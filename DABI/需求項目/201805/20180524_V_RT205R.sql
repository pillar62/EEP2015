...
SELECT          A.*, B.ENTRYNO, B.IOBOUND, C.CODENC AS NM_IOBOUND, B.ADDUSR, D .NAME, B.ADDDAT, 
                            replace(B.MEMO, ' ', '¡@') AS MEMOADD
FROM              
(SELECT          a.workno, a.linkno, g.comtype, l.CODENC, b.codenc AS worktype, isnull(c.shortnc, d .name) 
...
n.CODENC AS faqreasonnm, replace(g.MEMO, ' ', '¡@') as MEMO, m.CMTYIP, m.strbillingdat, m.dropdat, m.overdue, m.WtlApplyDat, 
