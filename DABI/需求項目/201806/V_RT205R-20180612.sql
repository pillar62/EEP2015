...
SELECT          A.*, B.ENTRYNO, B.IOBOUND, C.CODENC AS NM_IOBOUND, B.ADDUSR, D .NAME, B.ADDDAT, replace(B.MEMO, ' ', 
                            '¡@') AS MEMOADD
FROM              (SELECT          a.workno, a.linkno, g.comtype, l.CODENC, b.codenc AS worktype, isnull(c.shortnc, d .name) 
                                                        ...
                                                        m.docketdat, m.duedat, g.tel + '/' + g.mobile AS tel, m.cusnc, a.FINISHDAT, g.FAQREASON, 
                                                        n.CODENC AS faqreasonnm, replace(g.MEMO, ' ', '¡@') AS MEMO, m.CMTYIP, m.strbillingdat, m.dropdat, 
                                                        m.overdue, m.WtlApplyDat, m.paycycle, m.paytype, m.freecode, m.comtypenc, LEFT
                                                            ((SELECT          LINEIP + '/'
                                                                FROM              RTLessorAVSCmtyLine
                                                                WHERE          COMQ1 = G.COMQ1 AND isnull(dropdat, '') = '' FOR XML PATH('')), len
                                                            ((SELECT          LINEIP + '/'
                                                                FROM              RTLessorAVSCmtyLine
                                                                WHERE          COMQ1 = G.COMQ1 AND isnull(dropdat, '') = '' FOR XML PATH(''))) - 1) AS PP, 
                                                        m.SPEED, g.RCVUSR, o.NAME as RCVNM
                            FROM               RTSndWork a LEFT OUTER JOIN
														...
														left outer join RTEmployee o on o.EMPLY=g.RCVUSR
