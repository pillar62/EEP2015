<%  
  ...
  '必須於經銷商與isp關係檔中存在經銷商與速博或中華isp的關係(不包含東森)資料者,才可進入
     sql = "SELECT rtconsignee.CUSID, rtconsignee.CUSNO, rtconsignee.PASSWORD,rtconsignee.COMTYPE FROM RTConsignee  inner join rtconsigneeisp on rtconsignee.cusid=rtconsigneeisp.cusid and (rtconsigneeisp.isp='01' or rtconsigneeisp.isp='03') WHERE CUSNO='" & USR & "'"     
     ...
     If Not rs.EOF Then
        if pwd <> trim(rs("PASSWORD")) then
           ...
           if typ = "login" then '重導至[經銷商]的主畫面
              '速博
              if TRIM(rs("cusid"))="70774626" then
                 Response.Redirect "http://www.cbbn.com.tw/consignee/ConsigneeMain2.asp"
              else
                 Response.Redirect "http://www.cbbn.com.tw/consignee/ConsigneeMain.asp"
              end if
           elseif typ = "chgpw" then '重導至[修改密碼]的頁面
              Response.Redirect "http://www.cbbn.com.tw/consignee/chgpwd2.asp?uid="&usr
           end if   
        end if
     else
        Msg="帳號不存在!"
