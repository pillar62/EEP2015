...
Sub btn_onClick()
  ...
  if len(trim(s2)) > 0 then
     s="  客戶名稱：含('" & s2 & "')字元"  
     t=t & " HB2002ACT221.name like '%" & s2 & "%'"
  else
     s=s & "  客戶名稱：全部  "
     t=t & " HB2002ACT221.name <>'' "
  end if  
  ...
  s=s & "  申請日期︰自('" & s3 & "')-('" & s4 & "')"  
  t=t & " and hb2002act221.edat >= '" & s3 & "' and hb2002act221.edat <='" & S4 & "' "
  ...
  '---已確認
  if s5ary(0)="Y" then
     t=t & " and hb2002ACT221.cfmdat is not null and hb2002act221.dropdat is null "
  end if
  '---未確認
  if s5ary(0)="Y" then
     t=t & " and hb2002ACT221.cfmdat is null and hb2002act221.dropdat is null "
  end if  
  '---已作廢
  if s5ary(0)="Y" then
     t=t & " and hb2002ACT221.dropdat is not null "
  end if  
  
  s6=document.all("search6").value
  if len(trim(s6)) > 0 then
     s=s & "  客戶地址：含('" & s6 & "')字元"  
     t=t & " and (RTCounty.CUTNC + HB2002ACT221.TOWNSHIP + HB2002ACT221.RADDR )  like '%" & s6 & "%'"
  end if      
