...
Sub btn_onClick()
  ...
  IF LEN(TRIM(S2)) > 0 THEN
     s=s &"  社區序號:" &s2 & " "
     t=t &" AND (RTEBTFtpAdslRpl.COMQ1 =" & S2 & ") "
  END IF
 '----主線序號  
  s3=document.all("search3").value  
  IF LEN(TRIM(S3)) > 0 THEN
     s=s &"  主線序號:" &s3 & " "
     t=t &" AND (RTEBTFtpAdslRpl.LINEQ1 =" & S3 & ") "
  END IF  
  '----轉檔類別
  ...
  Elseif s4ary(0)="A" then
     s=s &"  轉檔類別:" &s4ary(1)
     t=t &" AND (RTEBTFtpAdslRpl.FLAG='" & S4ARY(0) & "') "
  ...
  Elseif s5ary(0)="1" then
     '未結案
     s=s &"  處理狀態:" &s5ary(1)
     t=t &" AND (RTEBTFtpAdslRpl.CLOSEDAT IS NULL AND RTEBTFtpAdslRpl.DROPDAT IS NULL ) "
  Elseif s5ary(0)="2" then
     '已作廢
     s=s &"  處理狀態:" &s5ary(1)
     t=t &" AND (RTEBTFtpAdslRpl.DROPDAT IS not NULL ) "
  elseif s5ary(0)="3" then
     '已結案
     s=s &"  處理狀態:" &s5ary(1)
     t=t &" AND (RTEBTFtpAdslRpl.CLOSEDAT IS NOT NULL ) "
  elseif s5ary(0)="4" then
     '已清除轉檔記錄但尚未結案
     s=s &"  處理狀態:" &s5ary(1)
     t=t &" AND (RTEBTFtpAdslRpl.CLOSEDAT IS NULL AND RTEBTFtpAdslRpl.CLRFLAG IS not NULL ) "
  ELSE
      s=s &"  處理狀態:全部"      
  End If        
