...
Sub SrEnvironment()
  ...
  functionOptName="清除記錄; 結  案 ; 作  廢 "
  functionOptProgram="rtebtcmtyTNSclr.asp;rtEBTcmtyTNSF.asp;rtEBTcmtyTNSDROP.asp"
  functionOptPrompt="Y;Y;Y"
  If V(1)="Y" then
     accessMode="U"
  Else
     accessMode="I"
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;社區序號;轉檔旗標;社區名稱;社區地址;住宅種類;轉檔序號;EBT回覆日;回覆狀態;錯誤訊息;結<BR>案;作<BR>廢;清<BR>除"
  sqlDelete="SELECT RTEBTFtpBuildingRpl.COMQ1,RTEBTFtpBuildingRpl.FLAG,RTEBTFtpBuildingRpl.COMQ1, case when RTEBTFtpBuildingRpl.FLAG='A' THEN '申請' when RTEBTFtpBuildingRpl.FLAG='F' THEN '報竣' WHEN RTEBTFtpBuildingRpl.FLAG='C' THEN '取消' ELSE '' END, RTEBTCMTYH.COMN, " _
           &"RTEBTFtpBuilding.CUTNC + RTEBTFtpBuilding.TOWNSHIP + RTEBTFtpBuilding.RADDR AS Expr2, RTCode.CODENC, " _
           &"RTEBTFtpBuilding.TRANNO, RTEBTFtpBuildingRpl.EBTREPLYDAT, RTEBTFtpBuildingRpl.EBTUPDSTATUS, RTEBTFtpBuildingRpl.ERRORMSG,CASE WHEN RTEBTFtpBuildingRpl.CLOSEDAT IS NOT NULL THEN 'Y' ELSE '' END,CASE WHEN RTEBTFtpBuildingRpl.DROPDAT IS NOT NULL THEN 'Y' ELSE '' END,CASE WHEN RTEBTFtpBuildingRpl.CLRFLAG IS NOT NULL THEN 'Y' ELSE '' END " _
           &"FROM RTEBTFtpBuilding LEFT OUTER JOIN RTCode ON RTEBTFtpBuilding.BUILDTYPE = RTCode.CODE RIGHT OUTER JOIN " _
           &"RTEBTFtpBuildingRpl INNER JOIN RTEBTCMTYH ON RTEBTFtpBuildingRpl.COMQ1 = RTEBTCMTYH.COMQ1 ON " _
           &"RTEBTFtpBuilding.TRANNO = RTEBTFtpBuildingRpl.TRANNO " _
           &"WHERE RTEBTFtpBuildingRpl=0 "            
  dataTable="RTEBTFtpBuildingRpl"
...
' When first time enter this keylist default query string to RTcmty.ComQ1 <> 0
  ...
  If searchQry="" AND searchQry2="" AND searchQry3="" Then
     searchQry=" and RTEBTFtpBuildingRpl.EBTUPDSTATUS='N' "
 ...
 if userlevel=31 then DAreaID="<>'*'"  
        ...
        sqlList="SELECT RTEBTFtpBuildingRpl.COMQ1,RTEBTFtpBuildingRpl.FLAG,RTEBTFtpBuildingRpl.COMQ1,  case when RTEBTFtpBuildingRpl.FLAG='A' THEN '申請' when RTEBTFtpBuildingRpl.FLAG='F' THEN '報竣' WHEN RTEBTFtpBuildingRpl.FLAG='C' THEN '取消' ELSE '' END, RTEBTCMTYH.COMN, " _
           &"RTEBTFtpBuilding.CUTNC + RTEBTFtpBuilding.TOWNSHIP + RTEBTFtpBuilding.RADDR AS Expr2, RTCode.CODENC, " _
           &"RTEBTFtpBuilding.TRANNO, RTEBTFtpBuildingRpl.EBTREPLYDAT, RTEBTFtpBuildingRpl.EBTUPDSTATUS, RTEBTFtpBuildingRpl.ERRORMSG,CASE WHEN RTEBTFtpBuildingRpl.CLOSEDAT IS NOT NULL THEN 'Y' ELSE '' END,CASE WHEN RTEBTFtpBuildingRpl.DROPDAT IS NOT NULL THEN 'Y' ELSE '' END,CASE WHEN RTEBTFtpBuildingRpl.CLRFLAG IS NOT NULL THEN 'Y' ELSE '' END " _ 
           &"FROM RTEBTFtpBuilding LEFT OUTER JOIN RTCode ON RTEBTFtpBuilding.BUILDTYPE = RTCode.CODE RIGHT OUTER JOIN " _
           &"RTEBTFtpBuildingRpl INNER JOIN RTEBTCMTYH ON RTEBTFtpBuildingRpl.COMQ1 = RTEBTCMTYH.COMQ1 ON " _
           &"RTEBTFtpBuilding.TRANNO = RTEBTFtpBuildingRpl.TRANNO " _
           &"WHERE RTEBTFtpBuildingRpl.comq1 <>0 and RTEBTFtpBuildingRpl.EBTUPDSTATUS='N' " & searchqry & " " _
           &"ORDER BY  RTEBTFtpBuildingRPL.EBTREPLYDAT "
