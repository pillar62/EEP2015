...
<%
   ...
   DSN="DSN=RtLib"
   conn.Open DSN
   SQL="select * from RTLessorCustFaqH  where cusid ='" & key(0) & "' and faqno ='" & key(1) & "' "
   ...
   rs.close
   SQL="select * from RTLessorCustFaqSndwork  where cusid ='" & key(0) & "' and faqno ='" & key(1) & "' and prtno ='" & key(2) & "' "
   rs.Open sql,conn,3,3
   if not rs.EOF then
      rs("prtusr")=V(0)
      rs("prtdat")=now()      
...
<form name=frm1 method=post action="RTLessorCustFaqSndWorkPV.asp">
