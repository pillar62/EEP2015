...
<%
	...
	Response.AddHeader "Content-Disposition","filename=收款明細表(AVS+ET).xls"
 ...
 <Worksheet ss:Name="AVS+ET">
  <Table>
   <Row ss:AutoFitHeight="0"> 
    <Cell ss:MergeAcross="13" ss:StyleID="Title"><Data ss:Type="String">元訊寬頻網路股份有限公司</Data></Cell>
   </Row>
<!--
   <Row>
...
<%
	...
	rs.Open sql, CONN
	Do While Not rs.Eof
		sqlstr="usp_RTRcvMoneyList '" &v(0)& "', '" &v(1)& "', 'A" &rs("code")& "' "
		rsx.Open sqlstr, CONN

		if Not rsx.Eof then
			response.Write "<Row ss:AutoFitHeight=""0"">" &_
				"<Cell ss:MergeAcross=""14"" ss:StyleID=""SubTitle""><Data ss:Type=""String"">收款明細表(" &rs("codenc")& ")</Data></Cell>" &_
				"</Row>"
		...
		if serno >0 then
			response.Write "<Row>" &_
				...
				"<Cell></Cell>" &_
				"</Row><Row></Row>"
			'response.Write "<Row><Cell ss:MergeAcross=""14""></Cell></Row><Row><Cell ss:MergeAcross=""13""></Cell></Row>"
		end if
		rs.MoveNext
	Loop
	rs.Close
