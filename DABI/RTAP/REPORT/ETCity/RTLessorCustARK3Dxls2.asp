<%
	...
	Response.AddHeader "Content-Disposition","filename=ET應收應付列表"& v(0) &".xls"
...
-->
   <Row>
    ...
    <Cell ss:StyleID="HeaderS"><Data ss:Type="String">心視界</Data></Cell>
   </Row>
<%
	sql="SELECT 	d.comn, c.cusnc, a.BATCHNO, b.ACNAMEC, " &_
		...
		"a.AMT - a.REALAMT as surplus, a.CDAT, a.MDAT, a.CANCELDAT, a.CANCELMEMO, case c.RCVMONEYFLAG1 + substring(a.batchno,1,2) when 'YBH' then '心視界收款' else '' end as flag "&_
	...
	Do While Not rs.Eof
	    ...
	    response.Write "<Row>" &_
			...
			"<Cell ss:StyleID=""toChar""><Data ss:Type=""String"">"& rs("flag") &"</Data></Cell>" &_
			"</Row>"
      rs.MoveNext
    Loop
    rs.Close
