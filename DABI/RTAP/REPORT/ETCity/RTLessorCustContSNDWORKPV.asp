...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       'msgbox "ETCity用戶續約派工單列印完成",0
       Set winP=window.Opener
       Set docP=winP.document       
       winP.focus()           
       docP.all("keyform").Submit     
       WINDOW.Form2.action="/RTAP/REPORT/ETCity/RTLessorCustContSNDWORKP.asp" & "?parm=" & document.all("key1").value & ";" & document.all("key2").value & ";" & document.all("key3").value & ";" & document.all("key4").value
...
<form name=frm1 method=post action="RTLessorCustContSNDWORKPV.asp" ID="Form1">
