              ...
              '   On Error Resume Next
                      if i=0 then  
                         ...
                         if len(rsc("caseno")) > 0 then
                            dspkey(i)="T" & yymmdd & right("000" & cstr(cint(mid(rsc("caseno"),8,3)) + 1),3)
                         else
                            dspkey(i)="T" & yymmdd & "001"
                         end if                                                             
                      end if
...
Sub SrEnvironment()
  ...
  title="調撥單資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT TRANSNO,TRANSDAT,OUTWAREHOUSE,INWAREHOUSE,outdept,OUTSIGN,indept,INSIGN,INSIGNDAT,EUSR,EDAT,UUSR,UDAT,CHECKDEPT,CHECKUSR,DROPUSR,DROPDAT " _
             &"FROM RTSTOCKTNSH WHERE TRANSNO='*' "
  sqlList="SELECT TRANSNO,TRANSDAT,OUTWAREHOUSE,INWAREHOUSE,outdept,OUTSIGN,indept,INSIGN,INSIGNDAT,EUSR,EDAT,UUSR,UDAT,CHECKDEPT,CHECKUSR,DROPUSR,DROPDAT " _
             &"FROM RTSTOCKTNSH WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"  
...
<p>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="21%" class=dataListHead>單號</td><td width="79%" READONLY bgcolor=silver>
...
conn.Open dsn
'當已作廢時,不可再維護資料
IF LEN(TRIM(DSPKEY(15)))>0 OR LEN(TRIM(DSPKEY(16))) > 0 THEN
...
<table border="1" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">調撥日期</font></td>
    <td width="35%" bgcolor="#C0C0C0" >
    <% If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then     
    ...
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">入庫日期</font></td>
    <td width="35%" bgcolor="#C0C0C0" colspan=3>
    <% If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then     
         SXX8=" onclick=""SrbtnOnclick"" "
       ELSE
         SXX8="" 
       END IF
    %>
     <input  <%=classxx%>  READONLY name="key8" <%=dataprotect%>  SIZE=10 style="TEXT-ALIGN: left" value
            ="<%=dspkey(8)%>" ID="Text8">
    <input type="button" id="B8"  name="B8" height="100%" width="100%" style="Z-INDEX: 1" value="...." <%=SXX8%>>
   ...
   <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">撥出倉庫</font></td>
    <td width="35%" bgcolor="#C0C0C0">
 <% If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then 
       sql="SELECT warehouse,warename from hbwarehouse order by warehouse "
    ...
    Else
       sql="SELECT warehouse,warename from hbwarehouse where warehouse='" & dspkey(2) & "' "
       XXCLASS=" class="" datalistDATA "" "
    End If
    rs.Open sql,conn,1,1
    Do While Not rs.Eof
       If rs("warehouse")=dspkey(2) Then sx=" selected "
    ...
    　</td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">撥出人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
   <%s=""
    sx=" selected " 
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then 
    ...
    Else
       sql="SELECT * FROM RTdept WHERE dept='" &dspkey(4) &"' " & vbcrlf
       SXX5=""
    End if 
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("dept")=dspkey(4) Then sx=" selected "
    ...
    rs.Close%>      
       ...
       <select <%=CLASSXX%> name="key4" <%=dataProtect%> ID="Select3"><%=s%></select>
    <input <%=classxx%>  name="key5"  maxlength=6 size=6 style="TEXT-ALIGN: left" value
            ="<%=dspkey(5)%>"  readOnly ID="Text7">
    <input type="button" id="B5"  name="B5"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX5%>  > 
   </td>
   <TR> 
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">撥入倉庫</font></td>
    <td width="35%" bgcolor="#C0C0C0">
 <% S=""
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then 
       sql="SELECT warehouse,warename from hbwarehouse order by warehouse "
       ...
       XXCLASS=" class="" datalistENTRY "" "
    Else
       sql="SELECT warehouse,warename from hbwarehouse where warehouse='" & dspkey(3) & "' "
       XXCLASS=" class="" datalistDATA "" "
    End If
    rs.Open sql,conn,1,1
    Do While Not rs.Eof
       If rs("warehouse")=dspkey(3) Then sx=" selected "
       s=s &"<option value=""" &rs("warehouse") &"""" &sx &">" &rs("warename") &"</option>"
       rs.MoveNext
       sx=""
    Loop
    rs.Close%>       
       <select <%=XXCLASS%> name="key3" <%=dataProtect%> ID="Select2"><%=s%></select>
    　</td>        
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">撥入人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
   <%s=""
    sx=" selected " 
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then 
    ...
    Else
       sql="SELECT * FROM RTdept WHERE dept='" &dspkey(6) &"' " & vbcrlf
       SXX7=""
    End if 
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("dept")=dspkey(6) Then sx=" selected "
          s=s &"<option value=""" &rs("dept") &"""" &sx &">" &trim(rs("deptn3")) & trim(rs("deptn4")) & trim(rs("deptn5"))  &"</option>" & vbcrlf
          rs.MoveNext
          sx=""
    Loop
    rs.Close%>      
       ...
       <select <%=CLASSXX%> name="key6" <%=dataProtect%> ID="Select4"><%=s%></select>    
    <input <%=classxx%>  name="key7"  maxlength=6 size=6 style="TEXT-ALIGN: left" value
            ="<%=dspkey(7)%>"  readOnly >
    <input type="button" id="B7"  name="B7"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX7%>  >  </td>            
  </tr>
  <tr>
      <td width="15%" bgcolor="#008080"><font color="#FFFFFF">驗收部門</font></td>
    <td width="35%" bgcolor="#C0C0C0">
   <%s=""
    sx=" selected " 
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND (LEN(TRIM(DSPKEY(15)))=0 AND LEN(TRIM(DSPKEY(16))) = 0) Then 
    ...
    rs.Close%>      
       ...
       <select <%=CLASSXX%> name="key13" <%=dataProtect%> ID="Select5"><%=s%></select>    
    </TD>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">驗收人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    
    <input <%=classxx%>  name="key14"  maxlength=6 size=6 style="TEXT-ALIGN: left" value
            ="<%=dspkey(14)%>"  readOnly ID="Text9">
    <input type="button" id="B14"  name="B14"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX14%>  >  </td>            
  </tr>
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">作廢人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListDATA name="key15" <%=dataprotect%> maxlength=6 size=6 style="TEXT-ALIGN:  left" 
     value="<%=dspkey(15)%>" readOnly ID="Text5"><%=dusrNc%>　</td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">作廢日期</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListDATA name="key16"  maxlength=10 size=10 style="TEXT-ALIGN: left" value
            ="<%=dspkey(16)%>"  readOnly ID="Text6">
    </td>
  </tr>  
 <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">輸入人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input <%=classxx%> name="key9" <%=dataprotect%> maxlength=6 size=6 style="TEXT-ALIGN:  left" 
     value="<%=dspkey(9)%>" readOnly><%=EusrNc%>　</td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">輸入日期</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input <%=classxx%>  name="key10"  maxlength=10 size=10 style="TEXT-ALIGN: left" value
            ="<%=dspkey(10)%>"  readOnly>　</td>
  </tr>
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">異動人員</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input <%=classxx%>  name="key11" readOnly size=6 maxlength=6 style="TEXT-ALIGN: left "
            value="<%=dspkey(11)%>"><%=UUsrNC%>　</td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">異動日期</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input <%=classxx%> name="key12" maxlength=10 size=10    
