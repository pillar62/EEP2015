...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTSparq0809/RTSparq0809CUSTTELD.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTSparq0809CustTEL where cusid='" & dspkey(0) & "' " ,conn
                         if len(rsc("SEQ")) > 0 then
                            dspkey(1)=rsc("SEQ") + 1
                         else
                            dspkey(1)=1
                         end if
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTSparq0809/RTSparq0809CUSTTELD.asp") then
          Set rsc=Server.CreateObject("ADODB.Recordset")
          rsc.open "select max(seq) AS seq from RTSparq0809CustTEL where cusid='" & dspkey(0) & "' " ,conn
          if not rsC.eof then
            dspkey(1)=rsC("seq")
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=2
  title="[有話好說]輕鬆省專案客戶申請電話明細"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT  CUSID, seq, TEL11,TEL12 , CANCELDAT " _
             &"from RTSparq0809CustTEL WHERE CUSID='' "
  sqlList="SELECT  CUSID, seq, TEL11,TEL12, CANCELDAT " _
             &"from RTSparq0809CustTEL WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrCheckData(message,formValid)
  ...
  IF LEN(TRIM(DSPKEY(2))) > 0 OR LEN(TRIM(DSPKEY(3))) > 0  THEN
      ...
      connxx.Open DSNxx
      '排除本身資料
      sqlXX="SELECT count(*) AS CNT FROM RTSparq0809CustTEL where CANCELDAT is null and TEL11='" & trim(dspkey(2)) & "' and TEL12='" & trim(dspkey(3)) & "' AND NOT (seq=" & DSPKEY(1) & ")"
...
IF ERRFLAG <> "Y" THEN
  If len(trim(dspkey(2)))=0 or len(trim(dspkey(3)))=0 then
       formValid=False
       message="電話號碼不可空白"    
  elseif len(trim(dspkey(2))) > 0 AND len(trim(dspkey(2))) < 2 then
       formValid=False
       message="電話區碼不可小於2位數"    
  elseif len(trim(dspkey(3))) > 0 AND len(trim(dspkey(3))) < 6 then
       formValid=False
       message="電話號碼不可小於6位數"           
  end if
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">0809用戶電話資訊</span>
...
<table width="100%" border=1 cellPadding=0 cellSpacing=0 id="tag1">
        <tr><td width=15%  class=dataListHEAD>電話號碼</td>
        ...
        <input type="text" name="key4" size="10" value="<%=dspKey(4)%>"  <%=fieldpe%><%=fieldpG%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text4">     
       <!-- 
