...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   ...
   if LEN(TRIM(CANCELDAT)) = 0 OR ISNULL(CANCELDAT) then
     ...
     endpgm="1"
     sql="update RTSparq0809CustTEL set canceldat=NULL where cusid='" & key(0) & "' AND SEQ=" & KEY(1)  
     conn.Execute sql
   end if
   conn.Close
   SET RS=NOTHING
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="2" then
       msgbox "此電話尚未作廢，不可執行作廢返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    else
       msgbox "無法執行用戶0809電話資料作廢返轉,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
