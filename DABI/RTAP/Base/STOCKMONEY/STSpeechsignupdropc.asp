   ...
   if LEN(TRIM(CANCELDAT)) = 0 OR ISNULL(CANCELDAT) then
     endpgm="3"
      ...
      sqlyy="insert into Speechsignuplog " _
           &"SELECT  YYMMDD, EMAIL," & SEQ & ",getdate(),'R','" & V(0) & "', NAME, TEL,  LIVEORNET, CONFIRMDAT, CONFIRMUSR, CANCELDAT, CANCELUSR,  APPLYDAT   " _
    ...
    ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update speechsignup set CANCELdat=NULL,CANCELUSR='' where yymmdd='" & key(0) & "' and email='" & key(1) & "' "  
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此演講報名資料尚未作廢，不可執行作廢返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    else
       msgbox "無法執行演講報名作廢返轉,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=STSPEECHSIGNUPDROPC.asp ID="Form1">
