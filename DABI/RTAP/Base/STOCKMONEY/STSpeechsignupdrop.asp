   ...
   ELSEif LEN(TRIM(confirmdat)) > 0 then
     endpgm="4"  
      ...
      sqlyy="insert into Speechsignuplog " _
           &"SELECT  YYMMDD, EMAIL," & SEQ & ",getdate(),'C','" & V(0) & "', NAME, TEL,  LIVEORNET, CONFIRMDAT, CONFIRMUSR, CANCELDAT, CANCELUSR,  APPLYDAT   " _
    ...
    ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update speechsignup set CANCELdat=getdate(),CANCELUSR='" & V(0) & "' where yymmdd='" & key(0) & "' and email='" & key(1) & "' "  
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此演講報名資料已作廢，不可重複作廢" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此演講報名資料已審核完成，不可作廢(欲作廢請先執行審核返轉)" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    else
       msgbox "無法執行演講報名作廢,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=STSPEECHSIGNUPDROP.asp ID="Form1">
