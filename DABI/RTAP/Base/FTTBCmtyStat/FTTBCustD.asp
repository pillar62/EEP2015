...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/FTTBCMTYSTAT/FTTBCUSTd.asp")
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/FTTBCMTYSTAT/FTTBCUSTd.asp") then
          cusidxx="F" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(cusid) AS cusid from FTTBCust where cusid like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(1)=rsC("CUSID")
...
<%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(43)))=0  and len(trim(dspkey(31)))=0 Then 
       sql="SELECT Code,CodeNC FROM RTCode WHERE KIND ='O7' AND CODE <> '01'  AND CODE <> '02' ORDER BY CODE" 
...
<tr>                                 
        ...
        <%IF DSPKEY(33)="Y" THEN
          ...
          END IF
        %>
        <input type="CHECKBOX" ID="KEY33" DISABLED value="Y" <%=FREECODE2%> name="key33" <%=fieldRole(1)%><%=dataProtect%> 
