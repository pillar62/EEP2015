...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE SQL
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/FTTBCMTYSTAT/FTTBCmtyStatD.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from FTTBCmtySTAT where  COMQ1=" & DSPKEY(0) ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/FTTBCMTYSTAT/FTTBCmtyStatD.asp") then
          rsc.open "select max(entryno) AS ENTRYNO from FTTBCMTYSTAT where  COMQ1=" & DSPKEY(0)  ,conn
          if not rsC.eof then
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=2
  title="FTTB社區光纖建設模式維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT comq1,entryno,sdat,smode,eusr,EDAT,uusr,udat,canceldat,CANCELUSR,MEMO " _
             &"FROM  fttbcmtystat WHERE comq1=0 "
  sqlList="SELECT comq1,entryno,sdat,smode,eusr,EDAT,uusr,udat,canceldat,CANCELUSR,MEMO " _
         &"from fttbcmtystat WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrCheckData(message,formValid)
  ...
  IF formValid=true then
     ...
     connxx.open DSN
     sqlxx="select count(*) as cnt from fttbcmtystat where comq1=" & dspkey(0) & " AND entryno <>" & dspkey(1) & " and canceldat is null and smode='" & dspkey(3) & "'" 
     rsxx.Open sqlxx,connxx
     if rsxx("cnt") > 0 then
        formValid=False
        message="建設模式已存在，不可重複建檔。"  
     end if
     rsxx.close
     connxx.close
     set rsxx=nothing
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">FTTB社區光纖建設模式資訊</span>
...
<td>     
    ...
    <tr>    <td bgcolor="BDB76B" align="center">建設模式內容</td></tr></table></DIV>
    <DIV ID=SRTAB0 >
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
<tr>
    <td width="25%" class=dataListHEAD>進度日期</td>
    <td width="25%" bgcolor="silver" >
    <% IF LEN(TRIM(DSPKEY(2)))=0 THEN 
          DSPKEY(2)=DATEVALUE(DATE())
       END IF
    %>
    ...
    <td width="25%" class=dataListHEAD>建設模式</td>
    <td width="25%" bgcolor="silver" >
      <%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(dspKey(8))) = 0  Then  
       IF LEN(TRIM(DSPKEY(1))) = 0 THEN DSPKEY(1)=0
       IF DSPKEY(1) > 0 THEN
           sql="SELECT CODE, CODENC FROM RTCode " _
              &"WHERE(KIND = 'O6') AND CODE NOT IN (SELECT SMODE FROM FTTBCMTYSTAT WHERE CANCELDAT IS NULL AND COMQ1=" & DSPKEY(0) & " AND ENTRYNO <>" & DSPKEY(1)  & ") "
       ELSE
           sql="SELECT CODE, CODENC FROM RTCode " _
               &"WHERE(KIND = 'O6') AND CODE NOT IN (SELECT SMODE FROM FTTBCMTYSTAT WHERE CANCELDAT IS NULL AND COMQ1=" & DSPKEY(0) & " AND ENTRYNO <>" & DSPKEY(1)  & ") "
          ' RESPONSE.WRITE SQL
       ...
       If len(trim(dspkey(3))) < 1 Then
          sx=" selected " 
          s=s & "<option value=""""" & sx & ">(建設模式)</option>"  
          sx=""
       else
          s=s & "<option value=""""" & sx & ">(建設模式)</option>"  
          sx=""
    ...
    Else
       sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='O6' AND CODE='" & dspkey(3) & "'"
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
...
<tr>
  ...
  %>    <input type="text" name="key4" size="6" READONLY value="<%=dspKey(4)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
...
<tr>
  ...
  %>    <input type="text" name="key6" size="6" READONLY value="<%=dspKey(6)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
    ...
    <tr><td bgcolor="BDB76B" align="center">其它說明</td></tr></table></DIV>
   <DIV ID="SRTAB2" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table9">
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%"  name="key10" rows=8  MAXLENGTH=500  class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(10)%>" ID="Textarea1"><%=dspkey(10)%></TEXTAREA>
   </td></tr>
 </table> 
  </div> 
<%
