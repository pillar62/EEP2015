          ...
          For i = 0 To numberOfField-1
              ...
              If Instr(cTypeDate,sType) > 0 And Len(Trim(dspKey(i))) = 0 Then dspKey(i)=Null
				if i=0 then
					...
					rsc.open "select max(wrkno) AS maxwrkno from RTSonetSndWrk where wrkno LIKE '" & cusidxx & "%' " ,conn
...
Sub SrEnvironment()
  ...
  sqlFormatDB=	"SELECT WRKNO, COMQ1, LINEQ1, CUSID, WRKTYP, IMPNO, IMPDAT, SNDWRKUSR, " &_
				...
				"WORKINGDAT, LINEDROPTYP " &_
				"FROM RTSonetSndWrk " &_
				"WHERE WRKTYP ='' "
...
Sub SrCheckData(message,formValid)
  ...
  if len(trim(dspkey(4)))=0 then
       formValid=False
       message="[派工別]不可空白"
  elseif dspkey(4)="02" and len(trim(dspkey(25)))=0 then
       formValid=False
       message="[預定裝機日]不可空白"
  elseif dspkey(4)="03" and len(trim(dspkey(25)))=0 then
       formValid=False
       message="[預定欠拆日]不可空白"
  elseif dspkey(4)="04" and len(trim(dspkey(25)))=0 then
       formValid=False
       message="[預定退租日]不可空白"
  elseif dspkey(4)="05" and len(trim(dspkey(25)))=0 then
       formValid=False
       message="[預定復機日]不可空白"
  ...
  '客戶檔已作廢則不可建立派工單
   ...
   connxx.open DSN
   sqlxx="select canceldat from RTSonetCust where comq1 =" & aryparmkey(1) & " and lineq1 =" & aryparmkey(2)& " and cusid = '" & aryparmkey(3) &"' "
   rsxx.Open sqlxx,connxx
   if not rsxx.eof then
     IF NOT ISNULL(RSXX("canceldat")) THEN
        formValid=False
        message="客戶檔已作廢，不可建立派工單" 
     END IF
   end if
   rsxx.close
   connxx.Close   
   ...
   End Sub
	...
	Sub SrWrkTypChange()
	      SELECT CASE document.all("key4").value
	         case "02"
				document.all("colWorkingDat").value = "預定裝機日"
				window.trWrkTypAbout.style.display=""
	         case "03"
				document.all("colWorkingDat").value = "預定欠拆日"
				window.trWrkTypAbout.style.display=""
	         case "04"
				document.all("colWorkingDat").value = "預定退租日"
				window.trWrkTypAbout.style.display=""
	         case "05"
				document.all("colWorkingDat").value = "預定復機日"
				window.trWrkTypAbout.style.display=""
	         case else
...
Sub SrGetUserDefineKey()%>
	<table width="100%" border=1 cellPadding=0 cellSpacing=0>
		<tr><td width="10%" class=dataListHead>派工單號</td>
			...
			<td width="23%"  bgcolor="silver">
				<input type="text" name="key3" readonly size="10" value="<%=dspKey(3)%>" class=dataListdata>
			</td>
		</tr>
	</table>
<%
...
<%
	sql="select isnull(d.shortnc, f.cusnc) as belongnc, c.comn, g.cutnc+a.township+a.raddr as addr, a.linetel, a.lineip, " &_
		"substring(h.codenc,1,2) as lineisp, i.codenc as lineiptype, j.codenc as linerate, a.hardwaredat, c.salesid, c.consignee, k.cusnc " &_
		...
		"inner join RTSonetCust k on k.comq1 = a.comq1 and k.lineq1 = a.lineq1 " &_
		"where a.comq1 =" &aryparmkey(1) & " and a.lineq1 =" &aryparmkey(2) & " and k.cusid ='" &aryparmkey(3)& "' "
	'response.Write "sql=" &sql
	...
	ELSE
		...
		salesid =rs("salesid")			:	consignee =rs("consignee")	:	cusnc =rs("cusnc")
	END IF
	RS.CLOSE
%>
<DIV ID="SRTAG0">
<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table2">
	<tr><td bgcolor="BDB76B" align="center">主線及客戶基本資料</td></tr>
</table>
</DIV>

<DIV ID=SRTAB0 >
<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
	<tr><td width="10%" class=dataListsearch>客戶名稱</td>
		<td width="23%" bgcolor="silver">
			<input type="text" readonly size="20" name="colCusnc" value="<%=cusnc%>" class="dataListsearch3" ID="TextCusnc">
		</td>
		<td width="10%" class=dataListsearch>社區名稱</td>
		<td width="23%" bgcolor="silver" colspan=3 >
			<input type="text" readonly size="30" name="colComn" value="<%=comn%>" class="dataListsearch3" ID="TextComn">
		</td>
	</tr>
	<tr><td width="10%" class=dataListsearch>轄區業務</td>
		<td width="23%" bgcolor="silver">
			<input type="text" readonly size="10" name="colBelongnc" value="<%=belongnc%>" class="dataListsearch3" ID="TextBelongnc">
		</td>
		<td class=dataListsearch>社區地址</td>
		<td bgcolor="silver" colspan=3>
			<input type="text" readonly size="50" name="colAddr" value="<%=addr%>" class="dataListsearch3" ID="TextAddr">
		</td>
	</tr>
...
<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
	...
	<!-- 撤線種類 -->
	<input type="text" name="key26" value="<%=dspKey(26)%>" style="display:none;" ID="Text26">
	</td></tr>
	...
	<tr><td width="10%" class=dataListHEAD>派工別</td>
		<%
			...
			sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='P6' and parm1 like ('%sonet客戶%') " 
	...
	<tr><td width="10%" class=dataListHEAD>預定施工人員</td>
		<td width="23%" bgcolor="silver" >
