...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/hbcalloutproject/HBPotCustAddD.asp")
         'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTPotCustD where caseno ='" & dspkey(0)&"' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/hbcalloutproject/HBPotCustAddD.asp") then
          rs.open "select max(entryno) AS entryno from RTPotCustD where caseno ='" & dspkey(0)&"' " ,conn
          if not rs.eof then
            dspkey(1)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  title="潛在客戶追件資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB=	"SELECT CASENO, ENTRYNO, ADDDAT, ADDUSR, ADDMEMO, CANCELDAT, CANCELUSR, UDAT, UUSR " &_
				"FROM   RTPotCustD WHERE caseno='' "
  sqlList=		"SELECT CASENO, ENTRYNO, ADDDAT, ADDUSR, ADDMEMO, CANCELDAT, CANCELUSR, UDAT, UUSR " &_
				"FROM   RTPotCustD WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrGetUserDefineKey()%>
	...
	<tr>
		...
		<td width="23%" bgcolor="silver">
			<input type="text" name="key1" readonly size="4" value="<%=dspKey(1)%>" class=dataListdata>
		</td>
	</tr>
	</table>
<%
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">潛在用戶資料</span>
...
<table width="100%">
	...
	<DIV ID="SRTAG2">
		...
		<tr><td bgcolor="BDB76B" align="center">潛在用戶追件資訊</td></tr></table>
	</DIV>
	...
	<tr>
		...
		<td width="23%" bgcolor="silver">
			<input type="text" name="key2" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
				value="<%=dspKey(2)%>" READONLY size="25" class=dataListDATA ID="Text3">
		</td>
		...
		<%
			...
			If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 and fieldpa="" Then  
				sql="SELECT RTEmployee.EMPLY, RTObj.CUSNC " _
					...
					&"WHERE  (DEPT='B400' or RTEmployee.emply ='" &dspkey(3)& "') AND TRAN2='' order by RTObj.CUSNC"
			...
			Else
				sql="SELECT RTEmployee.EMPLY, RTObj.CUSNC " _
					&"FROM   RTObj INNER JOIN RTEmployee ON RTObj.CUSID = RTEmployee.CUSID " _
					&"WHERE  (DEPT='B400' or RTEmployee.emply ='" &dspkey(3)& "') AND TRAN2='' AND RTEmployee.emply ='" &dspkey(3)& "' order by RTObj.CUSNC"
			End If
			rs.Open sql,conn
			Do While Not rs.Eof
			If rs("EMPLY")=dspkey(3) Then sx=" selected "
	...
	<tr>
		...
		<td bgcolor="silver" height="23" colspan=5>
			<TEXTAREA cols="100%" rows=8 MAXLENGTH=512 <%=fieldPa%> class="dataListentry" <%=FIELDROLE(1)%> <%=dataprotect%> value="<%=dspkey(4)%>" name="key4"><%=dspkey(4)%></TEXTAREA>
		</td>
	</tr>
	<tr>
		<td width="10%" class=dataListHEAD>作廢日</td>
		<td width="23%" bgcolor="silver"  >
			<input type="text" name="key5" value="<%=dspKey(5)%>" READONLY size="25" class=dataListData ID="Text11">
		</td>
        <td  class="dataListHEAD" height="23">作廢員</td>
        <td  height="23" bgcolor="silver" colspan=3>
			<input type="text" name="key6" size="6" READONLY value="<%=dspKey(6)%>" <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text12">
	...
	<tr>
		...
		<td width="23%" bgcolor="silver"  >
			<input type="text" name="key7" value="<%=dspKey(7)%>" READONLY size="25" class=dataListData ID="Text4">
		</td>
        <td  class="dataListHEAD" height="23">資料修改員</td>
        <td  height="23" bgcolor="silver" colspan=3>
			<input type="text" name="key8" size="6" READONLY value="<%=dspKey(8)%>" <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text5">
