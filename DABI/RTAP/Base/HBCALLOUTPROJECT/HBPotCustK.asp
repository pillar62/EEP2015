...
Sub SrEnvironment()
  ...
  functionOptName="結案;結案返轉;作廢;作廢返轉;追件"
  functionOptProgram="HBCotCustF.asp;HBCotCustFR.asp;HBCotCustDrop.asp;HBCotCustDropC.asp;HBPotCustAddK.asp"
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="客服單號;轄區;縣市;鄉鎮;社區名;客戶名;連絡電話;行動電話;需求;受理人;受理日;結案日;作廢日;追件數"
  sqlDelete="select a.caseno, isnull(d.shortnc, c.cusnc), e.cutnc, a.township, a.comn, a.cusnc, " &_
			"a.tel, a.mobile, f.codenc, h.cusnc, a.rcvdat, a.finishdat, a.canceldat, isnull(num, 0) " &_
			"from RTPotCustH a " &_
			"left outer join RTEmployee b inner join RTObj c on c.cusid = b.cusid on b.emply = a.salesid " &_
			"left outer join RTObj d on d.cusid = a.consignee " &_
			"left outer join RTCounty e on e.cutid = a.cutid " &_
			"left outer join RTCode f on f.code = a.requestid and f.kind ='P4' " &_
			"left outer join RTEmployee g inner join RTObj h on h.cusid = g.cusid on g.emply = a.rcvusr " &_
			"left outer join (select caseno, count(*) as num from RTPotCustD " &_
			"				where canceldat is null group by caseno) i on i.caseno = a.caseno " &_
			"where a.caseno='' "
  dataTable="RTPotCustH"
  userDefineDelete="Yes"
  numberOfKey=1
  dataProg="HBPotCustD.asp"
  datawindowFeature=""
...
' Open search program when first entry this keylist
  'searchProg="HBPotCustS.asp"
  searchFirst=FALSE
  If searchQry="" Then
     searchQry=" a.caseno is not null "
  ...
  'userlevel=2:為業務工程師==>只能看所屬社區資料
    sqlList="select a.caseno, isnull(d.shortnc, c.cusnc), e.cutnc, a.township, a.comn, a.cusnc, " &_
			"a.tel, a.mobile, f.codenc, h.cusnc, a.rcvdat, a.finishdat, a.canceldat, isnull(num, 0) " &_
			"from RTPotCustH a " &_
			"left outer join RTEmployee b inner join RTObj c on c.cusid = b.cusid on b.emply = a.salesid " &_
			"left outer join RTObj d on d.cusid = a.consignee " &_
