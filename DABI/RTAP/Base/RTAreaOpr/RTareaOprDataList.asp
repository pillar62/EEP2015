...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT areaid,areanc,areatype,eusr,edat,uusr,udat FROM rtarea WHERE rtarea.areatype='2' and rtarea.areaid='*' ;"
  sqlList="SELECT areaid,areanc,areatype,eusr,edat,uusr,udat " _
         &"FROM rtarea WHERE areatype='2' and "
  userDefineData="Yes"
  userDefineKey="Yes"
  datatable="rtarea"
End Sub
...
'       status2=""
'    End Select
    if dspkey(2)="2" then status1="checked"
...
%>   
     ...
     <tr><td width="35%" class=dataListHead><%=aryKeyName(2)%></td><td width="65%" bgcolor=silver><INPUT id=radio1 <%=status1%> name=key2  readonly type=radio value="2">施工轄區</tr> 
