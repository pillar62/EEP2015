...
If	filename <> "*TooBig*" Then
	...
	'二聯匯入 =================================================================================
	intI=0
	Set rs = XSLconn.Execute("SELECT 2 as INVTYPE,* FROM [二聯$] WHERE [發票號碼] is not null")
	While Not rs.EOF
		For intJ = 0 to 21 
...
'response.Write sql				  
	...
	'三聯匯入 =================================================================================
	intI=0
	Set rs = XSLconn.Execute("SELECT 3 as INVTYPE,* FROM [三聯$] WHERE [發票號碼] is not null")
	While Not rs.EOF
		For intJ = 0 to 21 
			if rs.Fields(intJ).Value <> Empty then 
			col = rs.Fields(intJ).Value
...
'response.Write sql			  
    ...
    SQLconn.Execute(sql)
    Response.Write "<br><br>""" & filename & """ 資料庫更新完畢..."
    ...
    Response.Write "<p><a href=""/rtap/Base/RTInvoice/RTInvReport1.asp?parm=" &batch&";1900/1/1;"& DateValue(Now()) &""">發票列印</a></p>"
Else
