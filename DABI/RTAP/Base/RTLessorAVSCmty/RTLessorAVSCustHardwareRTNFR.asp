...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   endpgm="1"
   sqlxx="select * FROM RTLessorAVSCustRtnHardware WHERE rcvprtno='" & key(0) & "'"
   ...
   if xxdatasrc="01" then
      sqlxx="select * FROM RTLessorAVSCmtyLineDropsndwork WHERE prtno='" & xxprtno & "'"
   end if
   rsxx.open sqlxx,conn
   if rsxx.eof then
      endpgm="7"
...
if endpgm="1" then
  ...
  '已作廢，不可結案返轉
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustHardwareRTNFR " & "'" & key(0) & "','" & rsxx("datasrc") & "','" & V(0) & "'" 
...
<form name=frm1 method=post action=RTLessorAVSCustHardwareRTNFR.asp ID="Form1">
