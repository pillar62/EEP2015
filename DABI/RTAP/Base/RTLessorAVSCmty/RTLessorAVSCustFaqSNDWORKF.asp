...
<% KEY=SPLIT(REQUEST("KEY"),";")
  ...
  elseif LEN(TRIM(RSyy("cancelDAT"))) <> 0 then
   ...
   sqlxx="select count(*) as CNT FROM RTLessorAVSCustFAQHardware WHERE CUSID='" & KEY(0) & "' AND faqno='" & key(1) & "' and prtno='" & key(2) & "' and dropdat is null and rcvfinishdat is null "
...
if endpgm="" then
   ...
   sqlyy="select count(*) as cnt FROM RTLessorAVSCustFaqSndworkFixCode WHERE CUSID='" & KEY(0) & "' and FAQNO='" & key(1) & "' and prtno='" & key(2) & "' "
   RSXX.OPEN SQLXX,CONN
   RSyy.OPEN SQLyy,CONN
   ...
   elseif LEN(TRIM(RSXX("BATCHNO"))) <> 0  then
      endpgm="13"      
   elseif rsyy("cnt") < 1 and LEN(TRIM(RSXX("MEMO"))) = 0 then
      endpgm="14"            
   ELSE
      '呼叫store procedure更新相關檔案
      strSP="usp_RTLessorAVSCustFAQSndworkF " & "'" & key(0) & "','" & key(1) & "','" & key(2) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶維修派工單完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="14" then
       msgbox "維修派工單結案時，必須輸入維修處理過程說明" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                                  
    else
