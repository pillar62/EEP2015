...
if endpgm="" then
   ...
   sqlyy="select count(*) as cnt FROM RTLessorAVSCustFAQhardware WHERE CUSID='" & KEY(0) & "' and FAQNO='" & key(1) & "' and prtno='" & key(2) & "'  and dropdat is null and RCVPRTNO <> '' "
   RSXX.OPEN SQLXX,CONN
   RSyy.OPEN SQLyy,CONN
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustFAQSndworkUF " & "'" & key(0) & "','" & key(1) & "','" & key(2) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶維修派工單未完工結案成功",0
...
<form name=frm1 method=post action=RTLessorAVSCustFAQsndworkUf.asp ID="Form1">
