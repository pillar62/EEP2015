...
Sub SrEnvironment()
  ...
  sqlDelete="SELECT  a.comq1,a.comn,a.cutnc,a.township,a.comcnt,a.b.linecnt,b.applycnt,C.COMQ1,c.cuscnt,c.cancelcnt,c.dropcnt,c.finishcnt,c.docketcnt,c.billingcnt " _
           &"FROM (SELECT        RTLessorAVSCmtyH.COMQ1, RTLessorAVSCmtyH.COMN, " _
...
' When first time enter this keylist default query string to RTcmty.ComQ1 <> 0
  ...
  set connxx=nothing
  '-------------------------------------------------------------------------------------------
  '20150206把salesid放在group條件拿掉,否則會出現一個社區因業務員不同而出現2筆
...
'				"from RTLessorAVSCmtyH x "& limitjoin &" join RTLessorAVSCmtyLine y on x.comq1 = y.comq1 " &_
...
'				"from RTLessorAVSCmtyH x " &_
'				"inner join RTLessorAVSCmtyLine y on x.comq1 = y.comq1 " &_
'				"inner join RTLessorAVSCust z on y.comq1 = z.comq1 and y.lineq1 = z.lineq1 " &_
...
'				"order by 	a.comq1 "
        sqlList="select a.comq1, a.comn, d.cutnc, a.township, a.comcnt, isnull(b.linecnt,0) as linecnt, isnull(b.applycnt, 0) as applycnt, " &_
				...
				"from	RTLessorAVSCmtyH a " &_
				"left outer join RTCounty d on a.cutid = d.cutid " &_
				"left outer join (select x.comq1, count(*) as linecnt, sum(case when y.hardwaredat is not null then 1 else 0 end) as applycnt " &_
				"from RTLessorAVSCmtyH x "& limitjoin &" join RTLessorAVSCmtyLine y on x.comq1 = y.comq1 " &_
				"group by x.comq1 "& searchqry2 &") b on b.comq1 = a.comq1 " &_
				"left outer join (select x.comq1, count(*) as cuscnt, sum(case when z.canceldat is not null then 1 else 0 end) as cancelcnt, " &_
				"sum(case when z.dropdat is not null and z.docketdat is not null then 1 else 0 end) as dropcnt, " &_
				"sum(case when z.dropdat is null and z.finishdat is not null then 1 else 0 end) as finishcnt, " &_
				"sum(case when z.dropdat is null and z.docketdat is not null then 1 else 0 end) as docketcnt, " &_
