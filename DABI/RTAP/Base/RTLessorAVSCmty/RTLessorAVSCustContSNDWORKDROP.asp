...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustContARSndworkDrop " & "'" & key(0) & "'," & key(1) & ",'" & key(2) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此收款派工單已完工(未完工)結案，不可作廢(欲作廢請先清除裝機完工日)：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此收款派工單已作廢，不可重覆執行作廢作業：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    elseIF frm1.htmlfld.value="5" then
       ...
       msgbox "無法執行收款派工單作廢作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTLessorAVScustContsndworkdrop.asp ID="Form1">
