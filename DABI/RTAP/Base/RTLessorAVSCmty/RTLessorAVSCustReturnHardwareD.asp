...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=3 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTLessorAVSCustReturnHARDWARE where CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTLessorAVSCustReturnHARDWARE WHERE  CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' " ,conn
          if not rs.eof then
            dspkey(3)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,ENTRYNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             ...
             &"FROM RTLessorAVSCustReturnHARDWARE WHERE cusid='' "
  sqlList="SELECT cusid,ENTRYNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             ...
             &"FROM RTLessorAVSCustReturnHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTLessorAVSCustReturnSNDWORK WHERE CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "
