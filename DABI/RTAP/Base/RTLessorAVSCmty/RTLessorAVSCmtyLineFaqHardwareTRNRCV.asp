...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  檢查該派工單是否已結案或作廢或已無可轉設備項目
   ...
   rsxx.close
   '檢查是否仍有未轉物品領用單的設備
   sqlxx="select count(*) as CNT FROM RTLessorAVSCmtylineFaqHardware WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND FAQNO='" & KEY(2) & "' AND PRTNO='" & KEY(3) & "' AND dropdat is null and rcvprtno='' and rcvfinishdat is null and batchno='' and qty > 0 "
...
if endpgm="" then
   ...
   strSP="usp_RTLessorAVSCmtylineFaqHardwareTRNRCVExe " & key(0) & "," & key(1) & ",'" & key(2) & "','" & key(3) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線維修派工設備轉物品領用單作業成功",0
    ...
    elseIF frm1.htmlfld.value="7" then
       ...
       msgbox "無法執行主線維修派工設備轉物品領用單作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTLessorAVSCmtylineFaqhardwaretrnrcv.asp ID="Form1">
