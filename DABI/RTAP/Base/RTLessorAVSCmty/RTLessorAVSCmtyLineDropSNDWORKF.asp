...
<% KEY=SPLIT(REQUEST("KEY"),";")
  ...
  elseif LEN(TRIM(RSyy("cancelDAT"))) <> 0 then
   ...
   elseif len(trim(rsyy("SNDCLOSEDAT"))) > 0 then
      '此撤線派工單所屬撤線單已有派工單結案日，請連絡資訊部
      endpgm="12"            
   end if
   rsxx.close
   rsyy.close
   '檢查該派工單下的設備是否皆已辦妥物品移轉程序
   sqlxx="select count(*) as CNT FROM RTLessorAVSCMTYLINEDROPHardware WHERE COMQ1=" & KEY(0) & " AND LINEQ1=" & KEY(1) & " AND ENTRYNO=" & key(2) & " and prtno='" & key(3) & "' and dropdat is null and rcvfinishdat is null "
...
if endpgm="" then
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtyLineDROPSndworkF " & key(0) & "," & key(1) & "," & key(2) & ",'" & key(3) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線撤線派工單完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="4" then
       msgbox "此撤線派工單已完工結案或未完工結案，不可重複執行完工結案或未完工結案" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    elseIF frm1.htmlfld.value="5" then
       msgbox "此撤線派工單已月結，不可異動" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close      
    elseIF frm1.htmlfld.value="6" then
       msgbox "此撤線派工單完工時，必須先輸入實際裝機人員或實際裝機經銷商" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                  
    ...
    elseIF frm1.htmlfld.value="15" then
       msgbox "主線已撤線,不可重覆執行(請連絡資訊部)。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                 
    elseIF frm1.htmlfld.value="9" then
       msgbox "此撤線派工單設備資料中，尚有設備未辦妥物品移轉程序(未移轉或移轉未結案)，不可執行完工結案作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    elseIF frm1.htmlfld.value="10" then
       msgbox "找不到此撤線派工單所屬撤線單資料" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                 
    elseIF frm1.htmlfld.value="11" then
       msgbox "此撤線派工單所屬撤線單資料已作廢，不可執行完工結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close               
    elseIF frm1.htmlfld.value="12" then
       msgbox "此撤線派工單所屬撤線單已有派工單結案日，請連絡資訊部" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                    
    elseIF frm1.htmlfld.value="13" then
       msgbox "此撤線派工單已產生應收帳款，無法重複結案，請連絡資訊部" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
...
<form name=frm1 method=post action=RTLessorAVSCmtylineDROPsndworkf.asp ID="Form1">
