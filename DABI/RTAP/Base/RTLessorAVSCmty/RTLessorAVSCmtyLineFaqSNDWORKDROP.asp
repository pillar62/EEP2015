...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlzz="select count(*) as cnt FROM RTLessorAVSCustRCVHardware WHERE prtno='" & KEY(3) & "' and canceldat is null "
   ...
   elseif LEN(TRIM(RSYY("FINISHDAT"))) <> 0 then
      endpgm="5"      
   elseif not rszz("cnt") > 0 then
      endpgm="7"  
   ELSE
      '呼叫store procedure更新相關檔案
      strSP="usp_RTLessorAVSCmtylineFaqSndworkDrop " & key(0) & "," & key(1) & ",'" & key(2) & "','" & key(3) & "','" & V(0) & "'" 
...
<form name=frm1 method=post action=RTLessorAVSCmtylineFAQsndworkdrop.asp ID="Form1">
