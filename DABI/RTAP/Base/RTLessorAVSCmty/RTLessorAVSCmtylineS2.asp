...
'----------經銷商
    S11=""
    rs.Open "SELECT  a.CONSIGNEE,  CASE WHEN a.CONSIGNEE = '' THEN '直銷' ELSE RTObj.SHORTNC  END as shortnc FROM RTLessorAVSCmtyline a LEFT OUTER JOIN RTObj ON a.CONSIGNEE = RTObj.CUSID GROUP BY  a.CONSIGNEE,  CASE WHEN a.CONSIGNEE = '' THEN '直銷' ELSE RTObj.SHORTNC  END ",CONN
...
'----------營運點
    ...
    s13="<option value=""*"" selected>全部</option>" &vbCrLf    
    Do While Not rs.Eof
       s13=s13 &"<option value=""" & rs("OPERATIONNAME") & """>" &rs("OPERATIONNAME") &"</option>"
       rs.MoveNext
    Loop
    s13=s13 &"<option value=""無法歸屬"">無法歸屬</option>"
    rs.Close    
'    
    conn.Close
    Set rs=Nothing
...
Sub btn_onClick()
  ...
  If s13 <> "*" and s13<>"無法歸屬" Then
     s=s &"  營運點︰" &s13 & " "
     t=t &" AND (RTCTYTOWN.OPERATIONNAME='" & S13 & "') "
  elseif s13="無法歸屬" then
     t=t &" AND (RTCTYTOWN.OPERATIONNAME='')"
  End If                   
...
<body>
<table width="100%">
  <tr class=dataListTitle align=center>AVS-City主線資料搜尋條件</td><tr>
</table>
<table width="100%" border=1 cellPadding=0 cellSpacing=0>
<tr><td class=dataListHead width="40%">營運點</td>
    <td width="60%" bgcolor="silver">
      <select name="search13" size="1" class=dataListEntry ID="Select1">
        <%=S13%>
    </select>      
    </td></tr>
