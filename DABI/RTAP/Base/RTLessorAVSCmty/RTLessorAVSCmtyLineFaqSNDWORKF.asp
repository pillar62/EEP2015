...
<% KEY=SPLIT(REQUEST("KEY"),";")
  ...
  elseif LEN(TRIM(RSyy("cancelDAT"))) <> 0 then
   ...
   sqlxx="select count(*) as CNT FROM RTLessorAVSCMTYLINEFAQHardware WHERE COMQ1=" & KEY(0) & " AND LINEQ1=" & KEY(1) & " AND faqno='" & key(2) & "' and prtno='" & key(3) & "' and dropdat is null and rcvfinishdat is null "
...
if endpgm="" then
   ...
   sqlyy="select count(*) as cnt FROM RTLessorAVSCmtylineFaqSndworkFixCode WHERE COMQ1=" & KEY(0) & " AND LINEQ1=" & KEY(1) & " AND faqno='" & key(2) & "' and prtno='" & key(3) & "' "
   RSXX.OPEN SQLXX,CONN
   RSyy.OPEN SQLyy,CONN
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtyLineFAQSndworkF " & key(0) & "," & key(1) & ",'" & key(2) & "','" & key(3) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線維修派工單完工結案成功",0
...
<form name=frm1 method=post action=RTLessorAVSCmtylineFAQsndworkf.asp ID="Form1">
