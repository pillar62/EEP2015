...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineFAQHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=4 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTLessorAVSCmtyLineFAQHARDWARE where COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND FAQNO='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineFAQHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTLessorAVSCmtyLineFAQHARDWARE WHERE  COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND FAQNO='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' " ,conn
          if not rs.eof then
            dspkey(4)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,FAQNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             ...
             &"FROM RTLessorAVSCmtyLineFaqHARDWARE WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,FAQNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             ...
             &"FROM RTLessorAVSCmtyLineFaqHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    IF ACCESSMODE="A" THEN
       ...
       connyy.Open dsn     
       sqlyy="SELECT * FROM RTLessorAVSCmtyLineFaqSNDWORK WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" &  DSPKEY(1) & " AND FAQNO='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' "
    ...
    if dspkey(22) <> "" then
       ...
       connyy.Open dsn     
       '同一社區下的設備網路編號不可相同
       sqlyy="select count(*) as cnt from RTLessorAVSCmtyLineFaqHardware where hostno='" & dspkey(22) & "' and COMQ1=" & DSPKEY(0) & " AND ( LINEQ1<>" &  DSPKEY(1) & " or FAQNO<>'" & DSPKEY(2) & "' or PRTNO<>'" & DSPKEY(3) & "' or seq<>" & dspkey(4) & ")"
       sqlzz="select count(*) as cnt from RTLessorAVSCmtyLineHardware where hostno='" & dspkey(22) & "' and COMQ1=" & DSPKEY(0) 
