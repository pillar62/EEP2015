...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustdropTRNSndwork " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "當退租單已作廢時，不可轉派工單" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "已退租單結案時，不可轉派工單" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    elseIF frm1.htmlfld.value="5" then
       ...
       msgbox "無法執行退租單轉派工單作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTLessorAVScustdropTRNSndwork.asp ID="Form1">
