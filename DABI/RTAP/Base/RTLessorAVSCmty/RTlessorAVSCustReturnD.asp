...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE SQL
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnd.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorAVSCustReturn where  CUSID='" & DSPKEY(0) & "' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnD.asp") then
          rsc.open "select max(entryno) AS ENTRYNO from RTLessorAVSCustReturn where  CUSID='" & DSPKEY(0) & "' " ,conn
          if not rsC.eof then
...
'       message="信用卡發卡銀行不可空白"         
 ...
 IF formValid=TRUE THEN
   ...
   connxx.open DSN
   sqlxx="select dropdat from RTLessorAVScust where cusid='" & aryparmkey(0) & "'"
 ...
 IF formValid=TRUE AND accessmode="A" THEN
   ...
   Set rsxx=Server.CreateObject("ADODB.Recordset")
   connxx.open DSN
   sqlxx="select count(*) as cnt from RTLessorAVScustReturn where cusid='" & aryparmkey(0) & "' and finishdat is null and canceldat is null "
   rsxx.Open sqlxx,connxx
   if rsxx("cnt") > 0 then
      formValid=False
...
Sub SrActiveXScript()%>
   <SCRIPT Language="VBScript">
	Sub srChangeMoney()
		s28=document.all("key28").value			'方案類型
      ...
      ' s5Y=document.all("key5Y").value
		...
		if document.all("key5X").CHECKED then	'第二戶
			document.all("key6").value = document.all("key"& S28 & S3 & "amt2").value
		else
			document.all("key6").value = document.all("key"& S28 & S3 & "amt").value
		end if
		document.all("key4").value = document.all("key"& S28 & S3 & "period").value	
	END SUB
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">AVS-City用戶復機資訊</span>
