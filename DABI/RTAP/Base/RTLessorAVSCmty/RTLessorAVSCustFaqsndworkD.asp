...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustFaqSNDWORKd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 2 then rs.Fields(i).Value=dspKey(i)    
                       if i=2 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         cusidxx="EF" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustFaqSNDWORKd.asp") then
          cusidxx="EF" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT cusid,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTLessorAVSCustFaqSNDWORK WHERE cusid='' "
  sqlList="SELECT cusid,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTLessorAVSCustFaqSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=99
...
Sub SrCheckData(message,formValid)
   ...
   ELSEif len(trim(DSPKEY(5))) = 0 and len(trim(DSPKEY(6))) = 0 THEN
       formValid=False
       message="預定施工人員或經銷商必須輸入其中一項"        
   ELSEif len(trim(DSPKEY(5))) > 0 and len(trim(DSPKEY(6))) > 0 THEN
       formValid=False
       message="預定施工人員及經銷商只能輸入其中一項"    
   ELSEif len(trim(DSPKEY(7))) = 0 and len(trim(DSPKEY(8))) = 0 AND LEN(TRIM(DSPKEY(11))) > 0 THEN
       formValid=False
       message="實際施工人員或經銷商必須輸入其中一項" 
   ELSEif len(trim(DSPKEY(7))) > 0 and len(trim(DSPKEY(8))) > 0 AND LEN(TRIM(DSPKEY(11))) > 0 THEN
       formValid=False
       message="實際施工人員及經銷商只能輸入其中一項"    
   END IF       
...
Sub SrActiveX() %>
    <OBJECT classid="CLSID:B8C54992-B7BF-11D3-AACE-0080C8BA466E"   codebase="/webap/activex/EF2KDT.CAB#version=9,0,0,3" 
	        height=60 id=objEF2KDT style="DISPLAY: none; HEIGHT: 0px; LEFT: 0px; TOP: 0px; WIDTH: 0px" width=60 VIEWASTEXT>
      ...
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="12%" class=dataListHead>用戶序號</td>
           ...
           <input type="text" name="key1"
                 <%=fieldRole(1)%> readonly size="13" value="<%=dspKey(1)%>" maxlength="12" class=dataListdata ID="Text24"></td>
...
<tr><td width="14%" class=dataListHead>派工日期</td>
    <td width="40%" bgcolor="silver" COLSPAN=3>
        <input type="text" name="key3" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               ...
               value="<%=dspKey(3)%>" size="25" class=dataListEntry ID="Text1">
       <input  type="button" id="B3" name="B3" height="100%" width="100%" style="Z-INDEX: 1" value="...." <%=FIELDPC%>>
       <IMG  SRC="/WEBAP/IMAGE/IMGDELETE.GIF" <%=fieldpc%> alt="清除" id="C3"  name="C3"   style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" <%=FIELDPD%>>                              
    </td>
 </tr>
 <tr>
    ...
    <td width="40%" bgcolor="silver" >
        <input type="text" name="key25" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(25)%>" size="25" READONLY  class=dataListDATA ID="Text8">
     </td>    
    ...
    <td width="35%" bgcolor="silver" >
        <input type="text" name="key4" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               ...
               value="<%=dspKey(4)%>" size="6" READONLY class=dataListDATA><font size=2><%=SrGetEmployeeName(dspKey(4))%></font>
    </td></tr>

<tr><td class=dataListHEAD>預定施工人員</td>
<td bgcolor="silver" >
<%
  sql="SELECT * FROM   RTLessorAVSCustFaqh LEFT OUTER JOIN RTLessorAVScust on RTLessorAVSCustFaqh.cusid=RTLessorAVScust.cusid left outer join " _
	 &"RTLessorAVSCmtyLine ON RTLessorAVSCust.COMQ1 = RTLessorAVSCmtyLine.COMQ1 AND " _
	 &"RTLessorAVSCust.LINEQ1 = RTLessorAVSCmtyLine.LINEQ1 WHERE RTLessorAVSCustFaqh.cusid='" & DSPKEY(0) & "'" 
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
 ...
 %>
        ...
        <input type="TEXT" name="key5" <%=fieldRole(1)%><%=dataProtect%> 
           ...
           <input type="BUTTON" name="B5"  width="100%" style="Z-INDEX: 1"  value="...." <%=FIELDP6%> <%=consignee%> >
...
<td class=dataListHEAD>實際施工人員</td>
<td bgcolor="silver" >
        <input type="TEXT" name="key7" <%=fieldRole(1)%><%=dataProtect%> 
               style="text-align:left;" size="6" maxlength="6" 
               value="<%=dspKey(7)%>"  <%=fieldpa%> readonly class="dataListENTRY" ID="Text3">
...
<tr><td  class=dataListHead>預定施工經銷商</td>
    <td  bgcolor="silver" >
        <%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 and len(trim(dspkey(11)))=0 Then 
    ...
    <td  class=dataListHead>實際施工經銷商</td>
    <td  bgcolor="silver" >
        <%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 and len(trim(dspkey(11)))=0  Then 
       sql="SELECT RTObj.CUSID,RTObj.SHORTNC " _
          &"FROM RTObj INNER JOIN " _
  ...
  <tr>
    ...
    <td  bgcolor="silver"  colspan=3>
        <input type="text" name="key28" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(28)%>" size="10" READONLY class=dataListDATA>
...
<td  class=dataListHead>作廢日期</td>
    ...
    <td  bgcolor="silver" >
        <input type="text" name="key27" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(27)%>" size="10" READONLY class=dataListDATA>
 ...
 <td class=dataListHEAD>獎金計算月</td>
  ...
  <tr>        
   <td class=dataListHead>獎金結帳員</td>          
    <td  bgcolor="silver">     
    <input type="text" name="key14" size="6" MAXLENGTH=6 value="<%=dspKey(14)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA"><font size=2><%=SrGetEmployeeName(dspKey(14))%></font>
    </td>
 ...
 <td class=dataListHEAD>庫存計算月</td>
  ...
  <tr>        
   <td class=dataListHead>庫存結帳員</td>          
    <td  bgcolor="silver">     
    <input type="text" name="key18" size="6" MAXLENGTH=6 value="<%=dspKey(18)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA"><font size=2><%=SrGetEmployeeName(dspKey(18))%></font>
    </td>
...
<tr>
 <td class=dataListSEARCH>應收帳款編號</td>
    <td bgcolor="silver">   
    <input type="text" name="key33" size="13" MAXLENGTH=13 value="<%=dspKey(33)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text10">
    </td>                            
   <td class=dataListSEARCH>轉應收帳款日</td>          
    <td bgcolor="silver">   
    <input type="text" name="key34" size="10" MAXLENGTH=10 value="<%=dspKey(34)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text11">
    </td>  
  </TR>
<tr>
        <td  class="dataListHEAD" height="23">建檔人員</td>                                 
        <td  height="23" bgcolor="silver">
	    <input type="text" name="key29" size="6" READONLY value="<%=dspKey(29)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA"><font size=2><%=SrGetEmployeeName(dspKey(29))%></font>
        </td>  
...
<tr>
        ...
        <td  height="23" bgcolor="silver">
		<input type="text" name="key31" size="6" READONLY value="<%=dspKey(31)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=SrGetEmployeeName(dspKey(31))%></font>
        </td>  
   ...
   <DIV ID="SRTAB6" > 
    ...
    sqlfaqlist="SELECT * FROM RTLessorAVSCustFaqSndworkFixCode RIGHT OUTER JOIN " _
            &"RTCode ON RTLessorAVSCustFaqSndworkFixCode.FIXcod = RTCode.CODE AND faqno ='" & DSPKEY(1) & "' AND PRTNO='" & DSPKEY(2) & "' "  _
  ...
  %>                               
     ...
     <TEXTAREA  cols="100%" name="key24" rows=7  MAXLENGTH=300 <%=fieldpa%>  class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(24)%>" ID="Textarea1"><%=dspkey(24)%></TEXTAREA>
   </td></tr>
 </table>            
   </table> 
  </div> 
...
'------ RTObj ---------------------------------------------------
    DELFAQlist="delete from RTLessorAVSCustFaqSndworkFixCode where CUSID='" & dspkey(0) & "' AND faqno='" & dspkey(1) & "' AND PRTNO='" & DSPKEY(2) & "' "
    conn.Execute DELFAQlist  
    For i=0 to 99
        if len(trim(extdb(i))) > 0  then
           rs.Open "SELECT * FROM RTLessorAVSCustFaqSndworkFixCode WHERE CUSID='" & dspkey(0) & "' AND faqno='" &dspKey(1) &"' AND PRTNO='" & DSPKEY(2) & "' and fIXcod='" & extDB(i) & "'" ,conn,3,3
           If rs.Eof Or rs.Bof Then
              rs.AddNew
              rs("cusid")=dspKey(0)
              rs("faqno")=dspKey(1)
