...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnSNDWORKd.asp")
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustReturnSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustReturnSNDWORKd.asp") then
          cusidxx="ER" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustReturnSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTLessorAVSCustReturnSNDWORK WHERE cusid='' "
  sqlList="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat " _
             &"FROM RTLessorAVSCustReturnSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connXX.open DSN
    SQLXX="SELECT * FROM RTLessorAVSCustReturnHARDWARE WHERE cusid='" & dspkey(0) & "' and entryno=" & dspkey(1) & " and prtno='" & dspkey(2) & "' "
 ...
 '      message="客戶尚未分配IP，不可建立派工資料。"       
    ...
    if formValid=True then
       ...
       Set rsxx=Server.CreateObject("ADODB.Recordset")
       connxx.open DSN
       sqlxx="select PAYTYPE from RTLessorAVScustReturn where cusid='" & aryparmkey(0) & "' and entryno ='" & aryparmkey(1) & "' "
       rsxx.Open sqlxx,connxx
...
<td bgcolor="silver" >
<%
  sql="SELECT * FROM RTLessorAVSCmtyLine RIGHT OUTER JOIN RTLessorAVSCust ON RTLessorAVSCmtyLine.COMQ1 = RTLessorAVSCust.COMQ1 AND " _
     &"RTLessorAVSCmtyLine.LINEQ1 = RTLessorAVSCust.LINEQ1 WHERE cusid='" & dspkey(0) & "'"
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
