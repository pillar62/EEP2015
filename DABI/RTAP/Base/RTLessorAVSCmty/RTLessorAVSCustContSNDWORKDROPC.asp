...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustContARSndworkDropC " & "'" & key(0) & "'," & key(1) & ",'" & key(2) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="4" then
       msgbox "此用戶收款派工單尚未作廢，不可執行作廢返轉作業：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行用戶收款派工單作廢返轉作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
