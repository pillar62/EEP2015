...
if endpgm="" then
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustContARSndworkFR " & "'" & key(0) & "'," & key(1) & ",'" & key(2) & "','" & V(0) & "','" & batchnoxx & "'" 
 ...
 sub window_onload()
   ...
   elseIF frm1.htmlfld.value="7" then
    ...
    elseIF frm1.htmlfld.value="13" then
       msgbox "找不到客戶續約資料檔，無法返轉。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close            
    elseIF frm1.htmlfld.value="14" then
       msgbox "客戶續約資料已作廢，不可返轉。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                   
    else
