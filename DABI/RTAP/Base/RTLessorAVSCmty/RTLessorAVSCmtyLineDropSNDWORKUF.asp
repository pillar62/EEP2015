...
if endpgm="" then
   ...
   sqlyy="select count(*) as cnt FROM RTLessorAVSCMTYLINEdropHardware WHERE COMQ1=" & KEY(0) & " AND LINEQ1=" & KEY(1) & " AND entryno=" & key(2) & " and prtno='" & key(3) & "' and dropdat is null and rcvprtno <> '' "
   RSXX.OPEN SQLXX,CONN
   RSyy.OPEN SQLyy,CONN
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtylinedropSndworkUF " & key(0) & "," & key(1) & "," & key(2) & ",'" & key(3) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線撤線派工單未完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "此撤線派工單已產生物品移轉單，請先返轉移轉單才能執行未完工結案作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                
  elseIF frm1.htmlfld.value="7" then
    ...
    elseIF frm1.htmlfld.value="11" then
       msgbox "此撤線派工單所屬撤線單資料已作廢，不可執行未完工結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close               
    ...
    elseIF frm1.htmlfld.value="13" then
       ...
       msgbox "無法執行撤線派工單未完工結案作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTLessorAVSCMTYLINEdropsndworkUf.asp ID="Form1">
