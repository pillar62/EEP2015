...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineDropSNDWORKd.asp")
                       ...
                       if i=3 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCmtyLineDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineDropSNDWORKd.asp") then
          cusidxx="EX" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCmtyLineDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(3)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,entryno, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTLessorAVSCmtyLineDropSNDWORK WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,entryno, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTLessorAVSCmtyLineDropSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=0
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="10%" class=dataListHead>社區序號</td>
           ...
           <td width="10%" class=dataListHead>撤線單次</td>
...
<tr><td>&nbsp;</td><td>        
    ...
    <tr>    <td bgcolor="BDB76B" align="center">撤線派工單內容</td></tr></table></DIV>
    <DIV ID=SRTAB2 >
...
<tr><td class=dataListHEAD>預定撤線人員</td>
<td bgcolor="silver" >
<%
...
<td class=dataListHEAD>實際撤線人員</td>
<td bgcolor="silver" >
<%
  name="" 
           if dspkey(8) <> "" then
...
<tr><td  class=dataListHead>預定撤線經銷商</td>
    <td  bgcolor="silver" >
    ...
    <td  class=dataListHead>實際撤線經銷商</td>
    <td  bgcolor="silver" >
        <%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 and len(trim(dspkey(12)))=0  Then 
       sql="SELECT RTObj.CUSNC, RTObjLink.CUSTYID, RTObj.CUSID,RTObj.SHORTNC " _
          &"FROM RTObj INNER JOIN " _
...
<tr>
  ...
  %>    <input type="text" name="key32" size="6" READONLY value="<%=dspKey(32)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
    ...
    <tr><td bgcolor="BDB76B" align="center">撤線過程說明</td></tr></table></DIV>
   <DIV ID="SRTAB6" > 
