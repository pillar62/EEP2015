...
<% KEY=SPLIT(REQUEST("KEY"),";")
  ...
  '當所屬派工單已計算庫存後，不可作廢之(stockcloseym<>'')
   sqlxx="select * FROM RTLessorAVSCmtyLineHardware WHERE comq1=" & key(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' and seq=" & key(3)
   sqlyy="select * FROM RTLessorAVSCmtyLinesndwork WHERE comq1=" & key(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "'"
  ...
  '已作廢
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtyLineHardwareDrop " & key(0) & "," & key(1) & ",'" & key(2) & "'," & key(3) & ",'" & V(0) & "'" 
...
<form name=frm1 method=post action=RTLessorAVScmtylinehardwaredrop.asp ID="Form1">
