...
'----------主線序號
    S3=""
    rs.Open "SELECT * FROM RTLessorAVSCustAR WHERE  CUSID='" & ARYPARMKEY(0) & "' AND BATCHNO='" & ARYPARMKEY(1) & "'",CONN
    IF RS.EOF THEN
       TEMPPERIOD=0
       TEMPCOD1=""
       TEMPCOD2=""
...
Sub btn_onClick()
  ...
  ELSEIF len(trim(S4))=0 then
      msgbox "應收帳款編號不可空白。"      
  ELSEIF len(trim(S11))=0 or cint(S11) =0 then
      msgbox "實沖金額不可等於零。"         
  ...
  ELSE
   ...
   '              &"location=no,menubar=no,width=5px,height=5px" 
     prog="RTLessorAVSCustARClearExe.asp" & "?key=" & s3 & ";" & s4 & ";" & s11
...
<body><CENTER>
<table width="80%">
  <tr class=dataListTitle align=center>AVS-City用戶應收應付帳款沖帳</td><tr>
</table>
