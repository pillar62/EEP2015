...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   RSXX.OPEN SQLXX,conn
     ...
     ELSEIF not isnull(RSYY("DROPdat")) THEN
        ENDPGM="10"
    'batchno為空白或結案日為空白時，表示此筆復機資料尚未轉應收帳款，不可執行返轉作業
   ...
   if endpgm="" then
      ...
      strSP="usp_RTLessorAVSCustReturnTRNFR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "','" & BATCHNOXX & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶復機返轉應收結案作業成功",0
   ...
   elseIF frm1.htmlfld.value="3" then
       msgbox "此筆復機資料尚未轉應收帳款，不可執行返轉應收結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
   elseIF frm1.htmlfld.value="4" then
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "復機資料已作廢時，不可執行返轉應收結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
     elseIF frm1.htmlfld.value="7" then
       msgbox "找不到客戶復機主檔資料，不可執行返轉應收結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    ...
    elseIF frm1.htmlfld.value="11" then
       ...
       msgbox "無法執行用戶復機返轉應收結案作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTLessorAVSCustReturnTRNFR.asp ID="Form1">
