...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCUSTSNDWORKd.asp")
                       ...
                       if i=1 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCUSTSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCUSTSNDWORKd.asp") then
          cusidxx="ET" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCUSTSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(1)=rsC("PRTNO")
...
Sub SrCheckData(message,formValid)
    ...
    connXX.open DSN
    SQLXX="SELECT * FROM RTLessorAVSCUSTHARDWARE WHERE cusid='" & dspkey(0) & "' and prtno='" & dspkey(1) & "' "
    ...
    SQLXX="SELECT * FROM RTLessorAVSCUST WHERE cusid='" & dspkey(0) & "'"
    SQLYY="SELECT CUSNC,COUNT(*) AS CNT FROM RTLessorAVSCUST WHERE cusid<>'" & dspkey(0) & "' AND CANCELDAT IS NULL AND EQUIP='" & dspkey(21) & "' AND PORT='" & dspkey(22) & "' AND EQUIP+PORT<>'00' GROUP BY CUSNC"
    RSXX.OPEN SQLXX,CONNXX
...
<tr><td width="14%" class=dataListHead>完工日期</td>
    ...
    <td width="40%" bgcolor="silver">
        <input type="text" name="key33" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               value="<%=dspKey(33)%>" size="11" class=dataListEntry ID="Text7">
 ...
 <tr>
    ...
    <td width="40%" bgcolor="silver" >
        <input type="text" name="key24" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               value="<%=dspKey(24)%>" size="10" READONLY  class=dataListDATA ID="Text8">
     </td>    
...
<td bgcolor="silver" >
<%
  sql="SELECT * FROM   RTLessorAVSCust a inner join " _
	 &"RTLessorAVSCmtyLine b ON a.COMQ1 = b.COMQ1 AND a.LINEQ1 = b.LINEQ1 WHERE cusid='" & DSPKEY(0) & "'" 
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
...
%>  
        ...
        <input type="TEXT" name="key4" <%=fieldRole(1)%><%=dataProtect%> 
           ...
           <input type="BUTTON" id="B4" <%=FIELDPA%> name="B4" width="100%" style="Z-INDEX: 1"  value="...." <%=FIELDP6%> >   
...
<tr>
    ...
    <td  bgcolor="silver" >
        <input type="text" name="key10" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               value="<%=dspKey(10)%>" size="25" READONLY class=dataListdata>
               <!--
    ...
    <td  bgcolor="silver" >
        <input type="text" name="key25" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               value="<%=dspKey(25)%>" size="25" READONLY class=dataListdata>
               <!--
  ...
  <tr>
    ...
    <td  bgcolor="silver" colspan=3>
        <input type="text" name="key27" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               value="<%=dspKey(27)%>" size="7" READONLY class=dataListDATA>
 ...
 <td class=dataListHEAD>獎金計算月</td>
  ...
  <tr>        
   <td class=dataListHead>獎金結帳員</td>          
    <td  bgcolor="silver">     
    <input type="text" name="key13" size="6" MAXLENGTH=6 value="<%=dspKey(13)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text15">
 ...
 <td class=dataListHEAD>庫存計算月</td>
  ...
  <tr>        
   <td class=dataListHead>庫存結帳員</td>          
    <td  bgcolor="silver">     
    <input type="text" name="key17" size="6" MAXLENGTH=6 value="<%=dspKey(17)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text25">
...
<tr>
	    ...
	    <input type="text" name="key28" size="6" READONLY value="<%=dspKey(28)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2">
