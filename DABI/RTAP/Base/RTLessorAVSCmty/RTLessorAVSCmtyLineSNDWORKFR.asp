...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlZZ="select COUNT(*) AS CNT FROM RTLessorAVSCUST WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND canceldat is null and dropdat is null and finishdat is not null "
  ...
  '當已作廢時，不可執行完工結案或未完工結案
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtylineSndworkFR " & key(0) & "," & key(1) & ",'" & key(2) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線派工單完工/未完工結案返轉成功",0
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "無法找到此主線派工單之主檔資料，請確認AVS-City主線主檔資料正常" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
...
<form name=frm1 method=post action=RTLessorAVScmtylinesndworkfr.asp ID="Form1">
