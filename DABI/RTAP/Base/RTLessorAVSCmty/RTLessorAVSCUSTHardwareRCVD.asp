          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCUSTHardwareRCVD.asp")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="select RCVPRTNO,DATASRC,APPLYDAT,ASSIGNENGINEER,ASSIGNCONSIGNEE,REALENGINEER,REALCONSIGNEE,CLOSEDAT," _
             ...
             &"FROM RTLessorAVSCustRCVHardware WHERE RCVPRTNO='' "
  sqlList="select RCVPRTNO,DATASRC,APPLYDAT,ASSIGNENGINEER,ASSIGNCONSIGNEE,REALENGINEER,REALCONSIGNEE,CLOSEDAT," _
             &"CLOSEUSR, PRTNO, CANCELDAT, CANCELUSR, RCVUSR  FROM RTLessorAVSCustRCVHardware where " 
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=0
...
Sub SrGetUserDefineKey()%>
      <table width="30%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="10%" class=dataListHead>物品領用單號</td>
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">物品領用單資料</span>
...
<tr><td width="14%" class=dataListHead>領用類別</td>
    ...
    Else
       sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='N8' AND CODE='" & dspkey(1) & "'"
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
   ...
   <select size="1" name="key1" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Select15">                                                                  
    ...
    <td width="14%" class=dataListHead>領用申請日</td></td>
    <td width="40%" bgcolor="silver" >
...
<td  class=dataListHead>所屬派工單號</td>
    ...
    <td  class=dataListHead>轉領用單人員</td>
    <%
  name="" 
           if dspkey(12) <> "" then
...
<tr><td class=dataListHEAD>領用申請人</td>
<td bgcolor="silver" >
<%
  name="" 
           if dspkey(3) <> "" then
...
<td class=dataListHEAD>實際領用人</td>
<td bgcolor="silver" >
<%
...
<tr><td  class=dataListHead>領用申請經銷商</td>
    <td  bgcolor="silver" >
        <%  
    ...
    <td  class=dataListHead>實際領用經銷商</td>
    <td  bgcolor="silver" >
        <%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 AND len(trim(dspKey(7))) = 0 Then 
       sql="SELECT RTObj.CUSNC, RTObjLink.CUSTYID, RTObj.CUSID,RTObj.SHORTNC " _
          &"FROM RTObj INNER JOIN " _
...
<tr>
    <td  class=dataListHead>領用結案日</td>
    <td  bgcolor="silver" >
