...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   ...
   sqlzz="select count(*) as cnt FROM RTLessorAVSCustADJDAY WHERE CUSID='" & KEY(0) & "' AND canceldat is null and adjclosedat is null "
   ...
   ELSE
     ...
     elseif len(trim(XXSTRBILLINGDAT)) > 0 and XX2DUEDAT < xxstrbillingdat then
       ...
       '呼叫store procedure更新相關檔案
        strSP="usp_RTLessorAVSCustAdjDayFR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶調整到期日數資料結案返轉成功",0
...
<form name=frm1 method=post action=RTLessorAVSCustADJDAYFR.asp ID="Form1">
