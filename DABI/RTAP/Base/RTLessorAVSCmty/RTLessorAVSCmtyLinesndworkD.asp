...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineSNDWORKd.asp")
                    '   response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCmtyLineSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineSNDWORKd.asp") then
          cusidxx="EL" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCmtyLineSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,PRTNO,SENDWORKDAT,PRTUSR,ASSIGNENGINEER,ASSIGNCONSIGNEE,REALENGINEER,REALCONSIGNEE,DROPDAT,DROPDESC," _ 
             ...
             &"FROM RTLessorAVSCmtyLineSNDWORK WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,PRTNO,SENDWORKDAT,PRTUSR,ASSIGNENGINEER,ASSIGNCONSIGNEE,REALENGINEER,REALCONSIGNEE,DROPDAT,DROPDESC," _ 
             ...
             &"MDF2,HOSTNO,HOSTPORT,MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,EUSR,EDAT," _
             &"UUSR, UDAT, BATCHNO, CDAT,SNDKIND,EQUIPSETUPDAT,ADSLAPPLYDAT " _
             &"FROM RTLessorAVSCmtyLineSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=0
...
<%
  ...
  name="" 
           if dspkey(5) <> "" then
              sql=" select cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid where rtemployee.emply='" & dspkey(5) & "' "
              rs.Open sql,conn
              if rs.eof then
                 name=""
              else
 ...
 %>
        ...
        <input type="TEXT" name="key5" <%=FIELDPA%><%=fieldRole(1)%><%=dataProtect%> 
           ...
           <input type="BUTTON" id="B5" name="B5"  width="100%" style="Z-INDEX: 1"  <%=FIELDP6%> value="....">
