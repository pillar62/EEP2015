...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlzz="select count(*) as cnt FROM RTLessorAVSCustReturnsndwork WHERE CUSID='" & KEY(0) & "' and entryno=" & key(1) & " and dropdat is null and unclosedat is null and closedat is null "
   ...
   RSzz.OPEN SQLzz,conn
    ...
    '不論繳費方式為何，若有存在收款派工單資料，則必須由派工單進行結案產生應收帳款
     ...
     ELSE
        ...
        strSP="usp_RTLessorAVSCustReturnTRNAR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
   ...
   elseIF frm1.htmlfld.value="3" then
       msgbox "此筆復機資料已轉應收帳款，不可重複產生" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "復機資料已作廢時，不可執行轉應收帳款作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
     elseIF frm1.htmlfld.value="7" then
       msgbox "找不到客戶復機主檔資料，無法執行轉應收帳款結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    ...
    elseIF frm1.htmlfld.value="9" then
       msgbox "客戶資料已作廢，必須作廢復機資料" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                
    elseIF frm1.htmlfld.value="10" then
       msgbox "客戶資料尚未退租，不可執行復機轉應收帳款作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                  
    ...
    elseIF frm1.htmlfld.value="12" then
       msgbox "此復機資料已存在收款派工單，必須由派工單進行結案作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                               
    else
       msgbox "無法執行用戶復機轉應收帳款作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action="RTLessorAVSCustReturnTRNAR.asp" ID="Form1">
