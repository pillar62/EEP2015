...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustDropd.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorAVSCustDrop where  CUSID='" & DSPKEY(0) & "' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustDropD.asp") then
          rsc.open "select max(entryno) AS ENTRYNO from RTLessorAVSCustDrop where  CUSID='" & DSPKEY(0) & "' " ,conn
          if not rsC.eof then
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT CUSID,ENTRYNO,DROPKIND,APPLYDAT,ENDDAT,FINISHDAT,FUSR,SNDPRTNO,SNDWORKCLOSE,CANCELDAT,CANCELUSR," _
             ...
             &"FROM  RTLessorAVSCustDrop WHERE cusid='' "
  sqlList="SELECT CUSID,ENTRYNO,DROPKIND,APPLYDAT,ENDDAT,FINISHDAT,FUSR,SNDPRTNO,SNDWORKCLOSE,CANCELDAT,CANCELUSR,MEMO," _
             ...
             &"from RTLessorAVSCustDrop WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrCheckData(message,formValid)
 ...
 IF formValid=TRUE THEN
   ...
   connxx.open DSN
   sqlxx="select * from RTLessorAVScust where cusid='" & aryparmkey(0) & "'"
 ...
 IF formValid=TRUE AND accessmode="A" THEN
   ...
   Set rsxx=Server.CreateObject("ADODB.Recordset")
   connxx.open DSN
   sqlxx="select count(*) as cnt from RTLessorAVSCustDrop where cusid='" & aryparmkey(0) & "' and finishdat is null and canceldat is null "
   rsxx.Open sqlxx,connxx
   if rsxx("cnt") > 0 then
      formValid=False
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">AVS-City用戶退租資訊</span>
