...
Sub SrEnvironment()
  ...
  functionOptProgram="RTLessorAVSCustBillingPrtCtlTRN_duedate.asp"
  functionOptPrompt="Y"
  functionoptopen="1"
  If V(1)="Y" then
     accessMode="U"
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;none;主線;社區名稱;用戶;週期;繳款;連絡電話;申請日;完工日;開始計費;最近<br>續約日;調整<br>日數;到期日;退租日;作廢日;期數;可用<BR>日數;已收"
  sqlDelete="SELECT a.comq1, a.lineq1, a.cusid, " &_
			...
			"else DATEDiFF(d,getdate(),a.DUEDAT) end as validdat,i.codenc " &_
			"FROM RTLessorAVSCustBillingPrt " &_
			"inner join RTLessorAVSCustBillingPrtsub on RTLessorAVSCustBillingPrt.batch=RTLessorAVSCustBillingPrtsub.batch " &_
			"inner join RTLessorAVSCust a on a.cusid=RTLessorAVSCustBillingPrtsub.cusid and a.canceldat is null " &_
			...
			"left outer join rtcode i on i.code = g.paytype and i.kind ='M9' " &_
 ...
 If searchQry="" Then
	...
	searchQry2="0"
	searchQry3="0"
	searchShow="本程式只會產生過期用戶(即可用日數小於0)"
	due1S = "1999/1/1"
	due1E = "1999/1/1"
	due2S = "1999/1/1"
	due2E = "1999/1/1"
  ...
  sqlList=	"SELECT a.comq1, a.lineq1, a.cusid, " &_
			...
			"inner join RTLessorAVSCmtyLine b on a.COMQ1 = b.COMQ1 and a.LINEQ1 = b.LINEQ1 " &_
			"inner join RTLessorAVSCmtyH c on a.COMQ1 = c.COMQ1 " &_
			"left outer join RTLessorAVSCustcont g on g.cusid=RTLessorAVSCustBillingPrtsub.cusid " &_
			"left outer join rtcode e on a.paycycle=e.code and e.kind='M8' " &_
			"left outer join rtcode f on a.paytype=f.code and f.kind='M9' " &_
			"left outer join rtcode i on i.code = g.paytype and i.kind ='M9' " &_
			"WHERE 	a.DROPDAT is null and a.CANCELDAT is null and a.FINISHDAT is not null and a.freecode<>'Y' " &_
			"and 	b.DROPDAT is null and b.CANCELDAT is null " &_
			" and g.paytype is null " &_
                        " and case when a.DUEDAT is null then 0 " &_
			"when a.canceldat is not null or a.dropdat is not null then 0 " &_
			"else DATEDiFF(d,getdate(),a.DUEDAT) end < 0 " &_
