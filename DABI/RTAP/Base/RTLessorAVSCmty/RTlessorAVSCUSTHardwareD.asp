...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVScustHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=2 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                       '  RESPONSE.WRITE "select max(entryno) AS entryno from RTLessorAVScustHARDWARE WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "
                         rsc.open "select max(entryno) AS entryno from RTLessorAVScustHARDWARE WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVScustHARDWAREd.asp") then
          rs.open "select max(entryno) AS ENTRYNO from RTLessorAVScustHARDWARE WHERE  CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' " ,conn
          if not rs.eof then
            dspkey(2)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             ...
             &"FROM RTLessorAVScustHARDWARE WHERE cusid='' "
  sqlList="SELECT cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             ...
             &"FROM RTLessorAVScustHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTLessorAVSCUSTSNDWORK WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "
