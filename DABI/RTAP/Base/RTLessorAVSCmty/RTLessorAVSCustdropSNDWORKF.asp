...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlyy="select * FROM RTLessorAVSCustdrop WHERE  CUSID='" & KEY(0) & "' and entryno=" & key(1) 
...
if endpgm="" then
   ...
   sqlxx="select * FROM RTLessorAVSCustdropsndwork WHERE CUSID='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' "
   RSXX.OPEN SQLXX,CONN
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCustdropSndworkF " & "'" & key(0) & "'," & key(1) & ",'" & key(2) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶拆機派工單完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="5" then
       msgbox "此拆機派工單已月結，不可異動" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close      
    elseIF frm1.htmlfld.value="6" then
       msgbox "此拆機派工單完工時，必須先輸入實際拆機人員或實際拆機經銷商" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                  
    ...
    elseIF frm1.htmlfld.value="8" then
       msgbox "客戶已作廢，無法結案(拆機派工單必須作廢)。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close            
   elseIF frm1.htmlfld.value="10" then
       msgbox "找不到此拆機派工單所屬退租單資料" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                 
    elseIF frm1.htmlfld.value="11" then
       msgbox "此拆機派工單所屬退租單資料已作廢，不可執行完工結案作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close               
...
<form name=frm1 method=post action=RTLessorAVSCustdropsndworkf.asp ID="Form1">
