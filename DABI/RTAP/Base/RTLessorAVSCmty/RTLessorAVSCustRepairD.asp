...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE	SQL
		  ...
		  For i	= 0	To numberOfField-1
			  ...
			  '	  On Error Resume Next
				...
				select case	ucase(runpgm)	
				   ...
				   case	ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCUSTRepaird.asp")
					'response.write	"I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
					   if i	<> 1 then rs.Fields(i).Value=dspKey(i)	  
					   if i=1 then
						 Set rsc=Server.CreateObject("ADODB.Recordset")
						 rsc.open "select max(entryno) AS entryno from RTLessorAVSCustRepair where	CUSID='" & DSPKEY(0) & "' "	,conn
	...
	if accessmode ="A" then
	   runpgm=Request.ServerVariables("PATH_INFO")
	   if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustRepairD.asp") then
		  rsc.open "select max(entryno)	AS ENTRYNO from	RTLessorAVSCustRepair where	 CUSID='" &	DSPKEY(0) &	"' " ,conn
		  if not rsC.eof then
...
Sub	SrEnvironment()
  ...
  sqlFormatDB=	"SELECT	 CUSID,	ENTRYNO, APPLYDAT, EQUIP, EQUIPAMT,	SETAMT,	MOVEAMT, " &_
				...
				"FROM	RTLessorAVSCustRepair WHERE	cusid='' "
  sqlList=		"SELECT	 CUSID,	ENTRYNO, APPLYDAT, EQUIP, EQUIPAMT,	SETAMT,	MOVEAMT, " &_
						"PAYTYPE, CREDITCARDTYPE, CREDITBANK, CREDITCARDNO,	CREDITNAME,	" &_
						"CREDITDUEM, CREDITDUEY, TARDAT, BATCHNO, TUSR,	" &_
						"FINISHDAT,	CANCELDAT, CANCELUSR, MEMO,	REALENGINEER, "	&_
						"REALCONSIGNEE,	UDAT, UUSR,	RCVMONEYDAT	" &_
				"from RTLessorAVSCustRepair	WHERE "
  userDefineRead="Yes"		
  userDefineSave="Yes"		 
  userDefineKey="Yes"
  userDefineData="Yes"
...
%>
  ...
  -->
  <span	id="tags1" class=dataListTagOn>AVS-City用戶維修收款資訊</span>
  <!--
