...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   ...
   sqlyy="select max(entryno) as xxentryno FROM RTLessorAVSCustDrop WHERE  CUSID='" & KEY(0) & "'"
 ...
 '  RESPONSE.END
  ...
  ELSE
     '呼叫store procedure更新相關檔案
      strSP="usp_RTLessorAVSCustdropDropC " & "'" & key(0) & "'," & key(1) & ",'" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City用戶退租單資料作廢返轉成功",0
...
<form name=frm1 method=post action=RTLessorAVSCustdropdropc.asp ID="Form1">
