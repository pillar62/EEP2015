...
<% 
  ...
  DSN="DSN=RTLib"
  Conn.Open DSN
  sql="SELECT groupid,groupnc FROM RTsalesgroup WHERE areaid = '" & areaid & "' AND EDATE IS NULL " _
       &"ORDER BY groupnc " 
  rs.Open sql,conn
  s1=""
  Do While Not rs.Eof
     s1=s1 &"<option value=""" &rs("groupid") &""">" & rs("groupid") &rs("groupnc") &"</option>"
     rs.MoveNext
  Loop
  rs.Close    
...
<SCRIPT LANGUAGE="VBScript">
  ...
  Sub cmdSure_onClick()
    ...
    if len(trim(window.document.all("cmdtext").value)) = 0 then
       msgbox "請選擇業務群組!",vbokonly,"錯誤訊息視窗"
    else    
