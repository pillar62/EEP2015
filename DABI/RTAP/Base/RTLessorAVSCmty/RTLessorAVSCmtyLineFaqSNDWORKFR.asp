...
if endpgm="" then
   ...
   ELSE
      ...
      strSP="usp_RTLessorAVSCmtylineFaqSndworkFR " & key(0) & "," & key(1) & ",'" & key(2) & "','" & key(3) & "','" & V(0) & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS-City主線維修派工單結案返轉成功",0
   ...
   elseIF frm1.htmlfld.value="10" then
    ...
    elseIF frm1.htmlfld.value="13" then
       msgbox "找不到主線客服單資料檔，無法返轉。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close            
    elseIF frm1.htmlfld.value="14" then
       msgbox "主線客服單資料已作廢，不可返轉。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                   
    else
...
<form name=frm1 method=post action=RTLessorAVSCmtylineFAQsndworkfr.asp ID="Form1">
