...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineContd.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 2 then rs.Fields(i).Value=dspKey(i)    
                       if i=2 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorAVSCmtyLineCont where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1),conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCmtyLineContd.asp") then
          rs.open "select max(entryno) AS entryno from RTLessorAVSCmtyLineCont where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) ,conn
          if not rs.eof then
            dspkey(2)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,ENTRYNO,CONTAPPLYDAT,APPLYNAME,APPLYSOCIAL,APPLYCONTACTTEL,APPLYMOBILE,LOANCONTACTTEL,LOANMOBILE,LINERATE, " _
             ...
             &"FROM RTLessorAVSCmtyLineCont WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,ENTRYNO,CONTAPPLYDAT,APPLYNAME,APPLYSOCIAL,APPLYCONTACTTEL,APPLYMOBILE,LOANCONTACTTEL,LOANMOBILE,LINERATE,  " _
             ...
             &"PPPOEPASSWORD,HINETNOTIFYDAT,ADSLAPPLYDAT,CLOSEDAT,CLOSEUSR,CANCELDAT,CANCELUSR,MEMO,EUSR,EDAT," _
             &"UUSR, UDAT,LOANNAME,LOANSOCIAL  " _
         &"FROM RTLessorAVSCmtyLineCont WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=0
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">AVS-City主線續約資訊</span>
