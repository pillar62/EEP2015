...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustDropSNDWORKd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 2 then rs.Fields(i).Value=dspKey(i)    
                       if i=2 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         cusidxx="EG" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
                         rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorAVScmty/RTLessorAVSCustDropSNDWORKd.asp") then
          cusidxx="EG" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTLessorAVSCustDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTLessorAVSCustDropSNDWORK WHERE cusid='' "
  sqlList="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTLessorAVSCustDropSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=0
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="12%" class=dataListHead>用戶序號</td>
           ...
           <td width="15%" class=dataListHead>退租序號</td>
           <td width="25%"  bgcolor="silver">
...
<tr><td>&nbsp;</td><td>        
    ...
    <tr>    <td bgcolor="BDB76B" align="center">退租拆機派工單內容</td></tr></table></DIV>
    <DIV ID=SRTAB2 >
...
<td bgcolor="silver" >
<%
  sql="SELECT * FROM   RTLessorAVSCustDrop LEFT OUTER JOIN RTLessorAVScust on RTLessorAVSCustDrop.cusid=RTLessorAVScust.cusid left outer join " _
                     &"RTLessorAVSCmtyLine ON RTLessorAVSCust.COMQ1 = RTLessorAVSCmtyLine.COMQ1 AND " _
                     &"RTLessorAVSCust.LINEQ1 = RTLessorAVSCmtyLine.LINEQ1 WHERE RTLessorAVSCustDrop.cusid='" & DSPKEY(0) & "'" 
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
...
<tr>
  ...
  %>    <input type="text" name="key31" size="6" READONLY value="<%=dspKey(31)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
    ...
    <tr><td bgcolor="BDB76B" align="center">拆機過程說明</td></tr></table></DIV>
   <DIV ID="SRTAB6" > 
