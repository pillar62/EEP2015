...
<% parm=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   sqlxx="select * FROM RTSparqWagalySndWrk WHERE workno='" & parm(0) & "' "
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       'msgbox "AVS主機派工單列印完成",0
       Set winP=window.Opener
       Set docP=winP.document       
       winP.focus()           
       docP.all("keyform").Submit     
       'WINDOW.Form2.action="/REPORT/SparqVoIP/RTSparqWagalySndWrkP.asp?parm=" & document.all("key0").value & ";" 
       WINDOW.Form2.action="/REPORT/SparqVoIP/RTSparqWagalySndWrkP.asp?key=" & "<%=parm(0)%>" &";"& "<%=parm(2)%>" &";"
...
<form name=frm1 method=post action="RTSparqWagalySndWrkPV.asp" ID="Form1">
