...
Sub SrEnvironment()
	...
	title="Sparq 網路電話用戶派工資料維護"
	buttonName=" 新增 ; 刪除 ; 結束 ;重新整理;頁數;功能選項"
	V=split(SrAccessPermit,";")
	AllowA=V(0):AllowU=V(1):AllowD=V(2):AllowP=V(3)
	ButtonEnable=V(0) & ";N;Y;Y;Y;Y"
	if V(0)="Y" then
		functionOptName="作 廢;列 印;結 案;結案返轉"
		functionOptProgram="RTSparqWagalySndWrkDrop.asp;RTSparqWagalySndWrkPV.asp;RTSparqWagalySndWrkDialog.asp;RTSparqWagalySndWrkFR.asp"
		functionOptPrompt="Y;N;N;Y"
	elseif V(3)="Y" then	'工務 only
		functionOptName="列 印;結 案;結案返轉"
		functionOptProgram="RTSparqWagalySndWrkPV.asp;RTSparqWagalySndWrkDialog.asp;RTSparqWagalySndWrkFR.asp"
		functionOptPrompt="N;N;Y"
	else
		functionOptName="列 印"
		functionOptProgram="RTSparqWagalySndWrkPV.asp"
		functionOptPrompt="N"
	end if
	...
	End IF
	DSN="DSN=RTLib"
	formatName="派工單號;客戶代號;none;客戶名稱;派工別;預定<br>施工人;實際<br>施工人;派工人;派工日;結案人;結案日;作廢日"
	sqlDelete=	"select workno, cusid, worktype, cusid, worktype, assigneng, finisheng, " &_
				"sndwrkusr, sndwrkdat, closeusr, closedat, canceldat " &_
				"from	RTSparqWagalySndWrk a " &_
				"where workno='' "
	dataTable="RTSparqWagalyCust"
	userDefineDelete="Yes"
	numberOfKey=3
	dataProg="RTSparqWagalySndWrkD.asp"
	datawindowFeature=""
	...
	searchFirst=FALSE
	'If searchQry="" Then
	'	searchQry=" and a.linkno='" & aryparmkey(0) & "' "
		searchShow="客戶代號︰"& aryparmkey(0) 
	'ELSE
	'   SEARCHFIRST=FALSE
	'End If
	...
	'-------------------------------------------------------------------------------------------
	sqlList="select a.workno, a.cusid, a.worktype, b.cusnc, c.codenc, isnull(g.shortnc, d.name) as assignusr, " &_
			"isnull(i.shortnc, f.name) as finishusr, e.name as sndwrkusr, a.sndwrkdat, j.name as ckoseusr, " &_
			"a.closedat, a.canceldat " &_
			"from	RTSparqWagalySndWrk a " &_
			"inner join RTSparqWagalyCust b on a.cusid = b.cusid " &_
			"left outer join RTCode c on c.code = a.worktype and c.kind ='Q6' " &_
			...
			"left outer join RTEmployee j on j.emply = a.closeusr " &_
			"where a.cusid='" & aryparmkey(0) & "' order by a.sndwrkdat "
