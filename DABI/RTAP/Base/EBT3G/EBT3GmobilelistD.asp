...
Sub SrEnvironment()
  ...
  title="東森行動電話號碼資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT  EBT3GMOBILENO, USEDAT, USECUSID, LOCKUSR, DROPMARKDAT,EUSR,EDAT,UUSR,UDAT " _
             &"from EBT3GMOBILELIST WHERE EBT3GMOBILENO='' "
  sqlList="SELECT  EBT3GMOBILENO, USEDAT, USECUSID, LOCKUSR, DROPMARKDAT,EUSR,EDAT,UUSR,UDAT " _
             &"from EBT3GMOBILELIST WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrCheckData(message,formValid)
  If len(trim(dspkey(0)))=0 or len(trim(dspkey(0))) <> 10 then
       formValid=False
       message="行動電話號碼不可空白或長度必須為10位"    
  end if
...
%>
      <table width="50%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="50%" class=dataListHead>行動電話號碼</td>
           <td width="50%"  bgcolor="silver">
           <input type="text" name="key0"
                 <%=fieldRole(1)%><%=KEYProtect%>  size="15" value="<%=dspKey(0)%>" maxlength="15" class=dataListENTRY></td>
  </tr>
      </table>
<%
End Sub
...
<td>     
    ...
    <tr><td bgcolor="BDB76B" align="LEFT">電話基本資料</td></tr></table></div>
 <DIV ID=SRTAB0 >   
<table width="100%" border=1 cellPadding=0 cellSpacing=0 id="tag1">
<tr>
<td width="10%" class=dataListHEAD>使用日期</td>
    <td width="10%" bgcolor="silver" >
        <input type="text" name="key1" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(1)%>"  READONLY size="10" class=dataListDATA ID="Text48">
      </td>      
    <td width="10%" class=dataListHEAD>使用客戶</td>       
    <td width="20%" bgcolor="silver" >
    <%
    SQLXX="SELECT * from EBT3GCUST WHERE CUSID='" & DSPKEY(2) & "' "
    RS.OPEN SQLXX,CONN
    IF RS.EOF THEN
       CUSNC=""
    ELSE
    ...
    %>
        <input type="text" name="key2" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(2)%>"   size="10" class=dataListDATA ID="Text1"><%=CUSNC%>
      </td>         
      </tr>      
<td  class=dataListHEAD>號碼領取業務</td>
    <td  bgcolor="silver" >
        <%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(DSPKEY(65))) = 0  Then  
       sql="SELECT RTEmployee.EMPLY, RTObj.CUSNC, RTEmployee.AUTHLEVEL FROM RTEmployee LEFT OUTER JOIN " _
          &"RTObj ON RTEmployee.CUSID = RTObj.CUSID WHERE (RTEmployee.DEPT IN ('B100', 'B106', 'B107', 'B200', 'B300', 'B600')) " _
          &"AND (RTEmployee.TRAN2 <> '10') AND (RTEmployee.AUTHLEVEL = '2')  ORDER BY RTObj.CUSNC " 
    ...
    Else
       sql="SELECT RTEmployee.EMPLY, RTObj.CUSNC, RTEmployee.AUTHLEVEL FROM RTEmployee LEFT OUTER JOIN " _
          &"RTObj ON RTEmployee.CUSID = RTObj.CUSID WHERE (RTEmployee.DEPT IN ('B100', 'B106', 'B107', 'B200', 'B300', 'B600')) " _
          &"AND (RTEmployee.TRAN2 <> '10') AND (RTEmployee.AUTHLEVEL = '2')  AND RTEmployee.CUSID='" & DSPKEY(3) & "' "
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("CUSNC")=dspkey(3) Then sx=" selected "
   ...
   <select size="1" name="key3" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select5">                                                                  
    ...
    <td class=dataListHEAD>註銷日期</td>       
    <td  bgcolor="silver" >
      <input type="text" name="key4" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(4)%>"  READONLY size="10" class=dataListDATA ID="Text12">
      </td>         
      </tr>            
<tr>
