...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE SQL
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTlessorCmtyLinefaqd.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(FAQNO) AS FAQNO from RTlessorCmtyLineFAQH where  FAQNO LIKE '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTlessorCmtyLineFAQD.asp") then
          cusidxx="FAH" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(FAQNO) AS FAQNO from RTlessorCmtyLineFAQH where  FAQNO LIKE '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("FAQNO")
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City主線客服資訊</span>
...
<tr>
  ...
  %>         
   ...
   <DIV ID="SRTAB6" > 
    ...
    sqlfaqlist="SELECT * FROM RTlessorCmtyLineFaqList RIGHT OUTER JOIN " _
            &"RTCode ON RTlessorCmtyLineFaqList.faqcod = RTCode.CODE AND COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno ='" & DSPKEY(2) & "'" _
  ...
  %>                               
   ...
   <DIV ID="SRTAB7" > 
    ...
    <TR><TD >
      <%   Set rsxx=Server.CreateObject("ADODB.Recordset")
           sqlfaqlist="SELECT RTlessorCmtyLineFaqSNDWORK.comq1,RTlessorCmtyLineFaqSNDWORK.lineq1,RTlessorCmtyLineFaqSNDWORK.faqno,RTlessorCmtyLineFaqSNDWORK.PRTNO,RTlessorCmtyLineFaqSNDWORK.SENDWORKDAT,RTlessorCmtyLineFaqSNDWORK.memo," _ 
                     ...
                     &"RTlessorCmtyLineFaqSNDWORK.closedat, RTlessorCmtyLineFaqSNDWORK.batchno, " _
                     &"SUM(CASE WHEN RTlessorCmtyLinefaqHARDWARE.dropdat IS NULL AND RTlessorCmtyLinefaqHARDWARE.QTY > 0 " _
                     &"THEN RTlessorCmtyLinefaqHARDWARE.QTY ELSE 0 END) AS amt1, " _
                     &"SUM(CASE WHEN RTlessorCmtyLinefaqHARDWARE.dropdat IS NULL AND RCVPRTNO <> '' " _
                     &"THEN RTlessorCmtyLinefaqHARDWARE.QTY ELSE 0 END) AS amt2, " _
                     &"SUM(CASE WHEN RTlessorCmtyLinefaqHARDWARE.dropdat IS NULL AND RCVPRTNO <> '' AND " _
                     &"RTlessorCmtyLinefaqHARDWARE.rcvfinishdat IS NOT NULL THEN RTlessorCmtyLinefaqHARDWARE.QTY ELSE 0 END) AS amt3 " _
                     &"FROM RTlessorCmtyLineFaqSNDWORK LEFT OUTER JOIN RTObj RTObj_4 ON RTlessorCmtyLineFaqSNDWORK.REALCONSIGNEE = " _
                     &"RTObj_4.CUSID LEFT OUTER JOIN RTEmployee RTEmployee_2 INNER JOIN RTObj RTObj_3 ON " _
                     &"RTEmployee_2.CUSID = RTObj_3.CUSID ON RTlessorCmtyLineFaqSNDWORK.REALENGINEER = RTEmployee_2.EMPLY " _
                     &"LEFT OUTER JOIN RTObj RTObj_2 ON RTlessorCmtyLineFaqSNDWORK.ASSIGNCONSIGNEE = RTObj_2.CUSID " _
                     &"LEFT OUTER JOIN RTEmployee RTEmployee_1 INNER JOIN RTObj RTObj_1 ON RTEmployee_1.CUSID = " _
                     &"RTObj_1.CUSID ON RTlessorCmtyLineFaqSNDWORK.ASSIGNENGINEER = RTEmployee_1.EMPLY LEFT OUTER JOIN " _
                     &"RTObj INNER JOIN RTEmployee ON RTObj.CUSID = RTEmployee.CUSID ON RTlessorCmtyLineFaqSNDWORK.PRTUSR = " _
                     &"RTEmployee.EMPLY LEFT OUTER JOIN RTlessorCmtyLine ON RTlessorCmtyLineFaqSNDWORK.COMQ1 = RTlessorCmtyLine.COMQ1 AND RTlessorCmtyLineFaqSNDWORK.LINEQ1=RTlessorCmtyLine.LINEQ1  " _
                     &"LEFT OUTER JOIN RTlessorCmtyLinefaqHARDWARE ON RTlessorCmtyLineFaqSNDWORK.COMQ1 = " _
                     &"RTlessorCmtyLinefaqHARDWARE.COMQ1 AND RTlessorCmtyLineFaqSNDWORK.LINEQ1=RTlessorCmtyLinefaqHARDWARE.LINEQ1 AND RTlessorCmtyLineFaqSNDWORK.FAQNO=RTlessorCmtyLinefaqHARDWARE.FAQNO AND RTlessorCmtyLineFaqSNDWORK.PRTNO = RTlessorCmtyLinefaqHARDWARE.PRTNO " _
                     &"WHERE RTlessorCmtyLineFaqSNDWORK.COMQ1=" & DSPKEY(0) & " AND RTlessorCmtyLineFaqSNDWORK.LINEQ1=" & DSPKEY(1) & " AND RTlessorCmtyLineFaqSNDWORK.faqno ='" & dspkey(2) & "' and RTlessorCmtyLineFaqSNDWORK.dropdat is null " _
                     &"GROUP BY  RTlessorCmtyLineFaqSNDWORK.comq1,RTlessorCmtyLineFaqSNDWORK.lineq1,RTlessorCmtyLineFaqSNDWORK.faqno,RTlessorCmtyLineFaqSNDWORK.PRTNO, RTlessorCmtyLineFaqSNDWORK.SENDWORKDAT,RTlessorCmtyLineFaqSNDWORK.memo, " _
                     ...
                     &"RTlessorCmtyLineFaqSNDWORK.closedat, RTlessorCmtyLineFaqSNDWORK.batchno  "
           rsxx.open sqlfaqlist,conn
           IF rsxx.eof then
              xxcomq1=0
              xxlineq1=0
      ...
      %>    
       <% if xxprtno <> "" then %>
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr class="dataListsearch"><td colspan=9 align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorcmtylinefaqsndworkk.asp?V=<%=XRND%>&accessMode=U&key=<%=dspkey(0)%>;<%=dspkey(1)%>;<%=dspkey(2)%>;" TARGET="NEWWINDOW" ><font color=white>派工單內容</font></A></td></tr>
    <tr class="dataListHEAD"><td>派工單號</td><td>派工日期</td><td>預定維修人員</td><td>實際維修人員</td><td>派工單結案日</td><td>帳款編號</td><td>設備數量</td><td>轉領用單數量</td><td>已領數量</td></tr>
           ...
           <td><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorcmtylinefaqsndworkD.asp?V=<%=XRND%>&accessMode=U&key=<%=xxcomq1%>;<%=xxlineq1%>;<%=xxfaqno%>;<%=xxprtno%>" TARGET="NEWWINDOW" ><%=xxprtno%></A></td>
           <td><%=xxsendworkdat%></td>
           <td><%=xxassign%></td>
           <td><%=xxREAL%></td>
           <td><%=xxclosedat%></td>
     ...
     <%
    Set rsxx=Server.CreateObject("ADODB.Recordset")
    sqlfaqlist="SELECT * FROM RTlessorCmtyLineFaqSndworkFixCode inner JOIN " _
            &"RTCode ON RTlessorCmtyLineFaqSndworkFixCode.fixcod = RTCode.CODE AND COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno ='" & DSPKEY(2) & "'" _
            &"WHERE RTCODE.KIND = 'A9' "
  '  response.write sqlfaqlist
    rsxx.open sqlfaqlist,conn
    Dtlcnt=0
  ...
  %>                               
    ...
    <% if xxprtno <> "" then %>
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr class="dataListsearch"><td colspan=9 align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorcmtylinefaqhardwarek2.asp?V=<%=XRND%>&accessMode=U&key=<%=dspkey(0)%>;<%=dspkey(1)%>;<%=dspkey(2)%>;" TARGET="NEWWINDOW" ><font color=white>派工設備資料明細</font></A></td></tr>
    <tr class="dataListHEAD"><td>派工單號</td><td>項次</td><td>設備名稱/規格</td><td>數量</td><td>單價</td><td>金額</td><td>領用單號</td><td>領用結案日</td></tr>
    <%
           Set rsxx=Server.CreateObject("ADODB.Recordset")
           sqlfaqlist="SELECT " _
                     &"RTlessorCmtyLineFaqHARDWARE.comq1,RTlessorCmtyLineFaqHARDWARE.lineq1,RTlessorCmtyLineFaqHARDWARE.faqno,RTlessorCmtyLineFaqHARDWARE.prtno,RTlessorCmtyLineFaqHARDWARE.seq, RTProdH.PRODNC + '--' + RTProdD1.itemnc + '('+ RTProdD1.SPEC+')' as pdname, " _
                     &"RTlessorCmtyLineFaqHARDWARE.QTY, RTlessorCmtyLineFaqHARDWARE.amt,RTlessorCmtyLineFaqHARDWARE.QTY * RTlessorCmtyLineFaqHARDWARE.amt as totamt, " _
                     &"RTlessorCmtyLineFaqHARDWARE.BATCHNO,RTlessorCmtyLineFaqHARDWARE.TARDAT,RTlessorCmtyLineFaqHARDWARE.rcvprtno," _
                     &"RTlessorCmtyLineFaqHARDWARE.rcvfinishdat FROM RTProdH RIGHT OUTER JOIN " _
                     &"RTlessorCmtyLineFaqHARDWARE LEFT OUTER JOIN RTObj INNER JOIN RTEmployee ON " _
                     &"RTObj.CUSID = RTEmployee.CUSID ON RTlessorCmtyLineFaqHARDWARE.DROPUSR = RTEmployee.EMPLY " _
                     &"LEFT OUTER JOIN HBwarehouse ON RTlessorCmtyLineFaqHARDWARE.WAREHOUSE = HBwarehouse.WAREHOUSE " _
                     &"LEFT OUTER JOIN RTProdD1 ON RTlessorCmtyLineFaqHARDWARE.PRODNO = RTProdD1.PRODNO AND " _
                     ...
                     &"WHERE RTlessorCmtyLineFaqHARDWARE.COMQ1=" & dspkey(0) & " and RTlessorCmtyLineFaqHARDWARE.LINEQ1=" & DSPKEY(1) & " and "  _
                     &"RTlessorCmtyLineFaqHARDWARE.faqno='" & dspkey(2) & "' and " _
                     &"RTlessorCmtyLineFaqHARDWARE.prtno='" & xxprtno & "' and RTlessorCmtyLineFaqHARDWARE.dropdat is null order by RTlessorCmtyLineFaqHARDWARE.seq " 
           'response.write sqlfaqlist
           rsxx.open sqlfaqlist,conn
           do until rsxx.eof %>
           <tr class="dataListentry">
...
'------ RTObj ---------------------------------------------------
    DELFAQlist="delete from RTlessorCmtyLinefaqlist where COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno='" & dspkey(2) & "'"
    conn.Execute DELFAQlist  
    For i=0 to 99
        if len(trim(extdb(i))) > 0  then
           rs.Open "SELECT * FROM RTlessorCmtyLinefaqlist WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno='" &dspKey(2) &"' and faqcod='" & extDB(i) & "'" ,conn,3,3
           If rs.Eof Or rs.Bof Then
              rs.AddNew
              rs("COMQ1")=dspKey(0)
              rs("LINEQ1")=dspKey(1)
