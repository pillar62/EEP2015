...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLessorcmty/RTlessorCustFaqSNDWORKd.asp")
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTlessorCustFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTlessorCustFaqSNDWORKd.asp") then
          cusidxx="EF" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTlessorCustFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTlessorCustFaqSNDWORK WHERE cusid='' "
  sqlList="SELECT cusid,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTlessorCustFaqSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=99
...
<%
 ...
 %>
        ...
        <input type="TEXT" name="key5" <%=fieldRole(1)%><%=dataProtect%> 
               ...
               value="<%=dspKey(5)%>"  <%=fieldpa%> readonly class="dataListENTRY">
           <input type="BUTTON" id="B5" name="B5" width="100%" style="Z-INDEX: 1"  value="...."  <%=FIELDP6%> >
...
<tr><td  class=dataListHead>預定施工經銷商</td>
    <td  bgcolor="silver" >
       ...
       sql="SELECT RTObj.CUSID,RTObj.SHORTNC " _
          ...
          &"WHERE (RTConsigneeCASE.CASEID = '07') " 
...
<tr>
  ...
  %>    <input type="text" name="key29" size="6" READONLY value="<%=dspKey(29)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        ...
        <td  height="23" bgcolor="silver">
        <input type="text" name="key30" size="25" READONLY value="<%=dspKey(30)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA">
        </td>       
 </tr>  
<tr>
  ...
  %>    <input type="text" name="key31" size="6" READONLY value="<%=dspKey(31)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        ...
        <td  height="23" bgcolor="silver">
        <input type="text" name="key32" size="25" READONLY value="<%=dspKey(32)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>     
  </table> 
  </DIV>    
   ...
   <DIV ID="SRTAB6" > 
    ...
    sqlfaqlist="SELECT * FROM RTLessorCustFaqSndworkFixCode RIGHT OUTER JOIN " _
            &"RTCode ON RTLessorCustFaqSndworkFixCode.FIXcod = RTCode.CODE AND faqno ='" & DSPKEY(1) & "' AND PRTNO='" & DSPKEY(2) & "' "  _
...
'------ RTObj ---------------------------------------------------
    DELFAQlist="delete from RTLessorCustFaqSndworkFixCode where CUSID='" & dspkey(0) & "' AND faqno='" & dspkey(1) & "' AND PRTNO='" & DSPKEY(2) & "' "
    conn.Execute DELFAQlist  
    For i=0 to 99
        if len(trim(extdb(i))) > 0  then
           rs.Open "SELECT * FROM RTLessorCustFaqSndworkFixCode WHERE CUSID='" & dspkey(0) & "' AND faqno='" &dspKey(1) &"' AND PRTNO='" & DSPKEY(2) & "' and fIXcod='" & extDB(i) & "'" ,conn,3,3
           If rs.Eof Or rs.Bof Then
              rs.AddNew
              rs("cusid")=dspKey(0)
              rs("faqno")=dspKey(1)
