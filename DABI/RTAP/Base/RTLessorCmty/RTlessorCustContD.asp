...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE SQL
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTlessorCUSTcontd.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorCustcont where  CUSID='" & DSPKEY(0) & "' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTLessorCustcontD.asp") then
          rsc.open "select max(entryno) AS ENTRYNO from RTLessorCustcont where  CUSID='" & DSPKEY(0) & "' " ,conn
          if not rsC.eof then
...
'       message="信用卡發卡銀行不可空白"         
  ...
  elseif dspkey(3)="02" and ( dspkey(4)< 11 or dspkey(4)>18 )  then
       formValid=False
       message="期數範圍有異常，年繳用戶範圍應介於12-18個月"       
 ...
 IF formValid=TRUE THEN
   ...
   connxx.open DSN
   sqlxx="select duedat,dropdat from rtlessorcust where cusid='" & aryparmkey(0) & "'"
 ...
 IF formValid=TRUE and accessMode="A" THEN
   ...
   Set rsxx=Server.CreateObject("ADODB.Recordset")
   connxx.open DSN
   sqlxx="select count(*) as cnt from rtlessorcustCont where cusid='" & aryparmkey(0) & "' and finishdat is null and canceldat is null "
   rsxx.Open sqlxx,connxx
   if rsxx("cnt") > 0 then
      formValid=False
 ...
 IF formValid=TRUE and accessMode="A" THEN
   ...
   Set rsxx=Server.CreateObject("ADODB.Recordset")
   connxx.open DSN
   sqlxx="SELECT FINISHDAT FROM RTLESSORCUST WHERE CUSID='" & DSPKEY(0) & "'"
   RSxx.OPEN SQLxx,CONNxx
     ...
     ELSEIF S27="02" THEN
        select case s5X
          case false
               ...
               END IF
               if s7 = "06" then 
                  document.all("key6").value = 2000
				  document.All("key4").value = 5
			   end if 
          CASE ELSE
               '半年繳
               IF S3="01" THEN
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City用戶續約資訊</span>
          <FONT SIZE=2 COLOR=RED>
          <MARQUEE width=250 behavior=slide scrolldelay=30 behavior=alternate>
          <%
          
          SQLYY="SELECT FINISHDAT FROM RTLESSORCUST WHERE CUSID='" & DSPKEY(0) & "'"
          RS.OPEN SQLYY,CONN
...
<%  dim FREECODE1,FREECODE2
    ...
    If dspKey(5)="N" Then FREECODE2=" checked " %>                          
        ...
        <td  WIDTH="23%" height="23" bgcolor="silver" >   
             <input type="text" name="key6"  size="8"  value="<%=dspKey(6)%>"  <%=fieldRole(1)%> class="dataListentry" ID="Text56">
        </TD>      
      </TR>
    <tr>
   ...
   <DIV ID="SRTAG4" onclick="srtag4" style="cursor:hand">
    ...
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
        ...
        if len(trim(dspkey(26)))=0 then
           sql="select duedat from rtlessorcust where cusid='" & aryparmkey(0) & "'"
           rs.open sql,conn
           if rs.eof then
           else
