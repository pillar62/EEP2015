...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '     IF LEN(TRIM(RSYY("strbillingDAT"))) > 0 or LEN(TRIM(RSYY("newbillingDAT"))) > 0 THEN ENDPGM="7"
   ...
   RSYY.CLOSE
   '檢查應收帳款檔是否已沖帳
   sqlyy="select * FROM RTLessorCUSTAR WHERE CUSID='" & KEY(0) & "' AND BATCHNO='" & BATCHNOX & "'"
   rsyy.open sqlyy,conn
   if rsyy.eof THEN
   ELSE
   ...
   IF ENDPGM="" THEN
     ...
     ELSE
        ...
        strSP="usp_RTLessorCustARSndworkFR " & "'" & key(0) & "'" & ",'" & key(1) & "','" & V(0) & "'" & ",'" & batchnoxx & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "ET-City用戶裝機派工單完工/未完工結案返轉成功",0
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "無法找到此裝機派工單之用戶主檔資料，請確認ET-City用戶主檔資料正常" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                     
...
<form name=frm1 method=post action=RTLessorcustsndworkfr.asp ID="Form1">
