...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlyy="select count(*) as cnt FROM RTlessorCUSTConthardware WHERE CUSID='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' and dropdat is null and RCVPRTNO <> '' "
   ...
   ELSE
      ...
      strSP="usp_RTLessorCustContARSndworkUF " & "'" & key(0) & "'" & "," & key(1) & ",'" & key(2) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "ET-City用戶收款派工單未完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="6" then
       ...
       msgbox "無法執行收款派工單未完工結案作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=rtlessorcustcontsndworkUf.asp ID="Form1">
