...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTlessorCUSTd.asp")
                  '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(cusid) AS cusid from RTLessorCust where cusid like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTLessorCustD.asp") then
          cusidxx="E" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(cusid) AS cusid from RTLessorCust where cusid like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("CUSID")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT  COMQ1, LINEQ1, CUSID, CUSNC, SOCIALID, CUTID1, TOWNSHIP1, RADDR1, RZONE1, CUTID2, TOWNSHIP2," _
             ...
             &"ip12,ip13,ip14,CPEKIND " _
             &"from RTLessorCust WHERE COMQ1=0 "
...
Sub SrCheckData(message,formValid)
  ...
  if len(trim(dspkey(60)))=0 then dspkey(60)=""
  if len(trim(dspkey(67)))=0 then dspkey(67)=""
  if len(trim(dspkey(49)))=0 then dspkey(49)=0
  if len(trim(dspkey(54)))=0 then dspkey(54)=0
  if len(trim(dspkey(68)))=0 then dspkey(68)=0
  if len(trim(dspkey(69)))=0 then dspkey(69)=0
  if len(trim(dspkey(72)))=0 then dspkey(72)="N"
  ...
  elseif len(trim(dspkey(3)))=0 then
       ...
       message="用戶名稱不可空白"          
  '公關機時不檢查身份證
  elseif len(trim(dspkey(51)))=0 AND len(trim(dspkey(4)))<>0 AND dspKey(38)<> "Y" then
       formValid=False
       message="第一證照別不可空白"             
  elseif ( len(trim(dspkey(4)))<>0 and (len(trim(dspkey(4)))<>10 and len(trim(dspkey(4)))<>8 ) ) AND dspKey(38)<> "Y" then
       formValid=False
       message="用戶身分證(統編)長度不對"    
 ' elseif len(trim(dspkey(52)))=0 AND len(trim(dspkey(53)))<>0 AND dspKey(38)<> "Y" then
 '      formValid=False
 '      message="第二證照別不可空白"         
 ' elseif len(trim(dspkey(53)))=0  AND dspKey(38)<> "Y" then
 '      formValid=False
 '      message="第二證照號碼不可空白"                
 ' elseif len(trim(dspkey(17)))=0 AND COMPANT="N" AND dspKey(38)<> "Y" then
 '      formValid=False
 '      message="出生日期不可空白"             
  ...
  elseif len(trim(dspkey(14)))=0 and dspkey(13)<>"06" and dspkey(13)<>"15" then
       formValid=False
...
'       message="接續設備網路port號不為空白時，必須輸入上層設備網路編號"                  
  elseif len(trim(dspkey(58)))=0  AND dspKey(38)<> "Y" then
       formValid=False
       message="繳費週期不可空白"               
  elseif len(trim(dspkey(49)))=0 or dspkey(49)=0  AND dspKey(38)<> "Y" then
       formValid=False
       message="期數不可空白或0"                     
  elseif len(trim(dspkey(28)))=0 AND dspKey(38)<> "Y" then
       formValid=False
       message="繳費方式不可空白"        
...
'       message="信用卡發卡銀行不可空白"         
  ...
  elseif len(trim(dspkey(49)))=0 AND len(trim(dspkey(33)))<>0 AND dspKey(38)<> "Y" then
       formValid=False
 ...
 '      message="用戶CPE設備種類不可空白"             
  ...
  IF formValid=TRUE THEN
   ...
   connxx.open DSN
   sqlxx="select * from RTLessorCmtyLine where comq1=" & aryparmkey(0) & " AND LINEQ1=" & ARYPARMKEY(1)
 ...
 IF formValid=TRUE and (dspkey(66) <> "" or dspkey(67) <> "" )THEN
    ...
    Set rsxx=Server.CreateObject("ADODB.Recordset")
    connxx.open DSN
    SQLXX="Select count(*) as cnt from RTLessorCust where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " and port='" & dspkey(66) & "' and equip='" & dspkey(67) & "' and cusid<>'" & dspkey(2) & "' " 
    rsxx.open sqlxx,connxx
    if rsxx("cnt") > 0 then
       formValid=False
 ...
 IF formValid=TRUE THEN
    ...
    '(新增時)
    if accessmode ="A" then
       sqlxx="select * from RTLessorCust where cusnc='" & dspkey(3) & "' AND COMQ1=" & DSPKEY(0) & " and canceldat is null and secondcase<>'Y' "
    '(修改時--需排除本身資料)
    else
       sqlxx="select * from RTLessorCust where cusnc='" & dspkey(3) & "' AND COMQ1=" & DSPKEY(0) & " and canceldat is null and secondcase<>'Y' and cusid<>'" & dspkey(2)& "'"
    end if
    rsxx.Open sqlxx,connxx
    '判斷有無主約資料
    if rsxx.eof then
...
Sub srrecalulate()
  ' msgbox document.All("key68").value
   ...
   elseif document.All("key33").value <> "" then
      ...
      IF document.All("key33").value <> "" THEN
         document.All("key50").value = dateadd("m",document.All("key49").value,document.All("key33").value)
         '日期減1
         document.All("key50").value = dateadd("d",-1,document.All("key50").value)
         document.All("key50").value = dateadd("d",document.All("key68").value,document.All("key50").value)    
   ...
   Sub srChangeMoney()
       '開始計費日
       S33=document.all("key33").value
       '續約開始計費日
       S65=document.all("key65").value
       '無開始計費日時才可以重算應收金額
       IF len(trim(s33))=0 and len(trim(s65))=0 then
       '繳費週期
       s58=document.all("key58").value
       '第二戶
       s72X=document.all("key72X").CHECKED
    '   MSGBOX document.all("key72X").CHECKED
    '   MSGBOX document.all("key72Y").CHECKED
      ' s72Y=document.all("key72Y").value
       select case s72X
          case false
               '半年繳
               IF s58="01" THEN
                  document.All("key69").value = 2394
               '年繳
               ELSEIF s58="02" THEN
                  document.all("key69").value = 5000
               '二年繳
               ELSEIF s58="03" THEN
                  document.all("key69").value = 9500
               '季繳
               ELSEIF s58="05" THEN
                  document.all("key69").value = 1197
               '三年繳
               ELSEIF s58="04" THEN
                  document.all("key69").value = 14000                 
               ELSE
          ...
          CASE ELSE
            '半年繳
               IF s58="01" THEN
                  document.all("key69").value = 600
               '年繳
               ELSEIF s58="02" THEN
                  document.all("key69").value = 1200
               '二年繳
               ELSEIF s58="03" THEN
                  document.all("key69").value = 2400
               '季繳
               ELSEIF s58="05" THEN
                  document.all("key69").value = 300
               '三年繳
               ELSEIF s58="04" THEN
                  document.all("key69").value = 3600       
               ELSE
                  document.all("key69").value = 0                                                         
               END IF          
...
' -------------------------------------------------------------------------------------------- 
 ...
 '   End If
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City用戶資訊</span>
...
<%  
    ...
    ELSE
       FREECODE2=" checked "    
    END IF %>                          
        <input type="radio" ID="KEY72X" value="Y" <%=FREECODE1%> name="key72" <%=fieldRole(1)%><%=dataProtect%> ID="Radio1" onCLICK="Srchangemoney()">是
        <input type="radio" ID="KEY72Y" name="key72" value="N" <%=FREECODE2%><%=fieldRole(1)%><%=dataProtect%> ID="Radio2" onCLICK="Srchangemoney()">否</td>               
...
<%  dim FREECODE1,FREECODE2
    ...
    If dspKey(38)="N" Then FREECODE2=" checked " %>                          
        <input type="radio" value="Y" <%=FREECODE1%> name="key38" <%=fieldRole(1)%><%=dataProtect%> ID="Radio1">是
        <input type="radio" name="key38" value="N" <%=FREECODE2%><%=fieldRole(1)%><%=dataProtect%> ID="Radio2">否</td> </TR>   
...
<td width="10%" class=dataListHEAD>身分證(統編)</td>
    <td width="23%" bgcolor="silver" >
	<select size="1" name="key51"<%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry"><%=s%></select>    
...
<%
  ...
  '  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then 
   ...
   %>        
		...
		<select size="1" name="key52"<%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select16"><%=s%></select>	
        <input type="text" name="key53" size="15" maxlength="12" value="<%=dspkey(53)%>" <%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Text49"></td> 
     </tr>    
<tr>                                 
        <td  class="dataListHEAD" height="23">出生日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key17" size="10"  value="<%=dspKey(17)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListentry" ID="Text8">  
        <input type="button" id="B17"  <%=fieldpb%>  name="B17" height="100%" width="100%" style="Z-INDEX: 1" value="...." onclick="SrBtnOnClick"> 
        <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"  <%=fieldpb%> alt="清除" id="C17"  name="C17"   style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" onclick="SrClear">          
        </td>                                 
        <td  class="dataListHEAD" height="23">連絡電話(白天)</td>                                 
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key18" size="15" maxlength="15" value="<%=dspKey(18)%>" <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text16"></td>                                 
        <td  class="dataListHEAD" height="23">行動電話</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key19" size="15" maxlength="15" value="<%=dspKey(19)%>" <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text9"></td>                                 
 </tr>     
 <tr> 
       <td width="10%" class=dataListHEAD>聯絡Email</td>
    <td width="23%" bgcolor="silver"  >
      <input type="text" name="key20" size="30" maxlength="50" value="<%=dspKey(20)%>" <%=fieldpa%> <%=fieldRole(1)%> class="dataListEntry" ID="Text9"> 
     </td>
            <td width="10%" class=dataListSEARCH>用戶CPE設備種類</td>
  ...
  '  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then 
  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) Then 
       sql="SELECT Code,CodeNC FROM RTCode WHERE KIND ='C4' " 
    ...
    Else
       sql="SELECT Code,CodeNC FROM RTCode WHERE KIND='C4' AND CODE='" & dspkey(76) &"' " 
       'SXX60=""
    End If
    rs.Open sql,conn
    s=""
    s=s &"<option value=""" &"""" &sx &">(CPE設備)</option>"
    sx=""
    Do While Not rs.Eof
    ...
    rs.Close
   %>               
        <select size="1" name="key76" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select16"><%=s%></select>
...
<tr><td class=dataListHEAD>戶籍(公司)地址</td>
  ...
  <%s=""
   ...
   %>
        ...
        <input type="text" name="key6" readonly  size="8" value="<%=dspkey(6)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4"><font SIZE=2>(鄉鎮)                 
        ...
        <input type="text" name="key7"  size="50" value="<%=dspkey(7)%>" maxlength="60"  <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListENTRY" ID="Text4">
        <input type="text" name="key8" readonly  size="5" value="<%=dspkey(8)%>" maxlength="5" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
        </td>                                 
</tr>  
<tr><td class=dataListHEAD>安裝地址</td>
  ...
  '  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then 
    ...
    rs.Close
   %>
         <select size="1" name="key9" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select2" onchange="Srchangetelzip"><%=s%></select>
        <input type="text" name="key10" readonly  size="8" value="<%=dspkey(10)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4"><font SIZE=2>(鄉鎮)                 
        ...
        <input type="text" name="key11"  size="50" value="<%=dspkey(11)%>" maxlength="60"  <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListENTRY" ID="Text4">
        <input type="text" name="key12" readonly  size="5" value="<%=dspkey(12)%>" maxlength="5" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
        <input type="radio" name="rd1"  <%=fieldpb%> onClick="SrAddrEqual1()"><font SIZE=2>同戶籍(公司)地址</font></td>                                 
</tr>  
<tr><td class=dataListHEAD>帳單地址</td>
  ...
  '  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then 
    ...
    rs.Close
   %>
         <select size="1" name="key13" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select2" onchange="Srchangetelzip"><%=s%></select>
        <input type="text" name="key14" readonly  size="8" value="<%=dspkey(14)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4"><font SIZE=2>(鄉鎮)                 
         <input type="button" id="B14" <%=fieldpb%> name="B14"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX14%>  >        
        ...
        <input type="text" name="key15"  size="50" value="<%=dspkey(15)%>" maxlength="60"  <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListENTRY" ID="Text4">
        <input type="text" name="key16" readonly  size="5" value="<%=dspkey(16)%>" maxlength="5" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
        <input type="radio" name="rd2"  <%=fieldpb%> onClick="SrAddrEqual2()"><font SIZE=2>同戶籍(公司)地址</font><input type="radio" name="rd2"  <%=fieldpb%> onClick="SrAddrEqual3()"><font SIZE=2>同裝機地址</font></td>                                 
</tr>  
<tr>                                 
        <td  class="dataListHEAD" height="23">企業連絡人</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key21" size="12" maxlength="12" value="<%=dspKey(21)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text7">
       </td>                                 
        <td  class="dataListHEAD" height="23">企業連絡人電話</td>                                 
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key22" size="15" maxlength="15" value="<%=dspKey(22)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text11">                                
        <FONT SIZE=2>分機︰</FONT>
        <input type="text" name="key23" size="5" maxlength="5" value="<%=dspKey(23)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text11"></td>                                 
        <td  class="dataListHEAD" height="23">連絡人行動電話</td>                                 
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key24" size="17" maxlength="15" value="<%=dspKey(24)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text11"></td>                                 
        
 </tr>        
<TR>        
        <td  class="dataListHEAD" height="23">企業負責人</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key25" size="12" maxlength="12" value="<%=dspKey(25)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text7">
        <td  class="dataListHEAD" height="23">負責人身分證</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key26" size="10" maxlength="10" value="<%=dspKey(26)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text7">
...
<tr>
        ...
        <%  name="" 
           if dspkey(40) <> "" then
              sql=" select rtobj.cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
                   &"where rtemployee.emply='" & dspkey(40) & "' "
              rs.Open sql,conn
              if rs.eof then
                 name=""
              else
  ...
  %>    <input type="text" name="key40" size="6" READONLY value="<%=dspKey(40)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
        <td  class="dataListHEAD" height="23">建檔日期</td>                                 
        <td  height="23" bgcolor="silver" colspan=3>
        <input type="text" name="key41" size="10" READONLY value="<%=dspKey(41)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>  
<tr>
  ...
  %>    <input type="text" name="key42" size="6" READONLY value="<%=dspKey(42)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
        <td  class="dataListHEAD" height="23">修改日期</td>                                 
        <td  height="23" bgcolor="silver" colspan=3>
        <input type="text" name="key43" size="10" READONLY value="<%=dspKey(43)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>         
</table> </div>
<!--
	...
	<tr><td id="tagT1" WIDTH="10%" class="dataListHEAD" height="23">業務轄區</td>
        <td WIDTH="35%" height="23" colspan=3 bgcolor="silver">
<%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1    Then 
    ...
    rs.Close        
    %>    
           <select size="1" name="key34" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry" ID="Select1">                                            
              <%=s%>
           </select>
    <input type="text" name="key35" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> 
         ...
         <input type="button" id="B35"  <%=fieldpb%>  name="B35"   width="100%" style="Z-INDEX: 1"  value="...." readonly onclick="SrsalesGrouponclick()"  >  
          <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"  <%=fieldpb%> alt="清除" id="C35"  name="C35"   style="Z-INDEX: 1" border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut"  onclick="SrClear">                               
    <input type="TEXT" name="key36" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> 
               style="text-align:left;" size="15" maxlength="10" 
               value="<%=dspKey(36)%>"  readonly class="dataListentry" ID="Hidden1">
           <input type="BUTTON" id="B36"  <%=fieldpb%>  name="B36"  width="100%" style="Z-INDEX: 1"  value="...." onclick="Srsalesonclick()"  >   
           <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"  <%=fieldpb%> alt="清除" id="C36"  name="C36"   style="Z-INDEX: 1" border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut"  onclick="SrClear"></td>
...
%>
       ...
       <tr>
        ...
        <td width="23%" bgcolor="silver">
          <input type="text" name="key54" size="10" value="<%=dspKey(54)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="10"></td>
<%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then     
    ...
    rs.Close
   %>
        <td width="10%" class="dataListHead">保證金年限</td>                    
        <td width="23%" bgcolor="silver">
          <select name="key55" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%> size="1"
                  maxlength="8" class="dataListEntry"><%=s%></select></td>
        <td width="10%" class="dataListHead">保證金序號</td>
        <td width="23%" bgcolor="silver">
          <input type="text" name="key56" size="15" value="<%=dspKey(56)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="15"></td>
	  </tr>      
  </table>     
  </DIV> 
    </DIV>   
 <DIV ID="SRTAG2" onclick="srtag2" style="cursor:hand">
  ...
  <td  WIDTH="10%"  class="dataListHEAD" height="23">用戶IP</td>               
        <td  WIDTH="23%" height="23" bgcolor="silver" >
        <input type="text" name="key47" size="3" READONLY value="<%=dspKey(47)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="3">.
        <input type="text" name="key73" size="3" READONLY value="<%=dspKey(73)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="3">.
        <input type="text" name="key74" size="3" READONLY value="<%=dspKey(74)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="3">.
        <input type="text" name="key75" size="3" READONLY value="<%=dspKey(75)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="3">
        </TD>                        
  <td  WIDTH="10%"  class="dataListHEAD" height="23">用戶電腦MAC</td>               
        <td  WIDTH="23%" height="23" bgcolor="silver" >
        <input type="text" name="key48" size="17" value="<%=dspKey(48)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="17">
        </TD>
  <td  WIDTH="10%"  class="dataListHEAD" height="23">接續設備代號</td>               
        <td  WIDTH="23%" height="23" bgcolor="silver" >
        <%  set conn=server.CreateObject("ADODB.Connection")
    set rs=server.CreateObject("ADODB.recordset")
    conn.Open dsn
    ...
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false))  AND ERRSTOCKX="" Then 
       sql="SELECT HOSTNO FROM RTLessorCmtyLineHardware WHERE (DROPDAT IS NULL) AND (HOSTNO <> '') AND " _
          &"HOSTNO<>'" & DSPKEY(67) & "' and COMQ1 ="  & dspkey(0) & " AND LINEQ1=" & DSPKEY(1) & " " _
          &"UNION " _
          &"SELECT HOSTNO FROM RTLessorCmtyLineFAQHardware WHERE (DROPDAT IS NULL) AND (HOSTNO <> '') AND " _
          &"HOSTNO<>'" & DSPKEY(67) & "' and COMQ1 ="  & dspkey(0) & " AND LINEQ1=" & DSPKEY(1) & " " 
   ...
   ' RESPONSE.WRITE SQL
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("HOSTNO")=dspkey(67) Then sx=" selected "
    ...
    rs.Close
    %>           
    <select  name="key67" <%=fieldpB%> <%=dataProtect%> class=dataListEntry size="1" 
            style="text-align:left;" maxlength="8" ID="Select7"><br><%=s%></select>  
        <FONT SIZE=2>PORT位︰</FONT>
        <input type="text" name="key66" size="3"  value="<%=dspKey(66)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="3">
        </TD>         
        </TR>
     </TABLE>
  </DIV>   
    ...
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table2">
    <tr>    <td bgcolor="BDB76B" align="CENTER">方案內容</td></tr></table></DIV>
    <DIV ID=SRTAB3 >
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
     <tr>
...
<%
   ...
   ' If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  and len(trim(dspkey(33)))=0 Then  
     If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1   Then  
       sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='M8' " 
...
<%
   ...
   <select size="1" name="key28" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35" onchange="SrSHOWcreditcardOnClick">                                                                  
        <%=s%>
         ...
         <td  WIDTH="10%"  class="dataListSEARCH" height="23">當期收款金額</td>               
        <td  WIDTH="23%" height="23" bgcolor="silver" >   
        <%
        IF DSPKEY(69)=0 OR LEN(TRIM(DSPKEY(69)))=0 THEN
           IF DSPKEY(72)="Y" THEN
               IF DSPKEY(58)="01" THEN
                  DSPKEY(69) = 600
               '年繳
               ELSEIF DSPKEY(58)="02" THEN
                  DSPKEY(69) = 1200
               '二年繳
               ELSEIF DSPKEY(58)="03" THEN
                  DSPKEY(69) = 2400
               '季繳
               ELSEIF DSPKEY(58)="05" THEN
                  DSPKEY(69) = 300
               '三年繳
               ELSEIF DSPKEY(58)="04" THEN
                  DSPKEY(69) = 3600       
               ELSE
                  DSPKEY(69) = 0                                                         
               END IF
        ...
        END IF
        %>
             <input type="text" name="key69"  size="8" READONLY value="<%=dspKey(69)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text56">
        </TD>
  </tr>          
    <tr>
    <td colspan=6>
...
<%
  ...
  '  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  and len(trim(dspkey(33)))=0  Then  
   ...
   <select size="1" name="key60" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35">                                                                  
        ...
        <td WIDTH="23%" height="23" bgcolor="silver" >     
        <input type="text" name="key61" size="16" value="<%=dspKey(61)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="16">
        </TD>     
        </tr>    
   <TR>
   ...
   <TR>
        ...
        <td   height="23" bgcolor="silver" COLSPAN=3>     
        <input type="text" name="key71" size="10" value="<%=dspKey(71)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="10">
        </TD>     
   ...
   <DIV ID="SRTAG4" onclick="srtag4" style="cursor:hand">
    ...
    <tr><td bgcolor="BDB76B" align="CENTER">用戶申請、異動及施工進度狀態</td></tr></table></DIV>
    <DIV ID=SRTAB4 >  
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
        <tr>
        <td  WIDTH="10%" class="dataListHEAD" height="23">完工日期</td>                                 
        <td  WIDTH="23%" height="23" bgcolor="silver" >
        <input type="text" name="key31" size="10" READONLY value="<%=dspKey(31)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text57">

                <input type="button" id="B31"  name="B31" height="100%" width="100%" style="Z-INDEX: 1" value="...." onclick="SrBtnOnClick">
        ...
        <td  WIDTH="23%" height="23" bgcolor="silver" STYLE="DISPLAY:NONE">
        <input type="text" name="key32" STYLE="DISPLAY:NONE" size="10" READONLY value="<%=dspKey(32)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text57">
        </td>        
        <td  WIDTH="10%" class="dataListHEAD" height="23">開始計費日</td>                                 
        <td  WIDTH="23%" height="23" bgcolor="silver" >
        <input type="text" name="key33" size="10" READONLY value="<%=dspKey(33)%>" <%=fieldpC%> <%=fieldRole(1)%> class="dataListentry" ID="Text56" >
   ...
   <td  WIDTH="10%" class="dataListHEAD" height="23">最近續約計費日</td>                                 
        <td  WIDTH="23%" height="23" bgcolor="silver">
        <input type="text" name="key65" size="10" READONLY value="<%=dspKey(65)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text56">
...
<tr> 
       <td  WIDTH="10%"  class="dataListHEAD" height="23">可使用期數</td>               
        <td  WIDTH="23%" height="23" bgcolor="silver" >   
        <input type="text" name="key49" size="3"  value="<%=dspKey(49)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListENTRY" maxlength="3" onchange="srrecalulate()">
        ...
        <td  WIDTH="23%" height="23" bgcolor="silver">
                <input type="text" name="key50" size="10" READONLY value="<%=dspKey(50)%>" <%=fieldRole(1)%> class="dataListDATA" ID="Text56"></TD>
        ...
        %>
          <input type="text" name="remain"  size="10" READONLY value="<%=remainday%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text56">
          <FONT SIZE=2> 天</FONT></TD>
   
</tr> 
       <tr>
       <td WIDTH="10%"  class="dataListHEAD" height="23">退租日</td>                                 
        <td WIDTH="23%"  height="23" bgcolor="silver" >
        <input type="text" name="key37" size="10" READONLY value="<%=dspKey(37)%>" <%=fieldpe%> <%=fieldRole(1)%> class="dataListdata" ID="Text56">
      <!--  <input type="button" id="B37"  name="B37" height="100%" width="100%" <%=fieldpf%>style="Z-INDEX: 1" value="...." onclick="SrBtn33OnClick">
        ...
        <font size=2>欠費︰</font> 
        <input type="text" name="key39" size="2" READONLY value="<%=dspKey(39)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text48">  
       </td>               
        <td   class="dataListHEAD" height="23">作廢日期</td>                                 
        <td   height="23" bgcolor="silver">
        <input type="text" name="key45" size="10" value="<%=dspKey(45)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListdata" ID="Text41">
         </td>
        <%  name="" 
           if dspkey(46) <> "" then
              sql=" select rtobj.cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
                   &"where rtemployee.emply='" & dspkey(46) & "' "
              rs.Open sql,conn
              if rs.eof then
                 name=""
              else
  ...
  %>         
        ...
        <td    height="23" bgcolor="silver" >
        <input type="text" name="key46" size="10" value="<%=dspKey(46)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text43"><font size=2><%=name%></font>
        </td></tr>           
        <tr>
        <td   class="dataListHEAD" height="23">施工進度</td>                                 
        <td  height="23" bgcolor="silver" COLSPAN=5>
        <% name=""
           if dspkey(30) <> "" then
        ...
        %>                   
        <input type="text" name="key30" size="10" value="<%=dspKey(30)%>"  <%=fieldpb%><%=fieldRole(1)%> class="dataListdata" READONLY ID="Text56"><font size=2><%=name%></font>
      </tr>     
 ...
 <%END IF %>
  <% Set rsxx=Server.CreateObject("ADODB.Recordset")
     sqlfaqlist="SELECT COUNT(*) AS CNT FROM RTLessorCustCont WHERE CUSID='" & DSPKEY(2) & "' AND CANCELDAT IS NULL "
     rsxx.open sqlfaqlist,conn
     IF RSXX("CNT") > 0 THEN
        XXCONTFLAG="Y"
     ELSE
  ...
  <DIV ID="SRTAG7" onclick="SRTAG7" style="cursor:hand">
    ...
    <DIV ID="SRTAB7" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr class="dataListsearch"><td colspan=13 align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustCONTK.asp?V=<%=XRND%>&accessMode=U&key=<%=DSPKEY(0)%>;<%=DSPKEY(1)%>;<%=DSPKEY(2)%>;" TARGET="NEWWINDOW" ><FONT COLOR=WHITE>客戶續約單明細</FONT></A></td></tr>
    ...
    <%
           Set rsxx=Server.CreateObject("ADODB.Recordset")
           sqlfaqlist="SELECT RTLessorCustCont.CUSID, RTLessorCustCont.ENTRYNO,RTLessorCust.CUSNC, " _
                     &"RTLessorCustCont.ENTRYNO AS ENTRYNO, RTLessorCustCont.APPLYDAT, RTLessorCustCont.STRBILLINGDAT, " _
                     &"RTCode_2.CODENC AS PAYCYCLE, RTCode_1.CODENC AS PAYTYPE, RTLessorCustCont.PERIOD, RTLessorCustCont.SECONDCASE, " _
                     &"RTLessorCustCont.AMT, RTLessorCustCont.REALAMT, RTLessorCustCont.TARDAT, RTLessorCustCont.BATCHNO, " _
                     &"RTLessorCustCont.FINISHDAT, " _
                     &"CASE WHEN RTLessorCustCont.FINISHDAT IS NULL THEN DATEDIFF(dd,RTLessorCustCont.APPLYDAT,getdate())+1 ELSE DATEDIFF(dd,RTLessorCustCont.APPLYDAT,RTLessorCustCont.finishdat)+1  END AS PROCESSDAT " _ 
                     &"FROM RTLessorCustCont LEFT OUTER JOIN RTCode RTCode_1 ON RTLessorCustCont.PAYTYPE = RTCode_1.CODE " _
                     &"AND RTCode_1.KIND = 'M9' LEFT OUTER JOIN RTLessorCust ON RTLessorCustCont.CUSID = " _
                     &"RTLessorCust.CUSID LEFT OUTER JOIN RTCode RTCode_2 ON RTLessorCustCont.PAYCYCLE = RTCode_2.CODE " _
                     &"AND RTCode_2.KIND = 'M8' " _
                     &"WHERE    RTLessorCustCont.cusid = '" & DSPKEY(2) & "' AND RTLessorCustCont.canceldat IS NULL " 
           rsxx.open sqlfaqlist,conn
           Randomize
           XRND=RND()
           do until rsxx.eof %>
           <tr class="dataListentry">
           <td align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustContD.asp?V=<%=XRND%>&accessMode=U&key=<%=RSXX("CUSID")%>;<%=RSXX("ENTRYNO")%>;" TARGET="NEWWINDOW" ><%=RSXX("CUSID")%>-<%=RSXX("ENTRYNO")%></A></td>
           ...
           <A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustARDTLK.ASP?V=<%=XRND%>&accessMode=U&key=<%=RSXX("CUSID")%>;<%=RSXX("BATCHNO")%>;" TARGET="NEWWINDOW" ><%=rsxx("BATCHNO")%>&nbsp;</td>
           <% else %>
           &nbsp;
           <%end if %>
           <td align=center>
 ...
 <%END IF %> 
  <% Set rsxx=Server.CreateObject("ADODB.Recordset")
     sqlfaqlist="SELECT COUNT(*) AS CNT FROM RTLessorCustDrop WHERE CUSID='" & DSPKEY(2) & "' AND CANCELDAT IS NULL "
     rsxx.open sqlfaqlist,conn
     IF RSXX("CNT") > 0 THEN
        XXdropFLAG="Y"
     ELSE
  ...
  <DIV ID="SRTAG10" onclick="SRTAG10" style="cursor:hand">
    ...
    <DIV ID="SRTAB10" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr class="dataListsearch"><td colspan=13 align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustdropK.asp?V=<%=XRND%>&accessMode=U&key=<%=DSPKEY(0)%>;<%=DSPKEY(1)%>;<%=DSPKEY(2)%>" TARGET="NEWWINDOW" ><FONT COLOR=WHITE>客戶退租單明細</FONT></A></td></tr>
    ...
    <%
           Set rsxx=Server.CreateObject("ADODB.Recordset")
           sqlfaqlist="SELECT  RTLessorCustDrop.CUSID,RTLessorCustDrop.ENTRYNO,RTCODE.CODENC,RTLessorCustDrop.APPLYDAT," _
                   &"RTLessorCustDrop.ENDDAT,RTLessorCustDrop.FINISHDAT, RTObj.CUSNC as cusnc1, RTLessorCustDrop.SNDPRTNO, " _
                   &"RTLessorCustDrop.SNDWORK,RTLessorCustDrop.SNDWORKCLOSE,RTLessorCustDrop.tardat," _
                   &"RTLessorCustDrop.batchno, RTLessorCustDrop.CANCELDAT, " _
                   &"RTObj_1.CUSNC  as cusnc2, RTObj_2.CUSNC  as cusnc3, RTObj_3.CUSNC  as cusnc4, " _
                   &"case when RTLessorCustDrop.FINISHDAT is null then DATEDIFF(dd,RTLessorCustDrop.APPLYDAT,getdate())+1 else DATEDIFF(dd,RTLessorCustDrop.APPLYDAT,RTLessorCustDrop.FINISHDAT)+1 end as processdat " _
                   &"FROM RTEmployee RTEmployee_3 INNER JOIN RTObj RTObj_3 ON RTEmployee_3.CUSID = RTObj_3.CUSID RIGHT OUTER JOIN " _
                   &"RTLessorCustDrop ON RTEmployee_3.EMPLY = RTLessorCustDrop.UUSR LEFT OUTER JOIN " _
                   &"RTEmployee RTEmployee_2 INNER JOIN RTObj RTObj_2 ON RTEmployee_2.CUSID = RTObj_2.CUSID ON " _
                   &"RTLessorCustDrop.EUSR = RTEmployee_2.EMPLY LEFT OUTER JOIN RTEmployee RTEmployee_1 INNER JOIN " _
                   &"RTObj RTObj_1 ON RTEmployee_1.CUSID = RTObj_1.CUSID ON RTLessorCustDrop.CANCELUSR = " _
                   &"RTEmployee_1.EMPLY LEFT OUTER JOIN RTObj INNER JOIN RTEmployee ON RTObj.CUSID = RTEmployee.CUSID ON " _
                   &"RTLessorCustDrop.FUSR = RTEmployee.EMPLY  LEFT OUTER JOIN RTCODE ON RTLessorCustDrop.DROPKIND=RTCODE.CODE AND RTCODE.KIND='N7' " _
                   &"where RTLessorCustDrop.cusid='" & dspkey(2) & "' AND RTLessorCustDrop.canceldat is null "
           rsxx.open sqlfaqlist,conn
           Randomize
           XRND=RND()
           do until rsxx.eof %>
           <tr class="dataListentry">
           <td align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustdropD.asp?V=<%=XRND%>&accessMode=U&key=<%=RSXX("CUSID")%>;<%=RSXX("ENTRYNO")%>;" TARGET="NEWWINDOW" ><%=RSXX("CUSID")%>-<%=RSXX("ENTRYNO")%></A></td>
           <td align=center><%=rsxx("CODENC")%>&nbsp;</td>
           <td align=center><%=rsxx("APPLYDAT")%>&nbsp;</td>
           <td align=center><%=rsxx("ENDDAT")%>&nbsp;</td>
           <td align=center><%=rsxx("FINISHDAT")%>&nbsp;</td>
           <td align=right><%=rsxx("cusnc1")%>&nbsp;</td>
           <td align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustdropsndworkD.asp?V=<%=XRND%>&accessMode=U&key=<%=RSXX("CUSID")%>;<%=RSXX("ENTRYNO")%>;<%=RSXX("SNDPRTNO")%>" TARGET="NEWWINDOW" ><%=rsxx("SNDPRTNO")%>&nbsp;</A></td>
           <td align=right>
           <%
           if isnull(rsxx("SNDWORK")) then
              xxSNDWORK=""
 ...
 <%END IF %>  
  <% Set rsxx=Server.CreateObject("ADODB.Recordset")
     sqlfaqlist="SELECT COUNT(*) AS CNT FROM RTLessorCustReturn WHERE CUSID='" & DSPKEY(2) & "' AND CANCELDAT IS NULL "
     rsxx.open sqlfaqlist,conn
     IF RSXX("CNT") > 0 THEN
        XXreturnFLAG="Y"
     ELSE
  ...
  <DIV ID="SRTAG8" onclick="SRTAG8" style="cursor:hand">
    ...
    <DIV ID="SRTAB8" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr class="dataListsearch"><td colspan=13 align=center><A HREF="/webap/rtap/BASE/RTlessorCMTY/RTLessorCustRETURNK.asp?V=<%=XRND%>&accessMode=U&key=<%=DSPKEY(0)%>;<%=DSPKEY(1)%>;<%=DSPKEY(2)%>;" TARGET="NEWWINDOW" ><FONT COLOR=WHITE>客戶復機單明細</FONT></A></td></tr>
    ...
    <%
           Set rsxx=Server.CreateObject("ADODB.Recordset")
           sqlfaqlist="SELECT RTLessorCustReturn.CUSID, RTLessorCustReturn.ENTRYNO,RTLessorCust.CUSNC, " _
                     &"RTLessorCustReturn.ENTRYNO AS ENTRYNO, RTLessorCustReturn.APPLYDAT, RTLessorCustReturn.STRBILLINGDAT, " _
                     &"RTCode_2.CODENC AS PAYCYCLE, RTCode_1.CODENC AS PAYTYPE, RTLessorCustReturn.PERIOD, RTLessorCustReturn.SECONDCASE, " _
                     &"RTLessorCustReturn.AMT, RTLessorCustReturn.REALAMT, RTLessorCustReturn.TARDAT, RTLessorCustReturn.BATCHNO, " _
                     &"RTLessorCustReturn.FINISHDAT, " _
                     &"CASE WHEN RTLessorCustReturn.FINISHDAT IS NULL THEN DATEDIFF(dd,RTLessorCustReturn.APPLYDAT,getdate())+1 ELSE DATEDIFF(dd,RTLessorCustReturn.APPLYDAT,RTLessorCustReturn.finishdat)+1  END AS PROCESSDAT " _ 
                     &"FROM RTLessorCustReturn LEFT OUTER JOIN RTCode RTCode_1 ON RTLessorCustReturn.PAYTYPE = RTCode_1.CODE " _
                     &"AND RTCode_1.KIND = 'M9' LEFT OUTER JOIN RTLessorCust ON RTLessorCustReturn.CUSID = " _
                     &"RTLessorCust.CUSID LEFT OUTER JOIN RTCode RTCode_2 ON RTLessorCustReturn.PAYCYCLE = RTCode_2.CODE " _
                     &"AND RTCode_2.KIND = 'M8' " _
                     &"WHERE    RTLessorCustReturn.cusid = '" & DSPKEY(2) & "' AND RTLessorCustReturn.canceldat IS NULL " 
           rsxx.open sqlfaqlist,conn
           Randomize
           XRND=RND()
           do until rsxx.eof %>
