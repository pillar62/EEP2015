...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlyy="select * FROM RTLessorCustReturn WHERE  CUSID='" & KEY(0) & "' and entryno=" & key(1)
  ' response.write sqlyy
   ...
   sqlxx="select count(*) as CNT FROM RTLessorCustReturnHardware WHERE CUSID='" & KEY(0) & "' AND entryno=" & key(1) & " and prtno='" & key(2) & "' and dropdat is null and rcvfinishdat is null "
...
if endpgm="" then
   ...
   ELSE
      ...
      strSP="usp_RTLessorCustReturnARSndworkF " & "'" & key(0) & "'" & "," & key(1) & ",'" & key(2) & "','" & V(0) & "'" & "," & tempperiod & "," & temprcvmoney & ",'" & temppaytype & "','" & tempcardno & "'"
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="10" then
       msgbox "此用戶復機資料已產生應收帳款檔，不可重複執行(請洽資訊部)。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                 
    ...
    elseIF frm1.htmlfld.value="12" then
       msgbox "此客戶主檔未退租，必須改採續約作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close               
    elseIF frm1.htmlfld.value="13" then
       msgbox "找不到客戶復機資料檔，無法結案。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close            
    elseIF frm1.htmlfld.value="14" then
       msgbox "客戶復機資料已作廢，不可結案。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                   
    elseIF frm1.htmlfld.value="15" then
       msgbox "執行結案作業時，復機資料中[開始計費日]不可空白。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                     
    ...
    elseIF frm1.htmlfld.value="17" then
       msgbox "客戶復機主檔已結案，不可重複執行結案作業(請聯絡資訊部)。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                    
    else
...
<form name=frm1 method=post action=RTLessorCustReturnsndworkf.asp ID="Form1">
