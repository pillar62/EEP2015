...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT  COMQ1, LINEQ1, CUSID, CUSNC, SOCIALID, CUTID1, TOWNSHIP1, RADDR1, RZONE1, CUTID2, TOWNSHIP2," _
             ...
             &"ip12,ip13,ip14,CPEKIND,SETMONEY,CASEKIND, GTEQUIP, GTPRTDAT, GTPRTUSR, USERRATE " _
...
Sub SrCheckData(message,formValid)
  ...
  if len(trim(dspkey(14)))=0 then dspkey(14)=""
  if len(trim(dspkey(28)))=0 then dspkey(28)=""
  if len(trim(dspkey(30)))=0 then dspkey(30)=""
...
'       message="接續設備網路port號不為空白時，必須輸入上層設備網路編號"                  
 elseif len(trim(dspkey(78)))=0 AND dspKey(38)<> "Y" then
       formValid=False
    ...
    '有主約資料，再判斷採復機或第二戶方式建檔
      ...
      '當主約資料存在時且亦以第二戶方式建檔，其到期日若大於主約到期日時則必須修正到與主約相同，因此先記錄主約到期日，再來反推期數及調整日數和應收金額。 
       ...
       '   end if
          ...
          if len(trim(dspkey(33))) > 0 or len(trim(dspkey(65))) > 0 then
...
Sub srrecalulate()
  ' msgbox document.All("key68").value
   ...
   Sub srChangeMoney()
       ...
       s72X=document.all("key72X").CHECKED	'第二戶
     IF S78="01" THEN	'經濟型
       select case S72x
          case false
               '半年繳
               IF S58="01" THEN
                  document.All("key69").value = 2700
				  document.All("key49").value = 8
               '年繳
               ELSEIF S58="02" THEN
                  document.all("key69").value = 4800
				  document.All("key49").value = 16
               ELSEIF S58="03" THEN
                  document.all("key69").value = 9500
				  document.All("key49").value = 28
               ELSEIF S58="04" THEN
                  document.all("key69").value = 14000
				  document.All("key49").value = 36
               '季繳
               ELSEIF S58="05" THEN
                  document.all("key69").value = 1410
				  document.All("key49").value = 4
               ELSE
          ...
          CASE ELSE
               ...
               END IF
       END SELECT
     ELSEIF S78="02" THEN	'專業型
        select case S72x
          case false
               '半年繳
               IF S58="01" THEN
                  document.All("key69").value = 2700
				  document.All("key49").value = 6
               '年繳
               ELSEIF S58="02" THEN
                  document.all("key69").value = 4800
				  document.All("key49").value = 12
               '季繳
               ELSEIF S58="05" THEN
                  document.all("key69").value = 1410
				  document.All("key49").value = 3
               ELSE
                  document.all("key69").value = 0
				  document.All("key49").value = 0
               END IF
          CASE ELSE
               ...
               END IF
       END SELECT
     ELSEIF S78="03" THEN	'專案250
        select case S72x
          case false
               '半年繳
               IF S58="01" THEN
                  document.All("key69").value = 1500
				  document.All("key49").value = 6
               '年繳
               ELSEIF S58="02" THEN
                  document.all("key69").value = 3000
				  document.All("key49").value = 12
               '季繳
               ELSEIF S58="05" THEN
                  document.all("key69").value = 750
				  document.All("key49").value = 3
               ELSE
                  document.all("key69").value = 0
				  document.All("key49").value = 0
     ...
     ELSEIF S78="08" THEN	'專案300
       select case S72x
          case false
               '半年繳
               IF S58="01" THEN
                  document.All("key69").value = 1800
				  document.All("key49").value = 6
               '年繳
               ELSEIF S58="02" THEN
                  document.all("key69").value = 3600
     ...
     ELSEIF S78="09" THEN	'專業型+
        select case S72x
          case false
               '半年繳
               IF S58="01" THEN
                  document.All("key69").value = 2700
				  document.All("key49").value = 7
               '年繳
               ELSEIF S58="02" THEN
                  document.all("key69").value = 4800
				  document.All("key49").value = 14
               '季繳
               ELSEIF S58="05" THEN
                  document.all("key69").value = 1400
				  document.All("key49").value = 3
               ELSE
                  document.all("key69").value = 0
				  document.All("key49").value = 0
...
' -------------------------------------------------------------------------------------------- 
	...
	If len(trim(dspKey(31))) > 0 and basedata=false Then
...
<tr>
	...
	%>                          
        ...
        <input type="hidden" name="key72" value="<%=dspKey(72)%>" class=dataListENTRY>
	</td>
		...
		<%  
			...
			END IF 
		%>                          
        <input type="radio" name="key38" value="Y" <%=FREECODEX%> <%=fieldPo%> <%=fieldRole(1)%><%=dataProtect%> >是
        <input type="radio" name="key38" value="N" <%=FREECODEY%> <%=fieldPo%> <%=fieldRole(1)%><%=dataProtect%> >否
        </td> </TR>   

<tr><td width="10%" class=dataListHEAD>用戶(公司)名稱</td>
    <td  width="23%"  bgcolor="silver" >
        <input type="text" name="key3" <%=fieldPm%><%=fieldRole(1)%><%=dataProtect%>
               maxlength="30" value="<%=dspKey(3)%>"  size="30" class=dataListENTRY ID="Text22">
	</td>
<%
    s=""
    sx=" selected "
 ...
 <tr> 
    ...
    <td width="23%" bgcolor="silver"  >
      <input type="text" name="key20" size="30" maxlength="50" value="<%=dspKey(20)%>" <%=fieldRole(1)%> class="dataListEntry" ID="Text9"> 
            ...
            <td width="10%" class=dataListHEAD>用戶CPE設備種類</td>
    <td width="23%" bgcolor="silver">
      <%
    s=""
    sx=" selected "
   ...
   %>               
	...
	<% 
	aryOption=Array("","1M","2M","3M","4M","5M","6M","7M","8M")
	s=""
	If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and len(trim(dspkey(33)))=0 Then 
	...
	%>
    <td width="23%" bgcolor="silver">
        <select size="1" name="key82" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%> class="dataListEntry">                                                             
        	<%=s%>
        </select>
	</td>
      </tr> 
...
<tr><td class=dataListHEAD>帳單地址</td>
  ...
  <%s=""
   ...
   %>
        ...
        <input type="radio" name="rd2"  <%=fieldPn%> onClick="SrAddrEqual2()"><font SIZE=2>同戶籍(公司)地址</font><input type="radio" name="rd2"  <%=fieldPn%> onClick="SrAddrEqual3()"><font SIZE=2>同裝機地址</font></td>                                 
</tr>  
<tr>                                 
...
<TR>        
	...
	<tr>
        ...
        <td  height="23" bgcolor="silver">
			<input type="text" name="key40" size="6" READONLY value="<%=dspKey(40)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2">
			<font size=2><%=SrGetEmployeeName(dspKey(40))%></font>
        </td>  
        <td  class="dataListHEAD" height="23">建檔日期</td>                                 
        <td  height="23" bgcolor="silver" colspan=3>
        <input type="text" name="key41" size="25" READONLY value="<%=dspKey(41)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
	</tr>  
	<tr>
...
<!--
	...
	<tr><td WIDTH="10%" class="dataListHEAD" height="23">二線開發人員</td>
	...
	<tr><td width="10%" class="dataListHead">保證金年限</td>                    
		...
		<td  class="dataListHEAD" height="23">保證金收據列印人</td>                                 
			<td  height="23" bgcolor="silver">
				<input type="text" name="key81" size="10" READONLY value="<%=dspKey(81)%>" class="dataListDATA" ID="Text3">
				<font size=2>
				<%
        ...
        <td  height="23" bgcolor="silver">
			<input type="text" name="key80" size="25" READONLY value="<%=dspKey(80)%>" <%=fieldPg%><%=fieldRole(1)%> class="dataListDATA" ID="Text1">
        </td>       
	</tr>
...
<%
   ...
   <select size="1" name="key28" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35" onchange="SrSHOWcreditcardOnClick">                                                                  
        ...
        %>
             ...
             <input type="text" name="key69"  size="8"  <%=fieldpA%><%=fieldpC%><%=fieldpE%> value="<%=dspKey(69)%>"  <%=fieldRole(1)%> class="datalistENTRY" ID="Text56">
        </TD>
  </tr>          
    <tr>
    <td colspan=6>
...
<%
   ...
   <select size="1" name="key60" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35">                                                                  
        ...
        <input type="<%=xtype%>" name="key61" size="16" value="<%=dspKey(61)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListEntry" maxlength="16">
        </TD>     
        </tr>    
   <TR>
   ...
   <DIV ID="SRTAG4" onclick="srtag4" style="cursor:hand">
	...
	<tr><td WIDTH="10%" class="dataListHEAD" height="23">完工日期</td>                                 
        <td WIDTH="23%" height="23" bgcolor="silver" >
			<input type="text" name="key31" size="12" READONLY value="<%=dspKey(31)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text57">
        </td>
	...
	<tr><td WIDTH="10%" class="dataListHEAD" height="23">最近續約計費日</td>                                 
        <td WIDTH="23%" height="23" bgcolor="silver">
			<input type="text" name="key65" size="12" READONLY value="<%=dspKey(65)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text5">
	...
	<tr><td WIDTH="10%"  class="dataListHEAD" height="23">退租日</td>                                 
        <td WIDTH="23%"  height="23" bgcolor="silver" >
			<input type="text" name="key37" size="12" READONLY value="<%=dspKey(37)%>" <%=fieldpe%> <%=fieldRole(1)%> class="dataListdata" ID="Text12">
			...
			-->
			<font size=2>欠費︰</font> 
			<input type="text" name="key39" size="2" value="<%=dspKey(39)%>"  <%=fieldRole(1)%> class="dataListEntry" ID="Text13">
		</td>               
        ...
        <td WIDTH="23%" height="23" bgcolor="silver" >
			...
			%>
			<input type="text" name="remain"  size="5" READONLY value="<%=remainday%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text6">
			<FONT SIZE=2> 天</FONT>
		</TD>
		...
		%>
        <td height="23" bgcolor="silver">
			<input type="text" name="key30" size="3" value="<%=dspKey(30)%>" <%=fieldpb%><%=fieldRole(1)%> class="dataListdata" READONLY ID="Text10">
