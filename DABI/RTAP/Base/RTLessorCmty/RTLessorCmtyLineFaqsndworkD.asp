...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLessorcmty/RTlessorCmtyLineFaqSNDWORKd.asp")
                       ...
                       if i=3 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTlessorCmtyLineFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTlessorCmtyLineFaqSNDWORKd.asp") then
          cusidxx="EY" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTlessorCmtyLineFaqSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(3)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTlessorCmtyLineFaqSNDWORK WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,FAQNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTlessorCmtyLineFaqSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=99
...
<tr>
  ...
  %>    <input type="text" name="key32" size="6" READONLY value="<%=dspKey(32)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
   ...
   <DIV ID="SRTAB6" > 
    ...
    sqlfaqlist="SELECT * FROM RTlessorCmtyLineFaqSndworkFixCode RIGHT OUTER JOIN " _
            &"RTCode ON RTlessorCmtyLineFaqSndworkFixCode.FIXcod = RTCode.CODE AND faqno ='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' "  _
...
'------ RTObj ---------------------------------------------------
    DELFAQlist="delete from RTlessorCmtyLineFaqSndworkFixCode where COMQ1=" & dspkey(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno='" & dspkey(2) & "' AND PRTNO='" & DSPKEY(3) & "' "
    conn.Execute DELFAQlist  
    For i=0 to 99
        if len(trim(extdb(i))) > 0  then
           rs.Open "SELECT * FROM RTlessorCmtyLineFaqSndworkFixCode WHERE COMQ1=" & dspkey(0) & " AND LINEQ1=" & DSPKEY(1) & " AND faqno='" &dspKey(2) &"' AND PRTNO='" & DSPKEY(3) & "' and fIXcod='" & extDB(i) & "'" ,conn,3,3
           If rs.Eof Or rs.Bof Then
              rs.AddNew
              rs("COMQ1")=dspKey(0)
              rs("LINEQ1")=dspKey(1)
