...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  檢查該派工單是否已結案或作廢或已無可轉設備項目
   ...
   rsxx.close
   '檢查是否仍有未轉物品領用單的設備
   sqlxx="select count(*) as CNT FROM RTlessorCUSTCONTHardware WHERE CUSID='" & KEY(0) & "' AND ENTRYNO=" & KEY(1) & " AND PRTNO='" & KEY(2) & "' AND dropdat is null and rcvprtno='' and rcvfinishdat is null and batchno='' and qty > 0 "
...
if endpgm="" then
   ...
   strSP="usp_RTLessorCustCONTHardwareTRNRCVExe " & "'" & key(0) & "'," & key(1) & ",'" & key(2) & "','" & V(0) & "'" 
...
<form name=frm1 method=post action=rtlessorcustCONThardwaretrnrcv.asp ID="Form1">
