...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCmtyLineHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=3 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                       '  RESPONSE.WRITE "select max(SEQ) AS SEQ from RTLessorCmtyLineHARDWARE WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "
                         rsc.open "select max(SEQ) AS SEQ from RTLessorCmtyLineHARDWARE WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCmtyLineHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTLessorCmtyLineHARDWARE WHERE  COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' " ,conn
          if not rs.eof then
            dspkey(3)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1,LINEQ1,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             ...
             &"FROM RTLessorCmtyLineHARDWARE WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             ...
             &"FROM RTLessorCmtyLineHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    IF ACCESSMODE="Y" THEN
       ...
       connyy.Open dsn     
       sqlyy="SELECT * FROM RTLessorCmtyLineSNDWORK WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "
    ...
    if dspkey(21) <> "" then
       ...
       connyy.Open dsn     
       '同一社區下的設備網路編號不可相同
       sqlyy="select count(*) as cnt from RTlessorCmtyLineFaqHardware where hostno='" & dspkey(21) & "' and COMQ1=" & DSPKEY(0) & " DROPDAT IS NULL "
       sqlzz="select count(*) as cnt from RTlessorCmtyLineHardware where hostno='" & dspkey(21) & "' and COMQ1=" & DSPKEY(0) & " AND ( LINEQ1<>" &  DSPKEY(1) & " or PRTNO<>'" & DSPKEY(2) & "' or seq<>" & dspkey(3) & ") AND DROPDAT IS NULL "
