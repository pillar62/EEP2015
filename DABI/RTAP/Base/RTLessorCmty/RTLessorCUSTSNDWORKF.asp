...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlxx="select count(*) as CNT FROM RTLessorCUSTHardware WHERE CUSID='" & KEY(0) & "' and prtno='" & key(1) & "' and dropdat is null and rcvfinishdat is null "
   RSXX.OPEN SQLXX,CONN
   IF RSXX.EOF THEN
...
' (不檢查一定要有設備資料20060203)
   '檢查該派工單下的設備是否皆已辦妥物品領用程序
'   sqlxx="select count(*) as CNT FROM RTLessorCUSTHardware WHERE CUSID='" & KEY(0) & "' and prtno='" & key(1) & "' and dropdat is null "
'   RSXX.OPEN SQLXX,CONN
...
if endpgm="" then
   ...
   sqlxx="select * FROM RTLessorCUSTsndwork WHERE CUSID='" & KEY(0) & "' and prtno='" & key(1) & "' "
   RSXX.OPEN SQLXX,CONN
...
'      endpgm="18"                
   ...
   ELSE
      ...
      strSP="usp_RTLessorCustARSndworkF " & "'" & key(0) & "'" & ",'" & key(1) & "','" & V(0) & "'" & "," & tempperiod & "," & temprcvmoney & ",'" & temppaytype & "','" & tempcardno & "'"
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="18" then
       ...
       winP.focus()
       window.close                
    elseIF frm1.htmlfld.value="19" then
       msgbox "裝機結案時，用戶開始計費日不可空白" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                                 
    else
...
<form name=frm1 method=post action=RTLessorcustsndworkf.asp ID="Form1">
