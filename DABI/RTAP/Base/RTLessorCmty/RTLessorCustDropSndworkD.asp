...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLessorcmty/RTlessorCustDropSNDWORKd.asp")
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTlessorCustDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTlessorCustDropSNDWORKd.asp") then
          cusidxx="EG" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTlessorCustDropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTlessorCustDropSNDWORK WHERE cusid='' "
  sqlList="SELECT cusid,ENTRYNO, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat,BATCHNO,CDAT " _
             &"FROM RTlessorCustDropSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"
  extDBField=0
...
<td bgcolor="silver" >
<%
  sql="SELECT * FROM   RTlessorCustDrop LEFT OUTER JOIN rtlessorcust on RTlessorCustDrop.cusid=rtlessorcust.cusid left outer join " _
                     &"RTLessorCmtyLine ON RTlessorCust.COMQ1 = RTLessorCmtyLine.COMQ1 AND " _
                     &"RTlessorCust.LINEQ1 = RTLessorCmtyLine.LINEQ1 WHERE RTlessorCustDrop.cusid='" & DSPKEY(0) & "'" 
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
