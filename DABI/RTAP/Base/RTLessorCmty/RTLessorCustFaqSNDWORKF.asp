...
<% KEY=SPLIT(REQUEST("KEY"),";")
  ...
  elseif LEN(TRIM(RSyy("cancelDAT"))) <> 0 then
   ...
   sqlxx="select count(*) as CNT FROM RTLessorCustFAQHardware WHERE CUSID='" & KEY(0) & "' AND faqno='" & key(1) & "' and prtno='" & key(2) & "' and dropdat is null and rcvfinishdat is null "
...
if endpgm="" then
   ...
   sqlyy="select count(*) as cnt FROM RTLessorCustFaqSndworkFixCode WHERE CUSID='" & KEY(0) & "' and FAQNO='" & key(1) & "' and prtno='" & key(2) & "' "
   RSXX.OPEN SQLXX,CONN
   RSyy.OPEN SQLyy,CONN
   ...
   ELSE
      ...
      strSP="usp_RTLessorCustFAQSndworkF " & "'" & key(0) & "','" & key(1) & "','" & key(2) & "','" & V(0) & "'"
...
<form name=frm1 method=post action=RTLessorCustFAQsndworkf.asp ID="Form1">
