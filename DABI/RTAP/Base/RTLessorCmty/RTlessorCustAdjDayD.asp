...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE SQL
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTLessorCustAdjdayd.asp")
                    'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorCustAdjday where  CUSID='" & DSPKEY(0) & "' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTLessorCustAdjdayd.asp") then
          rsc.open "select max(entryno) AS ENTRYNO from RTLessorCustAdjday where  CUSID='" & DSPKEY(0) & "' " ,conn
          if not rsC.eof then
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=2
  title="ET-City用戶到期日資料調整維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT CUSID,ENTRYNO,ADJPERIOD,ADJDAY,ADJCLOSEDAT,ADJCLOSEUSR,CANCELDAT,CANCELUSR,EUSR,EDAT,UUSR," _
             &"UDAT,memo " _
             &"FROM  RTLessorCustAdjDay WHERE cusid='' "
  sqlList="SELECT CUSID,ENTRYNO,ADJPERIOD,ADJDAY,ADJCLOSEDAT,ADJCLOSEUSR,CANCELDAT,CANCELUSR,EUSR,EDAT,UUSR," _
         &"UDAT,memo " _
         &"from RTLessorCustAdjDay WHERE "
  userDefineRead="Yes"      
  userDefineSave="Yes"       
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub SrCheckData(message,formValid)
  ...
  if (len(trim(dspkey(2)))=0 and len(trim(dspkey(3)))=0) or (dspkey(2)=0 and dspkey(3)=0) then
       formValid=False
       message="調整期數及調整日數不空同時空白或為0"   
  elseif not isnumeric(dspkey(2)) or not isnumeric(dspkey(3)) then
       formValid=False
       message="調整期數及調整日數必須為數字"                
  elseif dspkey(3)> 30 then
       formValid=False
       message="若調整日數大於30日，請改用調整期數替代。"                  
  elseif len(trim(dspkey(12))) = 0 then
       formValid=False
       message="調整原因不可空白。"                              
  end if
  '檢查是否有未結案的調整資料(if exist ==> not insert new data)
  if accessmode="A" AND formValid=true then
     Set connxx=Server.CreateObject("ADODB.Connection")
     Set rsxx=Server.CreateObject("ADODB.Recordset")
     connxx.open DSN
     sqlxx="select count(*) as cnt from rtlessorcustadjday where cusid='" & dspkey(0) & "' and canceldat is null and ADJclosedat is null "
     rsxx.open sqlxx,connxx
     if rsxx("cnt") > 0 then
        formValid=False
        message="尚有未結案的調整資料，不可再新增。(請先結案先前的資料)"             
     end if
     rsxx.close
     set rsxx=nothing
     connxx.close
...
Sub SrActiveXScript()%>
   ...
   End Sub          
   Sub SrTAG0()
      ' msgbox window.SRTAB0.style.display
...
' -------------------------------------------------------------------------------------------- 
    ...
    '用戶結案或作廢後,資料 protect
    If len(trim(dspKey(4))) > 0 or len(trim(dspKey(6))) > 0 Then
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City用戶調整到期日資訊</span>
...
<td>     
    ...
    <tr>    <td bgcolor="BDB76B" align="center">調整資料內容</td></tr></table></DIV>
    <DIV ID=SRTAB0 >
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
<tr>
    <td width="25%" class=dataListHEAD>調整期數</td>
    <td width="25%" bgcolor="silver" >
        <input type="text" name="key2" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="5"
               value="<%=dspKey(2)%>"  size="5" class=dataListEntry><font size=2 color=red>　(可為正或負數)</font>
      </td>        
    <td width="25%" class=dataListHEAD>調整日數</td>
    <td width="25%" bgcolor="silver" >
      <input type="text" name="key3" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="5"
               value="<%=dspKey(3)%>"  size="5" class=dataListEntry><font size=2 color=red>　(可為正或負數)</font>
            </TD>      
      </TR>
<tr>
  ...
  %>    <input type="text" name="key8" size="6" READONLY value="<%=dspKey(8)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
...
<tr>
  ...
  %>    <input type="text" name="key10" size="6" READONLY value="<%=dspKey(10)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
   ...
   <DIV ID="SRTAG1" onclick="srtag1" style="cursor:hand">
    ...
    <tr><td bgcolor="BDB76B" align="center">調整進度狀態</td></tr></table></DIV>
    <DIV ID=SRTAB1 >  
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
 
        <tr>
        <td  WIDTH="25%" class="dataListHEAD" height="23">結案日期</td>                                 
        <td  WIDTH="25%" height="23" bgcolor="silver" >
        <input type="text" name="key4" size="10" READONLY value="<%=dspKey(4)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text57">
 
        <td  WIDTH="25%" class="dataListHEAD" height="23">結案人員</td>                                 
        <td  WIDTH="25%" height="23" bgcolor="silver" >
        <%  name="" 
           if dspkey(5) <> "" then
              sql=" select rtobj.cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
  ...
  %>              
        <input type="text" name="key5" size="6" READONLY value="<%=dspKey(5)%>" <%=fieldpC%> <%=fieldRole(1)%> class="dataListdata" ID="Text56" ><font size=2><%=name%></font>
        </td>
</TR>
       <tr>
       <td   class="dataListHEAD" height="23">作廢日期</td>                                 
        <td   height="23" bgcolor="silver">
        <input type="text" name="key6" size="10" value="<%=dspKey(6)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListdata" ID="Text41">
         </td>
        <%  name="" 
           if dspkey(7) <> "" then
              sql=" select rtobj.cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
  ...
  %>         
        ...
        <td    height="23" bgcolor="silver" >
        <input type="text" name="key7" size="6" value="<%=dspKey(7)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text43"><font size=2><%=name%></font>
        </td></tr>           
    ...
    <tr><td bgcolor="BDB76B" align="center">調整原因說明</td></tr></table></DIV>
   <DIV ID="SRTAB2" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table9">
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%"  name="key12" rows=8  MAXLENGTH=500  class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(12)%>" ID="Textarea1"><%=dspkey(12)%></TEXTAREA>
   </td></tr>
 </table> 
  </div> 
<%
