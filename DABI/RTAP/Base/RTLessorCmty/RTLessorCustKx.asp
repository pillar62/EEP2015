...
Sub SrEnvironment()
  ...
  functionOptName="用戶移動;調整到期;作　　廢;作廢返轉;歷史異動"
  functionOptProgram="RTLessorCustmove.asp;RTLessorCustadjdayK.asp;RTLessorCustCANCEL.asp;RTLessorCustCANCELRTN.asp;RTLessorCustLOGK.asp"
  functionOptPrompt="N;N;Y;Y;N"
  functionoptopen="2;1;1;1;1"
  If V(1)="Y" then
     accessMode="U"
  Else
     accessMode="I"
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;none;主線;社區;用戶;週期;繳款;IP位址;none;連絡電話;申請日;完工日;開始計費;最近<br>續約日;調整<br>日數;到期日;退租日;作廢日;none;none;期數;可用<BR>日數;最近<br>收款額"
  sqlDelete="SELECT RTLessorCust.COMQ1, RTLessorCust.LINEQ1, RTLessorCust.CUSID, " _
                ...
                &"RTLessorCmtyH.COMN,RTLessorCust.CUSNC,ltrim(rtrim(RTLessorCust.IP11))+'.'+ltrim(rtrim(RTLessorCust.IP12))+'.'+ltrim(rtrim(RTLessorCust.IP13))+'.'+ltrim(rtrim(RTLessorCust.IP14)), RTLessorCust.TOWNSHIP1 + " _
  ...
  numberOfKey=3
  dataProg="RTLessorCustDTEMP.asp"
  datawindowFeature=""
 ...
 ' if userlevel=31 or userlevel =1  or userlevel =5 then DAreaID="<>'*'"
  ...
  '業務工程師只能讀取該工程師的社區
         sqlList="SELECT RTLessorCust.COMQ1, RTLessorCust.LINEQ1, RTLessorCust.CUSID, " _
                &"RTRIM(LTRIM(CONVERT(char(6), RTLessorCust.COMQ1))) " _
                &"+ '-' + RTRIM(LTRIM(CONVERT(char(6), RTLessorCust.LINEQ1))) AS comqline, " _
                &"RTLessorCmtyH.COMN,RTLessorCust.CUSNC,RTCODE_1.CODENC,RTCODE_2.CODENC,ltrim(rtrim(RTLessorCust.IP11))+'.'+ltrim(rtrim(RTLessorCust.IP12))+'.'+ltrim(rtrim(RTLessorCust.IP13))+'.'+ltrim(rtrim(RTLessorCust.IP14)), RTLessorCust.TOWNSHIP1 + " _
                &"RTLessorCust.RADDR1 AS ADDR,RTLessorCust.CONTACTTEL + CASE WHEN RTLessorCust.CONTACTTEL = '' OR " _
                &"RTLessorCust.CONTACTTEL IS NULL OR RTLessorCust.MOBILE = '' OR RTLessorCust.MOBILE IS NULL " _
                &"THEN '' ELSE ',' END + RTLessorCust.MOBILE AS Expr1, RTLESSORCUST.APPLYDAT, RTLESSORCUST.FINISHDAT, " _
                &"RTLESSORCUST.STRBILLINGDAT,RTLESSORCUST.newBILLINGDAT,RTLESSORCUST.adjustday,RTLESSORCUST.DUEDAT,RTLESSORCUST.DROPDAT,RTLESSORCUST.CANCELDAT,ltrim(rtrim(RTLessorCust.IP11))+'.'+ltrim(rtrim(RTLessorCust.IP12))+'.'+ltrim(rtrim(RTLessorCust.IP13))+'.'+ltrim(rtrim(RTLessorCust.IP14)), RTLESSORCUST.MAC, RTLESSORCUST.PERIOD," _
