...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTLessorCmtyCONTRACTd.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         cusidxx="ET" & right("00" & trim(datePART("yyyy",NOW())),2) 
                         rsc.open "select max(CONTRACTNO) AS CONTRACTNO from RTLessorCmtyCONTRACT where CONTRACTNO LIKE '" & cusidxx & "%'",conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTLessorCmtyCONTRACTd.asp") then
          cusidxx="ET" & right("00" & trim(datePART("yyyy",NOW())),2) 
           rs.open "select max(CONTRACTNO) AS CONTRACTNO from RTLessorCmtyCONTRACT where CONTRACTNO LIKE '" & cusidxx & "%'",conn
          if not rs.eof then
...
Sub SrCheckData(message,formValid)
  ...
  if accessmode="A" AND formValid=true then
     ...
     connxx.open DSN
     sqlxx="select count(*) as cnt from rtlessorcmtycontract where comq1=" & dspkey(0) & " and canceldat is null "
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City社區合約資訊</span>
