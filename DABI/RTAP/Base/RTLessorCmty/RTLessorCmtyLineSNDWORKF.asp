...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlxx="select count(*) as CNT FROM RTLessorcmtylineHardware WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' and dropdat is null and rcvfinishdat is null "
   RSXX.OPEN SQLXX,CONN
   IF RSXX.EOF THEN
   ...
   RSXX.CLOSE
   '檢查該派工單下的設備是否皆已辦妥物品領用程序
   sqlxx="select count(*) as CNT FROM RTLessorcmtylineHardware WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' and dropdat is null "
   RSXX.OPEN SQLXX,CONN
...
if endpgm="" then
   ...
   ELSE
      ...
      strSP="usp_RTLessorcmtylineSndworkF "  & key(0) & "," & key(1) & ",'" & key(2) & "','" & V(0) & "'"
...
<form name=frm1 method=post action=RTLessorcmtylinesndworkf.asp ID="Form1">
