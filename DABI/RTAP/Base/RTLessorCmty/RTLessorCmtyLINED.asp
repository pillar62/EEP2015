...
Sub SrCheckData(message,formValid)
    ...
    if len(trim(DSPKEY(46))) = 0 THEN DSPKEY(46)="" 
    ...
    If len(trim(dspKey(0))) <= 0 Then
       dspkey(0)=0
    END IF       
    if len(trim(dspkey(6)))=0 AND len(trim(dspkey(3)))=0 AND len(trim(dspkey(7)))=0 THEN
       formValid=False
       message="開發業務員或經銷商不可同時空白"
    'elseif len(trim(dspkey(44)))=0 or Not Isdate(dspkey(44)) then
    '   formValid=False
...
<%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(dspKey(62))) = 0  Then 
    ...
    %>    
		...
		<input type="TEXT" name="key5" value="<%=dspKey(5)%>" style="display:none;" >
        ...
        <input type="TEXT" name="key6" <%=fieldRole(1)%><%=dataProtect%> 
               size="10" value="<%=dspKey(6)%>"  readonly class="dataListDATA" ID="Hidden1">
    ...
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table5">
      ...
      <tr><td WIDTH="15%" class="dataListHEAD" >可推行方案</td>
		  <td height="23" bgcolor="silver" colspan=3>
			<%
			s=""
			sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='O9' and code in ('01','02','05')" 
			rs.Open sql,conn
			Do While Not rs.Eof
