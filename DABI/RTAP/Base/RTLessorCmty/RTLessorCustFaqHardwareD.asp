...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCustFAQHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=3 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTLessorCustFAQHARDWARE where CUSID='" & DSPKEY(0) & "' AND FAQNO='" & DSPKEY(1) & "' AND PRTNO='" & DSPKEY(2) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCustFAQHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTLessorCustFAQHARDWARE WHERE  CUSID='" & DSPKEY(0) & "' AND FAQNO='" & DSPKEY(1) & "' AND PRTNO='" & DSPKEY(2) & "' " ,conn
          if not rs.eof then
            dspkey(3)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=4
  title="ET-City用戶維修派工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT cusid,FAQNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             &"WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo,BATCHNO,TARDAT,TUSR " _
             &"FROM RTLessorCustFaqHARDWARE WHERE cusid='' "
  sqlList="SELECT cusid,FAQNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             &"DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo,BATCHNO,TARDAT,TUSR " _
             &"FROM RTLessorCustFaqHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTLessorCustFaqSNDWORK WHERE CUSID='" & DSPKEY(0) & "' AND FAQNO='" & DSPKEY(1) & "' AND PRTNO='" & DSPKEY(2) & "' "
      ...
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="10%" class=dataListHead>用戶序號</td>
           ...
           <input type="text" name="key1"
                 <%=fieldRole(1)%> readonly size="13" value="<%=dspKey(1)%>" maxlength="13" class=dataListdata ID="Text3"></td>                 
