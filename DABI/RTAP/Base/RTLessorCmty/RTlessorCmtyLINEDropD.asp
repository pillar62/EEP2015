...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtlessorcmty/RTLessorCmtyLineDropd.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 2 then rs.Fields(i).Value=dspKey(i)    
                       if i=2 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTLessorCmtyLineDrop where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1),conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtlessorcmty/RTLessorCmtyLineDropd.asp") then
          rs.open "select max(entryno) AS entryno from RTLessorCmtyLineDrop where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) ,conn
          if not rs.eof then
            dspkey(2)=rs("entryno")
          end if
          rs.close
...
Sub SrCheckData(message,formValid)
    ...
    IF ACCESSMODE="A" THEN
       ...
       connXX.open DSN
       SQLXX="Select count(*) as cnt from rtlessorcmtylinedrop where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " and canceldat is null "
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City主線撤線資訊</span>
