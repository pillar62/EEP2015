...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCustReturnHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=3 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTLessorCustReturnHARDWARE where CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtLESSORcmty/RTLessorCustReturnHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTLessorCustReturnHARDWARE WHERE  CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' " ,conn
          if not rs.eof then
            dspkey(3)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT cusid,ENTRYNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             ...
             &"FROM RTLessorCustReturnHARDWARE WHERE cusid='' "
  sqlList="SELECT cusid,ENTRYNO,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             ...
             &"FROM RTLessorCustReturnHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTLessorCustReturnSNDWORK WHERE CUSID='" & DSPKEY(0) & "' AND ENTRYNO=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="10%" class=dataListHead>用戶序號</td>
           ...
           <td width="10%" class=dataListHead>復機單次</td>
