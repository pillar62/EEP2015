...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtLESSORcmty/RTLESSORcustHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=2 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                       '  RESPONSE.WRITE "select max(entryno) AS entryno from RTLESSORcustHARDWARE WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "
                         rsc.open "select max(entryno) AS entryno from RTLESSORcustHARDWARE WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtLESSORcmty/RTLESSORcustHARDWAREd.asp") then
          rs.open "select max(entryno) AS ENTRYNO from RTLESSORcustHARDWARE WHERE  CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' " ,conn
          if not rs.eof then
            dspkey(2)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=3
  title="ET-City用戶派工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON," _
             &"WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo,BATCHNO,TARDAT,TUSR " _
             &"FROM RTLESSORcustHARDWARE WHERE cusid='' "
  sqlList="SELECT cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, " _
             &"DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo,BATCHNO,TARDAT,TUSR " _
             &"FROM RTLESSORcustHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTLESSORCUSTSNDWORK WHERE CUSID='" & DSPKEY(0) & "' AND PRTNO='" & DSPKEY(1) & "' "
    ...
    IF LEN(DSPKEY(8))=0 THEN DSPKEY(8)=""
    IF ERRFLG="Y" THEN
       formValid=False
    ...
    ELSEif len(trim(DSPKEY(11))) = 0 OR NOT ISNUMERIC(DSPKEY(11))  THEN
       formValid=False
       message="請輸入金額或格式不符"        
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="10%" class=dataListHead>用戶序號</td>
           ...
           <td width="11%" class=dataListHead>竣工單</td>
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">ET-City用戶設備資料</span>
...
<%  set conn=server.CreateObject("ADODB.Connection")
    ...
    <input type="button" id="B4"  name="B4" height="100%" width="100%" style="Z-INDEX: 1" value="...." <%=SXX6%> >
...
<td  class=dataListHead>設備/規格名稱</td></td>
  ...
  <%
  DEVICENAME=""
  IF LEN(DSPKEY(3)) > 0 AND LEN(DSPKEY(6)) > 0 THEN
...
<tr><td width=15% class=dataListHead>數量</td>
    ...
    Else
       sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='B5' AND CODE='" & dspkey(10) & "'"
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
   ...
   <select size="1" name="key10" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select15">                                                                  
        <%=s%>
   </select>        </td>
<td width=15% class=dataListHead>金額</td>
<td width=35% bgcolor="silver" >
    <input type="text" name="key11" size="8" value="<%=dspkey(11)%>" maxlength="10" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Text4">   
</td></tr>
<tr>
...
<tr>
  ...
  %>    <input type="text" name="key9" size="6" READONLY value="<%=dspKey(9)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>                                            
 </tr>        
 <tr>
   <td class=dataListHEAD>作廢原因</td>          
   <td  height="23" bgcolor="silver" COLSPAN=3>
   <input type="text" name="key7" size="80" MAXLENGTH=80 value="<%=dspKey(7)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text16">
   </TD>
   ...
   <TR>
        ...
        <td   height="23" bgcolor="silver" >     
        <input type="text" name="key17" size="15" READONLY value="<%=dspKey(17)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="30">
        </TD>     
         <td    class="dataListHEAD" height="23">轉應收帳款日</td>               
        <td   height="23" bgcolor="silver" >     
        <input type="text" name="key18" size="10" value="<%=dspKey(18)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="10">
        </TD>     
  ...
  %>            
        <input type="text" name="key19" size="6" READONLY value="<%=dspKey(19)%>" <%=fieldRole(1)%><%=fieldpa%><%=dataProtect%> class="dataListDATA" maxlength="30"><font size=2><%=name%></font>
        </TD>     
   </TR>                    
<tr>
...
<tr>
 ...
 <tr>
   ...
   <td  height="23" bgcolor="silver" COLSPAN=3>
   <input type="text" name="key16" size="30" MAXLENGTH=30 value="<%=dspKey(16)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text16">
   </TD>
</tr>         
   </table> 
  </div> 
