...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlyy="select count(*) as cnt FROM RTlessorCUSThardware WHERE CUSID='" & KEY(0) & "' and prtno='" & key(1) & "' and dropdat is null and RCVPRTNO <> '' "
   ...
   ELSE
      ...
      strSP="usp_RTLessorCustARSndworkUF " & "'" & key(0) & "'" & ",'" & key(1) & "','" & V(0) & "'" 
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "ET-City用戶裝機派工單未完工結案成功",0
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "此裝機派工單已產生物品領用單，請先返轉領用單才能執行未完工結案作業。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                    
    else
       msgbox "無法執行竣工單未完工結案作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=rtlessorcustsndworkUf.asp ID="Form1">
