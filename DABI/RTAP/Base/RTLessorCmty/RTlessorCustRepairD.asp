...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE	SQL
		  ...
		  For i	= 0	To numberOfField-1
			  ...
			  '	  On Error Resume Next
				...
				select case	ucase(runpgm)	
				   ...
				   case	ucase("/webap/rtap/base/RTLessorCmty/RTLessorCUSTRepaird.asp")
					'response.write	"I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
					   if i	<> 1 then rs.Fields(i).Value=dspKey(i)	  
					   if i=1 then
						 Set rsc=Server.CreateObject("ADODB.Recordset")
						 rsc.open "select max(entryno) AS entryno from RTLessorCustRepair where	CUSID='" & DSPKEY(0) & "' "	,conn
	...
	if accessmode ="A" then
	   runpgm=Request.ServerVariables("PATH_INFO")
	   if ucase(runpgm)=ucase("/webap/rtap/base/RTLessorcmty/RTLessorCustRepairD.asp") then
		  rsc.open "select max(entryno)	AS ENTRYNO from	RTLessorCustRepair where	 CUSID='" &	DSPKEY(0) &	"' " ,conn
		  if not rsC.eof then
...
%>
  ...
  -->
  <span	id="tags1" class=dataListTagOn>ET-City用戶維修收款資訊</span>
  <!--
