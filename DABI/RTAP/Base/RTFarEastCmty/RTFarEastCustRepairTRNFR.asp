...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   '檢查應收帳款檔是否已沖帳
   sqlyy="select * FROM RTFareastCustAR WHERE CUSID='" & KEY(0) & "' AND BATCHNO='" & BATCHNOXx & "'"
   rsyy.open sqlyy,conn
   if rsyy.eof THEN
   ELSE
   ...
   if endpgm="" then
      ...
      strSP="usp_RTFareastCustRepairAR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "', 'MR', '" & BATCHNOXX & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "遠傳大寬頻用戶維修收款返轉應收結案作業成功",0
...
<form name=frm1 method=post action=RTFareastCustRepairTRNFR.asp ID="Form1">
