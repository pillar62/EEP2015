...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   sqlYY="select * FROM RTFareastCust WHERE CUSID='" & KEY(0) & "' "
   ...
   RSXX.OPEN SQLXX,conn
     ...
     elseif LEN(TRIM(RSXX("batchno"))) <> 0 OR LEN(TRIM(RSXX("FINISHDAT"))) > 0 then
        endpgm="3"
     '應收金額為0者，不可產生
     elseif RSXX("moveamt")=0 and RSXX("equipamt")=0 and RSXX("setamt")=0 then
        endpgm="4"      
     ELSE
        '呼叫store procedure更新相關檔案
        strSP="usp_RTFareastCustRepairAR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "', 'MF', '' "
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="9" then
       msgbox "客戶資料已作廢，必須作廢維修收款資料" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
...
<form name=frm1 method=post action="RTFareastCustRepairTRNAR.asp" ID="Form1">
