...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
    ...
    If rs.Eof Or rs.Bof Then
       response.write "accessmode=" & accessmode & "<BR>"
       If accessMode="A" and dspkeY(2)="" Then
          rs.AddNew
          For i = 0 To numberOfField-1
              sType=Right("000" &Cstr(aryKeyType(i)),3)
...
'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
				if i <> 2 and i<> 26  then rs.Fields(i).Value=dspKey(i)
				if i=2 then
				 Set rsc=Server.CreateObject("ADODB.Recordset")
				 cusidxx="S" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
				 rsc.open "select max(cusid) AS cusid from RTfareastCust where cusid like '" & cusidxx & "%' " ,conn
				 ...
				 rs.Fields(i).Value=dspKey(i) 
                                elseif i=26 then
                                 '遠傳用戶編號︰編碼規則FBB_XXXYYYYZZZ (XXX:社區郵遞區號,YYYY:社區編號,ZZZ:用戶流水號
                                  if dspkey(26)="" then
                                     Set rsc=Server.CreateObject("ADODB.Recordset")
				     rsc.open "SELECT RTCtyTown.ZIP AS zip FROM RTFAREASTCmtyH INNER JOIN " _
                                          &"RTCtyTown ON RTfareastCmtyH.CUTID = RTCtyTown.CUTID AND " _
                                          &"RTfareastCmtyH.TOWNSHIP = RTCtyTown.TOWNSHIP where RTfareastCmtyH.comq1=" & dspkey(0) & " " ,conn
                                     IF len(rsc("zip")) > 0 then
                                        ZIP=RSC("ZIP")
                                     else
                                        zip=""
                                     END IF
                                     RSC.CLOSE
                                     CMTYSEQ=right("000" & CSTR(DSPKEY(0)),4)
                                     rsc.open "select max(MEMBERID) AS MEMBERID from RTfareastCust where MEMBERID like 'FBB_" & ZIP & CMTYSEQ & "%' " ,conn
				     if len(rsc("MEMBERID")) > 0 then
				        dspkey(26)="FBB_" & ZIP & CMTYSEQ & right("000" & cstr(cint(right(rsc("MEMBERID"),3)) + 1),3)
				     else
				        dspkey(26)="FBB_" & ZIP & CMTYSEQ & "001"
				     end if
				     rsc.close
                                  end if
				  rs.Fields(i).Value=dspKey(i) 
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1, LINEQ1, CUSID, CUSNC, IDNUMBERTYPE, SOCIALID, " &_
			...
			"UUSR, UDAT, ACTIVATEDAT,setupamt,CUTID1,TOWNSHIP1,RADDR1 " &_
			"FROM RTfareastCust WHERE COMQ1=0 "
...
Sub SrCheckData(message,formValid)
  ...
  IF formValid=TRUE THEN
   ...
   connxx.open DSN
   sqlxx="select * from RTfareastCmtyLine where comq1=" & aryparmkey(0) & " AND LINEQ1=" & ARYPARMKEY(1)
   'response.write "SQL=" & SQLXX
...
Sub SrActiveXScript()%>
   ...
   Sub Srbtnonclick()
       ...
       '押上報竣日則押上新保證金序號(公關機不產生序號KEY27="Y")
       if clickkey ="KEY43" and document.All("key37").value="" and document.All("key27").value <> "Y" then
	       	<%
				...
				sql ="select isnull(max(gtserial),'') as maxgtserial from RTfareastCust where gtserial<>'' "
    			rs.Open sql,conn,3,3
				 if len(trim(rs("maxgtserial"))) =0 then
					maxgtserial="FBB0000001"
				 else
				    maxgtserial="FBB" & right("000000" & cstr(cint(right(rs("maxgtserial"),7)) + 1),7)
				 end if
   ...
   Sub SrCountyOnClick()
	...
	Sub SrAddrEqual1()
	   document.All("key8").value=document.All("key54").value
	   document.All("key9").value=document.All("key55").value
	   document.All("key10").value=document.All("key56").value
	End Sub 


Sub SrAddrEqual2()
	   document.All("key11").value=document.All("key54").value
	   document.All("key12").value=document.All("key55").value
	   document.All("key13").value=document.All("key56").value
	End Sub 
...
sub sr_xxx()
    msgbox "AAA"
    k24=document.all("key24").value
    k25=document.all("key25").value
    if k24<>"" and k25<>"" then
                        if k25="06" then
                           if k24="01" then
                              document.all("xxx").value="SBAA140603-007 -20M/1M-年約-月繳"
                           elseif k24="02" then
                              document.all("xxx").value="SBAA140602-007 -40M/3M-年約-月繳"
                           elseif k24="03" then
                              document.all("xxx").value="SBAA140601-007 -70M/6M-年約-月繳"
                           else
                              document.all("xxx").value=""
                           end if
                        elseif k25="02" then
                           if k24="01" then
                              document.all("xxx").value="SBAA140603-005 -20M/1M-年約-年繳"
                           elseif k24="02" then
                              document.all("xxx").value="SBAA140602-005 -40M/3M-年約-年繳"
                           elseif k24="03" then
                              document.all("xxx").value="SBAA140601-005 -70M/6M-年約-年繳"
                           else
                              document.all("xxx").value=""
                           end if
                        elseif k25="03" then
                           if k24="01" then
                              document.all("xxx").value="SBAA140603-006 -20M/1M-年約-二年繳"
                           elseif k24="02" then
                              document.all("xxx").value="SBAA140602-006 -40M/3M-年約-二年繳"
                           elseif k24="03" then
                              document.all("xxx").value="SBAA140601-006 -70M/6M-年約-二年繳"
                           else
                              document.all("xxx").value=""
                           end if
                        else
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
<span id="tags1" class="dataListTagsOn">遠傳用戶資訊</span>
...
<DIV ID=SRTAB0 >
	...
	<tr><td width="10%" class=dataListHEAD>用戶(公司)名稱</td>
        ...
        <td width="23%" height="25" bgcolor="silver" > 
	        ...
	        <input type="text" name="key7" size="15" maxlength="15" value="<%=dspkey(7)%>" <%=fieldpg%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Text49">
		</td> 
	</tr>
	...
	<tr><td  class="dataListHEAD" height="23">出生日期</td>
        <td  height="23" bgcolor="silver">
	        ...
	        <input type="button" id="B14" name="B14" onclick="SrBtnOnClick" height="100%" width="100%" <%=fieldpb%> style="Z-INDEX: 1" value="...."> 
	...
	<tr><td width="10%" class=dataListHEAD>聯絡Email</td>
		...
		<%
			...
			If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) and basedata=true Then
				...
				SXX56=" onclick=""SrCountyOnClick()""  "
			Else
				sql="SELECT Cutid,Cutnc FROM RTCounty where cutid='" & dspkey(54) & "' " 
				SXX56=""
			End If
			sx=""    
			rs.Open sql,conn
			Do While Not rs.Eof
				If rs("cutid")=dspkey(54) Then sx=" selected "
	    ...
	    <td bgcolor="silver" COLSPAN=5>
			<select size="1" name="key54" <%=fieldPm%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select54"><%=s%></select>
			<input type="text" name="key55" readonly  size="8" value="<%=dspkey(55)%>" maxlength="10" readonly <%=fieldPm%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4"><font SIZE=2>(鄉鎮)                 
			<input type="button" id="B55" <%=fieldPn%> name="B55"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX56%>  >        
			<IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF" <%=fieldPn%> alt="清除" id="C55"  name="C55"   style="Z-INDEX: 1" onclick="SrClear"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" >
			<input type="text" name="key55zip" size=8 style="color:blue;border:0px;background:transparent;" value="郵遞：<%=SrGetZipName(dspKey(54),dspKey(55))%>" readonly>			
			<input type="text" name="key56"  size="50" value="<%=dspkey(56)%>" maxlength="60"  <%=fieldPm%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListENTRY" ID="Text4">
		</td>
	</tr>
	<tr><td class=dataListHEAD>裝機地址</td>
	    ...
	    <td bgcolor="silver" COLSPAN=5>
			...
			<input type="text" name="key10"  size="50" value="<%=dspkey(10)%>" maxlength="60"  <%=fieldPm%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListENTRY" ID="Text4">
                        <input type="radio" name="rd1"  <%=fieldpb%> onClick="SrAddrEqual1()"><font SIZE=2>同戶籍地址</font>		
            </td>
	</tr>
	   ...
	   %>
	    <td bgcolor="silver" COLSPAN=5>
			...
			<input type="radio" name="rd2"  <%=fieldpb%> onClick="SrAddrEqual3()"><font SIZE=2>同裝機地址</font>
                        <input type="radio" name="rd2"  <%=fieldpb%> onClick="SrAddrEqual2()"><font SIZE=2>同戶籍地址</font>
		</td>
	</tr>

	<TR><td class="dataListHEAD" height="23">企業負責人</td>
		<td height="23" bgcolor="silver">
	        <input type="text" name="key18" size="12" maxlength="12" value="<%=dspKey(18)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text18">
		</td>
        <td class="dataListHEAD" height="23">負責人身分證</td>
        <td height="23" bgcolor="silver" colspan=3>
	        <input type="text" name="key19" size="10" maxlength="10" value="<%=dspKey(19)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text19">
		</td>
	</tr>

	<tr><td class="dataListHEAD" height="23">企業連絡人</td>
		<td height="23" bgcolor="silver">
			<input type="text" name="key20" size="20" maxlength="20" value="<%=dspKey(20)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text20">
		</td>
		<td class="dataListHEAD" height="23">企業連絡人電話</td>
		<td height="23" bgcolor="silver" >
			<input type="text" name="key21" size="30" maxlength="30" value="<%=dspKey(21)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text21">
		</td>
		<td class="dataListHEAD" height="23">企業連絡人手機</td>
		<td height="23" bgcolor="silver" >
			<input type="text" name="key22" size="30" maxlength="30" value="<%=dspKey(22)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text22">
		</td>
	</tr>
	</table>
</div>
...
<DIV ID="SRTAB3">
	...
	<tr><td width="10%" class=dataListHEAD>用戶速率</td>
		<%
		    ...
		    sx=" selected "
		    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) Then 
		       sql="SELECT Code,CodeNC FROM RTCode WHERE KIND ='R6' " 
		    ...
		    Else
		       sql="SELECT Code,CodeNC FROM RTCode WHERE KIND='R6' AND CODE='" & dspkey(24) &"' " 
		       'SXX60=""
		    End If
		    rs.Open sql,conn
		    s=""
	   	...
	   	%>
    	<td width="23%" bgcolor="silver">
        	<select size="1" name="key24" class="dataListEntry" ID="Select24" onselect="sr_xxx()"><%=s%></select>
     	</td>
        ...
        <td WIDTH="23%" height="23" bgcolor="silver" >
		   <select size="1" name="key25" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select25" onselect="sr_xxx()"><%=s%></select>
                    <% 
                     if dspkey(24) <> "" and dspkey(25) <> "" then
                        if dspkey(25)="06" then
                           if dspkey(24)="01" then
                              xxx="SBAA140603-007 -20M/1M-年約-月繳"
                           elseif dspkey(24)="02" then
                              xxx="SBAA140602-007 -40M/3M-年約-月繳"
                           elseif dspkey(24)="03" then
                              xxx="SBAA140601-007 -70M/6M-年約-月繳"
                           else
                              xxx=""
                           end if
                        elseif dspkey(25)="02" then
                           if dspkey(24)="01" then
                              xxx="SBAA140603-005 -20M/1M-年約-年繳"
                           elseif dspkey(24)="02" then
                              xxx="SBAA140602-005 -40M/3M-年約-年繳"
                           elseif dspkey(24)="03" then
                              xxx="SBAA140601-005 -70M/6M-年約-年繳"
                           else
                              xxx=""
                           end if
                        elseif dspkey(25)="03" then
                           if dspkey(24)="01" then
                              xxx="SBAA140603-006 -20M/1M-年約-二年繳"
                           elseif dspkey(24)="02" then
                              xxx="SBAA140602-006 -40M/3M-年約-二年繳"
                           elseif dspkey(24)="03" then
                              xxx="SBAA140601-006 -70M/6M-年約-二年繳"
                           else
                              xxx=""
                           end if
                          else
                    ...
                    %>
                    <input type="TEXT" name="XXX" size=30 class="dataListdata" readonly value="<%=XXX%>">
                </td>

		<td WIDTH="10%" class="dataListHEAD" height="23">遠傳用戶編號</td>
		<td WIDTH="23%" bgcolor="silver" >
			<input type="text" name="key26" size="18" maxlength="16" value="<%=dspKey(26)%>" <%=fieldpa%> <%=fieldRole(1)%> class="dataListdata" ID="Text26" readonly>
		</td>
	</tr>
	</table>
</DIV>
...
<DIV ID="SRTAB1">
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table7">
 	<tr><td width="10%" class="dataListHead" style="display:none">保證金序號</td>
        <td width="23%" bgcolor="silver"  style="display:none">
			 <input type="text" name="key37" size="13" value="<%=dspKey(37)%>"  class="dataListdata" readonly>
		</td>

        <td width="10%" class="dataListHead"  style="display:none">保證金</td>
        <td width="23%" bgcolor="silver"  style="display:none">
        ...
        <td width="10%" class="dataListHead">設定費</td>
        <td width="23%" bgcolor="silver" >
			<input type="text" name="key53" value="<%=dspKey(53)%>" size="10" maxlength="10" <%=fieldRole(1)%><%=fieldPg%><%=dataProtect%> class="dataListEntry">
		</td>
<td width="10%" class="dataListHead">&nbsp;</td>
 <td width="23%" bgcolor="silver">&nbsp;</td>
	</tr>

	<tr><td  class="dataListHEAD" height="23"  style="display:none">保證金收據列印人</td>
		<td height="23" bgcolor="silver"  style="display:none">
        ...
        <td  class="dataListHEAD" height="23"  style="display:none">保證金收據列印日</td>
        <td  height="23" bgcolor="silver"  style="display:none">
			<input type="text" name="key38" size="25" READONLY value="<%=dspKey(38)%>" <%=fieldPg%><%=fieldRole(1)%> class="dataListDATA">
        </td>

		<td WIDTH="10%" class="dataListHEAD" height="23"  style="display:none">保證金退還日</td>
...
<DIV ID="SRTAB4">
	...
	<tr><td  WIDTH="10%" class="dataListHEAD" height="23">遠傳啟用日</td>
        <td  WIDTH="23%" height="23" bgcolor="silver" >
			<input type="text" name="key52" size="12" READONLY value="<%=dspKey(52)%>" class="dataListData" ID="Text52" >
		</td>
		<td  WIDTH="10%" class="dataListHEAD" height="23">遠傳首次繳款日</td>
...
<DIV ID="SRTAB5" > 
  ...
  <DIV ID="SRTAG6">
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
    <tr><td bgcolor="BDB76B" align="CENTER">遠傳繳款記錄</td></tr></table>
</DIV>
    <DIV ID="SRTAB6" > 
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
