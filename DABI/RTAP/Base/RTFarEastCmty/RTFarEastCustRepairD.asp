...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE	SQL
		  ...
		  For i	= 0	To numberOfField-1
			  ...
			  '	  On Error Resume Next
				...
				select case	ucase(runpgm)	
				   ...
				   case	ucase("/webap/rtap/base/RTFareastCmty/RTFareastCustRepaird.asp")
					'response.write	"I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
					   if i	<> 1 then rs.Fields(i).Value=dspKey(i)	  
					   if i=1 then
						 Set rsc=Server.CreateObject("ADODB.Recordset")
						 rsc.open "select max(entryno) AS entryno from RTFareastCustRepair where	CUSID='" & DSPKEY(0) & "' "	,conn
	...
	if accessmode ="A" then
	   runpgm=Request.ServerVariables("PATH_INFO")
	   if ucase(runpgm)=ucase("/webap/rtap/base/RTFareastcmty/RTFareastCustRepairD.asp") then
		  rsc.open "select max(entryno)	AS ENTRYNO from	RTFareastCustRepair where	 CUSID='" &	DSPKEY(0) &	"' " ,conn
		  if not rsC.eof then
...
'	connxx.open	DSN
'	sqlxx="select count(*) as cnt from RTFareastCustCont where cusid='" &	aryparmkey(0) &	"' and finishdat is	null and canceldat is null "
...
%>
  ...
  -->
  <span	id="tags1" class=dataListTagOn>遠傳大寬頻用戶維修收款資訊</span>
  <!--
		   <FONT SIZE=2	COLOR=RED>
		  <MARQUEE width=250 behavior=slide	scrolldelay=30 behavior=alternate>
		  <%
		  SQLYY="SELECT	FINISHDAT FROM RTFareastCust WHERE CUSID='" &	DSPKEY(0) &	"'"
		  RS.OPEN SQLYY,CONN
