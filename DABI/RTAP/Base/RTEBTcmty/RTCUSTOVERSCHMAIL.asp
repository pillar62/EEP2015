...
<% 
   ...
   sql="SELECT  count(*) AS RSCOUNT " _
        ...
        &"WHERE           (RTEBTCUSTSNDWORK.DROPDAT IS NULL) AND (RTEBTCUST.CANCELDAT IS NULL) AND " _
   ...
   sql="SELECT  CASE WHEN RTOBJ_3.SHORTNC IS NULL OR RTOBJ_3.SHORTNC ='' THEN RTAREA.AREANC ELSE RTOBJ_3.SHORTNC END AS 轄區, " _
        ...
        &"                RTEBTCUST.AREAID = RTSalesGroup.AREAID AND " _
        &"                RTEBTCUST.GROUPID = RTSalesGroup.GROUPID " _
        &"WHERE           (RTEBTCUSTSNDWORK.DROPDAT IS NULL) AND " _
        &"                (RTEBTCUSTSNDWORK.UNCLOSEDAT IS NULL) AND " _
        &"                (RTEBTCUSTSNDWORK.CLOSEDAT IS NULL) AND (RTEBTCUST.CANCELDAT IS NULL) AND (DATEDIFF(day, RTEBTCUSTSNDWORK.SENDWORKDAT, GETDATE()) > 6) AND LEN(LTRIM(RTOBJ_3.SHORTNC)) > 0 " _
   ...
   sql="SELECT  CASE WHEN RTOBJ_3.SHORTNC IS NULL OR RTOBJ_3.SHORTNC ='' THEN RTAREA.AREANC ELSE RTOBJ_3.SHORTNC END AS 轄區, " _
        ...
        &"WHERE           (RTEBTCUSTSNDWORK.DROPDAT IS NULL) AND " _
        &"                (RTEBTCUSTSNDWORK.UNCLOSEDAT IS NULL) AND " _
        &"                (RTEBTCUSTSNDWORK.CLOSEDAT IS NULL)  AND (RTEBTCUST.CANCELDAT IS NULL) AND (DATEDIFF(day, RTEBTCUSTSNDWORK.SENDWORKDAT, GETDATE()) > 6) AND (LEN(LTRIM(RTOBJ_3.SHORTNC)) = 0 or rtobj_3.shortnc is null) " _
        &"order by  CASE WHEN RTOBJ_3.SHORTNC IS NULL OR RTOBJ_3.SHORTNC ='' THEN RTAREA.AREANC ELSE RTOBJ_3.SHORTNC END, " _
        &"CASE WHEN RTOBJ_2.SHORTNC IS NULL OR RTOBJ_2.SHORTNC = '' THEN RTOBJ_1.CUSNC ELSE RTOBJ_2.SHORTNC END ,RTEBTCMTYH.COMN, RTEBTCUSTSNDWORK.SENDWORKDAT "
   RS.Open SQL,CONN
   CNT=0
   ...
   IF NOT RS.EOF THEN
      ...
      DO until RS.EOF
         ...
         BODY=BODY & "<tr>" _
             ...
             &"<td bgcolor=pink align=left>" &RS("用戶名稱")&"</td>" _
             &"<td bgcolor=pink align=left>" &RS("已派工日數")&"</td></TR>" 
         if  RS("預定裝機人員") <> prvman then
             prvman=RS("預定裝機人員")
             setupman(i)=RS("信箱") 
             SETUPMAN2(I)=RS("預定裝機人員") 
             i=i+1
         end if
       ...
       I=0
       Do while setupman(i) <> ""
          if setupman(i)="" THEN EXIT DO
          JMail.AddRecipient setupman(i),setupman2(i)
          i=i+1
       loop
       ...
       JMail.AddRecipient "yellow@cbbn.com.tw","黃茂春主任"
       JMail.AddRecipient "maybe0606@cbbn.com.tw","經銷業助窗口"
   END IF         
   IF RSCOUNT1 > 0 OR RSCOUNT2 > 0 THEN
      IF RSCOUNT2 > 0 THEN
         ...
         if ary(2)=1 then
            ...
            JMail.AddRecipient "irene@cbbn.com.tw","台中業助"
         end if                  
         if ary(3)=1 then
            JMail.AddRecipient "jennifer@cbbn.com.tw","王懷瑾"
