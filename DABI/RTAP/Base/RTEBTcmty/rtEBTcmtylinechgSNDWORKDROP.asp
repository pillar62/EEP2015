 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="insert into RTEBTCMTYLINECHGSNDWORKlog " _
           &"SELECT   COMQ1, LINEQ1, PRTNO, PRTNO2, " & ENTRYNO & ", getdate(), 'C','" &  v(0) & "', " _
           ...
           &"REALENGINEER4, REALENGINEER5, REALCONSIGNEE, DROPDAT,'主線異動派工單作廢', " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCMTYLINECHGSNDWORK set dropdat=getdate(),dropdesc='主線異動派工單作廢' where comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' and prtno2='" & key(3) & "' "
   ...
   IF ENDPGM="1" THEN
      ...
      jmail.Subject = "東森AVS主線異動派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "-" &  KEY(3) & "，作廢通知"
      jmail.priority = 1  
      body="<html><body><table border=0 width=""100%""><tr><td colspan=2>" _
      &"<H3>東森AVS主線異動派工單作廢通知</h3></td></tr>" _
      ...
      JMail.AddRecipient "edson@cbbn.com.tw","資訊部"
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此異動派工單已完工/未完工結案，不可作廢(欲作廢請先清除裝機完工日)：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此異動派工單已作廢，不可重覆執行作廢作業：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行異動派工單作廢作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=RTEBTCMTYLINECHGSNDWORKdrop.asp ID="Form1">
