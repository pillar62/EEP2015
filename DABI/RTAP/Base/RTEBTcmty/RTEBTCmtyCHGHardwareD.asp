...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTCmtychgHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=4 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from rtEBTcmtychghardware where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND PRTNO='" & DSPKEY(2) & "' AND PRTNO2='" & DSPKEY(3) & "' "  ,conn
                         if len(trim(rsc("entryno"))) > 0 then 
                            dspkey(4)=rsc("entryno") + 1
                         else
                            dspkey(4)=1
                         end if
                         rsc.close
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTCmtychghardwared.asp") then
          rs.open "select max(entryno) AS ENTRYNO from rtEBTcmtychghardware WHERE  comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND PRTNO='" & DSPKEY(2) & "' AND PRTNO2='" & DSPKEY(3) & "' " ,conn
          if not rs.eof then
            dspkey(4)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=5
  title="AVS主線異動派工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1, LINEQ1, PRTNO,prtno2, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, DROPUSR,UNIT,eusr,edat,uusr,udat " _
             &"FROM RTEBTCmtychghardware WHERE COMQ1=0 "
  sqlList="SELECT COMQ1, LINEQ1, PRTNO,prtno2, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, DROPUSR,UNIT,eusr,edat,uusr,udat " _
             &"FROM RTEBTCmtychghardware WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTEBTCMTYLINEchgSNDWORK WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' AND PRTNO2='" & DSPKEY(3) & "' "
    ...
    IF ERRFLG="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(3) & "非標準工程，不可輸入設備明細"   
    elseif  ERRFLG1="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(3) & " 已執行庫存計算作業，不可再輸入設備明細"   
      ...
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="15%" class=dataListHead>社區序號</td>
           ...
           <input type="text" name="key1"
                 <%=fieldRole(1)%> readonly size="3" value="<%=dspKey(1)%>" maxlength="3" class=dataListdata ID="Text23"></td>                 
           ...
           <input type="text" name="key3"
                 <%=fieldRole(1)%> readonly size="12" value="<%=dspKey(3)%>" maxlength="12" class=dataListdata ID="Text6"></td>                 
...
<td width="15%" class=dataListHead>資產編號</td></td>
    <td width="35%" bgcolor="silver" >
        <input type="text" name="key11" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               ...
               value="<%=dspKey(11)%>" size="15" class=dataListEntry ID="Text1">
    </td>    
</tr>
<td width="10%" class=dataListHead>設備/規格名稱</td></td>
...
<tr><td class=dataListHead>數量</td>
    ...
    Else
       sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='B5' AND CODE='" & dspkey(13) & "'"
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
    ...
    Loop
    rs.Close%>         
   <select size="1" name="key13" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select15">                                                                  
        <%=s%>
   </select>        </td>
...
<tr>
 ...
 <tr>
   ...
   <input type="text" name="key9" size="90" MAXLENGTH=90 value="<%=dspKey(9)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListEntry" ID="Text16">
   </TD>
</tr>    
<tr>
