...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
  ...
  '先開放給jacqueline for 例外狀況(920416)
   elseif LEN(TRIM(RSXX("CLOSEdat"))) <> 0 and (v(0) <>"T94200" or v(0) <>"T92134" or v(0)<>"T95222" or v(0)<>"T94182") then
      endpgm="4"
   ELSE
      endpgm="1"
      errmsg=""
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS用戶竣工確認單列印完成",0
       Set winP=window.Opener
       Set docP=winP.document       
       winP.focus()           
       docP.all("keyform").Submit     
       WINDOW.Form2.action="rtebtCUSTsndp.asp" & "?key=" & document.all("key1").value & "," & document.all("key2").value & "," & document.all("key3").value & "," & document.all("key4").value & "," & document.all("key5").value
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此竣工單已作廢，不可列印" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此竣工單已完工結案，不可列印" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行竣工單列印作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action="rtebtCUSTsndworkdrop.asp" ID="Form1">
