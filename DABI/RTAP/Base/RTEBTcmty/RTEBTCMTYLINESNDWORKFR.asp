...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   sqlxx="select * FROM RTEBTCMTYLINEsndwork WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' "
 ...
 '  RESPONSE.END
   ...
   END IF 
   '當主線已測通回報時，不可執行結案返轉
   IF NOT ISNULL(RSYY("APPLYUPLOADDAT")) OR  NOT ISNULL(RSYY("APPLYUPLOADTNS")) THEN
      ENDPGM="7"
   '當已作廢時，不可執行結案返轉
   ...
   ELSE
      ...
      sqlyy="select max(entryno) as entryno FROM RTEBTCMTYLINEsndworklog WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' "
      ...
      IF NOT ISNULL(RSXX("FINISHDAT")) THEN
         sqlyy="insert into RTEBTCMTYLINEsndworklog " _ 
           &"SELECT   COMQ1, LINEQ1, PRTNO, " & ENTRYNO & ", GETDATE() ,'FR','" & V(0) & "',SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, " _
           ...
           &"FROM RTEBTCMTYLINEsndwork where comq1=" & key(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' "
         STS="1"
      '未完工結案返轉
      ELSE
         sqlyy="insert into RTEBTCMTYLINEsndworklog " _ 
           &"SELECT   COMQ1, LINEQ1, PRTNO, " & ENTRYNO & ", GETDATE() ,'UR','" & V(0) & "',SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, " _
           &"ASSIGNENGINEER3, ASSIGNENGINEER4, ASSIGNENGINEER5, ASSIGNCONSIGNEE, REALENGINEER1, REALENGINEER2, REALENGINEER3, " _
           &"REALENGINEER4, REALENGINEER5, REALCONSIGNEE, DROPDAT,DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, " _
           &"BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT,STOCKCLOSEUSR, STOCKFINCHK, SNDTYPE, HOSTCABLENO, MEMO, " _
           &"PRTDAT, EUSR, EDAT, UUSR, UDAT, CLOSEUSR, DROPUSR, UNCLOSEDAT,finishdat " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTcmtylinesndwork set FINISHdat=NULL,UNCLOSEDAT=NULL,CLOSEUSR='',UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1)  & " and prtno='" & key(2) & "' "
         ...
         If Err.number > 0 then
            ...
            IF (sndtype ="OC" OR  sndtype ="OP" OR  sndtype ="ST" OR  sndtype ="CB") and len(trim(finishdat)) > 0 THEN
               SQLXX=" update RTEBTcmtyline set adslapplydat=null,UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND RTEBTcmtyline.ADSLAPPLYDAT IS not NULL "
               STS="3"
               connxx.Execute SQLXX
               If Err.number > 0 then
                  endpgm="2"
 ...
 '  END IF
   ...
   IF ENDPGM="1" AND STS="3" THEN '當需更新主線之完工日的派工種類(STS=3)返轉才須要通知相關人員
      ...
      IF STS="1" THEN
         jmail.Subject = "AVS社區︰" & COMN & "，派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，完工結案返轉通知"
      ELSE
         jmail.Subject = "AVS社區︰" & COMN & "，派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，未完工結案返轉通知"
      END IF
      ...
      &"<tr><td bgcolor=lightblue align=center>派工單號</td><td bgcolor=pink align=left>" &KEY(2) &"</td></tr>" _
      &"</table>" _
      &"<P><U>此主線完工記錄已被返轉，尚不可執行用戶申請作業</U></body></html>"  
      jmail.HTMLBody = BODY
