...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS用戶欠費拆機工單列印完成",0
       Set winP=window.Opener
       Set docP=winP.document       
       winP.focus()           
       docP.all("keyform").Submit     
       WINDOW.Form2.action="RTEBTCUSTm2m3SNDWORKP.asp" & "?key=" & document.all("key1").value & "," & document.all("key2").value & "," & document.all("key3").value & "," & document.all("key4").value & "," & document.all("key5").value
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此拆機工單已作廢，不可列印" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此拆機工單已完工結案，不可列印" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行拆機工單列印作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
