...
Sub btn_onClick()
 ...
 ' End If
  ...
  elseif s7ary(0)="5" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND RTEBTCMTYLINE.ADSLAPPLYDAT IS NOT NULL " 
 End If        
  '----是否可建置
  ...
  if  Len(trim(s10))=0 Or s10="" then
     ...
     s=s & " 合約編號︰包含('" & s10 & "')字元 "
     t=t & " and (rtebtcust.avsno like '%" & s10 & "%') "
  end if  
  '----avs繳款方式
  ...
  If Len(trim(s16)) > 0 Then
     s=s &"  社區序號:('" &s16 & "') "
     t=t &" AND (RTEBTCUST.COMQ1=" & S16 & ") "
  End If   
  '----主線序號
  s17=document.all("search17").value
  If Len(trim(s17)) > 0 Then
     s=s &"  主線序號:('" &s17 & "') "
     t=t &" AND (RTEBTCUST.LINEQ1=" & S17 & ") "
  End If            
  '----用戶進度狀況
  ...
  ELSEIF s18ARY(0) = "1" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND (RTEBTCUST.APPLYDAT IS NULL ) "
  '已申請尚未取得合約編號
  ELSEIF s18ARY(0) = "2" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND (RTEBTCUST.APPLYDAT IS NOT NULL ) AND RTEBTCUST.AVSNO ='' "     
  '未完工
  ELSEIF s18ARY(0) = "3" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND (RTEBTCUST.AVSNO <> '' AND RTEBTCUST.FINISHDAT IS NULL ) "      
  '未報竣
  ELSEIF s18ARY(0) = "4" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND RTEBTCUST.FINISHDAT IS NOT NULL AND  RTEBTCUST.DOCKETDAT IS NULL "               
  '未轉檔
  ELSEIF s18ARY(0) = "5" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND  RTEBTCUST.DOCKETDAT IS NOT NULL AND RTEBTCUST.TRANSDAT IS NULL "               
  '已退租
  ELSEIF s18ARY(0) = "6" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND  RTEBTCUST.dropdat IS NOT NULL AND RTEBTCUST.docketdat IS not NULL "       
  '已作廢
  ELSEIF s18ARY(0) = "7" THEN
     s=s &"  用戶進度:('" &s18ARY(1) & "') "
     t=t &" AND  RTEBTCUST.CANCELDAT IS NOT NULL AND RTEBTCUST.docketdat IS NULL "                 
  End If              
  s19ary=split(document.all("search19").value,";")  
  IF S19ARY(0) <> "" THEN
     S=S & " 其它方案移轉︰('" & S19ARY(1) & "') "
     T=T & " AND RTEBTCUST.TNSCUSTCASE='" & S19ARY(0) & "' "
  END IF
  '----用戶方案
  s20ary=split(document.all("search20").value,";")  
  If Len(trim(s20ARY(0))) = 0 Then
  'AVS+CBC
  ELSEIF s20ARY(0) = "1" THEN
     s=s &"  方案:('" &s20ARY(1) & "') "
     t=t &" AND (RTEBTCUST.CASETYPE ='01' ) "
  'AVS-ONLY
  ELSEIF s20ARY(0) = "2" THEN
     s=s &"  方案:('" &s20ARY(1) & "') "
     t=t &" AND (RTEBTCUST.CASETYPE ='02' )  "  
  END IF     
  Dim winP,docP
...
<tr><td class=dataListHead width="40%">方案</td>
    <td width="60%"  bgcolor="silver">
      <select name="search20" size="1" class=dataListEntry ID="Select2">
        <option value=";全部" selected>全部</option>
        <option value="1;AVS+CBC">AVS+CBC</option>
        <option value="2;AVS-ONLY">AVS-ONLY</option>    
<tr><td class=dataListHead width="40%">用戶進度狀況</td>
    <td width="60%"  bgcolor="silver">
      <select name="search18" size="1" class=dataListEntry ID="Select1">
        <option value=";全部" selected>全部</option>
        <option value="1;尚未申請">尚未申請</option>
        <option value="2;已申請尚未取得合約編號">已申請尚未取得合約編號</option>
        <option value="3;未完工">未完工</option>                
        <option value="4;未報竣">未報竣</option>
        <option value="5;未轉檔">未轉檔</option>      
...
<tr><td class=dataListHead width="40%">用戶身份證/統編</td>
    <td width="60%" bgcolor="silver">
      <input type="password" size="10" name="search13" class=dataListEntry> 
    </td></tr>          
...
<tr><td class=dataListHead width="40%">其它方案移轉</td>
    <td width="60%" bgcolor="silver">
    <select name="SEARCH19" class=dataListEntry> <%=S19%></select>
    </td></tr>                  
