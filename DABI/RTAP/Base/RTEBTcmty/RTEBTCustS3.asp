...
Sub btn_onClick()
 ...
 End If        
  ...
  if s18ary(0) = "1" then
     s=s &"  竣工單:('" &s18ary(1) & "') "
     t=t &" AND (RTEBTCUSTsndwork.prtno <> null) "
  ...
  If Len(trim(s19)) > 0 Then
     s=s &"  竣工單號:包含('" &s19 & "') 字元"
     t=t &" AND (RTEBTCUSTsndwork.prtno like '%" & S19 & "%') "
  End If           
...
<tr><td class=dataListHead width="40%">派工狀況</td>
    <td width="60%" bgcolor="silver">
     <select name="search18" size="1" class=dataListEntry ID="Select1">
        <option value=";全部" selected>全部</option>
        <option value="1;已印竣工單">已印竣工單</option>
        <option value="2;未印竣工單">未印竣工單</option>
     </select>
    </td></tr>            
<tr><td class=dataListHead width="40%">竣工單號</td>
    <td width="60%" bgcolor="silver">
      <input type="text" size="20" name="search19" class=dataListEntry ID="Text7"> 
    </td></tr>      
