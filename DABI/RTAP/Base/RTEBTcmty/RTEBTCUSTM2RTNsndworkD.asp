...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTCUSTM2RTNSNDWORKd.asp")
                       ...
                       if i=3 then
                         ...
                         rsc.open "select max(RTNNO) AS RTNNO from RTEBTCUSTM2RTNSNDWORK where RTNNO like '" & cusidxx & "%' " ,conn
                         if len(rsc("RTNNO")) > 0 then
                            dspkey(3)=cusidxx & right("0000" & cstr(cint(right(rsc("RTNNO"),4)) + 1),4)
                         else
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTCUSTM2M3SNDWORKd.asp") then
          cusidxx="R-" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(RTNNO) AS RTNNO from RTEBTCUSTM2RTNSNDWORK where RTNNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(3)=rsC("RTNNO")
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=4
  title="AVS用戶M2復機派工單資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT AVSNO,M2M3,seq, RTNNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTEBTCUSTM2RTNSNDWORK WHERE AVSNO='' "
  sqlList="SELECT AVSNO,M2M3,seq, RTNNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat " _
             &"FROM RTEBTCUSTM2RTNSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    '新增復機工單時，要先檢查系統是否已有復機工單
    errcode=""    
    IF ACCESSMODE="A" THEN
       Set connXX=Server.CreateObject("ADODB.Connection")
       Set rsXX=Server.CreateObject("ADODB.Recordset")
       connXX.open DSN
       SQLXX="SELECT COUNT(*) as cnt FROM RTEBTCUSTM2RTNSNDWORK WHERE AVSNO='" & DSPKEY(0) & "' AND M2M3='" & DSPKEY(1) & "' and seq=" & dspkey(2) & " AND DROPDAT IS NULL "
    ...
    if ERRCODE="Y" THEN
       formValid=False
       message="此用戶已存在復機工單，不可重複建立" 
    ELSEIF LEN(TRIM(dspkey(4))) = 0 then
       formValid=False
       message="派工日期不可空白" 
    ELSEif len(trim(DSPKEY(6))) = 0 and len(trim(DSPKEY(7))) = 0 THEN
       formValid=False
       message="預定復機人員或經銷商必須輸入其中一項" 
    ELSEif len(trim(DSPKEY(6))) > 0 and len(trim(DSPKEY(7))) > 0 THEN
       formValid=False
       message="預定復機人員及經銷商只能輸入其中一項"    
    ELSEif len(trim(DSPKEY(8))) = 0 and len(trim(DSPKEY(9))) = 0 AND LEN(TRIM(DSPKEY(12))) > 0 THEN
       formValid=False
       message="實際復機人員或經銷商必須輸入其中一項" 
    ELSEif len(trim(DSPKEY(8))) > 0 and len(trim(DSPKEY(9))) > 0 AND LEN(TRIM(DSPKEY(12))) > 0 THEN
       formValid=False
       message="實際復機人員及經銷商只能輸入其中一項"    
 '   elseif len(trim(DSPKEY(12))) > 0 and len(trim(DSPKEY(10))) = 0  AND ERRCODE="1" THEN
 '      formValid=False
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="12%" class=dataListHead>AVS合約編號</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key0"
                 <%=fieldRole(1)%> readonly size="15" value="<%=dspKey(0)%>" maxlength="15" class=dataListdata ID="Text22"></td>
           <td width="12%" class=dataListHead>M2M3</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key1"
                 <%=fieldRole(1)%> readonly size="4" value="<%=dspKey(1)%>" maxlength="4" class=dataListdata ID="Text23"></td>    
           <td width="12%" class=dataListHead>單次</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key2"
                 <%=fieldRole(1)%> readonly size="3" value="<%=dspKey(2)%>" maxlength="3" class=dataListdata ID="Text30"></td>    
           <td width="15%" class=dataListHead>復機單號</td>
...
<tr><td class=dataListHEAD>預定復機人員</td>
<td bgcolor="silver" >
<%
  sql="SELECT * FROM RTEBTCUSTM2M3SNDWORK LEFT OUTER JOIN RTEBTCUST ON RTEBTCUSTM2M3SNDWORK.AVSNO=RTEBTCUST.AVSNO LEFT OUTER JOIN RTEBTCMTYLINE ON RTEBTCUST.COMQ1=RTEBTCMTYLINE.COMQ1 AND RTEBTCUST.LINEQ1=RTEBTCMTYLINE.LINEQ1 WHERE RTEBTCUSTM2M3SNDWORK.AVSNO='" & DSPKEY(0) & "' "
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
 ...
 %>
        ...
        <input type="TEXT" name="key6" <%=fieldRole(1)%><%=dataProtect%> 
           ...
           <input type="BUTTON" id="B6"   <%=FIELDPA%> name="B6"  width="100%" style="Z-INDEX: 1"  value="...." <%=FIELDP6%>  >   
...
<td class=dataListHEAD>實際復機人員</td>
<td bgcolor="silver" >
<%
  name="" 
           if dspkey(8) <> "" then
 ...
 %>
        <input type="TEXT" name="key8" <%=fieldRole(1)%><%=dataProtect%> 
           ...
           <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"  <%=FIELDPA%> alt="清除" id="C8"  name="C8"   style="Z-INDEX: 1" border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut"   <%=FIELDPD%>>
           <font size=2><%=name%></font>
</td>
 </tr>  
<tr><td  class=dataListHead>預定復機經銷商</td>
    <td  bgcolor="silver" >
    ...
    <td  class=dataListHead>實際復機經銷商</td>
    <td  bgcolor="silver" >
        <%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  Then 
       sql="SELECT RTObj.CUSNC, RTObjLink.CUSTYID, RTObj.CUSID,RTObj.SHORTNC " _
          &"FROM RTObj INNER JOIN " _
 ...
 <td class=dataListHEAD>獎金計算月</td>
  ...
  <tr>        
   ...
   <%   NAME=""
          if dspkey(15) <> "" then
              sql=" select cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
                   &"where rtemployee.emply='" & dspkey(154) & "' "
              rs.Open sql,conn
              if rs.eof then
                 name="(對象檔找不到結帳人員)"
              else
...
<tr>
  ...
  %>    <input type="text" name="key32" size="6" READONLY value="<%=dspKey(32)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
    ...
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%" name="key25" rows=5  MAXLENGTH=300 <%=fieldpa%>  class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(25)%>" ID="Textarea1"><%=dspkey(25)%></TEXTAREA>
   </td></tr>
 </table>            
   </table> 
  </div> 
