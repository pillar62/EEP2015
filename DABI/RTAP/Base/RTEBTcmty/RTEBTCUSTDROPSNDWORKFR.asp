...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   ...
   sqlYY="select * FROM RTEBTCUSTDROP WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' AND ENTRYNO=" & KEY(3) 
   ...
   '  response.end
     ...
     ' Response.Write sqlyy
        ...
        If Err.number > 0 then
           ...
           'IF4-1S
           IF LEN(TRIM(RSXX("CLOSEDAT"))) > 0 THEN
              SQLXX=" update RTEBTCUSTDROPsndwork set CLOSEdat=NULL,CLOSEUSR='' where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' AND ENTRYNO=" & KEY(3) & " and prtno='" & key(4) & "' "
              ...
              If Err.number > 0 then
                ...
                '將用戶主檔之FINISHDAT 清空
                 SQLXX=" update RTEBTCUSTDROP set FINISHdat=NULL,UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' AND ENTRYNO=" & KEY(3) 
                 connxx.Execute SQLXX
                 'IF6-1S
                 If Err.number > 0 then
                    endpgm="2"
           ...
           ELSE
           '未完工結案返轉
              SQLXX=" update RTEBTCUSTDROPsndwork set UNCLOSEdat=NULL,CLOSEUSR='' where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' AND ENTRYNO=" & KEY(3) & " and prtno='" & key(4) & "' "
              connxx.Execute SQLXX
              'IF5-2S
              If Err.number > 0 then
                 endpgm="2"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS用戶拆機竣工確認單完工/未完工結案返轉成功",0
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此拆機竣工單尚未結案，不可執行結案返轉作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此拆機竣工單已作廢，不可執行結案返轉作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "無法找到此拆機竣工單之用戶主檔資料，請確認AVS用戶主檔資料正常" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                     
    elseIF frm1.htmlfld.value="7" then
       msgbox "用戶拆機已報竣，不可執行完工結案返轉作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                     
    else
       msgbox "無法執行拆機竣工單完工結案返轉作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
