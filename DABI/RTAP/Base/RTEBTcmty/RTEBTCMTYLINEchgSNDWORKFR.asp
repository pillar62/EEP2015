...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
   ...
   RSYY.Open SQLYY,CONNXX
   '當主線異動(移機)已向EBT完工報竣後，不可執行結案返轉
   IF ( NOT ISNULL(RSYY("DOCKETDAT")) OR  NOT ISNULL(RSYY("TRANSDAT"))) AND RSYY("CHGCOD3") = 1 THEN
      ENDPGM="7"
  '當主線異動(頻寬異動)已向EBT申請後，不可執行結案返轉(基本上不會發生==>因為非移機之異動是不可能建立派工單的)
   ELSEIF ( NOT ISNULL(RSYY("UPDEBTCHKDAT")) OR  NOT ISNULL(RSYY("UPDEBTTNSDAT"))) AND RSYY("CHGCOD3") = 0 THEN
      ENDPGM="8"      
   '當已主線異動派工單作廢時，不可執行結案返轉
   ELSEIF LEN(TRIM(RSXX("DROPDAT"))) <> 0 THEN
      ENDPGM="3"
   '當已主線異動單作廢時，不可執行結案返轉
   ELSEIF LEN(TRIM(RSYY("DROPDAT"))) <> 0 THEN
      ENDPGM="9"      
   '當主線異動派工單完工結案或未完工結案日皆空白時，不可執行結案返轉
   ...
   ELSE
      ...
      IF NOT ISNULL(RSXX("FINISHDAT")) THEN
         sqlyy="insert into RTEBTCMTYLINECHGSNDWORKlog " _ 
           &"SELECT   COMQ1, LINEQ1, PRTNO, PRTNO2, " & ENTRYNO & ", GETDATE() ,'FR','" & V(0) & "',SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, " _
      ...
      ELSE
         sqlyy="insert into RTEBTCMTYLINECHGSNDWORKlog " _ 
           &"SELECT   COMQ1, LINEQ1, PRTNO, PRTNO2, " & ENTRYNO & ", GETDATE() ,'UR','" & V(0) & "',SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, " _
           &"ASSIGNENGINEER3, ASSIGNENGINEER4, ASSIGNENGINEER5, ASSIGNCONSIGNEE, REALENGINEER1, REALENGINEER2, REALENGINEER3, " _
           &"REALENGINEER4, REALENGINEER5, REALCONSIGNEE, DROPDAT,DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, " _
           &"BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT,STOCKCLOSEUSR, STOCKFINCHK, SNDTYPE, HOSTCABLENO, MEMO, " _
           &"PRTDAT, EUSR, EDAT, UUSR, UDAT, CLOSEUSR, DROPUSR, UNCLOSEDAT,finishdat " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCMTYLINECHGSNDWORK set FINISHdat=NULL,UNCLOSEDAT=NULL,CLOSEUSR='',UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1)  & " and prtno='" & key(2) & "' and prtno2='" & key(3) & "' "
         ...
         If Err.number > 0 then
            ...
            IF (sndtype ="OC" OR  sndtype ="OP" OR  sndtype ="ST" OR  sndtype ="CB") and len(trim(finishdat)) > 0 THEN
               SQLXX=" update RTEBTcmtylineCHG set FINISHDAT=null,UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1) & " and PRTNO='" & key(2) & "' AND RTEBTcmtylineCHG.FINISHDAT IS not NULL "
               STS="3"
               connxx.Execute SQLXX
               If Err.number > 0 then
                  endpgm="2"
 ...
 '  END IF
   ...
   IF ENDPGM="1" AND STS="3" THEN '當需更新主線之完工日的派工種類(STS=3)返轉才須要通知相關人員
      ...
      IF STS="1" THEN
         jmail.Subject = "AVS社區︰" & COMN & "，異動派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，完工結案返轉通知"
      ELSE
         jmail.Subject = "AVS社區︰" & COMN & "，異動派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，未完工結案返轉通知"
      END IF
      
      jmail.priority = 1  
      body="<html><body><table border=0 width=""60%""><tr><td colspan=2>" _
      &"<H3>東森AVS主線異動派工結案返轉通知</h3></td></tr>" _
      &"<tr><td width=""30%"">&nbsp;</td><td width=""70%"">&nbsp;</td></tr>" _
      &"<tr><td bgcolor=lightblue align=center>社區主線序號</td><td bgcolor=pink align=left>" &KEY(0) & "-" & KEY(1) &"</td></tr>" _
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "當已主線異動派工單作廢時，不可執行結案返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "當主線異動派工單完工結案或未完工結案日皆空白時，不可執行結案返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    ...
    elseIF frm1.htmlfld.value="7" then
       msgbox "當主線異動(移機)已向EBT完工報竣後，不可執行結案返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close        
    elseIF frm1.htmlfld.value="8" then
       msgbox "當主線異動(頻寬異動)已向EBT申請後，不可執行結案返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close        
    elseIF frm1.htmlfld.value="9" then
       msgbox "當已主線異動單作廢時，不可執行結案返轉" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                                    
    else
...
<form name=frm1 method=post action=RTEBTCMTYLINECHGSNDWORKFR.asp ID="Form1">
