...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustM2RTNSNDWORKHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=4 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTEBTcustM2RTNSNDWORKHARDWARE where AVSNO='" & dspkey(0) & "' and M2M3='" & dspkey(1) & "' AND SEQX=" & DSPKEY(2) & " AND RTNNO='" & DSPKEY(3) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustM2RTNSNDWORKHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTEBTcustM2RTNSNDWORKHARDWARE WHERE AVSNO='" & dspkey(0) & "' and M2M3='" & dspkey(1) & "' AND SEQX=" & DSPKEY(2) & " AND RTNNO='" & DSPKEY(3) & "' " ,conn
          if not rs.eof then
            dspkey(4)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT AVSNO,M2M3,seqx,RTNNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustM2RTNSNDWORKHARDWARE WHERE AVSNO='' "
  sqlList="SELECT AVSNO,M2M3,seqx,RTNNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustM2RTNSNDWORKHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTEBTCUSTM2RTNSNDWORK WHERE AVSNO='" & DSPKEY(0) & "' AND M2M3='" & DSPKEY(1) & "' and seq=" & dspkey(2) & " AND RTNNO='" & DSPKEY(3) & "' "
    ...
    IF LEN(DSPKEY(13))=0 THEN DSPKEY(13)=0
    IF ERRFLG="Y" THEN
       formValid=False
       message="竣工單" & DSPKEY(3) & " 已執行庫存計算，不可再輸入設備明細"    
    ...
    ELSEif len(trim(DSPKEY(13))) > 0 AND NOT ISNUMERIC(DSPKEY(13))  THEN
       formValid=False
       message="金額格式不符"        
...
' -------------------------------------------------------------------------------------------- 
  ...
  -->
  <span id="tags1" class="dataListTagsOn">AVS用戶設備資料</span>
...
<%  set conn=server.CreateObject("ADODB.Connection")
    ...
    <input type="button" id="B6"  name="B6" height="100%" width="100%" style="Z-INDEX: 1" value="...." <%=SXX5%> >
