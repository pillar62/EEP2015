...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=4 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTEBTcustHARDWARE where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND CUSID='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustHARDWAREd.asp") then
          rs.open "select max(entryno) AS ENTRYNO from RTEBTcustHARDWARE WHERE  comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND CUSID='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' " ,conn
          if not rs.eof then
            dspkey(4)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=5
  title="AVS用戶竣工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1, LINEQ1, cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustHARDWARE WHERE COMQ1=0 "
  sqlList="SELECT COMQ1, LINEQ1, cusid,PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTEBTCUSTSNDWORK WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND CUSID='" & DSPKEY(2) & "' AND PRTNO='" & DSPKEY(3) & "' "
