...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "AVS用戶拆機竣工確認單列印完成",0
       Set winP=window.Opener
       Set docP=winP.document       
       winP.focus()           
       docP.all("keyform").Submit     
       WINDOW.Form2.action="/rtap/base/rtebtcmty/rtebtCustDROPsndp.asp" & "?key=" & document.all("key1").value & "," & document.all("key2").value & "," & document.all("key3").value & "," & document.all("key4").value & "," & document.all("key5").value & "," & document.all("key6").value
...
<form name=frm1 method=post action="RTEBTCustDropSndWorkPV.asp" ID="Form1">
