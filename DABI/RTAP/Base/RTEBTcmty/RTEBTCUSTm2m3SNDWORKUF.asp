 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="insert into RTEBTCUSTm2m3sndworklog " _
           &"SELECT   avsno, m2m3,seq,PRTNO, " & ENTRYNO & ", getdate(), 'UF','" &  v(0) & "', " _
           &"SENDWORKDAT, PRTUSR, ASSIGNENGINEER,ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE, DROPDAT, 'AVS用戶欠費拆機未完工結案'," _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCUSTm2m3sndwork set UNCLOSEdat=getdate(),CLOSEUSR='" & V(0) & "' where avsno='" & key(0) & "' and m2m3='" & key(1) & "' and seq=" & key(2) & " and prtno='" & key(3) & "' "
      ...
      '         '發生錯誤時，刪除異動檔所新增的異動資料
      '         errmsg=cstr(Err.number) & "=" & Err.description
      '         sqlyy="delete * FROM RTEBTCUSTsndworklog WHERE comq1=" & key(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' and seq=" & key(2) & " and prtno='" & key(3) & "' AND ENTRYNO=" & ENTRYNO
      '         CONNXX.Execute sqlyy 
      '      ELSE
               endpgm="1"
               errmsg=""
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "此拆機工單未完工結案時，必須先輸入實際拆機人員或實際拆機經銷商" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close                  
    else
       msgbox "無法執行欠費拆機工單未完工結案作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=rtebtcustm2m3sndworkuf.asp ID="Form1">
