...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTCUSTSNDWORKd.asp")
                       ...
                       if i=3 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTEBTCUSTSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTCUSTSNDWORKd.asp") then
          cusidxx="S-" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTEBTCUSTSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(3)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1, LINEQ1,cusid, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER," _
             ...
             &"FROM RTEBTCUSTSNDWORK WHERE COMQ1=0 "
  sqlList="SELECT COMQ1, LINEQ1,cusid, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER, " _
             &"ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, MDF1,MDF2,HOSTNO,HOSTPORT, MEMO,PRTDAT,UNCLOSEDAT,DROPUSR,CLOSEUSR,eusr,edat,uusr,udat " _
             &"FROM RTEBTCUSTSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connXX.open DSN
    SQLXX="SELECT * FROM RTEBTCUSTHARDWARE WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " and cusid='" & dspkey(2) & "' and prtno='" & dspkey(3) & "' "
...
<%
 ...
 %>
        ...
        <input type="TEXT" name="key6" <%=fieldRole(1)%><%=dataProtect%> 
           ...
           <input type="BUTTON" id="B6" <%=FIELDPA%> name="B6"  width="100%" style="Z-INDEX: 1"  value="...." <%=FIELDP6%> <%=consignee%> >   
