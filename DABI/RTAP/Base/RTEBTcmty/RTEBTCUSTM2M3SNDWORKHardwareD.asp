...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustM2M3SNDWORKHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=4 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(SEQ) AS SEQ from RTEBTcustM2M3SNDWORKHARDWARE where AVSNO='" & dspkey(0) & "' and M2M3='" & dspkey(1) & "' and seqx=" & dspkey(2) & " AND PRTNO='" & DSPKEY(3) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTcustM2M3SNDWORKHARDWAREd.asp") then
          rs.open "select max(SEQ) AS SEQ from RTEBTcustM2M3SNDWORKHARDWARE WHERE AVSNO='" & dspkey(0) & "' and M2M3='" & dspkey(1) & "' and seqx=" & dspkey(2) & " AND PRTNO='" & DSPKEY(3) & "' " ,conn
          if not rs.eof then
            dspkey(4)=rs("SEQ")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT AVSNO,M2M3,seqx,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustM2M3SNDWORKHARDWARE WHERE AVSNO='' "
  sqlList="SELECT AVSNO,M2M3,seqx,PRTNO, SEQ, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, DROPUSR,UNIT,amt,eusr,edat,uusr,udat,memo " _
             &"FROM RTEBTcustM2M3SNDWORKHARDWARE WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTEBTCUSTM2M3SNDWORK WHERE AVSNO='" & DSPKEY(0) & "' AND M2M3='" & DSPKEY(1) & "' and seq=" & dspkey(2) & " AND PRTNO='" & DSPKEY(3) & "' "
...
Sub SrGetUserDefineKey()%>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="7%" class=dataListHead>合約編號</td>
           ...
           <td width="11%" class=dataListHead>拆機工單</td>
