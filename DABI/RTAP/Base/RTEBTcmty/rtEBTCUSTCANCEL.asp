...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
   ...
   endpgm="1"
   '當用戶已提出申請且尚未轉檔時，不可直接作廢==>必須先執行清除申請作業
   IF LEN(TRIM(RSXX("APPLYDAT"))) <> 0  AND ISNULL(RSXX("APPLYTNSDAT")) THEN
      ENDPGM="3"
   ...
   ELSE
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCUST set CANCELdat=getdate(),CANCELUSR='" & V(0) & "' where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND CUSID='" & KEY(2) & "' "
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此用戶已向EBT提出申請，不可作廢(欲作廢請先清除申請)" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="5" then
       msgbox "此用戶已報竣，不可作廢(請執行退租作業)" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
