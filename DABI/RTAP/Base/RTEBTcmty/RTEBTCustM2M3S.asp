...
Sub btn_onClick()
  ...
  Elseif s7ary(0)="1" then
     s=s &"  處理進度:" &s7ary(1)
     t=t &" AND (RTEBTCustM2M3.DROPDAT IS NOT NULL ) "
  elseif s7ary(0)="2" then
     s=s &"  處理進度:" &s7ary(1)
     t=t &" AND (RTEBTCustM2M3.CLOSEDAT IS NOT NULL ) " 
  elseif s7ary(0)="3" then
     s=s &"  處理進度:" &s7ary(1)
     t=t &" AND (RTEBTCustM2M3.CLOSEDAT IS NULL AND RTEBTCustM2M3.DROPDAT IS NULL) "      
  End If   
...
<tr><td class=dataListHead width="40%">M2M3資料選擇</td>
    <td width="60%"  bgcolor="silver">
      <select name="search6" size="1" class=dataListEntry ID="Select1">
        ...
        <option value="2;M3">M3</option>
      </select>
     </td>
</tr>    
<tr><td class=dataListHead width="40%">M2M3處理進度</td>
    <td width="60%"  bgcolor="silver">
      <select name="search7" size="1" class=dataListEntry ID="Select2">
        <option value=";全部" selected>全部</option>
        <option value="1;已作廢">已作廢</option>
        <option value="2;已結案">已結案</option>
        <option value="3;未處理">未處理</option>
      </select>
     </td>
</tr>    
