...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="select max(entryno) as entryno FROM RTEBTCMTYLINECHGSNDWORKlog WHERE comq1=" & KEY(0) & " and lineq1=" & key(1) & " and prtno='" & key(2) & "' and prtno='" & key(3) & "' "
      ...
      sqlyy="insert into RTEBTCMTYLINECHGSNDWORKlog " _ 
           &"SELECT   COMQ1, LINEQ1, PRTNO, PRTNO2, " & ENTRYNO & ", GETDATE() ,'UF','" & V(0) & "',SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCMTYLINECHGSNDWORK set UNCLOSEdat=getdate() ,CLOSEUSR='" & V(0) & "' where comq1=" & KEY(0) & " and lineq1=" & key(1)  & " and prtno='" & key(2) & "' and prtno2='" & key(3) & "' "
         ...
         '      '發生錯誤時，刪除異動檔所新增的異動資料
         '         errmsg=cstr(Err.number) & "=" & Err.description
         '         sqlyy="delete * FROM RTEBTCMTYLINECHGSNDWORKlog WHERE comq1=" & key(0) & " and lineq1=" & key(1) & "' and prtno='" & key(2) & "' AND ENTRYNO=" & ENTRYNO
         '         CONNXX.Execute sqlyy 
         '      ELSE
         '         endpgm="1"
         '         errmsg=""
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="4" then
       msgbox "當異動派工單裝機完工日不為空白時(或已完成結案或已未完工結案)，不可執行未完工結案" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
...
<form name=frm1 method=post action=RTEBTCMTYLINECHGSNDWORKuf.asp ID="Form1">
