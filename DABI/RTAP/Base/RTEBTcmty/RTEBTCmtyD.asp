...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtEBTcmty/RTEBTCmtyd.asp")
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtEBTcmty/RTEBTCmtyd.asp") then
          rs.open "select max(comq1) AS comq1 from rtEBTcmtyh",conn
          if not rs.eof then
            dspkey(0)=rs("comq1")
          end if
          rs.close
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1, COMN, CUTID, TOWNSHIP,RADDR,RZONE,CUTID2,TOWNSHIP2,RADDR2,CUTID3,TOWNSHIP3,RADDR3,CUTID4,TOWNSHIP4,RADDR4, " _
             ...
             &"UPDEBTDAT,TELCOMBOX,CONTACT,CONTACTTEL,eusr,edat,uusr,udat FROM RTEBTCmtyH WHERE COMQ1=0 "
  sqlList="SELECT COMQ1, COMN, CUTID, TOWNSHIP,RADDR,RZONE,CUTID2,TOWNSHIP2,RADDR2,CUTID3,TOWNSHIP3,RADDR3,CUTID4,TOWNSHIP4,RADDR4, " _
             &"COMCNT,BUILDTYPE,BUILDCNT,BUILDFLOOR,POWERJECT,POWERTYPE,POWERDISTANCE,SETUPTYPE,CABLELENGTH,AGREEDRILL,TELCOMROOM, " _
             &"SURVEYDAT,AGREEDAT,AGREE,UNAGREEDESC, UPDEBTCHKDAT, UPDEBTCHKUSR, " _
             &"UPDEBTDAT,TELCOMBOX,CONTACT,CONTACTTEL,eusr,edat,uusr,udat FROM RTEBTCmtyH WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    ELSEif LEN(TRIM(dspkey(2))) = 0 OR LEN(TRIM(dspkey(3))) = 0 or LEN(TRIM(dspkey(4))) = 0  then
       formValid=False
       message="請輸入社區地址" 
    ELSEif len(trim(DSPKEY(15))) = 0 OR DSPKEY(15) = 0 THEN
       formValid=False
       message="請輸入社區規模戶數" 
    ELSEif len(trim(DSPKEY(17))) = 0 OR DSPKEY(17) = 0 THEN
       formValid=False
       message="請輸入社區棟數" 
...
Sub SrActiveXScript()%>
   ...
   Sub Srbtnonclick()
       ...
       if objEF2KDT.strDateTime <> "" then
          ...
          '社區申請人
          if clickkey ="KEY30" and document.all("KEY38").value <>"" then
			document.all("KEY31").value = document.all("KEY38").value
          elseif clickkey ="KEY30" and document.all("KEY36").value <>"" then			
			document.all("KEY31").value = document.all("KEY36").value
		  end if
       end if
   END SUB
   Sub SrClear()
  ...
  <%s=""
   ...
   %>
    <td bgcolor="silver" colspan=3 >
        ...
        <input type="text" name="key3" size="8" value="<%=dspkey(3)%>" maxlength="10" readonly <%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Text4">
        <font size=2>(鄉鎮)</font>
        ...
        <input type="text" name="key4" size="32" value="<%=dspkey(4)%>" maxlength="60" <%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Text5">
		<input type="text" name="key5" value="<%=dspkey(5)%>" size="5" <%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListdata" readonly ID="Text6">
		<font size=2>(郵遞區號)</font>
		</td>
</tr>
...
<td bgcolor="silver" >
<input type="text" name="key26" size="10" READONLY value="<%=dspKey(26)%>" <%=fieldPa%><%=fieldRole(1)%><%=dataProtect%>  class="dataListEntry" ID="Text7">
...
<tr>
		...
		<input type="text" name="key36" size="6" READONLY value="<%=dspKey(36)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2">
		<font size=2><%=SrGetEmployeeName(dspKey(36))%></font>
        </td>  
        <td  class="dataListHEAD" height="23">建檔日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key37" size="25" READONLY value="<%=dspKey(37)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>  
<tr>
        <td  class="dataListHEAD" height="23">修改人員</td>                                 
        <td  height="23" bgcolor="silver">
			<input type="text" name="key38" size="6" READONLY value="<%=dspKey(38)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2">
			<font size=2><%=SrGetEmployeeName(dspKey(38))%></font>
        </td>  
        <td  class="dataListHEAD" height="23">修改日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key39" size="25" READONLY value="<%=dspKey(39)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>        
</table> 
<!--
    ...
    <tr><td bgcolor="BDB76B" align="center">社區基本資料申請及異動狀態</td></tr></table>
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
      ...
      <tr><td width="20%" class="dataListHead">社區申請日</td>
          ...
          <td width="25%" bgcolor="silver">
			<input type="text" name="key31" size="6" READONLY value="<%=dspKey(31)%>" <%=fieldPa%><%=fieldRole(1)%><%=dataProtect%> class="dataListData">
			<font size=2><%=SrGetEmployeeName(dspKey(31))%></font>
          </td>
	  </tr>
      <tr><td width="20%" class="dataListHead">社區轉檔日</td>
          <td width="35%" bgcolor="silver" colspan=3>
			<input type="text" name="key32" size="25" READONLY value="<%=dspKey(32)%>" <%=fieldPa%><%=fieldRole(1)%><%=dataProtect%> class="dataListDATA">
		  </td>
   </table> 
  </div> 
<% 
