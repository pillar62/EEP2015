 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="insert into RTEBTCUSTm2RTNsndworklog " _
           &"SELECT   avsno,m2m3,seq,RTNNO, " & ENTRYNO & ", getdate(), 'R','" &  v(0) & "', " _
           &"SENDWORKDAT, PRTUSR, ASSIGNENGINEER,ASSIGNCONSIGNEE, REALENGINEER, REALCONSIGNEE, DROPDAT,'AVS用戶復機工單作廢返轉', " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTEBTCUSTm2RTNsndwork set dropdat=null,dropdesc='已作廢返轉' where AVSNO='" & key(0) & "' and M2M3='" & key(1) & "' and seq=" & key(2) & " and RTNno='" & key(3) & "'"
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="4" then
       msgbox "此用戶復機工單尚未作廢，不可執行作廢返轉作業：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行用戶復機工單作廢返轉作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
...
<form name=frm1 method=post action=rtebtcustm2RTNsndworkdropc.asp ID="Form1">
