...
if showopt(0)="Y" then    
 ...
 IF Domain="T" then
    sql="SELECT RTEmployee.emply, RTObj.CUSNC " _
          ...
          &"RTObjLink ON RTEmployee.CUSID = RTObjLink.CUSID AND rtobjlink.custyid = '08' and tran2<>'10' " _
          &"and (rtemployee.dept in ('B100','B200','B300','B700','B701') or (rtemployee.dept='B600' and authlevel=2)) " _
          &"order by 2"
    '台中
...
if showopt(1)="Y" then
 ...
 elseif Domain="K" then
    sql="SELECT RTEmployee.emply, RTObj.CUSNC " _
          ...
          &"RTObj ON RTEmployee.CUSID = RTObj.CUSID INNER JOIN " _
          &"RTObjLink ON RTEmployee.CUSID = RTObjLink.CUSID AND rtobjlink.custyid = '08' " _
          &"and RTEmployee.EMPLY like 'K%' and rtemployee.authlevel in ('5','1','4')  and tran2<>'10' "   
 else
     sql="SELECT RTEmployee.emply, RTObj.CUSNC " _
          &"FROM RTEmployee INNER JOIN " _
          &"RTObj ON RTEmployee.CUSID = RTObj.CUSID INNER JOIN " _
    ...
    s4=""
    Do While Not rs.Eof
       s4=s4 &"<option value=""" &rs("emply") &""">" &rs("cusnc") &"</option>"
       rs.MoveNext
    Loop
    rs.Close    
end if
...
if showopt(3)="Y" then   
	...
	sql="SELECT	b.CUSID, b.SHORTNC "&_
		"FROM	RTConsignee a inner join RTObj b on a.CUSID = b.CUSID "&_
		"UNION "&_
		"SELECT	d.CUSID, d.SHORTNC "&_
		"FROM	RTSuppCty c inner join RTObj d on c.CUSID = d.CUSID "&_
		"GROUP BY d.CUSID, d.SHORTNC "&_
		"ORDER BY 2 "

    rs.Open sql,conn
    s3=""
    Do While Not rs.Eof
       s3=s3 &"<option value=""" &rs("cusid") &""">" &rs("shortnc") &"</option>"
       rs.MoveNext
    Loop
    rs.Close    
end if
...
<FIELDSET
 STYLE="HEIGHT: 308px; LEFT: 145px; POSITION: absolute; TOP: 20px; WIDTH: 125px" ID=select1>
<LEGEND>客服人員</LEGEND>
...
<FIELDSET
 STYLE="HEIGHT: 308px; LEFT: 275px; POSITION: absolute; TOP: 20px; WIDTH: 125px" ID=select2>
<LEGEND>技術發展部</LEGEND>
...
<FIELDSET
 STYLE="HEIGHT: 308px; LEFT: 405px; POSITION: absolute; TOP: 20px; WIDTH: 125px" ID=select3>
<LEGEND>廠商</LEGEND>
