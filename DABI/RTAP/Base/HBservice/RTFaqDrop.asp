...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      strSP ="update RTSndWork set canceldat=getdate(),cancelusr='"& V(0) &"', memo = memo+' [因客訴單作廢而取消派工單]' WHERE linkno='" & KEY(0) & "' and (worktype ='01' or worktype ='09') and canceldat is null" 
      Set ObjRS = connXX.Execute(strSP)
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="4" then
       msgbox "客訴單已作廢，不可重覆執行。" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
