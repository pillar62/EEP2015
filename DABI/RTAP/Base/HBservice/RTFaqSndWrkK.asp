...
Sub SrEnvironment()
	...
	if V(0)="Y" then
		functionOptName="作 廢;列 印;完 工;完工返轉"
	...
	if emply ="T89039" then
		functionOptName="作 廢;列 印;完 工;完工返轉;滿意度調查"
	...
	End IF
	DSN="DSN=RTLib"
	formatName="派工單號;客訴單號;none;派工別;預定<br>施工人;實際<br>施工人;社區名稱;聯絡人;派工人;派工日;完工人;完工日;完工<br>方式;作廢日"
	sqlDelete=	"select a.workno, a.linkno, g.comtype, b.codenc as worktype, isnull(c.shortnc, d.name) as assigneng, finisheng, " &_
				"h.comn, g.faqman, " &_
				"e.name as sndwrkusr, a.sndwrkdat, f.name as finishusr, a.finishdat, a.finishtyp, a.canceldat " &_
				"from	RTSndWork a " &_
				...
				"left outer join HBAdslCmty h on g.comtype = h.comtype and g.comq1 = h.comq1 and g.lineq1 = h.lineq1 " &_				
				"where a.workno='' "
	dataTable="RTSndWork"
	...
	sqlList="select a.workno, a.linkno, g.comtype, b.codenc as worktype, isnull(c.shortnc, d.name) as assigneng, " &_
			"isnull(i.shortnc, f.name) as finisheng, h.comn, g.faqman, " &_
			"e.name as sndwrkusr, a.sndwrkdat, j.name as finishsur, a.finishdat, k.codenc, a.canceldat " &_
			"from	RTSndWork a " &_
			"left outer join RTCode b on b.code = a.worktype and b.kind ='P6' " &_
			"left outer join RTObj c on c.cusid = a.assigncons " &_
			"left outer join RTEmployee d on d.emply = a.assigneng " &_
			...
			"left outer join RTFaqM g on g.caseno = a.linkno  " &_
			"left outer join HBAdslCmty h on g.comtype = h.comtype and g.comq1 = h.comq1 and g.lineq1 = h.lineq1 " &_
			"left outer join RTCode k on k.code = a.finishtyp and k.kind ='P9' " &_
			"where a.linkno='" & aryparmkey(0) & "' and (a.worktype ='01' or a.worktype ='09') " &_
			" order by isnull(c.shortnc, d.name), h.comn "
