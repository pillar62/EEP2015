...
Sub SrEnvironment()
  ...
  sqlFormatDB=	"SELECT WORKNO, LINKNO, WORKTYPE, ASSIGNENG, ASSIGNCONS, MEMO, SNDWRKUSR, SNDWRKDAT, " &_
				...
				"ASSIGNDAT, ASSIGNTIME, SCORE01, SCORE02, SCORE03, SCORE04, SCORE05, SCORE06, SCORE07, " &_
				"SCORE08, SCORE09, SCORE10, SCORE11, SCORE12, SCORE13, SCORE14, SCORE15 " &_
				"FROM RTSndWork " &_
				"WHERE WORKTYPE ='' "
	...
	select case aryparmkey(2)
		...
		case "A"
			sql="select n.codenc as comtypenc, case when j.comtype ='' then '' when d.consignee ='' then '直銷' else '經銷' end as belongnc,  " &_
				"isnull(o.shortnc, q.cusnc) as salesnc, convert(varchar(5),j.comq1)+'-'+convert(varchar(5),j.lineq1) as comq, d.comn,  " &_
				"b.linetel as LINETEL, b.lineip as CMTYIP, f.codenc as LINERATE, b.hardwaredat as ARRIVEDAT, b.dropdat as RCOMDROP,  " &_
				"b.gateway, c.cusnc, c.contacttel, c.companytel, c.raddr, a.ip11 as CUSTIP, r.codenc as CASEKIND, '' as paycycle, '' as paytype,  " &_
				"convert(varchar(10), overduedat, 111) as overdue, replace(a.freecode,'N','') as freecode, c.docketdat, a.strbillingdat,  " &_
				"null as newbillingdat, null as duedat, c.dropdat, c.canceldat, a.cusid, null as entryno, d.engid as salesid, d.consignee, j.faqman,  " &_
				...
				"left outer join RTCode f on f.code = b.linerate and f.kind ='D3' " &_
				"left outer join RTCode k on k.code = j.faqreason and k.kind ='P7' " &_
                                "left outer join RTCode r on a.userrate = r.code and r.kind ='R3' " &_
				"left outer join RTEmployee l on l.emply = j.rcvusr " &_
				"left outer join RTEmployee m on m.emply = j.closeusr " &_
				"left outer join RTCode n on n.code = j.comtype and n.kind ='P5' " &_
				"left outer join RTObj o on o.cusid = d.consignee " &_
				"left outer join RTEmployee p inner join RTObj q on p.cusid = q.cusid on p.emply = d.engid " &_
				"where j.caseno ='" &aryparmkey(1)& "' "
	case "B"
			sql="select n.codenc as comtypenc, case when j.comtype ='' then '' when d.consignee ='' then '直銷' else '經銷' end as belongnc,  " &_
				"isnull(o.shortnc, q.cusnc) as salesnc, convert(varchar(5),j.comq1)+'-'+convert(varchar(5),j.lineq1) as comq, d.comn,  " &_
				...
				"j.tel, j.mobile, k.codenc as faqreason, j.memo, l.name as rcvusr, j.rcvdat, m.name as closeusr, j.closedat  " &_
				"from RTFaqM j  " &_
				"left outer join RTfareastCust a on j.comq1 = a.comq1 and j.lineq1 = a.lineq1 and j.cusid = a.cusid  " &_
				"left outer join RTfareastCmtyLine b on j.comq1 = b.comq1 and j.lineq1 = b.lineq1  " &_
				"left outer join RTfareastCmtyH d on d.comq1 = b.comq1  " &_
				"left outer join HBAdslCmtyCust c on c.comq1 = j.comq1 and c.lineq1 = j.lineq1 and j.cusid = c.cusid and c.comtype = j.comtype  " &_
				"left outer join RTCode f on f.code = b.linerate and f.kind ='D3' " &_
				"left outer join RTCode k on k.code = j.faqreason and k.kind ='P7' " &_
                                "left outer join RTCode r on a.userrate = r.code and r.kind ='R6' " &_
				"left outer join RTEmployee l on l.emply = j.rcvusr " &_
				"left outer join RTEmployee m on m.emply = j.closeusr " &_
				"left outer join RTCode n on n.code = j.comtype and n.kind ='P5' " &_
				"left outer join RTObj o on o.cusid = d.consignee " &_
...
<DIV ID="SRTAG5">
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table5">
		<tr><td bgcolor="BDB76B" align="center">績效相關</td></tr>
    </table>
</DIV>
<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
	<tr><td width="10%" class="dataListHEAD" >績效點數</td>
	    <td width="90%" bgcolor="silver" >
	    	<% IF DSPKEY(19)="Y" THEN CHECK19=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text19" name="key19" value="Y" <%=CHECK19%> bgcolor="silver"><font size=2>(2點) 申請AVS、FBB(業務)</font><br>
	    	<% IF DSPKEY(20)="Y" THEN CHECK20=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text20" name="key20" value="Y" <%=CHECK20%> bgcolor="silver"><font size=2>(1點) 安裝Sonet、SQ、AVS Call Out續約與其他</font><br>
	    	<% IF DSPKEY(21)="Y" THEN CHECK21=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text21" name="key21" value="Y" <%=CHECK21%> bgcolor="silver"><font size=2>(1點) 勸用戶退租與撤線</font><br>
	    	<% IF DSPKEY(22)="Y" THEN CHECK22=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text22" name="key22" value="Y" <%=CHECK22%> bgcolor="silver"><font size=2>(1點) 主機安裝</font><br>
	    	<% IF DSPKEY(23)="Y" THEN CHECK23=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text23" name="key23" value="Y" <%=CHECK23%> bgcolor="silver"><font size=2>(1點) 主機開通</font><br>
	    	<% IF DSPKEY(24)="Y" THEN CHECK24=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text24" name="key24" value="Y" <%=CHECK24%> bgcolor="silver"><font size=2>(1點) 用戶端裝機</font><br>
	    	<% IF DSPKEY(25)="Y" THEN CHECK25=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text25" name="key25" value="Y" <%=CHECK25%> bgcolor="silver"><font size=2>(1點) 主機拆回、拆 Port</font><br>
	    	<% IF DSPKEY(26)="Y" THEN CHECK26=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text26" name="key26" value="Y" <%=CHECK26%> bgcolor="silver"><font size=2>(1點) 退租拆機</font><br>
	    	<% IF DSPKEY(27)="Y" THEN CHECK27=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text27" name="key27" value="Y" <%=CHECK27%> bgcolor="silver"><font size=2>(1點) 用戶端設備及文件收回</font><br>
	    	<% IF DSPKEY(28)="Y" THEN CHECK28=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text28" name="key28" value="Y" <%=CHECK28%> bgcolor="silver"><font size=2>(0.8點) 維修(需有派工單)</font><br>
	    	<% IF DSPKEY(29)="Y" THEN CHECK29=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text29" name="key29" value="Y" <%=CHECK29%> bgcolor="silver"><font size=2>(0.3點) 線上維修</font><br>
	    	<% IF DSPKEY(30)="Y" THEN CHECK30=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text30" name="key30" value="Y" disabled <%=CHECK30%> bgcolor="silver"><font size=2 color=red>(1點) 發DM(需有派工單)</font><br>
	    	<% IF DSPKEY(31)="Y" THEN CHECK31=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text31" name="key31" value="Y" <%=CHECK31%> bgcolor="silver"><font size=2>(1點) 社區深耕活動(需有派工單)</font><br>
	    	<% IF DSPKEY(32)="Y" THEN CHECK32=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text32" name="key32" value="Y" disabled <%=CHECK32%> bgcolor="silver"><font size=2 color=red>(0.3點) 放DM、整理DM</font><br>
	    	<% IF DSPKEY(33)="Y" THEN CHECK33=" CHECKED "%>
	        <input type="checkbox" READONLY ID="Text33" name="key33" value="Y" <%=CHECK33%> bgcolor="silver"><font size=2>(0.3點) 社區電信室勘查</font><br>
		</td>
	</tr>
</table>
