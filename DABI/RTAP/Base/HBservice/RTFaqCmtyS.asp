...
'--------- 經銷商
    rs.Open "SELECT GROUPNC FROM HBADSLCMTY " _
           &" where comtype in ('3','6','7','8','9','A','B') GROUP BY GROUPNC ",conn
    ...
    rs.Close
'--------- 工程師 
    rs.Open "SELECT leader FROM HBADSLCMTY " _
           &" where comtype in ('3','6','7','8','9','A','B') GROUP BY leader ",conn
...
Sub btn_onClick()
  ...
  IF S14ARY(0) = "2" THEN
     t=t & " and b.prtno IS NOT NULL AND b.dropdat IS NULL AND b.closedat IS NULL "
  ELSEIF S14ARY(0) = "1" THEN
     t=t & " and b.prtno IS NOT NULL AND b.dropdat IS NULL AND b.closedat IS not NULL AND b.AUDITdat IS  NULL"
  ELSEIF S14ARY(0) = "0" THEN
     t=t & " and b.prtno IS NOT NULL AND b.dropdat IS NULL AND b.closedat IS not NULL AND b.AUDITdat IS NOT NULL"
  ELSEIF S14ARY(0) = "3" THEN
     t=t & " and b.prtno IS NULL "
  END IF    
  
  's14ARY=SPLIT(document.all("search14").value,";")
  'if S14ARY(0) ="1" then
  '   t=t & " AND ( a.RCOMDROP <> '' OR a.rcomdrop IS NOT NULL ) " 
  '   s=s & " 撤線狀況︰只選已撤線社區 "
  'elseif S14ARY(0)="2" then
  '   t=t & " and ( a.rcomdrop ='' OR a.rcomdrop IS NULL) "
  '   S=S & " 撤線狀況︰只選未撤線社區 "
  'end if
...
-->
	   ...
	   <option value="7;AVS-City">AVS-City</option>
	   <option value="8;ET-City">ET-City</option>
...
<tr><td class=dataListHead width="40%">線路到位日期</td>
    <td width="60%" bgcolor="silver" >
