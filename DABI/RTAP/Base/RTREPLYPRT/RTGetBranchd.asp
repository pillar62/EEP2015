...
<% 
  ...
  DSN="DSN=RTLib"
  Conn.Open DSN
  sql="SELECT BRANCH FROM RTBranch WHERE CUSID = '" & stock & "'" _
       &"ORDER BY branch " 
  rs.Open sql,conn
  s1=""
  Do While Not rs.Eof
     s1=s1 &"<option value=""" &rs("branch") &""">" &rs("branch") &"</option>"
     rs.MoveNext
  Loop
  rs.Close    
...
<SCRIPT LANGUAGE="VBScript">
  ...
  Sub cmdSure_onClick()
    ...
    if len(trim(window.document.all("cmdtext").value)) = 0 then
       msgbox "請選擇券商分行!",vbokonly,"錯誤訊息視窗"
    else    
