...
<% 'stock="縣市" 
  STOCK=split(request("key"),";")
  set conn=server.CreateObject("ADODB.Connection")
  set rs=server.CreateObject("ADODB.Recordset")
  DSN="DSN=RTLib"
  Conn.Open DSN
  sql="SELECT category2,category2nm " _
     &"FROM HBCONTRACTtREEl2 " _
     &"WHERE propertyid = '" & stock(0) & "' and category1='" & stock(1) & "' " _
     &"ORDER BY propertyid,category1,category2 " 
  rs.Open sql,conn
  s1=""
  Do While Not rs.Eof
     s1=s1 &"<option value=""" &rs("category2") &""">" &rs("category2") & "--" &rs("category2nm") &"</option>"
     rs.MoveNext
  Loop
  s1="<OPTION VALUE=""" & "00" & """>" & "00" & "--" & "無小類別" & "</option> " & s1
  rs.Close    
...
<SCRIPT LANGUAGE="VBScript">
  ...
  Sub cmdSure_onClick()
    ...
    if len(trim(window.document.all("cmdtext").value)) = 0 then
       msgbox "請選擇合約小類別!",vbokonly,"錯誤訊息視窗"
    else    
