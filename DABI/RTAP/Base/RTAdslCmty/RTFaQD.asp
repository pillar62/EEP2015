...
Sub SrCheckData(message,formValid)
    IF LEN(TRIM(DSPKEY(21)))= 0 THEN DSPKEY(21)="2"
...
' -------------------------------------------------------------------------------------------- 
    ...
    IF len(trim(dspkey(8))) = 0 then
       sqlCMTY="select comn, ipaddr from RTCUSTADSLCMTY where RTCUSTADSLCMTY.CUTYID=" & session("COMQ1")
       dspkey(8)=session("COMQ1")       
    else
       sqlCMTY="select comn, ipaddr from RTCUSTADSLCMTY where RTCUSTADSLCMTY.CUTYID=" & dspkey(8)   
    end if
   ' RESPONSE.Write "K8=" & DSPKEY(8)
    ...
    SQLCUST="select a.cusid, isnull(rtcounty.cutnc, '')+ a.township2+ a.raddr2 as addr, rtobj.cusnc, rtobj.shortnc, "_
		   ...
		   &"from RTCustAdsl a inner join  rtobj on a.cusid=rtobj.cusid "_
