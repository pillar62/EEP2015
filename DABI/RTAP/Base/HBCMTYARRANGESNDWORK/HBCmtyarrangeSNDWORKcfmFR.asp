 ...
 '  On Error Resume Next
   ...
   '  RESPONSE.END
     ...
     elseif LEN(TRIM(RSXX("BONUSCLOSEYM"))) <> 0 OR LEN(TRIM(RSXX("STOCKCLOSEYM"))) <> 0 then
        endpgm="5"
     elseif isnull(RSXX("AUDITDAT")) then
   '  response.Write "aaa"
   '  response.end
        endpgm="3"      
     ELSE
     ...
     ' Response.Write sqlyy
        ...
        If Err.number > 0 then
           ...
           'IF4-1S
           IF LEN(TRIM(RSXX("CLOSEDAT"))) > 0 THEN
              SQLXX=" update hbcmtyarrangesndwork set AUDITdat=NULL,AUDITUSR='' where comq1=" & key(0) & " and comtype='" & key(1) & "' and prtno='" & key(2) & "' "
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此整線派工單尚未審核，不可執行審核返轉作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此整線派工單已作廢，不可執行結案審核返轉作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="5" then
       ...
       msgbox "無法執行整線工單結案審核返轉作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
