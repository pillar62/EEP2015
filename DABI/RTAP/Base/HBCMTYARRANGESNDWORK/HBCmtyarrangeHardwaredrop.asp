 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="insert into hbcmtyarrangehardwarelog " _
           &"SELECT  COMQ1, COMTYPE, PRTNO, ENTRYNO, " & SEQ & ", getdate(), 'C', '" & V(0) & "', PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO,  DROPUSR, UNIT, EUSR, EDAT, UUSR, UDAT " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update hbcmtyarrangehardware set DROPdat=getdate(),DROPUSR='" & V(0) & "' where comq1=" & KEY(0) & " and comtype='" & key(1)  & "' and prtno='" & key(2) & "'  and entryno=" & key(3) 
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "當設備資料已作廢時，不可重複作廢" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此整線派工單已完工結案，設備明細不可作廢" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close           
    ...
    elseIF frm1.htmlfld.value="6" then
       msgbox "此整線派工單已作廢，設備明細不可作廢" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close             
    else
       msgbox "無法執行整線派工單設備作廢作業,錯誤訊息" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
