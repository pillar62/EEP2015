...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   ...
   '已開通或已終止，不可作廢
      IF LEN(TRIM(RSXX("opendat"))) > 0 or LEN(TRIM(RSXX("enddat"))) > 0 THEN
         ENDPGM="3"
    '已作廢，不可重覆作廢
      ...
      ELSE
         ...
         sqlyy="insert into KTSCUSTD1log " _
              &"SELECT  CUSID, ENTRYNO," & SEQ & ", getdate(), 'D','" &  v(0) & "', " _
     ...
     ' Response.Write sqlyy
         ...
         If Err.number > 0 then
            ...
            SQLXX=" update KTSCUSTD1 set CANCELdat=getdate() where CUSID='" & key(0) & "' and ENTRYNO=" & key(1) 
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "電話已開通或已終止，不可作廢" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此電話已作廢，不可重覆執行作廢作業" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行KTS電話作廢作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
