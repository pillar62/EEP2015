...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/KTSCUST/KTSCUSTpulld.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=0 then
                         ...
                         rsc.open "select max(cusid) AS cusid from KTSWANTGOCUST where cusid like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/KTSCUST/KTSCUSTpulld.asp") then
          cusidxx="K" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(cusid) AS cusid from KTSWANTGOCUST where cusid like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(0)=rsC("CUSID")
...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT CUSID,CUSNC,SOCIALID,BUSINESSTYPE,COTEL11,COTEL12,COFAX11,COFAX12,COEMAIL,CUTID1,TOWNSHIP1,RADDR1," _
             ...
             &"RUNONCESALESDAT, CONSIGNEE1, CONSIGNEE2, EMPLY,eusr,edat,uusr,udat,LISTTELDETAIL,SERVICE0809,CBBNPULLDAT,CBBNPULLUSR " _
             &"from KTSWANTGOCUST WHERE CUSID='' "
...
Sub SrCheckData(message,formValid)
  ...
  IF LEN(TRIM(DSPKEY(36))) > 0 THEN
	  ...
	  '排除目前這筆資料本身
      sqlXX="SELECT count(*) AS CNT FROM KTSWANTGOCUST where NCICCUSID='" & trim(dspkey(36)) & "' and not ( CUSID='" & DSPKEY(0) & "')"
...
' -------------------------------------------------------------------------------------------- 
    ...
    End If    
       ...
       fieldPg=" class=""dataListData"" readonly "
       fieldph=" disabled "
       fieldPI=" class=""dataListData"" readonly "
   ...
   <tr>
    ...
    <%   IF DSPKEY(54)="Y" THEN CHECK54=" CHECKED "%>
        <input type="checkbox" name="key54" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               ...
               value="Y"  <%=CHECK54%> READONLY  bgcolor="silver"  ID="Text11"><font size=2>於帳單中增列長途電話通話明細</font>
    ...
    <tr id="SRTAR1" STYLE="display:none">
    ...
    <%
     s=""
     sql="SELECT  *  FROM  KTSWANTGOCUSTD1 where cusid='" & dspkey(0) & "' and dropdat is null and applydat is not null and canceldat is null ORDER BY ENTRYNO"
     rs.Open sql,conn
     cnt=0
     Do While Not rs.Eof
       RESPONSE.Write "<TR>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("ENTRYNO") &  "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("TEL11") & "-" & RS("TEL12") & "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & datevalue(RS("APPLYDAT")) & "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("APPLYNO") & "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("DROPDAT") & "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("DROPNO") & "&nbsp;</td>"
       RESPONSE.Write "<td WIDTH=10% ALIGN=""center""  class=dataListHEAD2>" & RS("CANCELDAT") & "&nbsp;</td></TR>"
       rs.MoveNext
     Loop
     rs.Close
    %>
...
<!--
    ...
    <tr><td id="tagT1" WIDTH="14%" class="dataListHEAD" height="23">開發經銷商</td>               
...
<%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(DSPKEY(38))) = 0 AND len(trim(DSPKEY(32))) = 0 Then 
    ...
    Else
       sql="SELECT RTConsignee.CUSID, RTObj.SHORTNC FROM RTConsignee INNER JOIN  RTConsigneeISP ON " _
          &"RTConsignee.CUSID = RTConsigneeISP.CUSID INNER JOIN RTObj ON RTConsignee.CUSID = RTObj.CUSID " _
          &"WHERE (RTConsigneeISP.ISP = '03') AND RTConsignee.CUSID='" & DSPKEY(48) & "' ORDER BY RTObj.SHORTNC "
    ...
    sx=""
    Do While Not rs.Eof
       If rs("CUSID")=dspkey(48) Then sx=" selected "
    ...
    rs.Close        
    %>    
           <select size="1" name="key48" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry" ID="Select1">                                            
              <%=s%>
           </select>
        </td>
   <td id="tagT1" WIDTH="10%" class="dataListHEAD" height="23">開發業務</td>               
...
<%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(DSPKEY(38))) = 0 AND len(trim(DSPKEY(32))) = 0 Then 
    ...
    sx=""
    Do While Not rs.Eof
       If rs("EMPLY")=dspkey(49) Then sx=" selected "
    ...
    rs.Close        
    %>    
           <select size="1" name="key49" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry" ID="Select3">                                            
              <%=s%>
           </select>
        </td>        
...
<%  If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(DSPKEY(38))) = 0 AND len(trim(DSPKEY(32))) = 0 Then 
    ...
    Else
       sql="SELECT RTConsignee.CUSID, RTObj.SHORTNC FROM RTConsignee INNER JOIN  RTConsigneeISP ON " _
          &"RTConsignee.CUSID = RTConsigneeISP.CUSID INNER JOIN RTObj ON RTConsignee.CUSID = RTObj.CUSID " _
          &"WHERE (RTConsigneeISP.ISP = '03') AND RTConsignee.CUSID='" & DSPKEY(47) & "' ORDER BY RTObj.SHORTNC "
   ...
   <DIV ID="SRTAG3" onclick="srtag3" style="cursor:hand">
    ...
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
       ...
       <input type="button" id="B32"  name="B32" height="100%" width="100%" style="Z-INDEX: 1" <%=fieldpf%> value="...." onclick="SrBtnOnClick">
        ...
        <td   WIDTH=20% height="23" bgcolor="silver" >
        <input type="text" name="key36" size="15" READONLY value="<%=dspKey(36)%>"  <%=fieldpb%><%=fieldRole(1)%> class="dataListDATA" ID="Text3">     
        </td>
        <td  width=10% class="dataListHEAD" height="23">用戶電話開通日</td>                                 
        <td  width=20% height="23" bgcolor="silver" colspan=3>
       ...
       <tr>       
        ...
        <td   width=20% height="23" bgcolor="silver" >
        <input type="text" name="key38" size="23" READONLY value="<%=dspKey(38)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text57">
        </td>
        ...
        <td  width=10%  class="dataListHEAD" height="23">報竣轉檔日</td>                                 
        <td   width=20% height="23" bgcolor="silver" >
       ...
       <tr>
        ...
        <td  width=20% height="23" bgcolor="silver">
        <input type="text" name="key43" size="23" READONLY value="<%=dspKey(43)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text56"></td>
        <td  width=10%  class="dataListHEAD" height="23">NCIC強退註記</td>     
        <td  width=20% height="23" bgcolor="silver">
        <input type="text" name="key44" size="5" READONLY value="<%=dspKey(44)%>"  <%=fieldRole(1)%> class="dataListdata" ID="Text21"></td>
       ...
       <tr>
        ...
        <td   width=20% height="23" bgcolor="silver" colspan=5>
        <%  name="" 
           if dspkey(42) <> "" then
              sql=" select cusnc from rtemployee inner join rtobj on rtemployee.cusid=rtobj.cusid " _
  ...
  %>            
   ...
   <DIV ID="SRTAB4" > 
    ...
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%"  name="key124" rows=8  MAXLENGTH=500 <%=fieldpI%> class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(124)%>" ID="Textarea1"><%=dspkey(124)%></TEXTAREA>
   </td></tr>
 </table> 
  </div> 
<%
