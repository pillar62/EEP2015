...
Sub SrEnvironment()
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;用戶名稱;派工單號;派工日期;列印人員;預定施工員;實際施工員;完工結案日;未完工結案;作廢日;none;none;none;none"
  sqlDelete="SELECT KTSCUSTSNDWORK.CUSID, KTSCUSTSNDWORK.PRTNO,KTSCUST.CUSNC,KTSCUSTSNDWORK.PRTNO, KTSCUSTSNDWORK.SENDWORKDAT, " _
           ...
           &"RTObj INNER JOIN RTEmployee ON RTObj.CUSID = RTEmployee.CUSID ON KTSCUSTSNDWORK.PRTUSR = RTEmployee.EMPLY LEFT OUTER JOIN KTSCUST ON KTSCUSTSNDWORK.CUSID=KTSCUST.CUSID "
  ...
  colSplit=1
  keyListPageSize=25
  searchProg="KTSCUSTSNDWORKS2.ASP"
  searchFirst=FALSE
  If searchQry="" Then
     searchQry=" KTSCUSTSNDWORK.DROPDAT IS NULL AND KTSCUSTSNDWORK.UNCLOSEDAT IS NULL AND KTSCUSTSNDWORK.CLOSEDAT IS NULL "
     searchShow="全部未完工(不含退租)"
  ELSE
     SEARCHFIRST=FALSE
  End If
 ...
 ' Response.Write "DOMAIN=" & domain & "<BR>"
  ...
  if userlevel=31 then DAreaID="<>'*'"
    ...
    If searchShow="全部" Then
         sqlList="SELECT KTSCUSTSNDWORK.CUSID, KTSCUSTSNDWORK.PRTNO,KTSCUST.CUSNC,KTSCUSTSNDWORK.PRTNO, KTSCUSTSNDWORK.SENDWORKDAT, " _
           &"RTOBJ.CUSNC,CASE WHEN RTOBJ_2.SHORTNC <>'' THEN RTOBJ_2.SHORTNC ELSE RTOBJ_1.CUSNC END,CASE WHEN RTOBJ_4.SHORTNC <>'' THEN RTOBJ_4.SHORTNC ELSE RTOBJ_3.CUSNC END, " _
           &"KTSCUSTSNDWORK.CLOSEDAT,KTSCUSTSNDWORK.UNCLOSEDAT,KTSCUSTSNDWORK.DROPDAT,KTSCUSTSNDWORK.BONUSCLOSEYM, KTSCUSTSNDWORK.BONUSFINCHK, KTSCUSTSNDWORK.STOCKCLOSEYM, KTSCUSTSNDWORK.STOCKFINCHK " _
           &"FROM KTSCUSTSNDWORK LEFT OUTER JOIN RTObj RTObj_4 ON KTSCUSTSNDWORK.REALCONSIGNEE = RTObj_4.CUSID LEFT OUTER JOIN " _
           &"RTEmployee RTEmployee_2 INNER JOIN RTObj RTObj_3 ON RTEmployee_2.CUSID = RTObj_3.CUSID ON KTSCUSTSNDWORK.REALENGINEER = RTEmployee_2.EMPLY LEFT OUTER JOIN " _
           &"RTObj RTObj_2 ON KTSCUSTSNDWORK.ASSIGNCONSIGNEE = RTObj_2.CUSID LEFT OUTER JOIN RTEmployee RTEmployee_1 INNER JOIN " _
           &"RTObj RTObj_1 ON RTEmployee_1.CUSID = RTObj_1.CUSID ON KTSCUSTSNDWORK.ASSIGNENGINEER = RTEmployee_1.EMPLY LEFT OUTER JOIN " _
           &"RTObj INNER JOIN RTEmployee ON RTObj.CUSID = RTEmployee.CUSID ON KTSCUSTSNDWORK.PRTUSR = RTEmployee.EMPLY LEFT OUTER JOIN KTSCUST ON KTSCUSTSNDWORK.CUSID=KTSCUST.CUSID " _
            &"where " & searchqry & " ORDER BY  KTSCUSTSNDWORK.SENDWORKDAT "
    Else
         sqlList="SELECT KTSCUSTSNDWORK.CUSID, KTSCUSTSNDWORK.PRTNO,KTSCUST.CUSNC,KTSCUSTSNDWORK.PRTNO, KTSCUSTSNDWORK.SENDWORKDAT, " _
           &"RTOBJ.CUSNC,CASE WHEN RTOBJ_2.SHORTNC <>'' THEN RTOBJ_2.SHORTNC ELSE RTOBJ_1.CUSNC END,CASE WHEN RTOBJ_4.SHORTNC <>'' THEN RTOBJ_4.SHORTNC ELSE RTOBJ_3.CUSNC END, " _
           &"KTSCUSTSNDWORK.CLOSEDAT,KTSCUSTSNDWORK.UNCLOSEDAT,KTSCUSTSNDWORK.DROPDAT,KTSCUSTSNDWORK.BONUSCLOSEYM, KTSCUSTSNDWORK.BONUSFINCHK, KTSCUSTSNDWORK.STOCKCLOSEYM, KTSCUSTSNDWORK.STOCKFINCHK " _
