...
Sub SrEnvironment()
  ...
  title="銀行基本資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT HEADNO,HEADNC,BANKTYPE,shortnc " _
             &"FROM RTBANK WHERE HEADNO='*' "
  sqlList="SELECT  HEADNO,HEADNC,BANKTYPE,shortnc " _
             &"FROM RTBANK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"  
...
Sub SrCheckData(message,formValid)
    ...
    IF formvalid <> False then
       ...
       DSN="DSN=RTLIB"
       conn.Open dsn    
       sqlxx="select COUNT(*) AS CNT from RTBank where headnc='" & dspkey(1) & "' "
       rs.open sqlxx,conn
       if rs("CNT") > 0 then
          formvalid=False
          message="銀行名稱資料已存在!不可重覆"
       end if
       rs.close
       conn.close
       set rs=nothing
...
<table border="1" width="100%" cellspacing="0" cellpadding="0">
  <tr>
   <td width="15%" bgcolor="#008080"><font color="#FFFFFF">銀行名稱</font></td>
    <td width="35%" bgcolor="#C0C0C0">
     <input class=dataListEntry name="KEY1" <%=dataprotect%> maxlength=30 size=30 style="TEXT-ALIGN: left" value
            ="<%=DSPKEY(1)%>" ID="Text1">
    　</td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">銀行類別</font></td>
    <td width="35%" bgcolor="#C0C0C0" >
 <% s=""
    sx=" selected "
    If sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false) Then 
       sql="SELECT CODE,CODENC FROM rtcode WHERE KIND='G1' ORDER BY code "
    ...
    Else
       sql="SELECT CODE,CODENC FROM RTCoDE WHERE KIND='G1' AND CODE='" &DSPKEY(2) &"' "
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
    ...
    Loop
    rs.Close%>
    <select class=dataListEntry name="KEY2" <%=dataProtect%> size="1" style="text-align:left;" maxlength="8"><%=s%></select>    
</td>
</tr>
  <tr>
   <td width="15%" bgcolor="#008080"><font color="#FFFFFF">銀行簡稱</font></td>
