...
Sub SrEnvironment()
  ...
  title="分行基本資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT HEADNO,branchno,Branchnc,note " _
             &"FROM RTBANKbranch WHERE HEADNO='*' "
  sqlList="SELECT  HEADNO,branchno,Branchnc,note " _
             &"FROM RTBANKbranch WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  userDefineSave="Yes"  
  extDBField=0
...
Sub SrCheckData(message,formValid)
    ...
    IF formvalid <> False then
       ...
       DSN="DSN=RTLIB"
       conn.Open dsn    
       sqlxx="select COUNT(*) AS CNT from RTBankbranch where branchnc='" & dspkey(2) & "' "
       rs.open sqlxx,conn
       if rs("CNT") > 0 then
          formvalid=False
          message="分行名稱資料已存在!不可重覆"
       end if
       rs.close
       conn.close
       set rs=nothing
...
<p>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="21%" class=dataListHead>銀行代號</td><td width="29%" bgcolor=silver>
           <input class=dataListdata type="text" name="key0" readonly size="10" 
           value="<%=dspKey(0)%>" maxlength="10" ></td>
           <td width="21%" class=dataListHead>分行代號</td><td width="29%" bgcolor=silver>
           <input class=dataListEntry type="text" name="key1" <%=keyprotect%> size="10" 
           value="<%=dspKey(1)%>" maxlength="10" ID="Text3"></td>
       </tr>
      </table>
<%
End Sub
...
<table border="1" width="100%" cellspacing="0" cellpadding="0">
  <tr>
   <td width="6%" bgcolor="#008080"><font color="#FFFFFF">分行名稱</font></td>
    <td width="35%" bgcolor="#C0C0C0" colspan=3>
     <input class=dataListEntry name="KEY2" <%=dataprotect%> maxlength=30 size=30 style="TEXT-ALIGN: left" value
            ="<%=DSPKEY(2)%>" ID="Text1">
    　</td>
  </tr>
  <tr>
    <td width="6%" bgcolor="#008080"><font color="#FFFFFF">備註</font></td>
    <td width="35%" bgcolor="#C0C0C0" colspan=3>
     <input class=dataListEntry name="KEY3" <%=dataprotect%> maxlength=50 size=50 style="TEXT-ALIGN: left" value
            ="<%=DSPKEY(3)%>" ID="Text2">
    　</td>
</table>
<% conn.close
