...
Sub SrEnvironment()
  ...
  functionOptName="自動排程ping記錄一覽;Ping及Reset; 作  廢 ;作廢反轉"
  functionOptProgram="RTResetSubK.asp;RTPingAndReset.asp;RTResetDrop.asp;RTResetDropC.asp"
  functionOptPrompt="N;N;Y;Y"
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="none;工程師;社區方案;主線;社區名稱;主線<br>撤線日;主線<br>作廢日;Reset電話;主線IP;iDslam IP;主線最後<br>回應訊息;主線<br>最後速度;iDslam最後<br>回應訊息;iDslam<br>最後速度;最後<br>撥號;reset<br>次數;alarm<br>次數;最後<br>ping日期;作廢"
  sqlDelete="select 	a.resetno, a.resetno, m.codenc as cmtytype, " &_
			...
			"isnull(k.codenc, '') as lastping1, f.pingms1 as lastpingms1, " &_
			"isnull(l.codenc, '') as lastping2, f.pingms2 as lastpingms2, " &_
			"isnull(f.dialaction, '') as lastdialaction, isnull(o.resetnum,0), isnull(n.alarmnum,0), " &_
			"convert(varchar(20), f.pingdat, 20) as lastpingdat, case when a.canceldat is null then '' else 'Y' end " &_
			"from	RTReset a " &_
			...
			"		left outer join RTResetSub f 	inner join (select resetno, max(rtbatch) as maxrtbatch from RTResetSub group by resetno) g " &_
			"			on f.resetno = g.resetno and f.rtbatch = g.maxrtbatch on f.resetno = a.resetno " &_
			...
			"		left outer join RTCode m on m.code = a.cmtytype and m.kind ='L5' " &_			"		left outer join (select resetno, count(*) as alarmnum from RTResetSub where dialaction ='alarm' group by resetno) n on n.resetno = a.resetno " &_			"		left outer join (select resetno, count(*) as resetnum from RTResetSub where dialaction like 'reset%' group by resetno) o on o.resetno = a.resetno " &_			"WHERE a.resetno='*' "
  dataTable="RTReset"
  userDefineDelete=""
  numberOfKey=1
  dataProg="RTResetD.asp"
  datawindowFeature=""
...
' When first time enter this keylist default query string to RTcmty.CASENO <> '*'
  ...
  sqlList=	"select 	a.resetno, " &_
			...
			"			when '07' then replace(e.lineip, ' ', '') else '' end as ip1, " &_
			"isnull(c.idslamip, '') as ip2, " &_
			"isnull(k.codenc, '') as lastping1, f.pingms1+case when pingms1<>'' then 'ms' end as lastpingms1, " &_
			"isnull(l.codenc, '') as lastping2, f.pingms2+case when pingms2<>'' then 'ms' end as lastpingms2, " &_
			"case when f.dialaction='alarm' then '<font color=red>'+isnull(f.dialaction, '')+'</font>' when f.dialaction like 'reset%' then '<font color=Maroon>'+isnull(f.dialaction, '')+'</font>' end as lastdialaction, " &_
			"isnull(o.resetnum,0) as resetnum, isnull(n.alarmnum,0) as alarmnum, convert(varchar(20), f.pingdat, 20) as lastpingdat, case when a.canceldat is null then '' else 'Y' end " &_
			"from	RTReset a " &_
			"		left outer join RTSparqAdslCmty b on a.comq1 = b.cutyid and a.cmtytype ='03' " &_
			"		left outer join RTSparq499CmtyLine c inner join RTSparq499CmtyH h on h.comq1 = c.comq1 on a.comq1 = c.comq1 and a.lineq1 = c.lineq1 and a.cmtytype ='05' " &_
			"		left outer join RTLessorCmtyLine d  inner join RTLessorCmtyH i on i.comq1 = d.comq1 on a.comq1 = d.comq1 and a.lineq1 = d.lineq1 and a.cmtytype ='06' " &_
