...
Sub SrEnvironment()
  ...
  functionOptName="重大訊息;客  戶;整線派工;主機客訴;設備管理"
  functionOptProgram="RTCMTYMSGK.ASP;RTCUSTOPTK.ASP;/webap/rtap/base/hbCmtyarrangesndwork/hbCmtyarrangesndworkk.asp;RTFAQK.ASP;RTCMTYEquipmentk.asp"
  functionOPTprompt="N;N;N;N;N;N;N"
  If V(1)="Y" then
     accessMode="U"
  Else
     accessMode="I"
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;none;序號;社區名稱;社區附掛;總戶數;寬頻戶;方案別;開通日;撤線日;經銷商;工程師;建置書;契約書;Reset電話;整線派工;整線完成;審核完成"
  sqlDelete="SELECT HBADSLCMTY.comq1,hbadslcmty.lineq1, HBADSLCMTY.comtype, HBADSLCMTY.comq1, " _
		   ...
		   &"CASE HBADSLCMTY.comtype WHEN '1' THEN '元訊599' WHEN '2' THEN '中華399' when '3' then '速博399' when '4' then '東訊599' when '5' then '東森499' when '6' then '速博499' ELSE '???' END AS comtype, " _           
           &"HBADSLCMTY.T1APPLYDAT,HBADSLCMTY.rcomdrop, case when hbadslcmty.comsource is not null and hbadslcmty.comsource not in ('元訊','東訊','先說','聚宇','福元','大同')  then hbadslcmty.comsource else HBADSLCMTY.GROUPNC end, " _
           &"HBADSLCMTY.leader, HBADSLCMTY.comagree,HBADSLCMTY.contract, " _
		   ...
		   &"CASE HBADSLCMTY.comtype when '3' then RTReset.Tel when '6' then RTReset1.Tel ELSE '' END, " _           
           &" case when hbadslcmty.comsource is not null and hbadslcmty.comsource not in ('元訊','東訊','先說','聚宇','福元','大同')  then hbadslcmty.comsource else HBADSLCMTY.GROUPNC end, HBADSLCMTY.leader, " _
  ...
  extTable=""
  numberOfKey=3
  dataProg="/webap/rtap/base/rtadslcmty/rtcmtyd.asp;/webap/rtap/base/rtcmty/rtcmtyd.asp;/webap/rtap/base/rtsparqadslcmty/rtcmtyd.asp;/webap/rtap/base/rtcmty/rtcmtyd.asp;/webap/rtap/base/rtebtcmty/rtebtcmtylined.asp;/webap/rtap/base/rtsparq499cmty/RTSPARQ499cmtyLINED.asp"
  ...
  Call SrGetEmployeeRef(Rtnvalue,1,logonid)
  xyz=split(rtnvalue,";") 
  sqllist="SELECT HBADSLCMTY.comq1,hbadslcmty.lineq1, HBADSLCMTY.comtype, "_
           &"convert(varchar(6),HBADSLCMTY.comq1)+ case hbadslcmty.lineq1 when 0 then '' else '-'+convert(varchar(2),HBADSLCMTY.lineq1) end, " _
           &"HBADSLCMTY.comn, HBADSLCMTY.LINETEL, HBADSLCMTY.comcnt, HBADSLCMTY.usercnt, " _
           &"CASE HBADSLCMTY.comtype WHEN '1' THEN '元訊599' WHEN '2' THEN '中華399' when '3' then '速博399' when '4' then '東訊599' when '5' then '東森499' when '6' then '速博499' ELSE '???' END AS comtype, " _
           &"HBADSLCMTY.T1APPLYDAT,HBADSLCMTY.rcomdrop,  case when hbadslcmty.comsource is not null and hbadslcmty.comsource not in ('元訊','東訊','先說','聚宇','福元','大同')  then hbadslcmty.comsource else HBADSLCMTY.GROUPNC end, " _
           &"HBADSLCMTY.leader, HBADSLCMTY.comagree,HBADSLCMTY.contract, " _
