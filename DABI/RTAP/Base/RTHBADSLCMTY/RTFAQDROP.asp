  ...
  ' RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="insert into HBADSLCMTYFIXHlog " _
           &"SELECT  FIXNO," & ENTRYNO & ", getdate(), 'C','" &  v(0) & "', " _
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update HBADSLCMTYFIXH set dropdat=getdate(),DROPUSR='" & V(0) & "' where FIXNO='" & KEY(0) & "' "
...
<script language=vbscript>
 sub window_onload()
    ...
    elseIF frm1.htmlfld.value="3" then
       msgbox "此社區主機客訴已結案，不可作廢(欲作廢請先執行結案返轉)" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close       
    elseIF frm1.htmlfld.value="4" then
       msgbox "此社區主機客訴已作廢，不可重覆執行作廢作業：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close              
    else
       msgbox "無法執行社區主機客訴作廢作業,錯誤訊息：" & "  " & frm1.htmlfld1.value
       Set winP=window.Opener
       winP.focus()
       window.close
    end if
