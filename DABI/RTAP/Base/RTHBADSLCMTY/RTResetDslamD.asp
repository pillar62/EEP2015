...
'                   case ucase("/webap/rtap/base/RTPowerBill/RTPowerBillD.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
						...
						if i=0 then
				            ...
				            cusidxx="I" & trim(datePART("yyyy",NOW())) & right("0" & trim(datePART("m",NOW())),2) 
							sql = "select max(resetno) AS maxresetno from RTResetDslam where resetno like '" & cusidxx & "%' "
							rsc.open sql,conn
...
'               end select
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTHBADSLCMTY/RTResetDslamD.asp") then
          cusidxx="I" & trim(datePART("yyyy",NOW())) & right("0" & trim(datePART("m",NOW())),2) 
          rs.open "select max(resetno) AS maxresetno from RTResetDslam where resetno like '" & cusidxx & "%' " ,conn
          if not rs.eof then
...
Sub SrEnvironment()
  ...
  title="社區Reset主檔資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB=	"SELECT RESETNO, IP1, IP2, IP3, IP4, PASSWD, CMTYTYPE, COMQ1, LINEQ1, MEMO, " &_
                "UDAT, UUSR, CANCELDAT, CANCELUSR FROM RTResetDslam " &_
                "WHERE RESETNO='' "
  sqlList=		"SELECT RESETNO, IP1, IP2, IP3, IP4, PASSWD, CMTYTYPE, COMQ1, LINEQ1, MEMO, " &_
                "UDAT, UUSR, CANCELDAT, CANCELUSR FROM RTResetDslam " &_
                "WHERE "
  'userDefineRead="Yes"      
  'userDefineSave="Yes"       
...
'    If accessMode="A" And sw="S" Then dspKey(0)=-1
	IF LEN(TRIM(DSPKEY(5))) = 0 THEN DSPKEY(5)="Dog2888"
    IF LEN(TRIM(DSPKEY(7))) = 0 THEN DSPKEY(7)=0
    IF LEN(TRIM(DSPKEY(8))) = 0 THEN DSPKEY(8)=0

   	if isnumeric(dspkey(1)) and isnumeric(dspkey(2)) and isnumeric(dspkey(3)) and isnumeric(dspkey(4)) then 
		If dspkey(4) < 0 or dspkey(4) >255 then
				formValid=False
				message="Restart IP欄4 請填0~255"
		end if
		If dspkey(3) < 0 or dspkey(3) >255 then
				formValid=False
				message="Restart IP欄3 請填0~255"
		end if
		If dspkey(2) < 0 or dspkey(2) >255 then
				formValid=False
				message="Restart IP欄2 請填0~255"
		end if
		If dspkey(1) < 0 or dspkey(1) >255 then
				formValid=False
				message="Restart IP欄1 請填0~255"
		end if
	else
			formValid=False
			message="Restart IP欄位請填字"
	end if
...
Sub SrActiveXScript()%>
   ...
   Sub Srcounty3onclick()
       ...
       prog="RTGetCmty.asp?KEY=" & document.all("KEY6").VALUE &";"& colComn
...
<tr><td width="15%" class=dataListHead height="23">社區類別</td>
    ...
    <td width="85%" height="23" bgcolor="silver" colspan="3">
      <select class=dataListEntry name="key6" <%=dataProtect%> size="1"><%=s%></select>
	</td>
</tr>
...
<tr><td width="15%" class=dataListHEAD>社區主線序號</td>
    <td width="85%" height="23" bgcolor="silver" colspan="3">
		<input type="text" name="key7" readonly size="4" value="<%=dspkey(7)%>" class="dataListDATA">-
		<input type="text" name="key8" readonly size="3" value="<%=dspkey(8)%>" class="dataListDATA">
		<%
			sql="select case a.cmtytype when '03' then b.comn when '05' then h.comn when '06' then i.comn when '07' then j.comn else '' end as comn, " &_
				...
				"where a.cmtytype ='"& dspkey(6) &"' and a.COMQ1 ='"& dspkey(7) &"' and a.LINEQ1 ='"& dspkey(8) &"' "
			rs.Open sql,conn
...
<tr><td width="15%" class=dataListHEAD>主線撤線日</td>
	<td width="35%" height="23" bgcolor="silver">
		<input type="text" ID="colDropdat" name="colDropdat" readonly size="25" value="<%=dropdat%>" style="color=red;" class="dataListDATA">
	</td>

	<td width="15%" class=dataListHEAD>主線作廢日</td>
	<td width="35%" height="23" bgcolor="silver">
		<input type="text" ID="colCanceldat" name="colCanceldat" readonly size="25" value="<%=canceldat%>" style="color=red;" class="dataListDATA">
	</td>
</tr>

<tr><td width="15%" class=dataListHead height="23">Restart IP</td>
    <td width="35%" height="23" bgcolor="silver">
        <input type="text" name="key1" value="<%=dspKey(1)%>" maxlength="3" size="4" <%=dataProtect%> class="dataListEntry noime">. 
        <input type="text" name="key2" value="<%=dspKey(2)%>" maxlength="3" size="4" <%=dataProtect%> class="dataListEntry noime">.
        <input type="text" name="key3" value="<%=dspKey(3)%>" maxlength="3" size="4" <%=dataProtect%> class="dataListEntry noime">.
        <input type="text" name="key4" value="<%=dspKey(4)%>" maxlength="3" size="4" <%=dataProtect%> class="dataListEntry noime">
	</td>

	<td width="15%" class=dataListHead height="23">iDslam 密碼</td>
    <td width="35%" height="23" bgcolor="silver">
        <input type="text" name="key5" value="<%=dspKey(5)%>" maxlength="15" size="15" <%=dataProtect%> class="dataListEntry">
	</td>
</tr>

<tr><td width="15%" class=dataListHead height="23">備註說明</td>
	<TD width="85%" align="Left" bgcolor="silver" colspan="3">
		<TEXTAREA cols="100%" name="key9" value="<%=dspkey(9)%>" <%=dataprotect%> rows=5 MAXLENGTH=255 class="dataListentry" ID="Textarea1"><%=dspkey(9)%></TEXTAREA>
	</td>
</tr>

<tr><td width="15%" class="dataListHEAD" height="23">作廢人員</td>
    <td width="35%" height="23" bgcolor="silver">
		<input type="text" name="key13" size="6" READONLY value="<%=dspKey(13)%>" class="dataListDATA">
		<font size=2><%=SrGetEmployeeName(dspKey(13))%></font>
	</td>

    <td width="15%" class="dataListHEAD" height="23">作廢日期</td>
    <td width="35%" height="23" bgcolor="silver">
		<input type="text" name="key12" size="25" READONLY value="<%=dspKey(12)%>" class="dataListDATA">
	</td>       
</tr>

<tr><td width="15%" class="dataListHEAD" height="23">最後修改人員</td>                                 
    <td height="23" bgcolor="silver">
		<input type="text" name="key11" size="6" READONLY value="<%=dspKey(11)%>" class="dataListDATA">
		<font size=2><%=SrGetEmployeeName(dspKey(11))%></font>
	</td>

    <td width="15%" class="dataListHEAD" height="23">最後修改日期</td>
    <td height="23" bgcolor="silver">
		<input type="text" name="key10" size="25" READONLY value="<%=dspKey(10)%>" class="dataListDATA">
	</td>
</tr>
</table><br><br>
