...
Sub SrEnvironment()
  ...
  system="專案用戶管理系統"
  title="專案用戶每月應收應付帳款查詢"
  buttonName=" 新增 ; 刪除 ; 結束 ;重新整理;頁數;功能選項"
  V=split(SrAccessPermit,";")
  AllowA=V(0):AllowU=V(1):AllowD=V(2):AllowP=V(3)
  ButtonEnable="N;N;Y;Y;Y;Y"  
  'buttonEnable="Y;Y;Y;Y;Y;Y"
  functionOptName="全部用戶Excel表;全部用戶"
  functionOptProgram="RTPrjCustARK3Dxls.asp;RTPrjCustARK3D.ASP"
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="帳款<BR>年度;帳款<BR>月份;總金額;已沖金額;未沖金額;總筆數;已沖<BR>筆數;未沖<BR>筆數"
  sqlDelete="SELECT TYY, TMM, SUM(AMT) AS Expr1, SUM(REALAMT) AS Expr2, COUNT(*) AS Expr3 " _
                &"FROM   RTPrjCustARDTL WHERE         (CANCELDAT IS NULL) " _
