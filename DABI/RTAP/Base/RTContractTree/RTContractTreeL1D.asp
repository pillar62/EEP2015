...
Sub SrGetUserDefineData()
    ...
    IF DSPKEY(3) > 0 THEN 
       CLS=" class=dataListData readonly"
    ELSE
       CLS=" class=dataListEntry "   
    END IF
%>
  ...
  <tr>
    ...
    <td width="30%" bgcolor="#C0C0C0">
     <input class=dataListEntry name="KEY2" <%=dataprotect%> maxlength=20 size=20 style="TEXT-ALIGN: left" value
            ="<%=DSPKEY(2)%>"></td>
    <td width="20%" bgcolor="#008080"><font color="#FFFFFF">每冊頁次</font></td>
    <td width="30%" bgcolor="#C0C0C0">
     <input <%=CLS%> name="KEY3" <%=dataprotect%>  maxlength=20 size=20 style="TEXT-ALIGN: left" value
            ="<%=DSPKEY(3)%>" ></td>            
  </tr>
    <tr>
