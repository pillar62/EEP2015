...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT CUSID, ENTRYNO,STOCKID,BRANCH,BUSSMAN,BUSSID,SEX,BIRTHDAY, " _
             ...
             &"SETFEEDESC,orderno,ss365,REPLYDATE,Lookdat,formaldat,deliverdat,socialid,agree,haveroom,homestat,LOOKDESC " _
...
<table width="100%" ><tr><td width="100%">&nbsp;</td></tr>                                                      
    ...
    <tr>
        ...
        <td width="30%" colspan="5" height="32" bgcolor="silver"> 
...
<% 
   ' sql="SELECT CUSID,cusnc FROM RTOBJ WHERE CUSID='" & dspkey(5) & "'"
   ' rs.Open sql,conn
   ' if Not rs.Eof
   '    s=rs("cusnc")
   ' else
...
<%  Call SrGetCountyTownShip(accessMode,sw,Len(Trim(FIELDROLE(1) &dataProtect)),dspkey(8),dspkey(9),s,t)%>
        ...
        <td width="16%" height="32" bgcolor="silver"><input type="text" name="key11" size="10" value="<%=dspkey(11)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>                                 
      <tr>                                 
...
<input type="text" name="key14" size="41" value="<%=dspkey(14)%>" maxlength="60" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
        <td width="8%" class="dataListHead" height="32">郵遞區號</td>                                 
        <td width="16%" height="32" bgcolor="silver"><input type="text" name="key15" size="10" value="<%=dspkey(15)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>
      <tr>                                 
...
<input type="text" name="key18" size="41" value="<%=dspkey(18)%>" maxlength="60" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
        <td width="8%" class="dataListHead" height="32">郵遞區號</td>                                 
        <td width="16%" height="32" bgcolor="silver"><input type="text" name="key19" size="10" value="<%=dspkey(19)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>                                
      <tr>          
<script language="vbscript">
Sub SrAddrEqual()
        ...
        <td width="15%" class="dataListHead" height="32">堪查日期</td>
         <td width="30%" height="32" bgcolor="silver">       
          ...
          <%  dim rdo1, rdo2
             ...
             %>
        <input type="radio" value="Y" <%=rdo1%> name="key84" <%=fieldRole(1)%>>可建置
