...
sql="select * FROM RTSPARQADSLCUST LEFT OUTER JOIN RTOBJ ON RTSPARQADSLCUST.CUSID=RTOBJ.CUSID WHERE  RTSPARQADSLCUST.CUSID='" & KEYARY(0) & "' AND RTSPARQADSLCUST.ENTRYNO=" & KEYARY(1) & " "
'response.write sql
RS.Open SQL,CONN
IF NOT RS.EOF THEN
   CUSNC=RS("CUSNC")
...
<%
KEYARY=SPLIT(REQUEST("KEY"),";")
sql="select * from NCICMonthlyaccountSRC where  CUSID='" & KEYARY(0) & "' AND ENTRYNO=" & KEYARY(1) & " order by SNDPAYDAT "
rs.Open sql,conn
cnt=0
DO WHILE not rs.EOF 
   cnt=cnt + 1
