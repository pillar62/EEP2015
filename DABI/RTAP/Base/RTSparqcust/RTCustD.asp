       ...
       If accessMode="A" Then
          ...
          For i = 0 To numberOfField-1
              ...
              select case ucase(runpgm)   
               ' 當程式為ADSL客戶基本資料維護作業時,因其dspkey(76)為identify欄位，故不搬入值（由sql自行產生)
...
'------社區名稱---固定(由社區序號讀取)
    ...
    s=""
    If rsxx.Eof Then
       message="社區代號:" &dspkey(2) &"在社區基本資料內找不到"
       formvalud=false
       consigneeXX=""       
    Else 
       dspkey(25)=rsxx("ComN") 
...
<% 
   ...
   '       freecode2=" disabled "
    ...
    If dspKey(96)="N" Then freecode2=" checked " %>                          
        ...
        <td width="17%" class="dataListsearch" height="25">繳費方式</td>       
   ...
   '       freecode2=" disabled "
    ...
    If dspKey(95)="H" Then PAYTYPE3=" checked " %>                                
        <td width="16%" height="25" bgcolor="silver" >
			<input type="radio" value="M"  name="key95" <%=PAYTYPE1%><%=FIELDROLE(1)%>><font size=2>月繳</font>
			<input type="radio" name="key95" value="Y" <%=PAYTYPE2%><%=FIELDROLE(1)%>><font size=2>年繳</font>
			<input type="radio" name="key95" value="H" <%=PAYTYPE3%><%=FIELDROLE(1)%><font size=2>年約月繳</font>        
        </td></TR>
      <tr>                                      
