...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT CUSID, ENTRYNO,STOCKID,BRANCH,BUSSMAN,BUSSID,SEX,BIRTHDAY, " _
             ...
             &"SETFEEDESC,orderno,ss365,REPLYDATE,Lookdat,formaldat,deliverdat,socialid,agree,haveroom,homestat,LOOKDESC,CHTSIGNDAT,SENDWORKING,WORKINGREPLY,cusno,comq1  " _
...
'------社區名稱---固定(由社區序號讀取)
    ...
    DSNXX="DSN=RTLIB"
    connxx.Open DSNxx
    sqlXX="SELECT COMN FROM RTCustAdslCmty where cutyid=" & dspkey(92)
    rsxx.Open sqlxx,connxx
    s=""
    If rsxx.Eof Then
       message="社區代號:" &dspkey(92) &"在社區基本資料內找不到"
       formvalud=false
    Else 
       dspkey(26)=rsxx("ComN") 
    End If
...
<table width="100%" border=1 cellPadding=0 cellSpacing=0>
    ...
    <td width="20%" bgcolor="orange" style="font-size:12px">收件編號</td>
    <td width="10%" bgcolor="silver">
...
<table width="100%" ><tr><td width="100%">&nbsp;</td></tr>                                                      
    ...
    <tr>
        ...
        <input type="text" name="key83" size="10" maxlength="10" value="<%=dspkey(83)%>" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td> 
        <td width="15%" class="dataListHead" height="25">HN號碼</td>       
        <td width="30%" colspan="3" height="25" bgcolor="silver"> 
        <input type="text" name="key91" size="10" maxlength="10" value="<%=dspkey(91)%>" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>         
    </tr>
    <tr>                   
...
<%    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 Then 
    ...
    rs.Close        
    %>
           <select size="1" name="key2" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry">                                            
              <%=s%>
           </select>
        <input type="text" name="key3" size="10" value="<%=dspkey(3)%>" maxlength="12" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" readonly><font size=2>(分行)               
         <input type="button" id="B3"  name="B3"   width="100%" style="Z-INDEX: 1"  value="..." onclick="SrBranchonclick()"  >  
        ...
        <input type="text" name="key4" size="8" value="<%=dspkey(4)%>" maxlength="12" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" readonly>
         <input type="button" id="B4"  name="B4"   width="100%" style="Z-INDEX: 1"  value="..." onclick="SrBranchmanonclick()"  >                  
...
<%  dim sexd1, sexd2
    ...
    If dspKey(6)="F" Then sexd2=" checked " %>                          
        <td width="16%" height="25" bgcolor="silver">
        <input type="radio" value="M" <%=sexd1%> name="key6" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtec%>><font size=2>男</font>
        <input type="radio" name="key6" value="F" <%=sexd2%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>><font size=2>女</font></td>                              
  ...
  <%s=""
    ...
    rs.Close
   %>
         <select size="1" name="key8"<%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry"><%=s%></select>
        <input type="text" name="key9" size="8" value="<%=dspkey(9)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"><font size=2>(鄉鎮市區)                 
         <input type="button" id="B9"  name="B9"   width="100%" style="Z-INDEX: 1"  value="..." onclick="Srcounty9onclick()"  >        
        ...
        <input type="text" name="key10" size="32" value="<%=dspkey(10)%>" maxlength="60" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
        <td width="8%" class="dataListHead" height="25">郵遞區號</td>                                 
        <td width="16%" height="25" bgcolor="silver"><input type="text" name="key11" size="10" value="<%=dspkey(11)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>                                 
      <tr>                                 
  ...
  <%s=""
   ...
   %>        
        ...
        <input type="text" name="key13" size="8" value="<%=dspkey(13)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"><font size=2>(鄉鎮市區)                 
         <input type="button" id="B13"  name="B13"   width="100%" style="Z-INDEX: 1"  value="..." onclick="Srcounty13onclick()"  >        
        ...
        <td width="8%" class="dataListHead" height="25">郵遞區號</td>                                 
        <td width="16%" height="25" bgcolor="silver"><input type="text" name="key15" size="10" value="<%=dspkey(15)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>
      <tr>                                 
  ...
  <%s=""
   ...
   %>        
        ...
        <input type="text" name="key17" size="8" value="<%=dspkey(17)%>" maxlength="10" readonly <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"><font size=2>(鄉鎮市區)                 
         <input type="button" id="B17"  name="B17"   width="100%" style="Z-INDEX: 1"  value="..." onclick="Srcounty17onclick()"  >        
        ...
        <td width="8%" class="dataListHead" height="25">郵遞區號</td>                                 
        <td width="16%" height="25" bgcolor="silver"><input type="text" name="key19" size="10" value="<%=dspkey(19)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>                                
      <tr>          
<script language="vbscript">
Sub SrAddrEqual()
...
<% aryOption=Array("ADSL")
   ...
   End If%>                                  
        <td width="16%" height="25" bgcolor="silver"><select size="1" name="key21" style="font-family: 新細明體; font-size: 10pt"<%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry">                                                                  
        <%=s%></select></td>                                     
      </tr>                                 
      <tr>                            
   ...
   <select size="1" name="key78" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry">                                            
      ...
      <tr>                            
        <td width="15%" class="dataListHead" height="25">堪查日期</td>
         <td width="30%" height="25" bgcolor="silver">       
          <input type="text" name="key80" size="10" value="<%=dspKey(80)%>"  <%=fieldpa%><%=FIELDROLE(3)%> class="dataListEntry" maxlength="10" >
          ...
          <%  dim rdo1, rdo2
             ...
             %>
        <input type="radio" value="Y" <%=rdo1%> name="key84" <%=fieldRole(3)%><%=dataProtect%>><font size=2>可建置
        <input type="radio" value="N" <%=rdo2%>  name="key84" <%=fieldRole(3)%><%=dataProtect%>><font size=2>無法建置
          </td> 
        <td width="8%" class="dataListHead" height="25">堪查結果</td>
         ...
         <% aryOption=Array("","有電信室","無電信室","無電信箱")
            ...
            End If%>               
         <select size="1" name="key85" <%=fieldpa%><%=FIELDROLE(3)%><%=dataProtect%> class="dataListEntry">                                            
           <%=s%>
         </select>
         <% aryOption=Array("","跨棟","獨棟","雙拼")
            ...
            End If%>               
         <select size="1" name="key86" <%=fieldpa%><%=FIELDROLE(3)%><%=dataProtect%> class="dataListEntry">                                            
           <%=s%>
         </select>         
         </td>
      ...
      <tr>
        ...
        <input type="text" name="key88" style="text-align:left;" maxlength="10" size="10"
               value="<%=dspKey(88)%>" class=dataListentry >
          <input type="button" id="B88"  name="B88" height="100%" width="100%" style="Z-INDEX: 1" value="..."  <%=fieldpa%><%=FIELDROLE(1)%><%=fieldpc%>>               
        </td>
        <td width="8%"   bgcolor="ORANGE" height="34">送營運處日期</td>                                 
        <td width="16%"  height="34" bgcolor="silver">
          <input type="text" name="key89" size="10" value="<%=dspKey(89)%>" maxlength="10" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry">
        ...
        <td width="16%"  height="34" bgcolor="silver">
          <input type="text" name="key90" size="10" value="<%=dspKey(90)%>" maxlength="10" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry">
...
<%
   ...
   &nbsp;社區名稱<input type="text" name="key26" size="15" MAXLENGTH="30" value="<%=dspKey(26)%>" <%=fieldpa%><%=FIELDROLE(3)%><%=dataProtect%> readonly class="dataListEntry">
        <!--
   ...
   共<input type="text" name="key27" size="4" value="<%=dspKey(27)%>" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> maxlength="4" class="dataListEntry">戶</td>                                 
      ...
      <tr>                                 
        ...
        <td width="30%" height="23"><input type="text" name="key32" size="15" value="<%=dspkey(32)%>" maxlength="15" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"><font size=2>分機<input type="text" name="key33" size="5" value="<%=dspkey(33)%>" maxlength="5" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      ...
      <tr>                                 
        ...
        <td width="30%" height="25" bgcolor="silver"><input type="text" name="key35" size="30" value="<%=dspkey(35)%>" maxlength="30" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
