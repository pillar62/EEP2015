...
<% 'sales(0)="業務轄區" sales(1)="業務群組"
  ...
  Conn.Open DSN
  if key="" then
  sql="SELECT  rtareasales.cusid,rtobj.cusnc from rtareasales inner join rtemployee on " _
     &"rtareasales.cusid=rtemployee.emply and rtemployee.tran2 <> '10' left outer join rtobj on " _
     &"rtemployee.cusid=rtobj.cusid group BY rtareasales.cusid,rtobj.cusnc " 
  else
  sql="SELECT  rtareasales.cusid,rtobj.cusnc from rtareasales inner join rtemployee on " _
     &"rtareasales.cusid=rtemployee.emply and rtemployee.tran2 <> '10' left outer join rtobj on " _
     &"rtemployee.cusid=rtobj.cusid " _
     &"WHERE RTareasales.AREAID like '" &  sales & "' " _
     &"ORDER BY RTObj.cusnc " 
  end if
  rs.Open sql,conn
  s1=""
  Do While Not rs.Eof
     s1=s1 &"<option value=""" &rs("cusid") &""">" &rs("cusid") &rs("cusnc") &"</option>"
     rs.MoveNext
  Loop
  rs.Close    
...
<FIELDSET
 STYLE="HEIGHT: 308px; LEFT: 16px; POSITION: absolute; TOP: 20px; WIDTH: 155px" ID=select1>
<LEGEND>業務員</LEGEND>
<SELECT id=lstOrder1 size=5 
style="HEIGHT: 269px; LEFT: 10px; POSITION: absolute; TOP: 26px; WIDTH: 140px" >
<%=s1%>
</SELECT>
</FIELDSET>&nbsp; 
