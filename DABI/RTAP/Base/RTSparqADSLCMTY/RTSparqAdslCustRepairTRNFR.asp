...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   'sqlyy="select max(entryno)as entryno from RTSparqAdslCUSTlog where CUSID='" & KEY(0)& "'"
   ...
   '檢查應收帳款檔是否已沖帳
   sqlyy="select * FROM RTSparqAdslCustAR WHERE CUSID='" & KEY(0) & "' AND BATCHNO='" & BATCHNOXx & "'"
   rsyy.open sqlyy,conn
   if rsyy.eof THEN
   ELSE
   ...
   if endpgm="" then
      ...
      strSP="usp_RTSparqAdslCustRepairAR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "', 'MR', '" & BATCHNOXX & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "速博399用戶維修收款返轉應收結案作業成功",0
...
<form name=frm1 method=post action=RTSparqAdslCustRepairTRNFR.asp ID="Form1">
