...
if showopt(0)="Y" then    
	...
	SQL="SELECT	RTEmployee.EMPLY, RTObj.CUSNC "_
		&"FROM	RTAreaSales "_
		&"inner join RTEmployee on RTAreaSales.CUSID = RTEmployee.EMPLY "_
		&"inner join RTObj on RTObj.CUSID = RTEmployee.CUSID "_
		&"inner join RTCtyTown on RTCtyTown.AREAID = RTAreaSales.AREAID "_
		&"and RTCtyTown.CUTID ='" & showopt(4) & "' and RTCtyTown.TOWNSHIP ='" & showopt(5) & "'"
 '   Response.Write sql
     rs.Open sql,conn
    s1=""
    Do While Not rs.Eof
...
if showopt(2)="Y" then
 ...
 if Domain="T" then
    sql="SELECT RTEmployee.emply, RTObj.CUSNC " _
          ...
          &"and rtemployee.dept in ('B500','B700','B701') "    
   'Response.Write sql          
    '其他(不挑選技術部資料) 
 else
     sql="SELECT RTEmployee.emply, RTObj.CUSNC " _
...
<FIELDSET
 STYLE="HEIGHT: 308px; LEFT: 16px; POSITION: absolute; TOP: 20px; WIDTH: 130px" ID=select1>
<LEGEND>業務工程師</LEGEND>
