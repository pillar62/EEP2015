...
<%
  ...
  Dim detailwindowFeature,rscount,searchqry2,EMAILFIELDFLAG
  searchFirst=False
  EMAILFIELDFLAG="N"
  userDefineDelete="No"
  functionOptPrompt=";;;;;;;;;;;;;;;;;;"
...
Sub SrEnvironment()
  ...
  functionOptName="轉應收結案;返轉應收結案;應收應付; 作　廢 ;作廢返轉"
  functionOptProgram="RTSparqAdslCustRepairTRNAR.asp;RTSparqAdslCustRepairTRNFR.asp;RTSparqAdslCustARK2.asp;RTSparqAdslCustRepairDROP.asp;RTSparqAdslCustRepairDROPC.asp"
  functionOptPrompt="Y;Y;N;Y;Y"
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="none;none;用戶名稱;項次;派單日期;繳費方式;設定費;移機費;設備費;覆機費;轉應收帳款日;帳款編號;結案日;作廢日"
  sqlDelete="SELECT  a.CUSID, a.ENTRYNO, d.CUSNC, a.ENTRYNO, a.APPLYDAT, b.CODENC, a.SETAMT, a.MOVEAMT, " &_
					"a.EQUIPAMT, RETURNAMT, a.TARDAT, a.BATCHNO, a.FINISHDAT, a.CANCELDAT " &_
			"FROM    RTSparqAdslCustRepair a " &_
					"LEFT OUTER JOIN RTCode b ON a.PAYTYPE = b.CODE AND b.KIND = 'M9' " &_
					"LEFT OUTER JOIN RTSparqAdslCust c ON a.CUSID = c.CUSID " &_
					"LEFT OUTER JOIN RTObj d ON d.CUSID = c.CUSID " &_
			"WHERE	a.cusid='' "

  dataTable="RTSparqAdslCustRepair"
  userDefineDelete="Yes"
  numberOfKey=2
  dataProg="RTSparqAdslCustRepairD.asp"
  datawindowFeature=""
  ...
  colSplit=1
  keyListPageSize=25
  searchProg="RTSparqAdslcustRepairS.asp"
  '========================================================================================================
  ...
  '-------------------------------------------------------------------------------------------
   sqlList=	"SELECT  a.CUSID, a.ENTRYNO, d.CUSNC, a.ENTRYNO, a.APPLYDAT, b.CODENC, a.SETAMT, a.MOVEAMT, " &_
					"a.EQUIPAMT, RETURNAMT, a.TARDAT, a.BATCHNO, a.FINISHDAT, a.CANCELDAT " &_
			"FROM    RTSparqAdslCustRepair a " &_
					"LEFT OUTER JOIN RTCode b ON a.PAYTYPE = b.CODE AND b.KIND = 'M9' " &_
					"LEFT OUTER JOIN RTSparqAdslCust c ON a.CUSID = c.CUSID " &_
					"LEFT OUTER JOIN RTObj d ON d.CUSID = c.CUSID " &_
			"WHERE	a.cusid='" & aryparmkey(0) & "' "
