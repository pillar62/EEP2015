...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtsparqadslcmty/RTsparqadslcustdropsndworkd.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from rtsparqadslcustdropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtsparqadslcmty/RTsparqadslcustdropsndworkd.asp") then
          cusidxx="P-" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from rtsparqadslcustdropSNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
<%
  name="" 
           if dspkey(3) = "" then
              sql=" select * from rtsparqadslcust left outer join rtsparqadslcmty on rtsparqadslcust.comq1=rtsparqadslcmty.cutyid " _
              ...
              rs.close
           ELSE
              sql=" select * from rtsparqadslcmty where cutyid=" & dspkey(3)
              rs.open sql,conn
              if rs.eof then
                 comn=""
              else
