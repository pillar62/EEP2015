...
Sub SrEnvironment()
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="序號;社區名稱;附掛電話;IP;電信室位置;none;直經銷;none;none;派工單號;測通日;申請戶;完工戶;報竣戶;退租戶;撤銷戶;待處理" 
  sqlDelete="SELECT RTSparqAdslCmty.CUTYID, RTSparqAdslCmty.COMN, RTSparqAdslCmty.CMTYTEL, RTSparqAdslCmty.IPADDR, " _
           ...
           &"RTSparqAdslCmty.RCVD, RTSparqAdslCmty.CASESNDWRK, RTSparqadslcmtysndwork.PRTNO, " _
           &"RTSparqAdslCmty.ADSLAPPLY,sum(case when rtsparqadslcust.cusid is not null then 1 else 0 end),sum(case when rtsparqadslcust.finishdat is not null then 1 else 0 end)," _
           ...
           &"THEN RTObj.SHORTNC ELSE RTArea.AREANC END, RTSparqAdslCmty.RCVD, RTSparqAdslCmty.CASESNDWRK, " _
           &"RTSparqadslcmtysndwork.PRTNO,RTSparqAdslCmty.ADSLAPPLY,RTSparqAdslCmty.rcomdrop "
