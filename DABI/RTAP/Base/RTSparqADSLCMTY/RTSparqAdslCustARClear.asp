...
'----------主線序號
    S3=""
    rs.Open "SELECT * FROM RTSparqAdslCustAR WHERE  CUSID='" & ARYPARMKEY(0) & "' AND BATCHNO='" & ARYPARMKEY(1) & "'",CONN
    IF RS.EOF THEN
       TEMPPERIOD=0
       TEMPCOD1=""
       TEMPCOD2=""
...
Sub btn_onClick()
  ...
  ELSE
   ...
   '              &"location=no,menubar=no,width=5px,height=5px" 
     prog="RTSparqAdslCustArClearExe.asp" & "?key=" & s3 & ";" & s4 & ";" & s11
...
<body><CENTER>
<table width="80%">
  <tr class=dataListTitle align=center>速博399用戶應收應付帳款沖帳</td><tr>
</table>
