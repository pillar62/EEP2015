...
<table width="100%" ID="Table1">
  <tr class=dataListTitle align=center>速博399用戶繳款狀況查詢</td><tr>
  </table>
  <u></u>
<%
...
sql="select * FROM RTSPARQADSLCUST LEFT OUTER JOIN RTOBJ ON RTSPARQADSLCUST.CUSID=RTOBJ.CUSID WHERE RTSPARQADSLCUST.COMQ1=" & KEYARY(0) & " AND RTSPARQADSLCUST.CUSID='" & KEYARY(1) & "' AND RTSPARQADSLCUST.ENTRYNO=" & KEYARY(2) & " "
RS.Open SQL,CONN
IF NOT RS.EOF THEN
   CUSNC=RS("CUSNC")
...
<%
KEYARY=SPLIT(REQUEST("KEY"),";")
sql="select * from NCICMonthlyaccountSRC where COMQ1=" & KEYARY(0) & " AND CUSID='" & KEYARY(1) & "' AND ENTRYNO=" & KEYARY(2) & " order by SNDPAYDAT "
rs.Open sql,conn
cnt=0
DO WHILE not rs.EOF 
   cnt=cnt + 1
