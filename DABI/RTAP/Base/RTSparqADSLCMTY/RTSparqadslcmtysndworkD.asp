...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/rtsparqadslcmty/RTsparqadslcmtysndworkd.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       ...
                       if i=1 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from rtsparqadslcmtySNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtsparqadslcmty/RTsparqadslcmtysndworkd.asp") then
          cusidxx="P-" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from rtsparqadslcmtySNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(1)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT CUTYID, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, ASSIGNENGINEER3, " _
             ...
             &"FROM RTSPARQADSLCMTYSNDWORK WHERE cutyid=0 "
  sqlList="SELECT CUTYID, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, ASSIGNENGINEER3, " _
             &"ASSIGNCONSIGNEE, REALENGINEER1, REALENGINEER2,  REALENGINEER3, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, SNDTYPE, HOSTCABLENO, MEMO,PRTDAT,eusr,edat,uusr,udat,finishdat,unclosedat " _
             &"FROM RTSPARQADSLCMTYSNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connXX.open DSN
    SQLXX="SELECT * FROM RTSPARQADSLCMTYHARDWARE WHERE CUTYID=" & DSPKEY(0) & " and prtno='" & dspkey(1) & "' "
...
<%
 sql="SELECT COUNT(*) as equipcnt FROM RTSPARQADSLCMTYHARDWARE WHERE CUTYID=" & DSPKEY(0) & " AND PRTNO='" & DSPKEY(1) & "' AND dropdat is null" 
 rs.Open sql,conn
 cnt=rs("equipcnt")
 RS.CLOSE
...
<tr><td  class=dataListHead>預定裝機經銷商</td>
    <td  bgcolor="silver" >
       ...
       sql="select a.cusid, c.shortnc " &_
		   "from RTConsignee a " &_
		   "inner join RTConsigneeCASE b on a.cusid = b.cusid and b.caseid ='03' " &_
