...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTSPARQADSLCMTY/RTSPARQADSLCMTYHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=2 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from rtSPARQADSLCMTYhardware where CUTYID=" & dspkey(0)  & " AND PRTNO='" & DSPKEY(1) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/rtSPARQADSLCMTY/RTSPARQADSLCMTYhardwared.asp") then
          rs.open "select max(entryno) AS ENTRYNO from rtSPARQADSLCMTYhardware WHERE  CUTYID=" & dspkey(0) & " AND PRTNO='" & DSPKEY(1) & "' " ,conn
          if not rs.eof then
            dspkey(2)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=3
  title="速博ADSL主線派工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT CUTYID, PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, DROPUSR,UNIT,eusr,edat,uusr,udat " _
             &"FROM RTSPARQADSLCMTYhardware WHERE CUTYID=0 "
  sqlList="SELECT CUTYID, PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, DROPUSR,UNIT,eusr,edat,uusr,udat " _
             &"FROM RTSPARQADSLCMTYhardware  WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTSPARQADSLCMTYSNDWORK WHERE CUTYID=" & DSPKEY(0) & " AND PRTNO='" & DSPKEY(1) & "' "
    ...
    IF ERRFLG="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(1) & "非標準工程，不可輸入設備明細"   
    elseif  ERRFLG1="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(1) & " 已執行庫存計算作業，不可再輸入設備明細"   
