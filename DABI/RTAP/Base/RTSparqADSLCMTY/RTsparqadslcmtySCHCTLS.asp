...
Sub btn_onClick()
  ...
  Else
     s=s &"  社區名稱:包含('" &S1 & "'字元)"
     t=t &" (RTsparqadslcmty.ComN LIKE '%" &S1 &"%')" 
  End If
  '----主線裝機地址
 ...
 ' End If
  ...
  IF LEN(TRIM(S3)) > 0 THEN
     s=s &"  主線IP:包含('" &s3 & "'字元) "
     t=t &" AND (RTsparqadslcmty.IPADDR LIKE '%" & S3 & "%') "
  END IF
  '----主線聯單編號
...
'  End If    
  ...
  If Len(trim(s5)) > 0 Then
     s=s &"  附掛電話:包含('" &s5 & "'字元) "
     t=t &" AND (RTsparqadslcmty.CMTYTEL LIKE '%" & S5 & "%') "
  End If      
  '----主線申請列印單號
...
'  End If       
  ...
  Elseif s7ary(0)="1" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND (RTsparqadslcmty.IPaddr <> '' and RTsparqadslcmty.adslapply is null and RTsparqadslcmty.rcomdrop is null) "
  elseif s7ary(0)="2" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND (RTsparqadslcmty.adslapply is NOT null) " 
  elseif s7ary(0)="3" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND (RTsparqadslcmty.rcomdrop is NOT null) " 
 End If        
  '----社區序號
...
<tr><td class=dataListHead width="40%">主線進度狀況</td>
    <td width="60%"  bgcolor="silver">
      <select name="search7" size="1" class=dataListEntry>
        ...
        <option value="3;主線已撤銷">主線已撤銷</option> 
      </select>
     </td>
</tr>
</table>
