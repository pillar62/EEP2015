...
Sub SrEnvironment()
  ...
  formatName="none;none;none;縣市;鄉鎮;社區名稱;目前申請戶數"
   sqlDelete="SELECT DISTINCT " _
            &"rtsparqadslcust.cutid2, rtsparqadslcust.TOWNSHIP2, rtsparqadslcust.HOUSENAME,RTCounty.CUTNC, rtsparqadslcust.TOWNSHIP2, rtsparqadslcust.HOUSENAME,count(*) " _
            &"FROM rtsparqadslcust INNER JOIN " _
            &"RTCounty ON rtsparqadslcust.CUTID2 = RTCounty.CUTID " _
            &"GROUP BY rtsparqadslcust.cutid2, rtsparqadslcust.TOWNSHIP2, rtsparqadslcust.HOUSENAME,RTCounty.CUTNC, rtsparqadslcust.TOWNSHIP2, rtsparqadslcust.HOUSENAME " 
  ...
  if len(trim(parm(0))) > 0 then
     searchqry=" rtsparqadslcust.cutid2='" & parm(0) & "' "
     CUTNC=SrGetCtyRef(parm(0))
     searchshow="縣市別：" & cutnc
     if len(trim(parm(1))) > 0 then
         searchqry=searchqry & " and " & "rtsparqadslcust.township2='" & parm(1) & "' "
         searchshow=searchshow & "  鄉鎮市區：" & parm(1) & " "
     end if
  end if
  If searchQry="" Then
     searchShow="全部"
     searchQry="rtsparqadslcust.cutid2<>'*' "
  ELSE
     SEARCHFIRST=FALSE
  End If  
