...
Sub btn_onClick()
  ...
  Else
     s=s &"  社區名稱:包含('" &S3 & "'字元)"
     t=t &" AND (RTSPARQADSLCMTY.ComN LIKE '%" &S3 &"%')" 
  End If
  '用戶名稱
  ...
  if  Len(trim(s9))=0 Or s9="" then
     ...
     s=s & " 拆機工單號︰包含('" & s9 & "')字元 "
     t=t & " and (RTSparqadslcustdropsndwork.prtno like '%" & s9 & "%') "
  end if   
  '-拆機進度
  ...
  ELSEIF s10ARY(0) = "1" THEN
     s=s &"  拆機進度:('" &s10aRY(1) & "') "
     t=t &" AND (RTSparqadslcustdropsndwork.FINISHDAT is not null) "    
  '已派工尚未拆機
  ELSEIF s10ARY(0) = "2" THEN
     s=s &"  拆機進度:('" &s10aRY(1) & "') "
     t=t &" AND (RTSparqadslcustdropsndwork.prtno is not null) AND (RTSparqadslcustdropsndwork.FINISHDAT is null) "         
  '尚未派工
  ELSEIF s10ARY(0) = "3" THEN
     s=s &"  拆機進度:('" &s10aRY(1) & "') "
     t=t &" AND (RTSparqadslcustdropsndwork.prtno is null) "              
  end if
...
<body>
<table width="100%">
  <tr class=dataListTitle align=center>Sparq用戶退租資料搜尋條件</td><tr>
</table>
