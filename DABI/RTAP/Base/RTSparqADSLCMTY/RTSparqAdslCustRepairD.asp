...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE	SQL
		  ...
		  For i	= 0	To numberOfField-1
			  ...
			  '	  On Error Resume Next
				...
				select case	ucase(runpgm)	
				   ...
				   case	ucase("/webap/rtap/base/RTSparqAdslcmty/RTSparqAdslCustRepairD.asp")
					'response.write	"I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
					   if i	<> 1 then rs.Fields(i).Value=dspKey(i)	  
					   if i=1 then
						 Set rsc=Server.CreateObject("ADODB.Recordset")
						 rsc.open "select max(entryno) AS entryno from RTSparqAdslCustRepair where	CUSID='" & DSPKEY(0) & "' "	,conn
	...
	if accessmode ="A" then
	   runpgm=Request.ServerVariables("PATH_INFO")
	   if ucase(runpgm)=ucase("/webap/rtap/base/RTSparqAdslcmty/RTSparqAdslCustRepairD.asp") then
		  rsc.open "select max(entryno)	AS ENTRYNO from	RTSparqAdslCustRepair where	 CUSID='" &	DSPKEY(0) &	"' " ,conn
		  if not rsC.eof then
...
Sub	SrEnvironment()
  ...
  sqlFormatDB=	"SELECT	 CUSID,	ENTRYNO, APPLYDAT, EQUIP, EQUIPAMT,	SETAMT,	MOVEAMT, " &_
						...
						"REALCONSIGNEE,	UDAT, UUSR,	RCVMONEYDAT, RETURNAMT " &_
				"FROM	RTSparqAdslCustRepair WHERE	cusid='' "
  sqlList=		"SELECT	 CUSID,	ENTRYNO, APPLYDAT, EQUIP, EQUIPAMT,	SETAMT,	MOVEAMT, " &_
						"PAYTYPE, CREDITCARDTYPE, CREDITBANK, CREDITCARDNO,	CREDITNAME,	" &_
						"CREDITDUEM, CREDITDUEY, TARDAT, BATCHNO, TUSR,	" &_
						"FINISHDAT,	CANCELDAT, CANCELUSR, MEMO,	REALENGINEER, "	&_
						"REALCONSIGNEE,	UDAT, UUSR,	RCVMONEYDAT, RETURNAMT " &_
				"from RTSparqAdslCustRepair	WHERE "
  userDefineRead="Yes"		
  userDefineSave="Yes"		 
  userDefineKey="Yes"
  userDefineData="Yes"
...
Sub	SrCheckData(message,formValid)
  ...
  IF len(trim(dspkey(26)))=0 then dspkey(26)=0	'AMT
  ...
  if len(trim(dspkey(2)))=0	then
	   formValid=False
	   message="派單日期不可空白"	
  elseif DSPKEY(4) <1 and DSPKEY(5) <1 and DSPKEY(6) <1 and DSPKEY(26) <1 then
	   formValid=False
	   message="收款金額必須大於0"
...
'	connxx.open	DSN
'	sqlxx="select count(*) as cnt from RTSparqAdslcustCont where cusid='" &	aryparmkey(0) &	"' and finishdat is	null and canceldat is null "
...
%>
  ...
  -->
  <span	id="tags1" class=dataListTagOn>速博399用戶維修收款資訊</span>
  <!--
		   <FONT SIZE=2	COLOR=RED>
		  <MARQUEE width=250 behavior=slide	scrolldelay=30 behavior=alternate>
		  <%
		  SQLYY="SELECT	FINISHDAT FROM RTSparqAdslCUST WHERE CUSID='" &	DSPKEY(0) &	"'"
		  RS.OPEN SQLYY,CONN
...
<DIV ID=SRTAB2 >
	<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table3">
		...
		<tr><td	WIDTH="10%"	class="dataListSEARCH" height="23">設定費</td>			   
			...
			<td	WIDTH="10%"	class="dataListSEARCH" height="23">覆機費</td>			   
			<td	WIDTH="23%"	height="23"	bgcolor="silver">
				<input type="text" size="8" name="key26" value="<%=dspKey(26)%>" <%=fieldpa%> <%=fieldRole(1)%> class="dataListENTRY noIme">
			</TD>
		</tr>
