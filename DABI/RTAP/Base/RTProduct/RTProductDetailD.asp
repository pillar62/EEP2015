...
Sub SrEnvironment()
  ...
  title="產品明細檔資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT PRODNO,ITEMNO,SPEC,UNIT,LISTPRICE,SALEPRICE,SAFESTOCK,BREAKDAT,CONSIGNEEPRICE,package,ITEMNC " _
             &"FROM RTProdd1 WHERE PRODNO='*' "
  sqlList="SELECT PRODNO,ITEMNO,SPEC,UNIT,LISTPRICE,SALEPRICE,SAFESTOCK,BREAKDAT,CONSIGNEEPRICE,package,ITEMNC " _
             &"FROM RTProdd1 WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=0
...
Sub SrCheckData(message,formValid)
    if len(trim(dspkey(9))) = 0 then dspkey(9)="N"
    ...
    If len(trim(dspKey(2))) < 1 Then
       formValid=False
       message="產品規格不可空白。"
    elseif len(trim(dspKey(10))) < 1 Then
       formValid=False
       message="產品品名不可空白。"       
    elseIf NOT IsNumeric(dspkey(4)) or len(trim(dspkey(4))) = 0 Then
       formValid=False
       message="定價資料錯誤。"
    elseIf NOT IsNumeric(dspkey(5)) or len(trim(dspkey(5))) = 0 Then
       formValid=False
       message="售價資料錯誤。"       
    elseIf NOT IsNumeric(dspkey(8)) or len(trim(dspkey(8))) = 0 Then
       formValid=False
       message="經銷價資料錯誤。"              
...
<p>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="20%" class=dataListHead>產品編號</td>
...
<%  Dim conn,rs,sql,s,sx
    ...
    set conn=nothing%>          
           ...
           <td width="20%" class=dataListHead>細項編號</td>
           <td width="30%" bgcolor=silver>
           <input class=dataListdata type="text" name="key1"
                 readonly size="4" value="<%=dspKey(1)%>" maxlength="3" >
    </td></tr>
    </table>
<%
End Sub
...
<table border="1" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">品名</font></td>
    <td width="35%" bgcolor="#C0C0C0" colspan="3">
    <input class=dataListEntry name="key10" maxlength=60 size=60 style="TEXT-ALIGN: left" value
            ="<%=dspkey(10)%>" >　       
  </td>
  </tr>  
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">規格</font></td>
    <td width="35%" bgcolor="#C0C0C0" colspan="3">
    <input class=dataListEntry name="key2" maxlength=150 size=60 style="TEXT-ALIGN: left" value
            ="<%=dspkey(2)%>" >　       
  </td>
  </tr>
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">安全庫存量</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListEntry name="key6" maxlength=6 size=6 style="TEXT-ALIGN:  left" 
     value="<%=dspkey(6)%>" ></td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">單位</font></td>
    <td width="35%" bgcolor="#C0C0C0">
 <% Dim conn,rs,sql,s,sx
    set conn=server.CreateObject("ADODB.Connection")
    set rs=server.CreateObject("ADODB.recordset")
    ...
    If sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false) Then 
       sql="SELECT CODE,CODENC FROM RTCode where kind='B5' ORDER BY CODE "
    ...
    Else
       sql="SELECT code,codenc FROM RTcode WHERE KIND='B5' AND CODE='" &dspkey(3) &"' "
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
  ...
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">定價</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListEntry  name="key4"  size=6 maxlength=6 style="TEXT-ALIGN: left "
            value="<%=dspkey(4)%>"></td>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">售價</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListEntry name="key5" maxlength=10 size=10    
            style="TEXT-ALIGN: left" value="<%=dspkey(5)%>" >　</td>
  <tr>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">經銷價</font></td>
    <td width="35%"  bgcolor="#C0C0C0">
    <input class=dataListEntry  name="key8"  size=6 maxlength=6 style="TEXT-ALIGN: left "
          value="<%=dspkey(8)%>"> </TD>
    <td width="15%" bgcolor="#008080"><font color="#FFFFFF">停售起日</font></td>
    <td width="35%" bgcolor="#C0C0C0">
    <input class=dataListDATA name="key7" maxlength=12 size=12    
   ...
   <tr>
       <td width="15%" bgcolor="#008080"><font color="#FFFFFF">是否為套餐</td>
...
<%  dim sexd1, sexd2
    ...
    If dspKey(9)="Y" Then sexd1=" checked "    
    If dspKey(9)="N" Then sexd2=" checked " %>                          
        <input type="radio" value="Y" <%=sexd1%> name="key9" <%=dataProtecT%>><font size=2>是</font>
        <input type="radio" name="key9" value="N" <%=sexd2%><%=dataProtect%>><font size=2>否</font></td>                              
   </td>
   </tr>
</table>
<%
