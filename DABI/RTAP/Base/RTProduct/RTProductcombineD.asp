...
Sub SrEnvironment()
  ...
  title="套餐組合產品資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT PRODNO,ITEMNO,prodno2,itemno2,qty,unitprice " _
             &"FROM RTProdd2 WHERE PRODNO='*' "
  sqlList="SELECT PRODNO,ITEMNO,prodno2,itemno2,qty,unitprice  " _
             &"FROM RTProdd2 WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=3
...
Sub SrCheckData(message,formValid)
    ...
    If NOT IsNumeric(dspkey(4)) or len(trim(dspkey(4))) = 0 OR dspkey(4)=0 Then
       formValid=False
       message="數量資料錯誤。"
    elseIf NOT IsNumeric(dspkey(5)) or len(trim(dspkey(5))) = 0 OR dspkey(5)=0  Then
       formValid=False
       message="單價資料錯誤。"       
    End If              
End Sub
...
<p>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="10%" class=dataListHead>套餐編號</td>
...
%>          
           ...
           <td width="10%" class=dataListHead>細項編號</td>
           <td width="15%" bgcolor=silver>
           <input class=dataListdata type="text" name="key1"
                 readonly size="4" value="<%=dspKey(1)%>" maxlength="3" >
    </td>
    <td width="10%" class=dataListHead>組合產品編號</td>
       <td width="15%" bgcolor=silver>
<% 
    s=""
    sx=" selected "
    If sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false) Then 
       sql="SELECT RTPRODH.PRODNO,rtprodh.prodnc FROM RTprodh "
    ...
    Else
       sql="SELECT RTPRODH.PRODNO,rtprodh.prodnc FROM RTprodh where prodno='" & dspkey(2) & "' "
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("prodno")=dspkey(2) Then sx=" selected "
...
%>
    <select class=dataListEntry name="key2" <%=dataProtect%> size="1" 
       style="text-align:left;" ><%=s%></select>    
</td>
    <td width="10%" class=dataListHead>組合細項編號</td>
       <td width="15%" bgcolor=silver>
<% 
    s=""
    sx=" selected "
    If sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false) Then 
       sql="SELECT RTPRODd1.PRODNO,rtprodd1.itemno,itemnc FROM RTprodd1 "
    ...
    Else
       sql="SELECT RTPRODd1.PRODNO,rtprodd1.itemno,itemnc FROM RTprodd1 where prodno='" & dspkey(2) & "' and itemno='" & dspkey(3) & "' "
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
       If rs("prodno")=dspkey(2) and rs("ITEMno")=dspkey(3)  Then sx=" selected "
       s=s &"<option value=""" &rs("ITEMno") &"""" &sx &">" &rs("ITEMnc") &"</option>"
       rs.MoveNext
       sx=""
    Loop
    rs.Close
...
<table border="1" width="100%" cellspacing="0" cellpadding="0">
  ...
  <tr>
   ...
   <tr>
       <td width="15%" class=SEARCHEntry bgcolor="#008080"><font color="#FFFFFF">總金額</td>
    <td width="45%" colspan=3 bgcolor="#C0C0C0">
<% if Isnumeric(dspkey(4)) AND Isnumeric(dspkey(5)) then 
      AMT=dspkey(4) * dspkey(5)
   else
      AMT=0
   END if
 %>                          
        <input class=dataListDATA type="TEXT" value="<%=AMT%>" READONLY SIZE=10 >
      </td>                              
   </tr>
</table>
<%
