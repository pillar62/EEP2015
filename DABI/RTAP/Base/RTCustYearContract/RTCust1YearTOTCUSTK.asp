...
Sub SrEnvironment()
  ...
  functionOptProgram="RTConsigneeStatD2.asp"
  functionOptPrompt="N"
  If V(1)="Y" then
     accessMode="U"
  Else
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="<CENTER>經銷商</CENTER>;<CENTER>同意書年月</CENTER>;<CENTER>同意書數量</CENTER>;<CENTER>報竣戶</CENTER>;<CENTER>退租戶</CENTER>;<CENTER>已使用量</CENTER>;<CENTER>可使用量</CENTER>"
  sqlDelete="SELECT  CASE WHEN b.comtype BETWEEN '01' AND '05' OR  b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END AS area, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2), COUNT(*),SUM(CASE WHEN a.docketdat IS NOT NULL THEN 1 ELSE 0 END),0,SUM(CASE WHEN a.docketdat IS NOT NULL THEN 1 ELSE 0 END)-SUM(CASE WHEN a.dropdat IS NOT NULL THEN 1 ELSE 0 END) - 0 " _
           &"FROM RTCust a INNER JOIN RTCmty b ON a.COMQ1 = b.COMQ1 LEFT OUTER JOIN RTCode c ON c.CODE = b.COMTYPE AND c.KIND = 'B3' LEFT OUTER JOIN " _
           &"rtctytown f INNER JOIN rtarea g ON g.areaid = f.areaid ON b.cutid = f.cutid AND b.township = f.township LEFT OUTER JOIN " _
           &"(SELECT  cutid, areaid  FROM  rtctytown  WHERE NOT (cutid = '03' AND areaid = 'C2') AND areaid <> '' " _
           &"GROUP BY   cutid, areaid) h INNER JOIN rtarea i ON i.areaid = h.areaid ON b.cutid = h.cutid " _
           &"WHERE CONSENTNO <> '' " _
           &"GROUP BY  CASE WHEN b.comtype BETWEEN '01' AND '05' OR b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2) " _
           &"ORDER BY  CASE WHEN b.comtype BETWEEN '01' AND '05' OR b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2) " 
  ...
  colSplit=2
  keyListPageSize=40
  searchProg="RTConsigneeStatS2.asp"
...
' When first time enter this keylist default query string to RTcmty.ComQ1 <> 0
  ...
  If searchQry="" AND searchQry2="" AND searchQry3="" Then
     searchQry=" consentDAT between '1900/1/1' and '2900/12/31' "
     searchShow="全部"
  ELSE
     SEARCHFIRST=FALSE
  End If
  ...
  '-------------------------------------------------------------------------------------------
    sqlList="SELECT  CASE WHEN b.comtype BETWEEN '01' AND '05' OR  b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END AS area, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2), COUNT(*),SUM(CASE WHEN a.docketdat IS NOT NULL THEN 1 ELSE 0 END),SUM(CASE WHEN a.dropdat IS NOT NULL THEN 1 ELSE 0 END) ,SUM(CASE WHEN a.consentuseno <> '' THEN 1 ELSE 0 END) ,SUM(CASE WHEN a.docketdat IS NOT NULL THEN 1 ELSE 0 END)-SUM(CASE WHEN a.dropdat IS NOT NULL THEN 1 ELSE 0 END) -SUM(CASE WHEN a.consentuseno <> '' THEN 1 ELSE 0 END) " _
           &"FROM RTCust a INNER JOIN RTCmty b ON a.COMQ1 = b.COMQ1 LEFT OUTER JOIN RTCode c ON c.CODE = b.COMTYPE AND c.KIND = 'B3' LEFT OUTER JOIN " _
           &"rtctytown f INNER JOIN rtarea g ON g.areaid = f.areaid ON b.cutid = f.cutid AND b.township = f.township LEFT OUTER JOIN " _
           &"(SELECT  cutid, areaid  FROM  rtctytown  WHERE NOT (cutid = '03' AND areaid = 'C2') AND areaid <> '' " _
           &"GROUP BY   cutid, areaid) h INNER JOIN rtarea i ON i.areaid = h.areaid ON b.cutid = h.cutid " _
           &"WHERE CONSENTNO <> '' and a.docketdat is not null and " & searchqry & " " _
           &"GROUP BY  CASE WHEN b.comtype BETWEEN '01' AND '05' OR b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2) " _
           &"ORDER BY   CASE WHEN b.comtype BETWEEN '01' AND '05' OR b.comtype = '14' THEN isnull(g.areanc, i.areanc) ELSE c.codenc END, " _
           &"CONVERT(varchar(3), Datepart(yy, a.consentdat) - 1911) + RIGHT('0' + CONVERT(varchar(2), Datepart(mm, a.consentdat)), 2) " 
