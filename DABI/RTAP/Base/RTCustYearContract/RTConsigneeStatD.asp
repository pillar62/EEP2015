...
Sub SrEnvironment()
  ...
  colSplit=1
  keyListPageSize=25
  searchProg="RTConsigneeStatDS.asp"
...
' response.Write "searchQry="&searchQry& "<br>"
  ...
  If searchQry="" Then
     searchQry=" DROPDAT between '1900/1/1' and '2900/12/31' AND Convert(varchar(3), Datepart(yy, a.DROPDAT)-1911)+ right('0'+Convert(varchar(2),Datepart(mm,a.DROPDAT)),2)='" & ARYPARMKEY(1) & "' AND case when b.comtype between '01' and '05' or b.comtype ='14' then isnull(g.areanc,i.areanc) else c.codenc end ='" & ARYPARMKEY(0) & "' "
  ...
  sqlList="SELECT a.comq1, a.cusid, a.entryno, case when b.comtype between '01' and '05' or b.comtype ='14' then isnull(g.areanc,i.areanc) else c.codenc end as area, "_
			...
			&"WHERE	a.DOCKETDAT is not Null AND a.DROPDAT is not Null "_
			&"AND	dateadd(yy, 1, DOCKETDAT) > DROPDAT "_
			&"AND   case when b.comtype between '01' and '05' or b.comtype ='14' then isnull(g.areanc,i.areanc) else c.codenc end ='" & aryparmkey(0) & "' "_			
			&"AND " & searchQry _         
			&" AND	CONSENTNO <>'' "_
			&"ORDER BY case when b.comtype between '01' and '05' or b.comtype ='14' then isnull(g.areanc,i.areanc) else c.codenc end, dropdat "
