...
'---當報竣日大於完工日(排除完工日為空白者==>當申請時完工日為空白,故必須排除),用戶ip不可空白
    if trim(dspkey(11)) = "計量制" OR trim(dspkey(11)) = "單機轉計量" then
       if dspKey(24) >= dspkey(23) and len(dspkey(23)) > 0 and len(dspkey(82))=0 then
...
<%  dim sexd1, sexd2
  ...
  <%
    ...
    rs.Close
   %>
         <select size="1" name="key59"<%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry"><%=s%></select>
  ...
  <%s=""
    ...
    rs.Close
   %>        
        <select name="key63" <%=fieldpg%><%=FIELDROLE(1)%><%=dataProtect%> size="1"  style="text-align:left;" maxlength="8" class="dataListEntry">
        <%=s%></select>
  ...
  <%s=""
    ...
    rs.Close
   %>        
        <select name="key75" <%=fieldpg%><%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" style="text-align:left;" maxlength="8" class="dataListEntry"><%=s%></select>
...
<%  dim seld1
    ...
    End If%>
        ...
        <td  COLSPAN="3" height="34" bgcolor="silver"><input type="text" name="key3" size="10" value="<%=dspKey(3)%>" maxlength="8" <%=fieldRole(1)%><%=fieldpg%><%=dataProtect%> class="dataListEntry"></td>
      </tr>                                 
      <tr>                            
...
<% aryOption=Array("HIBUILDING","ADSL")
   ...
   End If%>                                  
      ...
      <tr>                                    
        ...
        <td class="dataListHead"><font size=2>身份證(統編)</font></td>
        <td colspan="3" height="21"><input type="password" name="key71" size="10" value="<%=dspKey(71)%>" maxlength="10" <%=fieldpg%><%=fieldRole(1)%> <%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>         
      <tr>                                    
        <td  class="dataListHead" height="21">COT PORT</td>
         <td   height="21" ><input type="text" name="key72" size="10" value="<%=dspKey(72)%>" maxlength="10" <%=fieldpg%><%=fieldRole(1)%> <%=dataProtect%> class="dataListEntry"></TD>
        <td  height="21" class="dataListHead">MDF號碼1</TD>
        <td  height="21"><input type="text" name="key73" size="15" value="<%=dspKey(73)%>" maxlength="15" <%=fieldpg%><%=fieldRole(1)%> <%=dataProtect%> class="dataListEntry"></td>                                 
        <td  height="21" class="dataListHead">MDF號碼2</td>
        <td height="21"><input type="text" name="key74" size="15" value="<%=dspKey(74)%>" maxlength="15" <%=fieldpg%><%=fieldRole(1)%> <%=dataProtect%> class="dataListEntry"></td>                                 
      </tr>                                       
      <tr>                                 
        <td  class="dataListHead" height="23">連接方式</td>
<% aryOption=Array("單機型","計量制","單機轉計量")
   s=""
   If Len(Trim(fieldRole(1) &dataProtect)) < 1 and flg = "Y" Then 
      For i = 0 To Ubound(aryOption)
...
<% aryOption=Array("128KBPS","512KBPS")
   s=""
   If Len(Trim(fieldRole(1) &dataProtect)) < 1 and flg = "Y" Then 
      For i = 0 To Ubound(aryOption)
   ...
   End If%>                                      
        <td  colspan="3" height="23" bgcolor="silver"><select size="1" name="key12" <%=fieldpg%><%=fieldRole(1)%><%=dataProtect%> class="dataListEntry">                                                             
        <%=s%></select></td>                                    
      </tr>                                    
      <tr>                                    
      ...
      <tr>                                 
        ...
        <td  height="23"><input type="text" name="key4" size="15" value="<%=dspkey(4)%>" maxlength="15" <%=fieldRole(1)%><%=dataProtect%> class="dataListEntry">分機<input type="text" name="key5" size="5" value="<%=dspkey(5)%>" maxlength="5" <%=fieldpg%><%=fieldRole(1)%><%=dataProtect%> class="dataListEntry"></td>                                 
      ...
      <TR>
        ...
        <td width="14%" class="dataListsearch" height="25">有無年約同意書</td>       
   ...
   '       freecode2=" disabled "
    ...
    If dspKey(88)="N" Then CONSENT2=" checked " %>                          
      ...
      <tr>                                 
        ...
        <td  height="32" bgcolor="silver" colspan=3><input type="text" name="key90" size="10" value="<%=dspkey(90)%>" readonly maxlength="30" <%=dataProtect%> class="dataListdata" ID="Text6"></td>                                 
      </tr>                
...
%>
      ...
      -->             
          <input type="text" name="key35" size="72" value="<%=dspKey(35)%>" <%=fieldPa%><%=fieldRole(5)%><%=dataProtect%> class="dataListEntry" maxlength="50"></td>                     
      </tr>                                     
      <tr>                       
