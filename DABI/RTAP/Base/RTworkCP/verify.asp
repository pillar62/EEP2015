...
<script language=vbscript>
 sub window_onload()
    ...
    if frm1.HTMLfld.value="1" then
       msgbox "RT派工單列印撤銷完成",0
       docP.all("keyform").Submit
    elseif frm1.HTMLfld.value="2" then
       msgbox "RT派工單列印撤銷失敗,錯誤訊息：" & "  " & frm1.HTMLfld1.value
    elseif frm1.HTMLfld.value="3" then
       msgbox "RT派工單列印撤銷失敗,錯誤訊息：此列印資料非(廠商)或(技術部)施工，無權限撤銷！"
    elseif frm1.HTMLfld.value="4" then
       msgbox "RT派工單列印撤銷失敗,錯誤訊息：系統找不到批號--" & frm1.HTMLfld1.value & "的報表資料!"   
    elseif frm1.HTMLfld.value="5" then
       msgbox "RT派工單列印撤銷失敗,錯誤訊息：此列印批號--已完工,不可撤銷"              
    end if
    winP.focus()              
    window.close        
 end sub
