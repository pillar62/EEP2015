...
Sub btn_onClick()
  ...
  if s1ary(0) <> "<>'*'" and s1ary(0) <> "*" then 
     t=t & " and  a.reqdat is not null AND  a.settype in ('1') and setsales in (" & s1ary(0) &")"
  end if  
  ...
  t=t &" and a.reqdat between '" & document.all("search4").value + " 00:00:00.000 " & "' and '" & document.all("search5").value + " 23:59:59.997 " & "' "  
...
<tr><td class=dataListHead width="40%">派工進度狀態</td>
    <td width="60%" bgcolor="silver">
      <select name="search2" size="1"  class=dataListEntry>
        <option value="0;已發包,未列印,未完工" selected>已發包,未列印,未完工</option>      
        <option value="1;已發包,未列印,已完工" >已發包,未列印,已完工</option>
