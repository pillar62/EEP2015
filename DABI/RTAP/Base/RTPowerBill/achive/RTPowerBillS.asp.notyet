...
Sub btn_onClick()
  ...
  Else
     s=s &"  社區地址:包含('" &S2 & "'字元)"
     t=t &" AND (RTCounty.CUTNC+RTEBTCmtyH.Township+rtEBTcmtyH.Raddr LIKE '%" &S2 &"%')" 
  End If
  '----申請狀況
  s3ary=split(document.all("search3").value,";")  
  If Len(trim(s3ary(0)))=0 Or s3ary(0)="" Then
  Elseif s3ary(0)="1" then
     s=s &"  申請狀態:" &s3ary(1)
     t=t &" AND (RTEBTCMTYH.updebtchkdat is not null) "
  elseif s3ary(0)="2" then
      s=s &"  申請狀態:" &s3ary(1)
     t=t &" AND (RTEBTCMTYH.updebtchkdat is not null and rtebtcmtyh.updebtdat is null) "
  elseif s3ary(0)="3" then
     s=s &"  申請狀態:" &s3ary(1)
     t=t &" AND (RTEBTCMTYH.updebtchkdat is not null and rtebtcmtyh.updebtdat is not null) "     
  End If  
  '----勘察進度
  ...
  Elseif s4ary(0)="1" then
     s=s &"  勘察進度:" &s4ary(1)
     t=t &" AND (RTEBTCMTYH.surveydat is not null) "
  elseif s3ary(0)="2" then
      s=s &"  勘察進度:" &s4ary(1)
     t=t &" AND (RTEBTCMTYH.surveydat is null) "
  End If    
  '----是否可建置
  ...
  Elseif s5ary(0)="1" then
     s=s &"  是否可建置:" &s5ary(1)
     t=t &" AND (RTEBTCMTYH.agree='Y') "
  elseif s5ary(0)="2" then
     s=s &"  是否可建置:" &s5ary(1)
     t=t &" AND (RTEBTCMTYH.agree='N') " 
  elseif s5ary(0)="3" then
      s=s &"  是否可建置:" &s5ary(1)
     t=t &" AND (RTEBTCMTYH.agree='H') "  
  elseif s5ary(0)="4" then
     s=s &"  是否可建置:" &s5ary(1)
     t=t &" AND (RTEBTCMTYH.agree='') "          
  End If      
  '----社區規模戶數
  ...
  Else
     ...
     t=t &" AND (RTebtCmtyh.comCnt" &s6ary(0) &s7 &")"
  End If
  '----建築物型式
  ...
  Else
     s=s &"  建築物型式:" &s8ary(1)
     t=t &" AND (RTebtCmtyh.buildtype='" &s8ary(0) &"')"
  End If  
  '----主機建置方式
  ...
  Else
     s=s &"  主機建置方式:" &s9ary(1)
     t=t &" AND (RTebtCmtyh.setuptype='" &s9ary(0) &"')"
  End If    
  '----簽訂同意書
  ...
  Elseif s10ary(0)="1" then 
     s=s &"  簽訂同意書:" &s10ary(1)
     t=t &" AND (RTebtCmtyh.agreedat is not null)"
  Elseif s10ary(0)="2" then 
     s=s &"  簽訂同意書:" &s10ary(1)
     t=t &" AND (RTebtCmtyh.agreedat is null)"     
  End If      
  '----集中電信室(箱)
  ...
  Elseif s11ary(0)="1" then 
     ...
     t=t &" AND (RTebtCmtyh.telcomroom='Y')"
  Elseif s11ary(0)="2" then 
     s=s &"  集中電信室(箱):" &s11ary(1)
     t=t &" AND (RTebtCmtyh.telcombox='Y')"     
  Elseif s11ary(0)="3" then 
     s=s &"  集中電信室(箱):" &s11ary(1)
     t=t &" AND (RTebtCmtyh.telcomroom='Y') and (RTebtCmtyh.telcombox='Y')"     
  End If        
  '----主線申請數
  TX=""
  TY=""
  ...
  if len(trim(S12ARY(0))) <> 0 then
     ...
     if len(trim(TX))=0 then
        TX=" HAVING SUM(CASE WHEN RTEBTCMTYLINE.COMQ1 IS NOT NULL THEN 1 ELSE 0 END) " & S12ARY(0) & S13 & " "
     ELSE
        TX=TX & " AND SUM(CASE WHEN RTEBTCMTYLINE.COMQ1 IS NOT NULL THEN 1 ELSE 0 END) " & S12ARY(0) & S13 & " "
     END IF
  end if
  '----主線開通數
  ...
  if len(trim(S14ARY(0))) <> 0 then
     ...
     if len(trim(TX))=0 then
        TX=" HAVING SUM(CASE WHEN RTEBTCMTYLINE.ADSLAPPLYDAT IS NOT NULL THEN 1 ELSE 0 END)  " & S14ARY(0) & S15 & " "
     ELSE
        TX=TX & " AND SUM(CASE WHEN RTEBTCMTYLINE.ADSLAPPLYDAT IS NOT NULL THEN 1 ELSE 0 END)  " & S14ARY(0) & S15 & " "
     END IF
  end if  
  '----社區用戶申請數  
  ...
  if len(trim(S16ARY(0))) <> 0 then
     ...
     if len(trim(TY))=0 then
        TY=" HAVING SUM(CASE WHEN RTEBTCUST.COMQ1 IS NOT NULL THEN 1 ELSE 0 END) " & S16ARY(0) & S17 & " "
     ELSE
        TY=TY & " AND SUM(CASE WHEN RTEBTCUST.COMQ1 IS NOT NULL THEN 1 ELSE 0 END) " & S16ARY(0) & S17 & " "
     END IF
  end if  
  '----社區用戶完工數  
  ...
  if len(trim(S18ARY(0))) <> 0 then
     ...
     if len(trim(TY))=0 then
        TY=" HAVING  SUM(CASE WHEN FINISHDAT IS NOT NULL THEN 1 ELSE 0 END) " & S20ARY(0) & S21 & " "
     ELSE
        TY=TY & " AND  SUM(CASE WHEN FINISHDAT IS NOT NULL THEN 1 ELSE 0 END) " & S20ARY(0) & S21 & " "
     END IF
  end if      
...
<tr><td class=dataListHead width="40%">申請狀況</td>
    <td width="60%"  bgcolor="silver">
      <select name="search3" size="1" class=dataListEntry>
        <option value=";全部" selected>全部</option>
        <option value="1;已申請">已申請</option>
        <option value="2;已申請尚未轉檔">已申請尚未轉檔</option>        
        <option value="3;已申請且已轉檔">已申請且已轉檔</option>
      </select>
     </td>
</tr>
<!--
