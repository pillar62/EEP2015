...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTPowerBill/RTPowerBillCmtyD.asp")
                     '  response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       if i <> 1 then rs.Fields(i).Value=dspKey(i)    
                       if i=1 then
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTPowerBillD where BILLNO='" & dspkey(0) & "' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTPowerBill/RTPowerBillCmtyD.asp") then
          Set rsc=Server.CreateObject("ADODB.Recordset")
          rsc.open "select max(entryno) AS entryno from KTSCUSTd1 where cusid='" & dspkey(0) & "' " ,conn
          if not rsC.eof then
...
Sub SrCheckData(message,formValid)
  ...
  IF LEN(TRIM(DSPKEY(2))) > 0 OR LEN(TRIM(DSPKEY(3))) > 0  THEN
      ...
      connxx.Open DSNxx
      '排除本身資料
      sqlXX="SELECT count(*) AS CNT FROM KTSCUSTD1 where TEL11='" & trim(dspkey(2)) & "' and TEL12='" & trim(dspkey(3)) & "' AND NOT (ENTRYNO=" & DSPKEY(1) & ")"
...
<table width="100%" border=1 cellPadding=0 cellSpacing=0 id="tag1">
       ...
       <tr>
        ...
        <input type="text" name="key8" size="10" value="<%=dspKey(8)%>"  <%=fieldpe%><%=fieldpG%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text4">     
       <!-- 
       ...
       <tr>
        ...
        <td  width=25%  height="23" bgcolor="silver">
        <input type="text" name="key9" size="15" value="<%=dspKey(9)%>"  <%=fieldpe%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text5">     
        </td>
