...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTSparq499Cmty/RTSparq499CmtyHARDWAREd.asp")
                       'response.write "I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
                       IF I=3 THEN
                         Set rsc=Server.CreateObject("ADODB.Recordset")
                         rsc.open "select max(entryno) AS entryno from RTSparq499Cmtyhardware where comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND PRTNO='" & DSPKEY(2) & "' "  ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTSparq499Cmty/RTSparq499Cmtyhardwared.asp") then
          rs.open "select max(entryno) AS ENTRYNO from RTSparq499Cmtyhardware WHERE  comq1=" & dspkey(0) & " and lineq1=" & dspkey(1) & " AND PRTNO='" & DSPKEY(2) & "' " ,conn
          if not rs.eof then
            dspkey(3)=rs("entryno")
          end if
          rs.close
...
Sub SrEnvironment()
  DSN="DSN=RTLib"
  numberOfKey=4
  title="Sparq499主線派工單設備資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1, LINEQ1, PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, " _
             &"DROPUSR,UNIT,eusr,edat,uusr,udat, EQUIPSERNO, MEMO " _
             &"FROM RTSparq499Cmtyhardware WHERE COMQ1=0 "
  sqlList=	"SELECT COMQ1, LINEQ1, PRTNO, ENTRYNO, PRODNO, ITEMNO, QTY, DROPDAT, DROPREASON, WAREHOUSE, ASSETNO, " _
             &"DROPUSR,UNIT,eusr,edat,uusr,udat, EQUIPSERNO, MEMO " _
             &"FROM RTSparq499Cmtyhardware WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connyy.Open dsn     
    sqlyy="SELECT * FROM RTSparq499CmtyLINESNDWORK WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' "
    ...
    IF ERRFLG="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(2) & "非標準工程，不可輸入設備明細"   
    elseif  ERRFLG1="Y" THEN
       formValid=False
       message="派工單" & DSPKEY(2) & " 已執行庫存計算作業，不可再輸入設備明細"   
  '  ELSEif len(dspkey(10)) < 1 OR len(dspkey(10)) <> 10 Then
  '     formValid=False
  '     message="資產編號不可空白或長度不足10碼"       
      ...
      <table width="100%" border=1 cellPadding=0 cellSpacing=0 ID="Table2">
       <tr><td width="15%" class=dataListHead>社區序號</td>
           ...
           <input type="text" name="key0"
                 readonly size="5" value="<%=dspKey(0)%>" maxlength="5" class=dataListdata ID="Text22"></td>
           <td width="15%" class=dataListHead>主線序號</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key1"
                 readonly size="3" value="<%=dspKey(1)%>" maxlength="3" class=dataListdata ID="Text23"></td>                 
           <td width="15%" class=dataListHead>派工單號</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key2"
                  readonly size="12" value="<%=dspKey(2)%>" maxlength="12" class=dataListdata ID="Text3"></td>                 
           <td width="15%" class=dataListHead>項次</td>
           <td width="10%"  bgcolor="silver">
           <input type="text" name="key3"
                  readonly size="3" value="<%=dspKey(3)%>" maxlength="3" class=dataListdata ID="Text24"></td>
  </tr>
      </table>
<%
End Sub
...
<tr>
           <td width="20%" class=dataListHead>設備/規格</td>
...
<%  set conn=server.CreateObject("ADODB.Connection")
    ...
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) AND ERRSTOCK="" Then 
       sql="SELECT * FROM RTprodh  where substring(prodno,1,1)='N' ORDER BY prodnC "
...
<td width="15%" class=dataListHead>資產編號</td></td>
    <td width="35%" bgcolor="silver" >
        <input type="text" name="key10" <%=fieldpa%><%=dataProtect%>
               style="text-align:left;" maxlength="15"
               value="<%=dspKey(10)%>" size="15" class=dataListEntry ID="Text1">
    </td>    
</tr>

<tr><td width="10%" class=dataListHead>設備/規格名稱</td></td>
    <td width="40%" bgcolor="silver">
  <%
  DEVICENAME=""
  ...
  END IF
  %>
        <input type="text"  READONLY name="DEVICENAME" <%=fieldpa%><%=dataProtect%>
               style="text-align:left;" maxlength="90"
               value="<%=DEVICENAME%>" size="90" class=dataListDATA ID="Text5">

	<td width="15%" class=dataListHead>設備序號</td></td>
    <td width="35%" bgcolor="silver" >
        <input type="text" name="key17" <%=fieldpa%><%=dataProtect%> style="text-align:left;" maxlength="30" value="<%=dspKey(17)%>" size="30" class=dataListEntry>
    </td>
</tr>

<tr><td class=dataListHead>數量</td>
    <td bgcolor="silver" >
        <input type="text" name="key6" size="8" value="<%=dspkey(6)%>" maxlength="10" <%=fieldpa%><%=dataProtect%> class="dataListEntry">
     <%
    s=""
    sx=" selected "
    ...
    Loop
    rs.Close%>         
   <select size="1" name="key12" <%=fieldpa%><%=dataProtect%> class="dataListEntry" ID="Select15">                                                                  
        <%=s%>
   </select>        </td>
<td class=dataListHEAD>出庫別</td>
  ...
  <%s=""
    ...
    rs.Close
   %>
   <select size="1" name="key9"<%=fieldpa%><%=dataProtect%> size="1" class="dataListEntry" ID="Select4">
   <%=s%>
   </select>
</td>      
</tr>  
<tr>
        ...
        <input type="text" name="key7" size="10" READONLY value="<%=dspKey(7)%>"  <%=fieldpa%> class="dataListDATA" ID="Text9">
        </td>       
  ...
  %>    <input type="text" name="key11" size="6" READONLY value="<%=dspKey(11)%>"  <%=fieldpa%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>                                            
 </tr>        
 <tr>
   <td class=dataListHEAD>作廢原因</td>          
   <td  height="23" bgcolor="silver" COLSPAN=3>
   <input type="text" name="key8" size="90" MAXLENGTH=90 value="<%=dspKey(8)%>"  <%=fieldpa%> class="dataListEntry" ID="Text16">
   </TD>
</tr>
 <tr>
   <td class=dataListHEAD>備註</td>
   <td  height="23" bgcolor="silver" COLSPAN=5>
   <input type="text" name="key18" size="100" MAXLENGTH=200 value="<%=dspKey(18)%>"  <%=fieldpa%> class="dataListEntry">
   </TD>
</tr>    
...
<tr>
  ...
  %>    <input type="text" name="key13" size="6" READONLY value="<%=dspKey(13)%>"  <%=fieldpa%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
        <td  class="dataListHEAD" height="23">建檔日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key14" size="10" READONLY value="<%=dspKey(14)%>"  <%=fieldpa%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>  
<tr>
  ...
  %>    <input type="text" name="key15" size="6" READONLY value="<%=dspKey(15)%>"  <%=fieldpa%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
        </td>  
        <td  class="dataListHEAD" height="23">修改日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key16" size="10" READONLY value="<%=dspKey(16)%>"  <%=fieldpa%> class="dataListDATA" ID="Text9">
        </td>       
 </tr>               
   </table> 
  </div> 
