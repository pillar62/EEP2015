...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  RESPONSE.END
   ...
   END IF 
   '當主線已測通回報時，不可執行結案返轉
   IF NOT ISNULL(RSYY("NCICOPENDAT"))  THEN
      ENDPGM="7"
   '當已作廢時，不可執行結案返轉
   ...
   ELSE
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTSparq499Cmtylinesndwork set FINISHdat=NULL,UNCLOSEDAT=NULL,CLOSEUSR='',UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1)  & " and prtno='" & key(2) & "' "
         ...
         If Err.number > 0 then
            ...
            IF (sndtype ="OC" OR  sndtype ="OP" OR  sndtype ="ST" OR  sndtype ="CB") and len(trim(finishdat)) > 0 THEN
               SQLXX=" update RTSparq499Cmtyline set adslopendat=null,UUSR='" & V(0) & "',UDAT=GETDATE()  where comq1=" & KEY(0) & " and lineq1=" & key(1) & " AND RTSparq499Cmtyline.ADSLopenDAT IS not NULL "
               STS="3"
               connxx.Execute SQLXX
               If Err.number > 0 then
                  endpgm="2"
 ...
 '  END IF
   ...
   IF ENDPGM="1" AND STS="3" THEN '當需更新主線之完工日的派工種類(STS=3)返轉才須要通知相關人員
      ...
      IF STS="1" THEN
         jmail.Subject = "優質499社區︰" & COMN & "，派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，完工結案返轉通知"
      ELSE
         jmail.Subject = "優質499社區︰" & COMN & "，派工單︰" & KEY(0) & "-" & KEY(1) & "-" & KEY(2) & "，未完工結案返轉通知"
      END IF
      
      jmail.priority = 1  
      body="<html><body><table border=0 width=""60%""><tr><td colspan=2>" _
      &"<H3>速博優質社區499主線派工結案返轉通知</h3></td></tr>" _
      &"<tr><td width=""30%"">&nbsp;</td><td width=""70%"">&nbsp;</td></tr>" _
      &"<tr><td bgcolor=lightblue align=center>社區主線序號</td><td bgcolor=pink align=left>" &KEY(0) & "-" & KEY(1) &"</td></tr>" _
      &"<tr><td bgcolor=lightblue align=center>社區名稱</td><td bgcolor=pink align=left>" &comn & "-" & KEY(1) &"</td></tr>" _
      &"<tr><td bgcolor=lightblue align=center>派工單號</td><td bgcolor=pink align=left>" &KEY(2) &"</td></tr>" _
      &"</table>" _
      &"<P><U>此主線完工記錄已被返轉，尚不可執行用戶裝機作業</U></body></html>"  
      jmail.HTMLBody = BODY
