...
Sub btn_onClick()
  ...
  Else
     s=s &"  社區地址:包含('" &S2 & "'字元)"
     t=t &" AND (RTCounty.CUTNC+a.Township+a.Raddr LIKE '%" &S2 &"%')" 
  End If
  '----勘察進度
  ...
  if len(trim(S14ARY(0))) <> 0 then
     ...
     if len(trim(TX))=0 then
        TX=" HAVING SUM(CASE WHEN y.ADSLOPENDAT IS NOT NULL THEN 1 ELSE 0 END)  " & S14ARY(0) & S15 & " "
     ELSE
        TX=TX & " AND SUM(CASE WHEN y.ADSLOPENDAT IS NOT NULL THEN 1 ELSE 0 END)  " & S14ARY(0) & S15 & " "
     END IF
  end if  
  '----社區用戶申請數  
