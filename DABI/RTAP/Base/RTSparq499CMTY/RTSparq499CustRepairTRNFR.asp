...
<% KEY=SPLIT(REQUEST("KEY"),";")
   ...
   'sqlyy="select max(entryno)as entryno from RTSparq499CUSTlog where CUSID='" & KEY(0)& "'"
   ...
   sqlYY="select * FROM RTSparq499CUST WHERE  CUSID='" & KEY(0) & "' "
   rsyy.open sqlyy,conn
   ...
   '檢查應收帳款檔是否已沖帳
   sqlyy="select * FROM RTSparq499CUSTAR WHERE CUSID='" & KEY(0) & "' AND BATCHNO='" & BATCHNOXx & "'"
   rsyy.open sqlyy,conn
   if rsyy.eof THEN
   ELSE
   ...
   if endpgm="" then
      ...
      strSP="usp_RTSparq499CustRepairAR " & "'" & key(0) & "'" & "," & key(1) & ",'" & V(0) & "', 'MR', '" & BATCHNOXX & "'"
...
<script language=vbscript>
 sub window_onload()
    if frm1.htmlfld.value="1" then
       msgbox "速博499用戶維修收款返轉應收結案作業成功",0
...
<form name=frm1 method=post action=RTSparq499CustRepairTRNFR.asp ID="Form1">
