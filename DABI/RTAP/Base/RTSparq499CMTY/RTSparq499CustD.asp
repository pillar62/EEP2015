...
Sub SrEnvironment()
  ...
  sqlFormatDB="SELECT COMQ1, LINEQ1, CUSID, CUSNC, SOCIALID, SECONDIDTYPE, SECONDNO, BIRTHDAY, EMAIL, CONTACTTEL," _
			 ...
			 &"VALIDMONTH, VALIDYEAR, DEVELOPERID, SETEMPLY, REALRCVAMT, GAMT, APPLYNO, GTEQUIP, GTSERIAL, GTPRTDAT, GTPRTUSR, WTLAPPLYDAT "_
...
Sub SrCheckData(message,formValid)
  ...
  if len(trim(dspkey(78)))=0 then dspkey(78)=""
  ...
  IF dspkey(65)="02" AND LEADINGCHAR >="0" AND LEADINGCHAR <="9" THEN
     COMPANY="Y"
  ELSE
     COMPANT="N"
  END IF
...
'  END IF
  ...
  elseif len(trim(dspkey(19)))=0 and instr(dspkey(21),"郵政")=0 then
       formValid=False
       message="帳單地址(縣市)不可空白"   
  elseif len(trim(dspkey(20)))=0 and dspkey(19)<>"06" and dspkey(19)<>"15" and instr(dspkey(21),"郵政")=0 then
       formValid=False
...
'      end if
	...
	'  elseif len(trim(dspkey(46)))<> 0 AND len(trim(dspkey(74)))= 0 AND len(trim(rsxx("consignee"))) = 0 then
	'		formValid=False
	'		message="輸入完工日期時，裝機人員不可空白"
      end if
...
Sub SrActiveXScript()%>
   ...
   End Sub    
   Sub SrClearIP()
		document.all("key61").value =  ""
		document.all("key62").value =  ""
		document.all("key63").value =  ""
		document.all("key64").value =  ""
   End Sub    
...
<TR><td  class="dataListSEARCH" height="23">用戶IP</td>                                 
     ...
     '   IF LEN(TRIM(DSPKEY(63)))=0 THEN DSPKEY(63)=IP3
        %>
        <font color=red><input type="text" name="key61" size="3" maxlength="3" value="<%=dspKey(61)%>" <%=fieldRole(1)%> readonly class="dataListdata" ID="Text3">
        <font size=2>.</font>
        <input type="text" name="key62" size="3" maxlength="3" value="<%=dspKey(62)%>" <%=fieldRole(1)%> readonly class="dataListdata" ID="Text6">
        <font size=2>.</font>
        <input type="text" name="key63" size="3" maxlength="3" value="<%=dspKey(63)%>" <%=fieldRole(1)%> readonly class="dataListdata" ID="Text7">
        <font size=2>.</font>
        <input type="text" name="key64" size="3" maxlength="3" value="<%=dspKey(64)%>" <%=fieldRole(1)%> readonly class="dataListdata" ID="Text10"></font>
		<%If len(trim(dspKey(48))) = 0 Then 
			response.write "<IMG SRC=""/WEBAP/IMAGE/IMGDELETE.GIF"" fieldpb alt=""清除"" id=""C64"" name=""C64"" style=""Z-INDEX: 1"" onclick=""SrClearIP"" border=0 onmouseover=""ImageIconOver"" onmouseout=""ImageIconOut"" >"
		end if%>          
        </td>     
<td  class="dataListSEARCH" height="23">對帳序號</td>                                 
        <td  height="23" bgcolor="silver"  >
        <input type="text" name="key60" size="15" maxlength="15" <%=fieldpa%> value="<%=dspKey(60)%>" <%=fieldRole(1)%> class="dataListDATA" READONLY ID="Text1">
        <FONT SIZE=2>-</FONT>
        <input type="text" name="key66" size="3" maxlength="3" <%=fieldpa%> value="<%=dspKey(66)%>" <%=fieldRole(1)%> class="dataListDATA" READONLY ID="Text1">
</TD>
</TR>        

<tr><td  class="dataListHEAD" height="23">iDslam Port位</td>                                 
    <td  height="23" bgcolor="silver" >
		<select size="1" name="key29" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select3">	
			<%
				if dspKey(29) ="" then 
					idslamport = ""
				else
	...
	<td width="15%" class=dataListHEAD>WTL申請日</td>
	    <td width="35%" bgcolor="silver" >
	        <input type="text" name="key82" <%=fieldRole(1)%><%=dataProtect%>
	               value="<%=dspKey(82)%>"  READONLY size="10" class=dataListEntry>
	       <input  type="button" id="B82" name="B82" height="100%" width="100%" style="Z-INDEX: 1" value="...." onclick="SrBtnOnClick">
	    <IMG  SRC="/WEBAP/IMAGE/IMGDELETE.GIF" alt="清除" id="C82"  name="C82"   style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" onclick="SrClear">
	</td>
</tr>
...
<td width="15%" class=dataListHEAD>用戶申請日</td>
    <td width="35%" bgcolor="silver" >
        <input type="text" name="key45" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
...
<tr><td width="15%" class=dataListHEAD>用戶(公司)名稱</td>
    <td  width="35%"  bgcolor="silver" >
        <input type="text" name="key3" <%=fieldRole(1)%><%=dataProtect%>
               ...
               value="<%=dspKey(3)%>" size="30" class=dataListCht ID="Text22"></td>
<%
    s=""
    sx=" selected "
...
<td width="15%" class=dataListHEAD>身分證(統編)</td>
    <td width="35%" bgcolor="silver" >
		<select size="1" name="key65" <%=FIELDROLE(1)%><%=dataProtect%>
			 class="dataListEntry noIme uCase"> <%=s%> 
		</select>
        <input type="text" name="key4" value="<%=dspKey(4)%>" maxlength="10" size="12" 
			<%=fieldRole(1)%><%=dataProtect%> class="dataListENTRY noIme">
	</td>
</tr>
...
If accessMode="A" Then
	...
	rsc.open "select max(applyno) AS applyno from RTSparq499Cust where cusid <>'' " ,conn
	if len(rsc("applyno")) > 0 then
		dspkey(77)=cusidxx & right("0000" & cstr(cint(right(rsc("applyno"),4)) + 1),4)
	else
		dspkey(77)=cusidxx & "0001"
	end if
	rsc.close
End if
%>
<tr><td width="15%" class=dataListHEAD>申請書編號</td>
    <td  width="35%"  bgcolor="silver" >
        <input type="text" name="key77" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%> style="text-align:left;" maxlength="11"
               value="<%=dspKey(77)%>" readonly size="15" class=dataListdata ID="Text22"></td>
...
<%
   ...
   %>        
        ...
        <td width="18%" height="25" bgcolor="silver" colspan=5> 
			<select size="1" name="key5" <%=FIELDROLE(1)%><%=dataProtect%> 
				class="dataListEntry" >	<%=s%>
			</select>	
	        <input type="text" name="key6" size="15" maxlength="12" value="<%=dspkey(6)%>" 
				<%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry noIme uCase">
        </td> 
     </tr>    
<tr>                                 
        <td  class="dataListHEAD" height="23">出生日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key7" size="10" value="<%=dspKey(7)%>" <%=fieldRole(1)%> class="dataListEntry noIme" >  
        ...
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key8" size="30" maxlength="30" value="<%=dspKey(8)%>" <%=fieldRole(1)%> class="dataListEntry imeOff"></td>                                 
  </tr>
  
<TR>        
        <td  class="dataListHEAD" height="23">連絡電話</td>                                 
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key9" size="30" maxlength="30" value="<%=dspKey(9)%>" <%=fieldRole(1)%> class="dataListEntry imeOff"></td>     
        <td  class="dataListHEAD" height="23">行動電話</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key10" size="30" maxlength="30" value="<%=dspKey(10)%>" <%=fieldRole(1)%> class="dataListEntry imeOff"></td>                                 
 </tr>
<tr><td class=dataListHEAD>戶籍/公司地址</td>
	...
	<%
		...
		rs.Close
	%>
		<select size="1" name="key11" <%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select2" ><%=s%></select>
        <input type="text" name="key12" readonly  size="8" value="<%=dspkey(12)%>" maxlength="10" readonly <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
			<font SIZE=2>(鄉鎮)</font>
			<input type="button" id="B12"  name="B12"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX12%>  >        
        ...
        <input type="text" name="key13"  size="40" value="<%=dspkey(13)%>" maxlength="60" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListCht" ID="Text5">
		<input type="text" name="key14"  readonly size="5" value="<%=dspkey(14)%>" maxlength="5" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text12">
        </td>                                 
</tr>  
<tr><td class=dataListHEAD>裝機地址
  ...
  <%s=""
    ...
    rs.Close
   %>
         <select size="1" name="key15" <%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select2"><%=s%></select>
        <input type="text" name="key16" readonly  size="8" value="<%=dspkey(16)%>" maxlength="10" readonly <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
			<font SIZE=2>(鄉鎮)</font>               
			<input type="button" id="B16"    name="B16"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX16%>  >        
			<IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"   alt="清除" id="C16"  name="C16"   style="Z-INDEX: 1" onclick="SrClear"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" >          
        
        <input type="text" name="key17"  size="40" value="<%=dspkey(17)%>" maxlength="60" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListCht" ID="Text5">
		<input type="text" name="key18"  readonly size="5" value="<%=dspkey(18)%>" maxlength="5" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text13">
</td>                                 
</tr>  
<tr><td class=dataListHEAD>帳單地址<br>
		<input type="radio" name="rd2" <%=fieldpb%>  onClick="SrAddrEqual2()"><font SIZE=2>同戶籍</font>
		<input type="radio"  <%=fieldpb%> name="rd2" onClick="SrAddrEqual3()"><font SIZE=2>同裝機</font>
    </td>
	<%
		s=""
		sx=" selected "
	...
	%>
    <td bgcolor="silver" COLSPAN=3>
		<select size="1" name="key19" <%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select2"><%=s%></select>
		<input type="text" name="key20" readonly  size="8" value="<%=dspkey(20)%>" maxlength="10" readonly <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text4">
			<font SIZE=2>(鄉鎮)</fon>
			<input type="button" id="B20"    name="B20"   width="100%" style="Z-INDEX: 1"  value="...." <%=SXX20%>  >        
			<IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"   alt="清除" id="C20"  name="C20"   style="Z-INDEX: 1" onclick="SrClear"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" >          

		<input type="text" name="key21"  size="40" value="<%=dspkey(21)%>" maxlength="60" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListCht" ID="Text5"><font size=2>
		<input type="text" name="key22"  readonly size="5" value="<%=dspkey(22)%>" maxlength="5" <%=FIELDROLE(1)%><%=dataProtect%> class="dataListDATA" ID="Text14">
	</td>                                 
</tr>
<TR><td class="dataListHEAD" height="23">企業連絡人</td>                                 
    <td height="23" bgcolor="silver" >
	    <input type="text" name="key23" size="15" maxlength="15"  value="<%=dspKey(23)%>" <%=fieldRole(1)%> class="dataListCht" ID="Text54">
	</td>
	<td class="dataListHEAD" height="23">企業連絡電話</td>
    <td height="23" bgcolor="silver">
	    <input type="text" name="key24" size="15" maxlength="15"  value="<%=dspKey(24)%>" <%=fieldRole(1)%> class="dataListEntry imeOff" ID="Text58">
		<font size=2>分機: </font>
		<input type="text" name="key25" size="5" maxlength="5"  value="<%=dspKey(25)%>" <%=fieldRole(1)%> class="dataListEntry imeOff" ID="Text59">
		<font size=2>手機: </font>
		<input type="text" name="key26" size="10" maxlength="10"  value="<%=dspKey(26)%>" <%=fieldRole(1)%> class="dataListEntry imeOff" ID="Text28">
    </td>
	
</tr>
 <TR>   <td  class="dataListHEAD" height="23">企業負責人</td>                                 
        <td  height="23" bgcolor="silver" >
        <input type="text" name="key27" size="10" maxlength="10"  value="<%=dspKey(27)%>" <%=fieldRole(1)%> class="dataListCht" ID="Text62"></td>     
        <td  class="dataListHEAD" height="23">負責人身份證字號</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key28" size="12" maxlength="10"  value="<%=dspKey(28)%>" <%=fieldRole(1)%> class="dataListEntry noIme uCase" ID="Text65"></td>                                 
 </tr>

<TR><td width=15% class="dataListHEAD" height="23">代理人姓名</td>                                 
	<td width=35% height="23" bgcolor="silver">
		<input type="text" name="key41" size="10" maxlength="15" value="<%=dspKey(41)%>" <%=fieldRole(1)%> class="dataListCht" ID="Text8">
	</td>
	<td width=15% class="dataListHEAD" height="23">代理人身份證號</td>                                 
	<td width=35% height="23" bgcolor="silver" >
		<input type="text" name="key42" size="12" maxlength="10" value="<%=dspKey(42)%>" <%=fieldRole(1)%> class="dataListEntry noIme uCase" ID="Text16">
	</td>
</tr>
...
<TR>        
        ...
        <input type="text" name="key43" size="15" maxlength="15" value="<%=dspKey(43)%>" <%=fieldRole(1)%> class="dataListEntry imeOff" ID="Text11"></td>
</tr>     
...
<!--
	...
	<tr><td WIDTH="15%" class="dataListHEAD" height="23">裝機員工</td>
		<td width="35%"><input type="text" name="key74"value="<%=dspKey(74)%>" <%=fieldRole(1)%><%=dataProtect%> maxlength="6" readonly class="dataListEntry" ID="Text25">
			<input type="BUTTON" id="B74" name="B74" style="Z-INDEX: 1"  value="...." onclick="SrSetEmplyonclick()">
			<IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF"  <%=fieldpb%> alt="清除" id="C74" name="C74" style="Z-INDEX: 1" border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut"  onclick="SrClear">
			<font size=2><%=SrGetEmployeeName(dspKey(74))%></font>
		</td>

		<td  WIDTH="15%" class="dataListHEAD" height="23">實收金額</td>               
        <td  WIDTH="35%" height="23" bgcolor="silver">
			<input type="text" name="key75" size="15" maxlength="10" value="<%=dspKey(75)%>" <%=fieldpa%><%=fieldRole(1)%> class="dataListdata" ID="Text26">
        </td>
	</tr>

	<tr><td width="15%" class=dataListHEAD>經銷商(for原遠端用戶)</td>
		<td width="35%" bgcolor="silver">
		<%  
			If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) Then 
		    ...
		    sx=""
		    Do While Not rs.Eof
		       If rs("CUSID")=dspkey(83) Then sx=" selected "
		    ...
		    rs.Close
		%>
	       <select size="1" name="key83" <%=fieldpm%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry" ID="Select83">                                            
	          <%=s%>
	       </select>
		</td>

		<td WIDTH="15%" class="dataListHEAD" height="23">二線開發人員</td>
		<td width="35%"><input type="text" name="key73"value="<%=dspKey(73)%>" <%=fieldRole(1)%><%=dataProtect%> style="text-align:left;" size="8" maxlength="6" readonly class="dataListEntry" ID="Text21">
			<input type="BUTTON" id="B73" name="B73" style="Z-INDEX: 1"  value="...." onclick="Srdeveloperonclick()">
			...
			<font size=2><%=SrGetEmployeeName(dspKey(73))%></font>
		</td>
	</tr>
...
<DIV ID="SRTAG3" onclick="srtag3" style="cursor:hand">
	<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table12">
		<tr><td bgcolor="BDB76B" align="center">保證金相關</td></tr>
	</table>
</DIV>
<DIV ID="SRTAB3" >
	<table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table13">
	    <tr><td width="10%" class="dataListHead">保證金序號</td>
			<td width="23%" bgcolor="silver" colspan=3>
				<input type="text" name="key79" size="13" readonly value="<%=dspKey(79)%>" class="dataListData">
			</td>
		</tr>

		<tr><td WIDTH="15%" class="dataListHEAD" height="23">保證金</td>               
			<td WIDTH="35%" height="23" bgcolor="silver">
				<input type="text" name="key76" size="10" maxlength="10" value="<%=dspKey(76)%>" <%=fieldpg%> <%=fieldRole(1)%> class="dataListEntry noIme">
			</td>

			<td width="15%" class="dataListHead">用戶保管設備</td>
			<%
				s=""
				sx=" selected "
				If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) Then     
				...
				Else
				sql="SELECT Code,CodeNC FROM RTCode WHERE KIND='P2' AND CODE='" & dspkey(78) &"' " 
				End If
				rs.Open sql,conn
				s=""
			...
			%>
			<td width="35%" bgcolor="silver" >
				<select name="key78" <%=fieldpg%><%=fieldRole(1)%><%=dataProtect%> size="1" class="dataListEntry" ID="Select1"><%=s%></select>
			</td>
		</tr>

		<tr><td  class="dataListHEAD" height="23">保證金收據列印人</td>                                 
			<td  height="23" bgcolor="silver">
				<input type="text" name="key81" size="10" READONLY value="<%=dspKey(81)%>" class="dataListDATA">
				<font size=2>
				<%
			...
			<td  height="23" bgcolor="silver">
				<input type="text" name="key80" size="25" READONLY value="<%=dspKey(80)%>" class="dataListDATA">
			</td>       
		</tr>
	</table>     
</DIV>
    ...
    <tr><td bgcolor="BDB76B" align="center">寬頻服務</td></tr></table></DIV>
    <DIV ID=SRTAB2 >
...
<%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1  AND len(trim(DSPKEY(48))) = 0  Then  
    ...
    Loop
    rs.Close%>         
   <select size="1" name="key37" <%=fieldpC%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select8">                                                                  
        <%=s%>
   </select>
        </td>
...
<%  dim FREECODE1,FREECODE2
    ...
    If dspKey(38)="N" Then FREECODE2=" checked " %>                          
        ...
        <input type="radio" name="key38" value="N" <%=FREECODE2%>  <%=fieldRole(1)%><%=dataProtect%> ID="Radio2">否</td>
        </tr>
<tr>   <td  WIDTH="15%" class="dataListHEAD" height="23">PROMOTION CODE</td>               
        <td  WIDTH="35%" height="23" bgcolor="silver" >
        <input type="text" name="key39" size="15" maxlength="10" value="<%=dspKey(39)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListdata" ID="Text15"></td>                                 
        </td>
...
<%
    ...
    Loop
    rs.Close%>         
   <select size="1" name="key40" <%=fieldpC%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35">                                                                  
        <%=s%>
   </select>
        </td></tr>
...
<DIV ID="SRTAG4" onclick="srtag4" style="cursor:hand">
    ...
    <tr><td bgcolor="BDB76B" align="center">用戶申請及施工進度狀態</td></tr></table>
</DIV>
<DIV ID=SRTAB4 >  
    <table border="1" width="100%" cellpadding="0" cellspacing="0" >
        <tr>
        ...
        <td   height="23" bgcolor="silver" >
        <input type="text" name="key46" size="10" READONLY value="<%=dspKey(46)%>" <%=fieldpC%>  <%=fieldRole(1)%> class="dataListentry" ID="Text57">
                <input type="button" id="B46"  name="B46" height="100%" width="100%"  <%=fieldpD%> style="Z-INDEX: 1" value="...." onclick="SrBtnOnClick">
        <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF" alt="清除" id="C46"  name="C46"    <%=fieldpD%> style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" onclick="SrClear">     
        </td>
        <td   class="dataListHEAD" height="23">報竣日期</td>                                 
        <td  height="23" bgcolor="silver">
        <input type="text" name="key47" size="10" READONLY value="<%=dspKey(47)%>" <%=fieldpe%> <%=fieldRole(1)%> class="dataListDATA" ID="Text56">
           <!--     <input type="button" id="B47"  name="B47" height="100%" width="100%" <%=fieldpf%>style="Z-INDEX: 1" value="...." onclick="SrBtnOnClick">
        <IMG SRC="/WEBAP/IMAGE/IMGDELETE.GIF" alt="清除" id="C47"  name="C47"   <%=fieldpf%>style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" onclick="SrClear"> -->    
        </TD>
</tr> 
        <tr>
        <td   class="dataListHEAD" height="23">報竣轉檔日</td>                                 
        <td   height="23" bgcolor="silver" >
        <input type="text" name="key48" size="10" value="<%=dspKey(48)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text57">     
        </td>
                <td   class="dataListHEAD" height="23">退租/欠拆日</td>                                 
        <td   height="23" bgcolor="silver" >
        <input type="text" name="key49" size="10" value="<%=dspKey(49)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text24">
        <font size=2>欠拆︰</font> 
        <input type="text" name="key52" size="2" READONLY value="<%=dspKey(52)%>"  <%=fieldRole(1)%> class="dataListDATA" ID="Text48">  
        </td>
       ...
       <tr>
        ...
        <td  width=35% height="23" bgcolor="silver">
        <input type="text" name="key50" size="10" value="<%=dspKey(50)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListdata" ID="Text41">
         </td>

        <td   width=15% class="dataListHEAD" height="23">作廢人員</td>                                 
        <td   width=35% height="23" bgcolor="silver">
        <input type="text" name="key51" size="10" value="<%=dspKey(51)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text43">
        <font size=2><%=SrGetEmployeeName(dspKey(51))%></font>
        </td></tr>           
  </table></DIV>

<DIV ID="SRTAG8" onclick="SRTAG8" style="cursor:hand">
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table10">
    <tr><td bgcolor="BDB76B" align="center">信用卡資料</td></tr></table></DIV>
...
%>
	<tr><td width="15%" class="dataListHEAD" height="23">信用卡</td>
		<td width="35%" bgcolor="silver">
		<select size="1" name="key67" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> size="1" class="dataListEntry"><%=s%></select>

		<td width="15%" class="dataListHEAD" height="23">發卡銀行</td>
        <td width="35%" height="23" bgcolor="silver">
		<input type="text" name="key68" value="<%=dspKey(68)%>" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
			style="text-align:left;" maxlength="20" size="25" class=dataListENTRY></td></tr>

	<tr><td class="dataListHEAD" height="23">信用卡卡號</td>
        <td height="23" bgcolor="silver">
		<input type="text" name="key69" value="<%=dspKey(69)%>" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
			style="text-align:left;" maxlength="16" size="20" class="dataListENTRY noIme"></td>

		<td class="dataListHEAD" height="23">持卡人姓名</td>
        <td height="23" bgcolor="silver">
		<input type="text" name="key70" value="<%=dspKey(70)%>" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
			style="text-align:left;" maxlength="20" size="20" class=dataListCht></td></tr>

	<tr><td class="dataListHEAD" height="23">信用卡有效期限</td>
        <td height="23" bgcolor="silver" colspan=3>
		<input type="text" name="key71" value="<%=dspKey(71)%>" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
			style="text-align:left;" maxlength="2" size="5" class="dataListENTRY noIme">月/
		<input type="text" name="key72" value="<%=dspKey(72)%>" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
			style="text-align:left;" maxlength="2" size="5" class="dataListENTRY noIme">年</td></tr>
    </table></div>


   <DIV ID="SRTAG7" onclick="SRTAG7" style="cursor:hand">
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table4">
    <tr><td bgcolor="BDB76B" align="center">異動狀態</td></tr></table></DIV>
    <DIV ID="SRTAB7" >  
    <table border="1" width="100%" cellpadding="0" cellspacing="0" ID="Table5">
       <tr>
        ...
        %>                   
        <input type="text" name="key54" size="5" value="<%=dspKey(54)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text36">
        <input type="text" name="key55" size="5" value="<%=dspKey(55)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text38">
        ...
        %>                   
        <input type="text" name="key56" size="5" value="<%=dspKey(56)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text37">
        <input type="text" name="key57" size="5" value="<%=dspKey(57)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text39">
        <font size=2><%=comn%></font></td>
     </tr>      
       <tr>
        <td  width=15% class="dataListHEAD" height="23">移出異動結案日</td>                                 
        <td  width=35% height="23" bgcolor="silver">
        <input type="text" name="key58" size="10" value="<%=dspKey(58)%>"  <%=fieldpa%><%=fieldRole(1)%> readonly class="dataListDATA" ID="Text40">
         </td>
        <td   width=15% class="dataListHEAD" height="23">移入異動結案日</td>                                 
        <td   width=35% height="23" bgcolor="silver">
        <input type="text" name="key59" size="10" value="<%=dspKey(59)%>" <%=fieldRole(1)%> readonly class="dataListDATA" ID="Text42">
        </td></tr>
		<tr>
				<td  class="dataListHEAD" height="23">建檔人員</td>                                 
				<td  height="23" bgcolor="silver">
					<input type="text" name="key30" size="6" READONLY value="<%=dspKey(30)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2">
		...
		<tr>
				...
				<td  height="23" bgcolor="silver">
					<input type="text" name="key32" size="6" READONLY value="<%=dspKey(32)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text23">
					<font size=2><%=SrGetEmployeeName(dspKey(32))%></font>
				</td>  
				<td  class="dataListHEAD" height="23">修改日期</td>                                 
				<td  height="23" bgcolor="silver">
				<input type="text" name="key33" size="10" READONLY value="<%=dspKey(33)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text27">
				</td>       
		</tr>         
    </table> 
</div>
   ...
   <DIV ID="SRTAB6" > 
    ...
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%"  name="key53" rows=8  MAXLENGTH=500  class="dataListCht"  <%=dataprotect%>  value="<%=dspkey(53)%>" ID="Textarea1"><%=dspkey(53)%></TEXTAREA>
   </td></tr>
 </table> 
  </div> 
<%
