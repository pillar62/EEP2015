...
Sub btn_onClick()
 ...
 ' End If
  ...
  IF LEN(TRIM(S3)) > 0 THEN
     s=s &"  主線IP:包含('" &s3 & "'字元) "
     t=t &" AND (a.LINEIPSTR1+'.'+a.LINEIPSTR2+'.'+a.LINEIPSTR3+'.'+a.LINEIPSTR4 LIKE '%" & S3 & "%') "
  END IF
  '----主線聯單編號
  s4=document.all("search4").value
  If Len(trim(s4)) > 0 Then
     s=s &"  聯單編號:包含('" &s4 & "'字元) "
     t=t &" AND (a.CHTWORKINGNO LIKE '%" & S4 & "%') "
  End If    
  '----主線附掛電話
  ...
  Elseif s7ary(0)="1" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND (a.LINEIPSTR1 <> '' and a.adslopendat is null) "
  elseif s7ary(0)="2" then
     s=s &"  進度狀況:" &s7ary(1)
     t=t &" AND (a.adslopendat is NOT null) " 
 End If        
  '----社區序號
...
<body>
<table width="100%">
  <tr class=dataListTitle align=center>速博優質社區499主線資料搜尋條件</td><tr>
</table>
