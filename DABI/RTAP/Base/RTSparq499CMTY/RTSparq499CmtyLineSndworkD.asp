...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
          ...
          For i = 0 To numberOfField-1
              ...
              '   On Error Resume Next
                ...
                select case ucase(runpgm)   
                   ...
                   case ucase("/webap/rtap/base/RTSparq499Cmty/RTSparq499Cmtylinesndworkd.asp")
                       ...
                       if i=2 then
                         ...
                         rsc.open "select max(PRTNO) AS PRTNO from RTSparq499CmtyLINESNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
    ...
    if accessmode ="A" then
       runpgm=Request.ServerVariables("PATH_INFO")
       if ucase(runpgm)=ucase("/webap/rtap/base/RTSparq499Cmty/RTSparq499Cmtylinesndworkd.asp") then
          cusidxx="P-" & right("00" & trim(datePART("yyyy",NOW())),2) & right("00" & trim(datePART("m",NOW())),2)& right("00" & trim(datePART("d",NOW())),2)
          rsc.open "select max(PRTNO) AS PRTNO from RTSparq499CmtyLINESNDWORK where PRTNO like '" & cusidxx & "%' " ,conn
          if not rsC.eof then
            dspkey(2)=rsC("PRTNO")
...
Sub SrEnvironment()
  ...
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1, LINEQ1, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, ASSIGNENGINEER3, " _
             ...
             &"FROM RTSparq499CmtyLINESNDWORK WHERE COMQ1=0 "
  sqlList="SELECT COMQ1, LINEQ1, PRTNO, SENDWORKDAT, PRTUSR, ASSIGNENGINEER1, ASSIGNENGINEER2, ASSIGNENGINEER3, " _
             &"ASSIGNCONSIGNEE, REALENGINEER1, REALENGINEER2,  REALENGINEER3, REALCONSIGNEE,  " _
             &"DROPDAT, DROPDESC, CLOSEDAT, BONUSCLOSEYM, BONUSCLOSEDAT, BONUSCLOSEUSR, BONUSFINCHK, STOCKCLOSEYM, STOCKCLOSEDAT, " _
             &" STOCKCLOSEUSR, STOCKFINCHK, SNDTYPE, HOSTCABLENO, MEMO,PRTDAT,eusr,edat,uusr,udat,finishdat,unclosedat " _
             &"FROM RTSparq499CmtyLINESNDWORK WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=1
...
Sub SrCheckData(message,formValid)
    ...
    connXX.open DSN
    SQLXX="SELECT * FROM RTSparq499CmtyHARDWARE WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " and prtno='" & dspkey(2) & "' "
    ...
    if len(dspkey(24)) < 1 Then
       formValid=False
       message="請選擇派工種類"       
    ELSEif LEN(TRIM(dspkey(3))) = 0 then
       formValid=False
       message="派工日期不可空白" 
    ELSEif len(trim(DSPKEY(5))) = 0 and len(trim(DSPKEY(6))) = 0  and len(trim(DSPKEY(7))) = 0 and len(trim(DSPKEY(8))) = 0 THEN
       formValid=False
       message="預定裝機人員或經銷商必須輸入其中一項" 
    ELSEif (len(trim(DSPKEY(5))) > 0 or len(trim(DSPKEY(6))) > 0  or len(trim(DSPKEY(7))) > 0) and len(trim(DSPKEY(8))) > 0 THEN
       formValid=False
       message="預定裝機人員及經銷商只能輸入其中一項"    
    ELSEif (len(trim(DSPKEY(9))) = 0 and len(trim(DSPKEY(10))) = 0  and len(trim(DSPKEY(11))) = 0 and len(trim(DSPKEY(12))) = 0) AND len(trim(DSPKEY(15))) > 0   THEN
       formValid=False
       message="裝機完成，實際裝機人員或經銷商必須輸入其中一項" 
    ELSEif (len(trim(DSPKEY(9))) > 0 or len(trim(DSPKEY(10))) > 0  or len(trim(DSPKEY(11))) > 0) and len(trim(DSPKEY(12))) > 0 THEN
       formValid=False
       message="實際裝機人員及經銷商只能輸入其中一項"    
    elseif len(trim(DSPKEY(15))) > 0 and (dspkey(24)="ST" OR dspkey(24)="OC" OR dspkey(24)="EP") AND ERRCODE="1" THEN
       formValid=False
       message="當裝機完成時若派工種類為標準工程/擴充設備/測通及加裝/連絡纜工程等，則必須輸入設備資料"    
    elseif len(trim(DSPKEY(15))) > 0 and dspkey(24)="OP" AND ERRCODE="" THEN
       formValid=False
...
' -------------------------------------------------------------------------------------------- 
    ...
    '獎金計算後，資料protect
    If len(trim(dspKey(32))) > 0 OR LEN(TRIM(DSPKEY(33))) > 0 Then
...
<%
 sql="SELECT COUNT(*) as equipcnt FROM RTSparq499CmtyHARDWARE WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1) & " AND PRTNO='" & DSPKEY(2) & "' AND dropdat is null" 
 rs.Open sql,conn
 cnt=rs("equipcnt")
 RS.CLOSE
 sql="SELECT CODE,CODENC FROM RTCODE WHERE KIND='G9' AND CODE='" & dspkey(24) & "'"
rs.Open sql,conn
IF RS.EOF THEN 
   SNDTYPE="派工種類︰無" 
ELSE
...
<%
    s=""
    sx=" selected "
    If (sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false)) And protect<1 AND DSPKEY(16)=""  Then  
    ...
    Loop
    rs.Close%>         
   <select size="1" name="key24" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%> class="dataListEntry" ID="Select35">                                                                  
        <%=s%>
   </select>
    </td>
...
<%
  sql="SELECT * FROM RTSparq499CmtyLINE WHERE COMQ1=" & DSPKEY(0) & " AND LINEQ1=" & DSPKEY(1)
  rs.Open sql,conn
  IF RS.EOF THEN
     DOMAIN=""
  ELSE
...
<%  name="" 
  ...
  %>           
        <input type="TEXT" name="key6" <%=fieldRole(1)%><%=dataProtect%> 
               style="text-align:left;" <%=FIELDPA%> size="6" maxlength="6" 
               value="<%=dspKey(6)%>"  readonly class="dataListENTRY" ID="Text5">
...
<%  name="" 
  ...
  %>                      
           <input type="TEXT" name="key7" <%=fieldRole(1)%><%=dataProtect%> 
               style="text-align:left;" <%=FIELDPA%> size="6" maxlength="6" 
               value="<%=dspKey(7)%>"  readonly class="dataListENTRY" ID="Text7">
           <input type="BUTTON" id="B7" name="B7"  width="100%" style="Z-INDEX: 1"  value="...."  <%=FIELDP7%>  >   
...
<%  name="" 
  ...
  %>           
        <input type="TEXT" name="key10" <%=fieldRole(1)%><%=dataProtect%> 
               style="text-align:left;" <%=FIELDPA%> size="6" maxlength="6" 
               value="<%=dspKey(10)%>"  readonly class="dataListENTRY" ID="Text4">
...
<%  name="" 
  ...
  %>                      
           <input type="TEXT" name="key11" <%=fieldRole(1)%><%=dataProtect%> 
               style="text-align:left;" <%=FIELDPA%> size="6" maxlength="6" 
               value="<%=dspKey(11)%>"  readonly class="dataListENTRY" ID="Text6">
           <input type="BUTTON" id="B11" name="B11"  width="100%" style="Z-INDEX: 1"  value="...."  <%=FIELDP11%>   >   
...
'response.Write sql    
    ...
    Else
		sql="SELECT RTObj.CUSID,RTObj.SHORTNC " _
			...
			&"RTConsigneeCASE ON RTConsigneeCASE.CUSID = RTConsignee.CUSID " _
			&"WHERE (RTConsigneeCASE.CASEID = '08') " _
			&"AND rtobj.cusid='" & dspkey(12) & "' "          
    End If
    rs.Open sql,conn
    If rs.Eof Then s="<option value="""" >(經銷商)</option>"
    sx=""
    Do While Not rs.Eof
       If rs("CUSID")=dspkey(12) Then sx=" selected "
       s=s &"<option value=""" &rs("CUSID") &"""" &sx &">" &rs("SHORTNC") &"</option>"
       rs.MoveNext
       sx=""
    Loop
    rs.Close        
    %>
           <select size="1" name="key12" <%=fieldpa%><%=FIELDROLE(1)%><%=dataProtect%>  class="dataListEntry" ID="Select1">                                            
              <%=s%>
           </select>
    </td></tr>
<tr>
    ...
    <td  bgcolor="silver" >
        <input type="text" name="key15" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(15)%>" size="10" READONLY class=dataListENTRY ID="Text13">
       <input  type="button" id="B15" name="B15" height="100%" width="100%" style="Z-INDEX: 1" value="...." <%=FIELDPC%>>
       <IMG  SRC="/WEBAP/IMAGE/IMGDELETE.GIF" alt="清除" id="C15"  name="C15"   style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" <%=FIELDPD%>>                              
    </td>    
    <td  class=dataListHead>局端電纜編號</td>
    <td  bgcolor="silver" >
        <input type="text" name="key25" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(25)%>" size="10"  class=dataListENTRY ID="Text12">
    </td>    
</tr>        
<tr>
    ...
    <td  bgcolor="silver" >
        <input type="text" name="key32" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(32)%>" size="10" readonly class=dataListdata ID="Text28">
    </td>    
    <td  class=dataListHead>未完工結案日</td>
    <td  bgcolor="silver" >
        <input type="text" name="key33" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(33)%>" size="10"  readonly class=dataListdata ID="Text29">
    </td>    
</tr>        
<tr><td  class=dataListHead>作廢日期</td>
    <td  bgcolor="silver" COLSPAN=3>
        <input type="text" name="key13" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               style="text-align:left;" maxlength="10"
               value="<%=dspKey(13)%>" size="10" READONLY class=dataListDATA ID="Text27">
    </td>
</tr>        
 <tr>
   ...
   <td  height="23" bgcolor="silver" COLSPAN=3>
     <input type="text" name="key14" <%=fieldpa%><%=fieldRole(1)%><%=dataProtect%>
               ...
               value="<%=dspKey(14)%>" size="90" class=dataListEntry ID="Text2">
   </TD>
   </TR>
<tr>
 <td class=dataListHEAD>獎金計算月</td>
    <td bgcolor="silver">   
    <input type="text" name="key16" size="6" MAXLENGTH=6 value="<%=dspKey(16)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text10">
    </td>                            
   <td class=dataListHead>獎金計算日</td>          
    <td bgcolor="silver">   
    <input type="text" name="key17" size="10" MAXLENGTH=10 value="<%=dspKey(17)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text11">
    </td>  
  </TR>
  <tr>        
   ...
   <%   NAME=""
    ...
    <input type="text" name="key18" size="6" MAXLENGTH=6 value="<%=dspKey(18)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text15"><font size=3><%=NAME%></font>
    </td>
   <td class=dataListHead>獎金會計審核日</td>          
    <td  bgcolor="silver">     
 <input type="text" name="key19" size="10" MAXLENGTH=10 value="<%=dspKey(19)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text16">
    </td>    
</tr>             
<tr>
 <td class=dataListHEAD>庫存計算月</td>
    <td bgcolor="silver">   
    <input type="text" name="key20" size="6" MAXLENGTH=6 value="<%=dspKey(20)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text14">
    </td>                            
   <td class=dataListHead>庫存計算日</td>          
    <td bgcolor="silver">   
    <input type="text" name="key21" size="10" MAXLENGTH=10 value="<%=dspKey(21)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text21">
    </td>  
  </TR>
  <tr>        
   ...
   <%   NAME=""
    ...
    <input type="text" name="key22" size="6" MAXLENGTH=6 value="<%=dspKey(22)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text25"><font size=3><%=NAME%></font>
    </td>
   <td class=dataListHead>庫存會計審核日</td>          
    <td  bgcolor="silver">     
    <input type="text" name="key23" size="10" MAXLENGTH=10 value="<%=dspKey(23)%>"  READONLY  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text26">
    </td>    
</tr>  
<tr>
...
<tr>
  ...
  %>    <input type="text" name="key30" size="6" READONLY value="<%=dspKey(30)%>"  <%=fieldpa%><%=fieldRole(1)%> class="dataListDATA" ID="Text2"><font size=2><%=name%></font>
    ...
    <TR><TD align="CENTER">
     <TEXTAREA  cols="100%" name="key26" rows=5  MAXLENGTH=300   class="dataListentry"  <%=dataprotect%>  value="<%=dspkey(26)%>" ID="Textarea1"><%=dspkey(26)%></TEXTAREA>
   </td></tr>
 </table>            
   </table> 
  </div> 
