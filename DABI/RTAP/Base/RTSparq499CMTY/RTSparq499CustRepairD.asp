...
' -------------------------------------------------------------------------------------------- 
  Sub SrSaveData(message)
   ...
   ' RESPONSE.WRITE	SQL
		  ...
		  For i	= 0	To numberOfField-1
			  ...
			  '	  On Error Resume Next
				...
				select case	ucase(runpgm)	
				   ...
				   case	ucase("/webap/rtap/base/RTSparq499cmty/RTSparq499CUSTRepaird.asp")
					'response.write	"I=" & i & ";VALUE=" & dspkey(i) & "<BR>"
					   if i	<> 1 then rs.Fields(i).Value=dspKey(i)	  
					   if i=1 then
						 Set rsc=Server.CreateObject("ADODB.Recordset")
						 rsc.open "select max(entryno) AS entryno from RTSparq499CustRepair where	CUSID='" & DSPKEY(0) & "' "	,conn
	...
	if accessmode ="A" then
	   runpgm=Request.ServerVariables("PATH_INFO")
	   if ucase(runpgm)=ucase("/webap/rtap/base/RTSparq499cmty/RTSparq499CustRepairD.asp") then
		  rsc.open "select max(entryno)	AS ENTRYNO from	RTSparq499CustRepair where	 CUSID='" &	DSPKEY(0) &	"' " ,conn
		  if not rsC.eof then
...
'	connxx.open	DSN
'	sqlxx="select count(*) as cnt from RTSparq499custCont where cusid='" &	aryparmkey(0) &	"' and finishdat is	null and canceldat is null "
...
%>
  ...
  -->
  <span	id="tags1" class=dataListTagOn>速博499用戶維修收款資訊</span>
  <!--
		   <FONT SIZE=2	COLOR=RED>
		  <MARQUEE width=250 behavior=slide	scrolldelay=30 behavior=alternate>
		  <%
		  SQLYY="SELECT	FINISHDAT FROM RTSparq499CUST WHERE CUSID='" &	DSPKEY(0) &	"'"
		  RS.OPEN SQLYY,CONN
