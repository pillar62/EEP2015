...
Sub SrEnvironment()
  ...
  title="經銷商經銷ISP資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT cusid,ISP,eusr,edat,uusr,udat " _
             &"FROM RTConsigneeISP WHERE cusid='*' "
  sqlList="SELECT cusid,ISP,eusr,edat,uusr,udat " _
             &"FROM RTConsigneeISP WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
  extDBField=0
End Sub
...
<p>
      <table width="100%" border=1 cellPadding=0 cellSpacing=0>
       <tr><td width="20%" class=dataListHead>經銷商代碼</td><td width="30%" bgcolor=silver>
           ...
           <td width="20%" class=dataListHead>ISP代碼</td>
...
<%  Dim conn,rs,sql,s,sx
    ...
    If sw="E" Or (accessMode="A" And sw="") or (sw="S" and formvalid=false) Then 
       sql="SELECT * FROM RTcode where Kind='C3' ORDER BY Kind,Code "
       If len(trim(dspkey(1))) < 1 Then
          sx=" selected " 
    ...
    Else
       sql="SELECT * FROM RTcode WHERE Kind='C3' and code='" &dspkey(1) &"' order by Kind,CODE"
    End If
    rs.Open sql,conn
    Do While Not rs.Eof
