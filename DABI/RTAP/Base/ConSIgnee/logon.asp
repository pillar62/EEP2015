<%  
  ...
  If Not (IsEmpty(Request("Usr")) or Request("Usr") = "" or Request("PWD") = "" or IsEmpty(Request("PWD"))) Then
     ...
     sql = "SELECT CUSID, CUSNO, PASSWORD FROM RTConsignee WHERE CUSNO='" & USR & "'"     
     ...
     If Not rs.EOF Then
        if pwd <> trim(rs("PASSWORD")) then
           ...
           if typ = "login" then '重導至[經銷商]的主畫面
              Response.Redirect "http://w3c.intra.cbbn.com.tw/webap/rtap/base/consignee/ConsigneeMain.asp"
           elseif typ = "chgpw" then '重導至[修改密碼]的頁面
              Response.Redirect "http://w3c.intra.cbbn.com.tw/webap/rtap/base/consignee/chgpwd2.asp?uid="&usr
           end if   
        end if
     else
        Msg="帳號不存在!"
