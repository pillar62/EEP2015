...
Sub btn_onClick()
  ...
  s=s & "COT建置自付額審核狀況：" & s1ary(1) & "  " & "列印批號：含('" & s2 & "')字元"
  if s1ary(0)="1" then
     t=t & " and datalength(rtrim(rtcmty.payprtseq)) >0 "
  elseif s1ary(0)="2" then
     t=t & " and rtcmty.ACCOUNTCFM Is not null "
  elseif s1ary(0)="3" then
     t=t & " and rtcmty.ACCOUNTCFM Is null  "
  end if
  if len(trim(s2))> 0 then
     t=t & " and rtcmty.payprtseq like '%" & s2 & "%'"
  end if  
  '---t transfor to array
  't=t & ";" & s2   
...
<form name="form">
<center>
<div class=datalisttitle>請輸入（選擇）COT建置自付額審核搜尋條件</div><p>
<table border=1 cellspacing=0 cellpadding=0>
<tr>
<td class=datalisthead>COT建置自付額審核狀況</td>
<td bgcolor="silver">
