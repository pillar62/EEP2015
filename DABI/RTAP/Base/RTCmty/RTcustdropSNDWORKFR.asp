...
<% KEY=SPLIT(REQUEST("KEY"),";")
 ...
 '  On Error Resume Next
   sqlxx="select * FROM RTcustdropsndwork WHERE cusid='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' "
 ...
 '  RESPONSE.END
   ...
   ELSE
      ...
      sqlyy="select max(seq) as seq FROM RTcustdropsndworklog WHERE cusid='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' "
      ...
      IF NOT ISNULL(RSXX("FINISHDAT")) THEN
         sqlyy="insert into RTcustdropsndworklog " _ 
           ...
           &"FROM RTcustdropsndwork where cusid='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' "
         STS="1"
      '未完工結案返轉
      ELSE
         sqlyy="insert into RTcustdropsndworklog " _ 
     ...
     ' Response.Write sqlyy
      ...
      If Err.number > 0 then
         ...
         SQLXX=" update RTcustdropsndwork set FINISHdat=NULL,UNCLOSEDAT=NULL,CLOSEUSR='',UUSR='" & V(0) & "',UDAT=GETDATE()  where cusid='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' "
         connxx.Execute SQLXX
         If Err.number > 0 then
            endpgm="2"
            '發生錯誤時，刪除異動檔所新增的異動資料
            errmsg=cstr(Err.number) & "=" & Err.description
            sqlyy="delete * FROM RTcustdropsndworklog WHERE cusid='" & KEY(0) & "' and entryno=" & key(1) & " and prtno='" & key(2) & "' and seq=" & seq
            CONNXX.Execute sqlyy 
         else
            endpgm="1"
            errmsg=""
...
<form name=frm1 method=post action=RTcustdropsndworkfr.asp ID="Form1">
