...
Sub SrEnvironment()
  ...
  End IF
  DSN="DSN=RTLib"
  formatName="none;社區名稱;IP;社區位置;經銷商;派工單號;測通日;撤線日;申請;完工;報竣;退租;撤銷;有效戶;待處理" 
  sqlDelete="SELECT RTCmty.COMQ1, RTCmty.COMN, RTCmty.NETIP,RTCounty.CUTNC + RTCmty.TOWNSHIP + RTCmty.ADDR AS Expr1, " _
         &"RTCode.CODENC, RTSparqadslcmtysndwork.PRTNO,RTCmty.T1APPLY, RTCmty.RCOMDROP, " _
         ...
         &"RTSparqadslcmtysndwork.PRTNO,RTCmty.T1APPLY, RTCmty.RCOMDROP "
  dataTable="RTCMTY"
...
' When first time enter this keylist default query string to RTcmty.ComQ1 <> 0
  ...
  sqllist="SELECT RTCmty.COMQ1, RTCmty.COMN, RTCmty.NETIP,RTCounty.CUTNC + RTCmty.TOWNSHIP + RTCmty.ADDR AS Expr1, " _
         ...
         &"SUM(CASE WHEN RTCUST.FINISHDAT IS NOT NULL THEN 1 ELSE 0 END) AS FCNT, " _
         &"SUM(CASE WHEN RTCUST.DOCKETDAT IS NOT NULL THEN 1 ELSE 0 END) AS DCNT, " _
         &"SUM(CASE WHEN RTCUST.DROPDAT IS NOT NULL AND RTCUST.DOCKETDAT IS NOT NULL THEN 1 ELSE 0 END) AS RCNT, " _
         &"SUM(CASE WHEN RTCUST.DROPDAT IS NOT NULL AND RTCUST.DOCKETDAT IS NULL THEN 1 ELSE 0 END) AS CCNT, " _
         &"SUM(CASE WHEN RTCUST.DOCKETDAT IS NOT NULL AND RTCUST.DROPDAT IS NULL THEN 1 ELSE 0 END) AS UCNT, " _
         &"SUM(CASE WHEN RTCUST.CUSID IS NOT NULL THEN 1 ELSE 0 END) - " _
         &"SUM(CASE WHEN RTCUST.DOCKETDAT IS NOT NULL AND RTCUST.DROPDAT IS NULL THEN 1 ELSE 0 END) - " _
         &"SUM(CASE WHEN RTCUST.DROPDAT IS NOT NULL AND RTCUST.DOCKETDAT IS NOT NULL THEN 1 ELSE 0 END) - " _
         &"SUM(CASE WHEN RTCUST.DROPDAT IS NOT NULL AND RTCUST.DOCKETDAT IS NULL THEN 1 ELSE 0 END) AS PCNT " _
         &"FROM RTCmty LEFT OUTER JOIN  RTCust ON RTCmty.COMQ1 = RTCust.COMQ1 LEFT OUTER JOIN RTCode ON " _
         &"RTCmty.COMTYPE = RTCode.CODE AND RTCode.KIND = 'B3' LEFT OUTER JOIN RTCounty ON RTCmty.CUTID = RTCounty.CUTID " _
         &"LEFT OUTER JOIN RTSparqadslcmtysndwork ON RTCMTY.COMQ1 = RTSparqadslcmtysndwork.CUTYID AND " _
