...
Sub SrEnvironment()
  ...
  title="AVS客戶附加服務資料維護"
  formatName=";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  sqlFormatDB="SELECT COMQ1,LINEQ1,CUSID,ENTRYNO,TELNO,DIALERPAYTYPE,SRVTYPE,SDATE,DROPDAT,CHKDAT,TRANSDAT FROM RTEBTcustext WHERE COMQ1=0 "
  sqlList="SELECT COMQ1,LINEQ1,CUSID,ENTRYNO,TELNO,DIALERPAYTYPE,SRVTYPE,SDATE,DROPDAT,CHKDAT,TRANSDAT FROM RTEBTcustext WHERE "
  userDefineKey="Yes"
  userDefineData="Yes"
...
<tr>
    ...
    <td width="18%" bgcolor="silver" COLSPAN=7>
        <input class=dataListDATA type="text" name="key4" 
               style="text-align:left;" maxlength="15"
               value="<%=dspKey(4)%>" <%=keyProtect%>  ID="Text2"></td>         
    </TR>
</table>
<%
End Sub
...
<table width="100%" border=1 cellPadding=0 cellSpacing=0>
   ...
   ' Response.Write "SQL=" & sql
    ...
    Set rs=Nothing
    Set conn=Nothing%>
        <select class=dataListDATA name="key6" <%=keyProtect%> size="1"
   ...
   ' Response.Write "SQL=" & sql
    ...
    Set rs=Nothing
    Set conn=Nothing%>
        <select class=dataListDATA name="key5" <%=keyProtect%> size="1"
               style="text-align:left;" maxlength="8" ID="Select1"><%=s%></select></td>               
    </TR>
    <TR>
    <td width="20%" class=dataListHead>申請日期</td>
    <td width="30%" bgcolor="silver">
        <input class=dataListDATA type="text" READONLY name="key7" 
        ...
        <IMG SRC="/AVSCONSIGNEE/IMAGES/IMGDELETE.GIF" alt="清除" id="C7"  name="C7"   style="Z-INDEX: 1"  border=0 onmouseover="ImageIconOver" onmouseout="ImageIconOut" onclick="SrClear"></td>
               </td>
