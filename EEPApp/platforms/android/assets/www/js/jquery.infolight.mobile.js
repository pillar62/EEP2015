var webSiteUrl = 'http://192.168.1.102/JQWebClient';
var developerID = ''; var database = ''; var solution = '';
...
$.fn.logon.methods = {
    ...
    registerDevice: function (jq, options) {
        ...
        $.ajax({
            ...
            url: $.getSystemUrl(),
            data: {
                mode: 'registerDevice', userID: window.localStorage.getItem("userID"), deviceID: window.sessionStorage.getItem("uuid"), regID: options.regID, tokenID: options.tokenID
            },
            cache: false,
            async: true,
            success: function (data) {
...
$.extend({
    ...
    createListItem: function (text, url, imageUrl) {
        if (imageUrl) {
            ...
            return "<li><a href='" + url + "' data-transition='slide' rel='external'><img src='" + imageUrl + "' alt=" + text + " class='ui-li-icon'>" + text + "</a></li>";
        }
        else {
            return "<li><a href='" + url + "' data-transition='slide' rel='external'>" + text + "</a></li>"; //include multipage
        }
    },
    ...
    load: function (jq) {
        jq.each(function () {
            ...
            if (options.remoteName != undefined && options.remoteName != "" && options.tableName != undefined) {
                ...
                var selecteddata = window.localStorage.getItem($.cacheData.url + $(this).parent().parent().attr('id') + "selected");
                if ($.cacheData('loadCache', { id: $(this).parent().parent().attr('id'), cacheMode: cacheMode, isgrid: true }) && selecteddata) {
                    var datagrid = $(this);
                    ...
                    $.ajax({
                        ...
                        data: { remoteName: options.remoteName, page: options.pageNumber, rows: options.pageSize, queryWord: $.toJSONString(queryWord) }, //cordova 
                        ...
                        success: function (data) {
                            if (data.rows != undefined) {
                                ...
                                $.cacheData("cache", { id: datagrid.parent().parent().attr("id"), cacheMode: cacheMode, cacheData: data });
        ...
        if (jq.length > 0) {
            ...
            if (onlinestate == "true") {
                ...
                if (cacheMode && cacheMode != "none" && changedDataCount) {
                    ...
                    if (button.length == 0) {
                        button = $('#' + $(jq[0]).parent().parent().attr('id') + '_toolitem').find('.grid-offlineSend');
                        button.attr('data-iconpos', 'left');
                        button.attr('data-icon', 'online');
                    }
                    else {
                    ...
                    if (button.length == 0) {
                        button = $('#' + $(jq[0]).parent().parent().attr('id') + '_toolitem').find('.grid-offlineSend');
                        button.attr('data-iconpos', '');
                        button.attr('data-icon', '');
                        button.addClass('ui-icon-online');
                    }
                    else {
                        button.removeClass("ui-icon-plus");
    ...
    apply: function (jq) {
        jq.each(function () {
            ...
            if (onlinestate == "false") {
                ...
                //}
                var cacheMode = $("." + $.fn.datagrid.class, parentObjectID).datagrid('options').cacheMode;
                var url = document.location.pathname.substring(document.location.pathname.lastIndexOf('/') + 1);
                if (cacheMode && cacheMode != "none") {
                    ...
                    if (button.length == 0) {
                        button = $('#' + $("." + $.fn.datagrid.class, viewPage).parent().parent().attr('id') + '_toolitem').find('.grid-offlineSend');
                        button.attr('data-iconpos', 'left');
                        button.attr('data-icon', 'offline');
                    }
                    else {
                        //button.removeClass("ui-icon-plus");
    ...
    load: function (jq) {
        jq.each(function () {
            ...
            if (options.remoteName != undefined && options.tableName != undefined) {
                ...
                var selecteddata = window.localStorage.getItem($.cacheData.url + selects.attr('id') + "selected");
                if(selecteddata && $.cacheData('loadCache', {id:selects.attr('id'),cacheMode:cacheMode,cacheDateTimeField:cacheDateTimeField,remoteName:options.remoteName,tableName:options.tableName})) {
                    var sdata = JSON.parse(selecteddata);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.rows != undefined) {
                                ...
                                $.cacheData("cache", { id: selects.attr("id"), cacheMode: cacheMode ,cacheData:data});
                            }
                        },
                        error: function (data) {
                        }
...
$.cacheData.methods = {
    loadCache: function (options) { 
        ...
        if (isgrid) {
            ...
            if (button.length == 0) {
                button = $('#' + id + '_toolitem').find('.grid-offlineSend');
                button.attr('data-iconpos', '');
                button.attr('data-icon', '');
        ...
        if (onlinestate == "false") {
            ...
            var selecteddatatime = window.localStorage.getItem(url + id + "selectedTime");
            ...
            var year = selecteddatatime.getFullYear();
            var month = (selecteddatatime.getMonth() + 1).toString();
            if (month.length == 1) {
                month = '0' + month;
            }
            var date = selecteddatatime.getDate().toString();
            if (date.length == 1) {
                date = '0' + date;
            }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != "") {
                            ...
                            var month = (selecteddatetime.getMonth() + 1).toString();
                            if (month.length == 1) {
                                month = '0' + month;
                            }
                            var date = selecteddatetime.getDate().toString();
                            if (date.length == 1) {
                                date = '0' + date;
                            }
                            var hour = selecteddatetime.getHours();
                            var minutes = selecteddatetime.getMinutes();
                            var seconds = selecteddatetime.getSeconds();

                            var cacheTimes = localStorage.getItem(url + id + "selectedTime");
                            var cacheTime = new Date(cacheTimes)
                            var oyear = cacheTime.getFullYear();
                            var omonth = (cacheTime.getMonth() + 1).toString();
                            if (omonth.length == 1) {
                                omonth = '0' + omonth;
                            }
                            var odate = cacheTime.getDate().toString();
                            if (odate.length == 1) {
                                odate = '0' + odate;
                            }
                            var ohour = cacheTime.getHours();
                            var ominutes = cacheTime.getMinutes();
                            var oseconds = cacheTime.getSeconds();

                            if (year <= oyear && month <= omonth && date <= odate && hour <= ohour && minutes <= ominutes && seconds <= oseconds) { loadcache = true; }
                        }
                    },
                    error: function (data) { }
                });
        ...
        jq.each(function () {
            ...
            if (drillStyle == 'webform') {
                ...
                if (formname != "") {
                    ...
                    var url = webSiteUrl + "/handler/SystemHandle.ashx?Type=Encrypt";//20180828
                    if (developer) {
                        url = webSiteUrl + "/handler/SystemHandler.ashx?type=Encrypt";//20150828
                    }
                    $.ajax({
                        url: url,
                        data: { param: urlParam },
                        ...
                        success: function (param) {
                            ...
                            if (openMode == "newwindow") {
                                ...
                                var title = $.parseOption(drilldown.attr('infolight-options')).formName;
                                var t = $.parseOption(drilldown.attr('infolight-options')).formName.split("/");
                                var t2 = t[t.length - 1];
                ...
                if (reportName != undefined) {
                    ...
                    var url = webSiteUrl + "/ReportViewerTemplate.aspx?RemoteName=" + remoteName + "&TableName=" + tableName + "&ReportPath=" + reportName + "&WhereString=" + encodeURIComponent(where);//20150828
                    var height = $(window).height() - 20;
                    var width = $(window).width() - 20;
                    if ($.browser.msie) {
                ...
                if (DisplayFields != undefined && DisplayFields.length > 0) {
                    ...
                    for (var i = 0; i < DisplayFields.length; i++) {
                        var column = { field: DisplayFields[i].field, title: DisplayFields[i].caption, width: DisplayFields[i].width };
                    ...
                    var datagrid = $('<table />').appendTo(div)
                    .datagrid({
                        pageNumber: 1,
                        pageSize: 10,
                        pagination: true,
